...
def flatten_ast(node: Any, prefix="", indent=0, path="") -> str:
    if isinstance(node, ast.AST):
        ...
        if "lineno" in node._attributes:
            acc.append(f"{prefix}/_pos={node.lineno}:{indent - 1}:{path[2:-1]}[-]*\n")
        ...
        for (i, (name, x)) in enumerate(fields):
            ...
            if name == "orelse" and isinstance(node, (ast.For, ast.While, ast.AsyncFor)):
                ...
                name = "assignvalue"
            acc.append(flatten_ast(x, f"{prefix}/{name}", indent + push, f"{path}{i}-"))
        return "".join(acc)
    elif isinstance(node, list):
        acc = [f"{prefix}/length={len(node)}\n"]
        for (i, x) in enumerate(node):
            acc.append(flatten_ast(x, f"{prefix}/{i}", indent, f"{path}{i}-"))
        return "".join(acc)
    else:

...
def print_collatz(n): # added_block_label (-> +7), function:print_collatz (-> +7), function_parameter:n, function_parameter_flavor:arg, function_returning_nothing:print_collatz (-> +7), local_scope:n (-> +7), node:FunctionDef (-> +7), node:arg, procedural_style (-> +7), scope:n (-> +7), whole_span:8 (-> +7)
    while n != 1: # comparison_operator:NotEq, literal:1, loaded_variable:n, loop:while (-> +5), loop_with_late_exit:while (-> +5), node:Compare, node:Name, node:Num, node:While (-> +5), suggest_constant_definition
        print(n) # call_argument:n, external_free_call:print, free_call:print, free_call_without_result:print, loaded_variable:n, node:Call, node:Expr, node:Name
...
for i in range(1, 101): # call_argument:1, call_argument:101, external_free_call:range, for:i (-> +8), for_range:1:101 (-> +8), free_call:range, iteration_variable:i, literal:1, literal:101, loop:for (-> +8), loop_with_late_exit:for (-> +8), magic_number:101, node:Call, node:For (-> +8), node:Name, node:Num, range:1:101
    if i % 15 == 0: # binary_operator:Mod, comparison_operator:Eq, divisibility_test:15, if (-> +7), if_test_atom:0, if_test_atom:15, if_test_atom:i, literal:0, literal:15, loaded_variable:i, magic_number:15, modulo_operator, node:BinOp, node:Compare, node:If (-> +7), node:Name, node:Num
        ...
        print(i) # call_argument:i, external_free_call:print, free_call:print, free_call_without_result:print, if_else_branch, loaded_variable:i, node:Call, node:Expr, node:Name

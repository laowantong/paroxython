...
class RGB(numpy.ndarray): # class:RGB (-> +8), node:Attribute, node:ClassDef (-> +8), node:Name
    ...
    def from_str(cls, rgbstr): # function_argument:cls, function_argument:rgbstr, function_argument_flavor:arg, node:arg
        return numpy.array( # composition, member_call:array, member_call:view, member_call_object:numpy, method_chaining, node:Attribute, node:Attribute (-> +1), node:Call (-> +1), node:Call (-> +2), node:Name, node:Return (-> +2), return

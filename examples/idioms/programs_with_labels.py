...
# ----------------------------------------------------------------------------------------
print("Hello World") # argument:, external_free_call:print, flat_style, free_call:print, free_call_without_result:print, imperative_style, literal:Str, node:Call, node:Expr, node:Name, node:Str, one_liner_style, variety:*, whole_span:1

# ----------------------------------------------------------------------------------------
# 002.0011-print-hello-10-times.py
# ----------------------------------------------------------------------------------------
for i in range(10): # argument:10, external_free_call:range, for:i (-> +1), for_range:10 (-> +1), free_call:range, global_scope:i (-> +1), imperative_style (-> +1), iteration_variable:i, literal:10, loop:for (-> +1), loop_with_late_exit:for (-> +1), magic_number:10, node:Call, node:For (-> +1), node:Name, node:Num, range:10, scope:i (-> +1), variety:* (-> +1), whole_span:2 (-> +1)
...
# ----------------------------------------------------------------------------------------
for x in items: # for:x (-> +1), for_each:x (-> +1), global_scope:x (-> +1), imperative_style (-> +1), iteration_variable:x, loaded_variable:items, loop:for (-> +1), loop_with_late_exit:for (-> +1), node:For (-> +1), node:Name, scope:x (-> +1), variety:* (-> +1), whole_span:2 (-> +1)
...
# ----------------------------------------------------------------------------------------
x = {"one": 1, "two": 2} # assignment, assignment_lhs_identifier:x, assignment_rhs_atom:1, assignment_rhs_atom:2, flat_style, global_scope:x, imperative_style, literal:1, literal:2, literal:Dict, literal:Str, node:Assign, node:Dict, node:Name, node:Num, node:Str, one_liner_style, scope:x, single_assignment:x, variety:*, whole_span:1

# ----------------------------------------------------------------------------------------
# 009.1410-create-a-binary-tree-data-structure.py
# ----------------------------------------------------------------------------------------
class Node: # class:Node (-> +4), class_method_count:1 (-> +4), node:ClassDef (-> +4), object_oriented_style (-> +4), variety:** (-> +4), whole_span:5 (-> +4)
...
# ----------------------------------------------------------------------------------------
import random # flat_style (-> +1), imperative_style (-> +1), import:random, import_module:random, node:Import, one_liner_style (-> +1), variety:* (-> +1), whole_span:2 (-> +1)
...
# ----------------------------------------------------------------------------------------
import random # functional_style (-> +2), import:random, import_module:random, node:Import, one_liner_style (-> +2), variety:** (-> +2), whole_span:3 (-> +2)
...
# ----------------------------------------------------------------------------------------
class Node(object): # class:Node (-> +3), class_method_count:1 (-> +3), loaded_variable:object, node:ClassDef (-> +3), node:Name, object_oriented_style (-> +3), variety:** (-> +3), whole_span:4 (-> +3)
...
# ----------------------------------------------------------------------------------------
x = reversed(x) # argument:x, assignment:reversed, assignment_lhs_identifier:x, assignment_rhs_atom:x, external_free_call:reversed, flat_style, free_call:reversed, global_scope:x, imperative_style, loaded_variable:x, node:Assign, node:Call, node:Name, one_liner_style, scope:x, single_assignment:x, variety:*, whole_span:1
...
# ----------------------------------------------------------------------------------------
a, b = b, a # assignment, assignment_lhs_identifier:a, assignment_lhs_identifier:b, assignment_rhs_atom:a, assignment_rhs_atom:b, flat_style, global_scope:a, global_scope:b, imperative_style, literal:Tuple, loaded_variable:a, loaded_variable:b, node:Assign, node:Name, node:Tuple, one_liner_style, parallel_assignment:2, scope:a, scope:b, swap, update:a:b, update:b:a, update_by_assignment:a:b, update_by_assignment:b:a, update_by_assignment_with, update_with, variety:*, whole_span:1

# ----------------------------------------------------------------------------------------
# 022.0243-convert-string-to-integer.py
# ----------------------------------------------------------------------------------------
i = int(s) # argument:s, assignment:int, assignment_lhs_identifier:i, assignment_rhs_atom:s, external_free_call:int, flat_style, free_call:int, global_scope:i, imperative_style, loaded_variable:s, node:Assign, node:Call, node:Name, one_liner_style, scope:i, single_assignment:i, variety:*, whole_span:1

# ----------------------------------------------------------------------------------------
# 023.1102-convert-real-number-to-string-with-2-decimal-places.py
# ----------------------------------------------------------------------------------------
s = "{:.2f}".format(x) # argument:x, assignment:format, assignment_lhs_identifier:s, assignment_rhs_atom:x, flat_style, global_scope:s, imperative_style, literal:Str, loaded_variable:x, member_call_method:format, node:Assign, node:Attribute, node:Call, node:Name, node:Str, one_liner_style, scope:s, single_assignment:s, variety:*, whole_span:1

# ----------------------------------------------------------------------------------------
# 024.0664-assign-to-string-the-japanese-word-.py
# ----------------------------------------------------------------------------------------
s = "ネコ" # assignment, assignment_lhs_identifier:s, flat_style, global_scope:s, imperative_style, literal:Str, node:Assign, node:Name, node:Str, one_liner_style, scope:s, single_assignment:s, special_literal_string:ネコ, variety:*, whole_span:1

# ----------------------------------------------------------------------------------------
# 025.0195-send-a-value-to-another-thread.py
# ----------------------------------------------------------------------------------------
import Queue # flat_style (-> +5), global_scope:q (-> +5), global_scope:t (-> +5), imperative_style (-> +5), import:Queue, import_module:Queue, node:Import, scope:q (-> +5), scope:t (-> +5), variety:*** (-> +5), whole_span:6 (-> +5)
...
# ----------------------------------------------------------------------------------------
def exp(x, n): # function:exp (-> +1), function_line_count:2 (-> +1), function_parameter:n, function_parameter:x, function_parameter_flavor:arg, function_returning_something:exp (-> +1), functional_style (-> +1), local_scope:n (-> +1), local_scope:x (-> +1), node:FunctionDef (-> +1), node:arg, one_liner_style (-> +1), pure_function:exp (-> +1), scope:n (-> +1), scope:x (-> +1), variety:* (-> +1), whole_span:2 (-> +1)
...
# ----------------------------------------------------------------------------------------
import threading # flat_style (-> +6), global_scope:lock (-> +6), global_scope:x (-> +6), imperative_style (-> +6), import:threading, import_module:threading, node:Import, scope:lock (-> +6), scope:x (-> +6), variety:** (-> +6), whole_span:7 (-> +6)
...
# ----------------------------------------------------------------------------------------
def compose(f, g): # function:compose (-> +1), function_line_count:2 (-> +1), function_parameter:f, function_parameter:g, function_parameter_flavor:arg, function_returning_something:compose (-> +1), functional_style (-> +1), higher_order_function:f (-> +1), higher_order_function:g (-> +1), local_scope:a (-> +1), local_scope:f (-> +1), local_scope:g (-> +1), node:FunctionDef (-> +1), node:arg, one_liner_style (-> +1), pure_function:compose (-> +1), scope:a (-> +1), scope:f (-> +1), scope:g (-> +1), variety:* (-> +1), whole_span:2 (-> +1)
...
# ----------------------------------------------------------------------------------------
def compose(f, g): # function:compose (-> +1), function_line_count:2 (-> +1), function_parameter:f, function_parameter:g, function_parameter_flavor:arg, function_returning_something:compose (-> +1), functional_style (-> +1), higher_order_function:f (-> +1), higher_order_function:g (-> +1), local_scope:f (-> +1), local_scope:g (-> +1), local_scope:x (-> +1), node:FunctionDef (-> +1), node:arg, one_liner_style (-> +1), pure_function:compose (-> +1), scope:f (-> +1), scope:g (-> +1), scope:x (-> +1), variety:* (-> +1), whole_span:2 (-> +1)
...
# ----------------------------------------------------------------------------------------
from functools import partial # functional_style (-> +5), import:functools:partial, import_module:functools, import_name:partial, node:ImportFrom, variety:*** (-> +5), whole_span:6 (-> +5)
...
# ----------------------------------------------------------------------------------------
for v in a: # for:v (-> +7), for_each:v (-> +7), global_scope:u (-> +7), global_scope:v (-> +7), imperative_style (-> +7), iteration_variable:v, loaded_variable:a, loop:for (-> +7), loop_with_early_exit:for:raise (-> +7), loop_with_raise:for (-> +7), node:For (-> +7), node:Name, scope:u (-> +7), scope:v (-> +7), variety:**** (-> +7), whole_span:8 (-> +7)
...
# ----------------------------------------------------------------------------------------
for v in a: # for:v (-> +4), for_each:v (-> +4), global_scope:v (-> +4), global_scope:v_ (-> +4), imperative_style (-> +4), iteration_variable:v, loaded_variable:a, loop:for (-> +4), loop_with_late_exit:for (-> +4), node:For (-> +4), node:Name, scope:v (-> +4), scope:v_ (-> +4), variety:*** (-> +4), whole_span:5 (-> +4)
...
# ----------------------------------------------------------------------------------------
s.insert(i, x) # argument:i, argument:x, flat_style, imperative_style, loaded_variable:i, loaded_variable:s, loaded_variable:x, member_call:s:insert, member_call_method:insert, member_call_object:s, node:Attribute, node:Call, node:Expr, node:Name, one_liner_style, update:s:i, update:s:x, update_by_member_call:s:i, update_by_member_call:s:x, update_by_member_call_with:insert, update_with:insert, variety:*, whole_span:1
...
# ----------------------------------------------------------------------------------------
chunks = s.split() # assignment:split, assignment_lhs_identifier:chunks, assignment_rhs_atom:s, flat_style, global_scope:chunks, imperative_style, loaded_variable:s, member_call_method:split, node:Assign, node:Attribute, node:Call, node:Name, one_liner_style, scope:chunks, single_assignment:chunks, variety:*, whole_span:1
...
# ----------------------------------------------------------------------------------------
k in m # comparison_operator:In, flat_style, imperative_style, loaded_variable:k, loaded_variable:m, node:Compare, node:Expr, node:Name, one_liner_style, variety:*, whole_span:1

# ----------------------------------------------------------------------------------------
# 052.0666-check-if-map-contains-value.py
# ----------------------------------------------------------------------------------------
v in map.values() # comparison_operator:In, flat_style, imperative_style, loaded_variable:map, loaded_variable:v, member_call_method:values, node:Attribute, node:Call, node:Compare, node:Expr, node:Name, one_liner_style, variety:*, whole_span:1
...
# ----------------------------------------------------------------------------------------
s = sum(x) # argument:x, assignment:sum, assignment_lhs_identifier:s, assignment_rhs_atom:x, external_free_call:sum, flat_style, free_call:sum, global_scope:s, imperative_style, loaded_variable:x, node:Assign, node:Call, node:Name, one_liner_style, scope:s, single_assignment:s, variety:*, whole_span:1

# ----------------------------------------------------------------------------------------
# 055.0575-convert-integer-to-string.py
# ----------------------------------------------------------------------------------------
s = str(i) # argument:i, assignment:str, assignment_lhs_identifier:s, assignment_rhs_atom:i, external_free_call:str, flat_style, free_call:str, global_scope:s, imperative_style, loaded_variable:i, node:Assign, node:Call, node:Name, one_liner_style, scope:s, single_assignment:s, variety:*, whole_span:1

# ----------------------------------------------------------------------------------------
# 056.1424-launch-1000-parallel-tasks-and-wait-for-completion.py
# ----------------------------------------------------------------------------------------
from multiprocessing import Pool # import:multiprocessing:Pool, import_module:multiprocessing, import_name:Pool, node:ImportFrom, procedural_style (-> +5), variety:*** (-> +5), whole_span:6 (-> +5)
...
# ----------------------------------------------------------------------------------------
import sys # flat_style (-> +1), global_scope:x (-> +1), imperative_style (-> +1), import:sys, import_module:sys, node:Import, one_liner_style (-> +1), scope:x (-> +1), variety:* (-> +1), whole_span:2 (-> +1)
...
# ----------------------------------------------------------------------------------------
z = x ** n # assignment:Pow, assignment_lhs_identifier:z, assignment_rhs_atom:n, assignment_rhs_atom:x, binary_operator:Pow, flat_style, global_scope:z, imperative_style, loaded_variable:n, loaded_variable:x, node:Assign, node:BinOp, node:Name, one_liner_style, scope:z, single_assignment:z, variety:*, whole_span:1

# ----------------------------------------------------------------------------------------
# 067.1426-binomial-coefficient-n-choose-k.py
# ----------------------------------------------------------------------------------------
import math # functional_style (-> +2), import:math, import_module:math, node:Import, one_liner_style (-> +2), variety:* (-> +2), whole_span:3 (-> +2)
...
# ----------------------------------------------------------------------------------------
from __future__ import division # flat_style (-> +2), global_scope:x (-> +2), imperative_style (-> +2), import:__future__:division, import_module:__future__, import_name:division, node:ImportFrom, one_liner_style (-> +2), scope:x (-> +2), variety:** (-> +2), whole_span:3 (-> +2)
...
# ----------------------------------------------------------------------------------------
from fractions import gcd # flat_style (-> +1), global_scope:x (-> +1), imperative_style (-> +1), import:fractions:gcd, import_module:fractions, import_name:gcd, node:ImportFrom, one_liner_style (-> +1), scope:x (-> +1), variety:* (-> +1), whole_span:2 (-> +1)
...
# ----------------------------------------------------------------------------------------
y = float(x) # argument:x, assignment:float, assignment_lhs_identifier:y, assignment_rhs_atom:x, external_free_call:float, flat_style, free_call:float, global_scope:y, imperative_style, loaded_variable:x, node:Assign, node:Call, node:Name, one_liner_style, scope:y, single_assignment:y, variety:*, whole_span:1

# ----------------------------------------------------------------------------------------
# 080.1092-truncate-floating-point-number-to-integer.py
# ----------------------------------------------------------------------------------------
y = int(x) # argument:x, assignment:int, assignment_lhs_identifier:y, assignment_rhs_atom:x, external_free_call:int, flat_style, free_call:int, global_scope:y, imperative_style, loaded_variable:x, node:Assign, node:Call, node:Name, one_liner_style, scope:y, single_assignment:y, variety:*, whole_span:1
...
# ----------------------------------------------------------------------------------------
count = s.count(t) # argument:t, assignment:count, assignment_lhs_identifier:count, assignment_rhs_atom:s, assignment_rhs_atom:t, flat_style, global_scope:count, imperative_style, loaded_variable:s, loaded_variable:t, member_call_method:count, node:Assign, node:Attribute, node:Call, node:Name, one_liner_style, scope:count, single_assignment:count, variety:*, whole_span:1
...
# ----------------------------------------------------------------------------------------
def multiplyWillOverflow(x, y): # function:multiplyWillOverflow (-> +1), function_line_count:2 (-> +1), function_parameter:x, function_parameter:y, function_parameter_flavor:arg, function_returning_something:multiplyWillOverflow (-> +1), functional_style (-> +1), local_scope:x (-> +1), local_scope:y (-> +1), node:FunctionDef (-> +1), node:arg, one_liner_style (-> +1), pure_function:multiplyWillOverflow (-> +1), scope:x (-> +1), scope:y (-> +1), variety:* (-> +1), whole_span:2 (-> +1)
    return False # literal:False, node:NameConstant, node:Return, return:False
...
# ----------------------------------------------------------------------------------------
import json # flat_style (-> +2), global_scope:x (-> +2), imperative_style (-> +2), import:json, import_module:json, node:Import, scope:x (-> +2), variety:** (-> +2), whole_span:3 (-> +2)
...
# ----------------------------------------------------------------------------------------
import json # flat_style (-> +2), imperative_style (-> +2), import:json, import_module:json, node:Import, variety:** (-> +2), whole_span:3 (-> +2)
...
# ----------------------------------------------------------------------------------------
print(type(x)) # argument:, argument:x, composition, external_free_call:print, external_free_call:type, flat_style, free_call:print, free_call:type, free_call_without_result:print, imperative_style, loaded_variable:x, node:Call, node:Expr, node:Name, one_liner_style, variety:*, whole_span:1

# ----------------------------------------------------------------------------------------
# 094.1864-print-type-of-variable.py
# ----------------------------------------------------------------------------------------
print(x.__class__) # argument:, external_free_call:print, flat_style, free_call:print, free_call_without_result:print, imperative_style, loaded_variable:x, node:Attribute, node:Call, node:Expr, node:Name, one_liner_style, variety:*, whole_span:1

# ----------------------------------------------------------------------------------------
# 095.2140-get-file-size.py
# ----------------------------------------------------------------------------------------
import os # flat_style (-> +1), global_scope:x (-> +1), imperative_style (-> +1), import:os, import_module:os, node:Import, one_liner_style (-> +1), scope:x (-> +1), variety:* (-> +1), whole_span:2 (-> +1)
...
# ----------------------------------------------------------------------------------------
from datetime import date # flat_style (-> +2), global_scope:d (-> +2), global_scope:x (-> +2), imperative_style (-> +2), import:datetime:date, import_module:datetime, import_name:date, node:ImportFrom, scope:d (-> +2), scope:x (-> +2), variety:* (-> +2), whole_span:3 (-> +2)
...
# ----------------------------------------------------------------------------------------
items.sort(c) # argument:c, flat_style, imperative_style, loaded_variable:c, loaded_variable:items, member_call:items:sort, member_call_method:sort, member_call_object:items, node:Attribute, node:Call, node:Expr, node:Name, one_liner_style, variety:*, whole_span:1

# ----------------------------------------------------------------------------------------
# 101.2172-load-from-http-get-request-into-a-string.py
# ----------------------------------------------------------------------------------------
import urllib.request # flat_style (-> +2), global_scope:s (-> +2), imperative_style (-> +2), import:urllib.request, import_module:urllib.request, node:Import, scope:s (-> +2), variety:** (-> +2), whole_span:3 (-> +2)
...
# ----------------------------------------------------------------------------------------
import urllib # flat_style (-> +1), global_scope:filename (-> +1), global_scope:headers (-> +1), imperative_style (-> +1), import:urllib, import_module:urllib, node:Import, one_liner_style (-> +1), scope:filename (-> +1), scope:headers (-> +1), variety:* (-> +1), whole_span:2 (-> +1)
...
# ----------------------------------------------------------------------------------------
import pyxser as pyx # global_scope:a (-> +11), global_scope:b (-> +11), global_scope:c (-> +11), global_scope:ser (-> +11), global_scope:tst (-> +11), import:pyxser, import_module:pyxser, node:Import, object_oriented_style (-> +11), scope:a (-> +11), scope:b (-> +11), scope:c (-> +11), scope:ser (-> +11), scope:tst (-> +11), variety:*** (-> +11), whole_span:12 (-> +11)
...
# ----------------------------------------------------------------------------------------
import sys # flat_style (-> +1), global_scope:s (-> +1), imperative_style (-> +1), import:sys, import_module:sys, node:Import, one_liner_style (-> +1), scope:s (-> +1), variety:* (-> +1), whole_span:2 (-> +1)
...
# ----------------------------------------------------------------------------------------
if "x" in locals(): # comparison_operator:In, external_free_call:locals, free_call:locals, free_call_no_arguments:locals, if (-> +1), if_without_else (-> +1), imperative_style (-> +1), literal:Str, node:Call, node:Compare, node:If (-> +1), node:Name, node:Str, variety:* (-> +1), whole_span:2 (-> +1)
...
# ----------------------------------------------------------------------------------------
import pympler.asizeof # flat_style (-> +1), global_scope:n (-> +1), imperative_style (-> +1), import:pympler.asizeof, import_module:pympler.asizeof, node:Import, one_liner_style (-> +1), scope:n (-> +1), variety:* (-> +1), whole_span:2 (-> +1)
...
# ----------------------------------------------------------------------------------------
for k in sorted(mymap): # argument:mymap, external_free_call:sorted, for:k (-> +1), free_call:sorted, global_scope:k (-> +1), imperative_style (-> +1), iteration_variable:k, loaded_variable:mymap, loop:for (-> +1), loop_with_late_exit:for (-> +1), node:Call, node:For (-> +1), node:Name, scope:k (-> +1), variety:* (-> +1), whole_span:2 (-> +1)
...
# ----------------------------------------------------------------------------------------
b = x == y # assignment, assignment_lhs_identifier:b, assignment_rhs_atom:x, assignment_rhs_atom:y, comparison_operator:Eq, flat_style, global_scope:b, imperative_style, loaded_variable:x, loaded_variable:y, node:Assign, node:Compare, node:Name, one_liner_style, scope:b, single_assignment:b, variety:*, whole_span:1

# ----------------------------------------------------------------------------------------
# 115.2138-compare-dates.py
# ----------------------------------------------------------------------------------------
import datetime # flat_style (-> +1), global_scope:b (-> +1), imperative_style (-> +1), import:datetime, import_module:datetime, node:Import, one_liner_style (-> +1), scope:b (-> +1), variety:* (-> +1), whole_span:2 (-> +1)
...
# ----------------------------------------------------------------------------------------
s2 = s1.replace(w, "") # argument:, argument:w, assignment:replace, assignment_lhs_identifier:s2, assignment_rhs_atom:s1, assignment_rhs_atom:w, empty_literal:Str, flat_style, global_scope:s2, imperative_style, literal:Str, loaded_variable:s1, loaded_variable:w, member_call_method:replace, node:Assign, node:Attribute, node:Call, node:Name, node:Str, one_liner_style, scope:s2, single_assignment:s2, variety:*, whole_span:1

# ----------------------------------------------------------------------------------------
# 117.1297-get-list-size.py
# ----------------------------------------------------------------------------------------
n = len(x) # argument:x, assignment:len, assignment_lhs_identifier:n, assignment_rhs_atom:x, external_free_call:len, flat_style, free_call:len, global_scope:n, imperative_style, loaded_variable:x, node:Assign, node:Call, node:Name, one_liner_style, scope:n, single_assignment:n, variety:*, whole_span:1

# ----------------------------------------------------------------------------------------
# 118.1254-list-to-set.py
# ----------------------------------------------------------------------------------------
y = set(x) # argument:x, assignment:set, assignment_lhs_identifier:y, assignment_rhs_atom:x, external_free_call:set, flat_style, free_call:set, global_scope:y, imperative_style, loaded_variable:x, node:Assign, node:Call, node:Name, one_liner_style, scope:y, single_assignment:y, variety:*, whole_span:1

# ----------------------------------------------------------------------------------------
# 118.3266-list-to-set.py
# ----------------------------------------------------------------------------------------
set(x) # argument:x, external_free_call:set, flat_style, free_call:set, free_call_without_result:set, imperative_style, loaded_variable:x, node:Call, node:Expr, node:Name, one_liner_style, variety:*, whole_span:1

# ----------------------------------------------------------------------------------------
# 119.1253-deduplicate-list.py
# ----------------------------------------------------------------------------------------
x = list(set(x)) # argument:, argument:x, assignment:list, assignment_lhs_identifier:x, assignment_rhs_atom:x, composition, external_free_call:list, external_free_call:set, flat_style, free_call:list, free_call:set, global_scope:x, imperative_style, loaded_variable:x, node:Assign, node:Call, node:Name, one_liner_style, scope:x, single_assignment:x, variety:*, whole_span:1

# ----------------------------------------------------------------------------------------
# 119.3263-deduplicate-list.py
# ----------------------------------------------------------------------------------------
elements = ["b", "a", "b", "c"] # assignment, assignment_lhs_identifier:elements, global_scope:elements (-> +7), global_scope:elements_unique (-> +7), global_scope:i (-> +7), global_scope:unique_set (-> +7), imperative_style (-> +7), literal:List, literal:Str, node:Assign, node:List, node:Name, node:Str, scope:elements (-> +7), scope:elements_unique (-> +7), scope:i (-> +7), scope:unique_set (-> +7), single_assignment:elements, variety:*** (-> +7), whole_span:8 (-> +7)
...
# ----------------------------------------------------------------------------------------
class Suit: # class:Suit (-> +1), global_scope:CLUBS (-> +1), global_scope:DIAMONDS (-> +1), global_scope:HEARTS (-> +1), global_scope:SPADES (-> +1), node:ClassDef (-> +1), object_oriented_style (-> +1), one_liner_style (-> +1), scope:CLUBS (-> +1), scope:DIAMONDS (-> +1), scope:HEARTS (-> +1), scope:SPADES (-> +1), variety:* (-> +1), whole_span:2 (-> +1)
...
# ----------------------------------------------------------------------------------------
from enum import Enum # global_scope:CLUBS (-> +5), global_scope:DIAMONDS (-> +5), global_scope:HEARTS (-> +5), global_scope:SPADES (-> +5), import:enum:Enum, import_module:enum, import_name:Enum, node:ImportFrom, object_oriented_style (-> +5), scope:CLUBS (-> +5), scope:DIAMONDS (-> +5), scope:HEARTS (-> +5), scope:SPADES (-> +5), variety:** (-> +5), whole_span:6 (-> +5)
...
# ----------------------------------------------------------------------------------------
assert isConsistent # flat_style, imperative_style, loaded_variable:isConsistent, node:Assert, node:Name, variety:*, whole_span:1

# ----------------------------------------------------------------------------------------
# 124.2152-binary-search-for-a-value-in-sorted-array.py
# ----------------------------------------------------------------------------------------
import bisect # import:bisect, import_module:bisect, node:Import, procedural_style (-> +3), variety:** (-> +3), whole_span:4 (-> +3)
...
# ----------------------------------------------------------------------------------------
import time # flat_style (-> +4), global_scope:t1 (-> +4), global_scope:t2 (-> +4), imperative_style (-> +4), import:time, import_module:time, node:Import, scope:t1 (-> +4), scope:t2 (-> +4), variety:** (-> +4), whole_span:5 (-> +4)
...
# ----------------------------------------------------------------------------------------
def foo(): # function:foo (-> +1), function_line_count:2 (-> +1), function_returning_something:foo (-> +1), function_without_parameters:foo (-> +1), functional_style (-> +1), node:FunctionDef (-> +1), one_liner_style (-> +1), pure_function:foo (-> +1), variety:* (-> +1), whole_span:2 (-> +1)
...
# ----------------------------------------------------------------------------------------
import imp # flat_style (-> +1), global_scope:foo (-> +1), imperative_style (-> +1), import:imp, import_module:imp, node:Import, one_liner_style (-> +1), scope:foo (-> +1), variety:* (-> +1), whole_span:2 (-> +1)
...
# ----------------------------------------------------------------------------------------
from collections import deque # import:collections:deque, import_module:collections, import_name:deque, node:ImportFrom, procedural_style (-> +8), variety:**** (-> +8), whole_span:9 (-> +8)
...
# ----------------------------------------------------------------------------------------
f1 if c1 else f2 if c2 else f3 if c3 else None # flat_style, imperative_style, literal:None, loaded_variable:c1, loaded_variable:c2, loaded_variable:c3, loaded_variable:f1, loaded_variable:f2, loaded_variable:f3, node:Expr, node:IfExp, node:Name, node:NameConstant, one_liner_style, variety:*, whole_span:1

# ----------------------------------------------------------------------------------------
# 131.2766-successive-conditions.py
# ----------------------------------------------------------------------------------------
if c1: # if (-> +5), imperative_style (-> +5), loaded_variable:c1, node:If (-> +5), node:Name, variety:** (-> +5), whole_span:6 (-> +5)
...
# ----------------------------------------------------------------------------------------
import timeit # flat_style (-> +1), global_scope:duration (-> +1), imperative_style (-> +1), import:timeit, import_module:timeit, node:Import, one_liner_style (-> +1), scope:duration (-> +1), variety:* (-> +1), whole_span:2 (-> +1)
...
# ----------------------------------------------------------------------------------------
ok = word.lower() in s.lower() # assignment, assignment_lhs_identifier:ok, assignment_rhs_atom:s, assignment_rhs_atom:word, comparison_operator:In, flat_style, global_scope:ok, imperative_style, loaded_variable:s, loaded_variable:word, member_call_method:lower, node:Assign, node:Attribute, node:Call, node:Compare, node:Name, one_liner_style, scope:ok, single_assignment:ok, variety:*, whole_span:1

# ----------------------------------------------------------------------------------------
# 134.1850-create-a-new-list.py
# ----------------------------------------------------------------------------------------
items = [a, b, c] # assignment, assignment_lhs_identifier:items, assignment_rhs_atom:a, assignment_rhs_atom:b, assignment_rhs_atom:c, flat_style, global_scope:items, imperative_style, literal:List, loaded_variable:a, loaded_variable:b, loaded_variable:c, node:Assign, node:List, node:Name, one_liner_style, scope:items, single_assignment:items, variety:*, whole_span:1

# ----------------------------------------------------------------------------------------
# 135.2158-remove-item-from-list-by-its-value.py
# ----------------------------------------------------------------------------------------
items.remove(x) # argument:x, flat_style, imperative_style, loaded_variable:items, loaded_variable:x, member_call:items:remove, member_call_method:remove, member_call_object:items, node:Attribute, node:Call, node:Expr, node:Name, one_liner_style, update:items:x, update_by_member_call:items:x, update_by_member_call_with:remove, update_with:remove, variety:*, whole_span:1

# ----------------------------------------------------------------------------------------
# 136.2141-remove-all-occurrences-of-a-value-from-a-list.py
# ----------------------------------------------------------------------------------------
newlist = [item for item in items if item != x] # assignment, assignment_lhs_identifier:newlist, assignment_rhs_atom:item, assignment_rhs_atom:items, assignment_rhs_atom:x, comparison_operator:NotEq, comprehension:List, comprehension_for_count:1, filtered_comprehension, flat_style, imperative_style, iteration_variable:item, loaded_variable:item, loaded_variable:items, loaded_variable:x, local_scope:item, local_scope:newlist, node:Assign, node:Compare, node:ListComp, node:Name, one_liner_style, scope:item, scope:newlist, single_assignment:newlist, variety:*, whole_span:1

# ----------------------------------------------------------------------------------------
# 137.1823-check-if-string-contains-only-digits.py
# ----------------------------------------------------------------------------------------
b = s.isdigit() # assignment:isdigit, assignment_lhs_identifier:b, assignment_rhs_atom:s, flat_style, global_scope:b, imperative_style, loaded_variable:s, member_call_method:isdigit, node:Assign, node:Attribute, node:Call, node:Name, one_liner_style, scope:b, single_assignment:b, variety:*, whole_span:1
...
# ----------------------------------------------------------------------------------------
s = hex(x) # argument:x, assignment:hex, assignment_lhs_identifier:s, assignment_rhs_atom:x, external_free_call:hex, flat_style, free_call:hex, global_scope:s, imperative_style, loaded_variable:x, node:Assign, node:Call, node:Name, one_liner_style, scope:s, single_assignment:s, variety:*, whole_span:1

# ----------------------------------------------------------------------------------------
# 143.2256-iterate-alternatively-over-two-lists.py
# ----------------------------------------------------------------------------------------
for pair in zip(item1, item2): # argument:item1, argument:item2, external_free_call:zip, for:pair (-> +1), free_call:zip, global_scope:pair (-> +1), imperative_style (-> +1), iteration_variable:pair, loaded_variable:item1, loaded_variable:item2, loop:for (-> +1), loop_with_late_exit:for (-> +1), node:Call, node:For (-> +1), node:Name, scope:pair (-> +1), variety:* (-> +1), whole_span:2 (-> +1)
...
# ----------------------------------------------------------------------------------------
import os # flat_style (-> +1), global_scope:b (-> +1), imperative_style (-> +1), import:os, import_module:os, node:Import, one_liner_style (-> +1), scope:b (-> +1), variety:* (-> +1), whole_span:2 (-> +1)
...
# ----------------------------------------------------------------------------------------
import sys, logging # flat_style (-> +5), global_scope:logger (-> +5), imperative_style (-> +5), import:logging, import:sys, import_module:logging, import_module:sys, node:Import, scope:logger (-> +5), variety:** (-> +5), whole_span:6 (-> +5)
...
# ----------------------------------------------------------------------------------------
import locale # flat_style (-> +3), global_scope:f (-> +3), global_scope:s (-> +3), imperative_style (-> +3), import:locale, import_module:locale, node:Import, scope:f (-> +3), scope:s (-> +3), variety:** (-> +3), whole_span:4 (-> +3)
...
# ----------------------------------------------------------------------------------------
f = float(s) # argument:s, assignment:float, assignment_lhs_identifier:f, assignment_rhs_atom:s, external_free_call:float, flat_style, free_call:float, global_scope:f, imperative_style, loaded_variable:s, node:Assign, node:Call, node:Name, one_liner_style, scope:f, single_assignment:f, variety:*, whole_span:1

# ----------------------------------------------------------------------------------------
# 146.2739-convert-string-to-floating-point-number.py
# ----------------------------------------------------------------------------------------
float("1.3") # argument:, external_free_call:float, flat_style, free_call:float, free_call_without_result:float, imperative_style, literal:Str, node:Call, node:Expr, node:Name, node:Str, one_liner_style, variety:*, whole_span:1

# ----------------------------------------------------------------------------------------
# 147.2171-remove-all-non-ascii-characters.py
# ----------------------------------------------------------------------------------------
import re # flat_style (-> +1), global_scope:t (-> +1), imperative_style (-> +1), import:re, import_module:re, node:Import, one_liner_style (-> +1), scope:t (-> +1), variety:* (-> +1), whole_span:2 (-> +1)
...
# ----------------------------------------------------------------------------------------
import pathlib # flat_style (-> +2), global_scope:path (-> +2), imperative_style (-> +2), import:pathlib, import_module:pathlib, node:Import, scope:path (-> +2), variety:** (-> +2), whole_span:3 (-> +2)
...
# ----------------------------------------------------------------------------------------
PLANET = "Earth" # assignment, assignment_lhs_identifier:PLANET, flat_style, global_scope:PLANET, imperative_style, literal:Str, node:Assign, node:Name, node:Str, one_liner_style, scope:PLANET, single_assignment:PLANET, variety:*, whole_span:1

# ----------------------------------------------------------------------------------------
# 158.2163-random-sublist.py
# ----------------------------------------------------------------------------------------
import random # flat_style (-> +1), global_scope:y (-> +1), imperative_style (-> +1), import:random, import_module:random, node:Import, one_liner_style (-> +1), scope:y (-> +1), variety:* (-> +1), whole_span:2 (-> +1)
...
# 159.2281-trie.py
# ----------------------------------------------------------------------------------------
class Trie: # class:Trie (-> +4), class_method_count:1 (-> +4), node:ClassDef (-> +4), object_oriented_style (-> +4), variety:** (-> +4), whole_span:5 (-> +4)
...
# ----------------------------------------------------------------------------------------
import sys # imperative_style (-> +4), import:sys, import_module:sys, node:Import, variety:** (-> +4), whole_span:5 (-> +4)
...
# ----------------------------------------------------------------------------------------
import webbrowser # flat_style (-> +1), imperative_style (-> +1), import:webbrowser, import_module:webbrowser, node:Import, one_liner_style (-> +1), variety:* (-> +1), whole_span:2 (-> +1)
...
# ----------------------------------------------------------------------------------------
x = items[-1] # assignment, assignment_lhs_identifier:x, assignment_rhs_atom:-1, assignment_rhs_atom:items, flat_style, global_scope:x, imperative_style, index:-1, literal:-1, loaded_variable:items, negative_index:-1, node:Assign, node:Name, node:Num, node:Subscript, one_liner_style, scope:x, single_assignment:x, variety:*, whole_span:1

# ----------------------------------------------------------------------------------------
# 166.2272-concatenate-two-lists.py
# ----------------------------------------------------------------------------------------
ab = a + b # addition_operator, assignment:Add, assignment_lhs_identifier:ab, assignment_rhs_atom:a, assignment_rhs_atom:b, binary_operator:Add, flat_style, global_scope:ab, imperative_style, loaded_variable:a, loaded_variable:b, node:Assign, node:BinOp, node:Name, one_liner_style, scope:ab, single_assignment:ab, variety:*, whole_span:1

# ----------------------------------------------------------------------------------------
# 167.2611-trim-prefix.py
# ----------------------------------------------------------------------------------------
t = s[s.startswith(p) and len(p) :] # argument:p, assignment, assignment_lhs_identifier:t, assignment_rhs_atom:p, assignment_rhs_atom:s, boolean_operator:And, external_free_call:len, flat_style, free_call:len, global_scope:t, imperative_style, loaded_variable:p, loaded_variable:s, member_call_method:startswith, node:Assign, node:Attribute, node:BoolOp, node:Call, node:Name, node:Subscript, one_liner_style, scope:t, single_assignment:t, slice:_::, slice_lower:_, slice_step:, slice_upper:, variety:*, whole_span:1

# ----------------------------------------------------------------------------------------
# 167.3175-trim-prefix.py
# ----------------------------------------------------------------------------------------
t = s.lstrip(p) # argument:p, assignment:lstrip, assignment_lhs_identifier:t, assignment_rhs_atom:p, assignment_rhs_atom:s, flat_style, global_scope:t, imperative_style, loaded_variable:p, loaded_variable:s, member_call_method:lstrip, node:Assign, node:Attribute, node:Call, node:Name, one_liner_style, scope:t, single_assignment:t, variety:*, whole_span:1

# ----------------------------------------------------------------------------------------
# 168.2277-trim-suffix.py
# ----------------------------------------------------------------------------------------
t = s.rsplit(w, 1)[0] # argument:1, argument:w, assignment, assignment_lhs_identifier:t, assignment_rhs_atom:0, assignment_rhs_atom:1, assignment_rhs_atom:s, assignment_rhs_atom:w, flat_style, global_scope:t, imperative_style, index:0, literal:0, literal:1, loaded_variable:s, loaded_variable:w, member_call:s:rsplit, member_call_method:rsplit, member_call_object:s, node:Assign, node:Attribute, node:Call, node:Name, node:Num, node:Subscript, one_liner_style, scope:t, single_assignment:t, variety:*, whole_span:1

# ----------------------------------------------------------------------------------------
# 168.3174-trim-suffix.py
# ----------------------------------------------------------------------------------------
t = s.rstrip(w) # argument:w, assignment:rstrip, assignment_lhs_identifier:t, assignment_rhs_atom:s, assignment_rhs_atom:w, flat_style, global_scope:t, imperative_style, loaded_variable:s, loaded_variable:w, member_call_method:rstrip, node:Assign, node:Attribute, node:Call, node:Name, one_liner_style, scope:t, single_assignment:t, variety:*, whole_span:1

# ----------------------------------------------------------------------------------------
# 169.2233-string-length.py
# ----------------------------------------------------------------------------------------
n = len(s) # argument:s, assignment:len, assignment_lhs_identifier:n, assignment_rhs_atom:s, external_free_call:len, flat_style, free_call:len, global_scope:n, imperative_style, loaded_variable:s, node:Assign, node:Call, node:Name, one_liner_style, scope:n, single_assignment:n, variety:*, whole_span:1

# ----------------------------------------------------------------------------------------
# 170.2275-get-map-size.py
# ----------------------------------------------------------------------------------------
n = len(mymap) # argument:mymap, assignment:len, assignment_lhs_identifier:n, assignment_rhs_atom:mymap, external_free_call:len, flat_style, free_call:len, global_scope:n, imperative_style, loaded_variable:mymap, node:Assign, node:Call, node:Name, one_liner_style, scope:n, single_assignment:n, variety:*, whole_span:1

# ----------------------------------------------------------------------------------------
# 171.2446-add-an-element-at-the-end-of-a-list.py
# ----------------------------------------------------------------------------------------
s.append(x) # argument:x, flat_style, imperative_style, loaded_variable:s, loaded_variable:x, member_call:s:append, member_call_method:append, member_call_object:s, node:Attribute, node:Call, node:Expr, node:Name, one_liner_style, update:s:x, update_by_member_call:s:x, update_by_member_call_with:append, update_with:append, variety:*, whole_span:1

# ----------------------------------------------------------------------------------------
# 172.2442-insert-entry-in-map.py
# ----------------------------------------------------------------------------------------
m[k] = v # assignment, assignment_lhs_identifier:m, assignment_rhs_atom:v, flat_style, imperative_style, index:k, loaded_variable:k, loaded_variable:m, loaded_variable:v, node:Assign, node:Name, node:Subscript, one_liner_style, subscript_assignment:Index, variety:*, whole_span:1
...
# ----------------------------------------------------------------------------------------
s = a.hex() # assignment:hex, assignment_lhs_identifier:s, assignment_rhs_atom:a, flat_style, global_scope:s, imperative_style, loaded_variable:a, member_call_method:hex, node:Assign, node:Attribute, node:Call, node:Name, one_liner_style, scope:s, single_assignment:s, variety:*, whole_span:1

# ----------------------------------------------------------------------------------------
# 176.2614-hex-string-to-byte-array.py
# ----------------------------------------------------------------------------------------
a = bytearray.fromhex(s) # argument:s, assignment:fromhex, assignment_lhs_identifier:a, assignment_rhs_atom:bytearray, assignment_rhs_atom:s, flat_style, global_scope:a, imperative_style, loaded_variable:bytearray, loaded_variable:s, member_call_method:fromhex, node:Assign, node:Attribute, node:Call, node:Name, one_liner_style, scope:a, single_assignment:a, variety:*, whole_span:1

# ----------------------------------------------------------------------------------------
# 177.2709-find-files-with-a-given-list-of-filename-extensions.py
# ----------------------------------------------------------------------------------------
import os # flat_style (-> +2), global_scope:L (-> +2), global_scope:extensions (-> +2), imperative_style (-> +2), import:os, import_module:os, node:Import, scope:L (-> +2), scope:extensions (-> +2), variety:* (-> +2), whole_span:3 (-> +2)
...
# ----------------------------------------------------------------------------------------
import glob # flat_style (-> +2), imperative_style (-> +2), import:glob, import_module:glob, node:Import, one_liner_style (-> +2), variety:* (-> +2), whole_span:3 (-> +2)
...
# ----------------------------------------------------------------------------------------
requests # flat_style (-> +6), global_scope:content (-> +6), global_scope:content_type (-> +6), global_scope:data (-> +6), global_scope:headers (-> +6), global_scope:r (-> +6), global_scope:status_code (-> +6), imperative_style (-> +6), loaded_variable:requests, node:Expr, node:Name, scope:content (-> +6), scope:content_type (-> +6), scope:data (-> +6), scope:headers (-> +6), scope:r (-> +6), scope:status_code (-> +6), variety:** (-> +6), whole_span:7 (-> +6)
...
# ----------------------------------------------------------------------------------------
from datetime import date, timedelta # flat_style (-> +1), imperative_style (-> +1), import:datetime:date, import:datetime:timedelta, import_module:datetime, import_name:date, import_name:timedelta, node:ImportFrom, one_liner_style (-> +1), variety:* (-> +1), whole_span:2 (-> +1)
...
# ----------------------------------------------------------------------------------------
import threading # flat_style (-> +2), global_scope:timer (-> +2), imperative_style (-> +2), import:threading, import_module:threading, node:Import, scope:timer (-> +2), variety:* (-> +2), whole_span:3 (-> +2)
...
# ----------------------------------------------------------------------------------------
class UnionFind: # class:UnionFind (-> +14), class_method_count:4 (-> +14), node:ClassDef (-> +14), object_oriented_style (-> +14), variety:*** (-> +14), whole_span:15 (-> +14)
...
# ----------------------------------------------------------------------------------------
import numpy as np # flat_style (-> +1), global_scope:c (-> +1), imperative_style (-> +1), import:numpy, import_module:numpy, node:Import, one_liner_style (-> +1), scope:c (-> +1), variety:* (-> +1), whole_span:2 (-> +1)
...
# ----------------------------------------------------------------------------------------
with open(path) as f: # argument:path, external_free_call:open, flat_style (-> +1), free_call:open, global_scope:lines (-> +1), imperative_style (-> +1), loaded_variable:path, node:Call, node:Name, node:With (-> +1), scope:lines (-> +1), variety:* (-> +1), whole_span:2 (-> +1)

...
# ----------------------------------------------------------------------------------------
for i in range(10): # call_argument:10, external_free_call:range, for:i (-> +1), for_range:10 (-> +1), free_call:range, global_scope:i (-> +1), imperative_style (-> +1), iteration_variable:i, literal:10, loop:for (-> +1), loop_with_late_exit:for (-> +1), magic_number:10, node:Call, node:For (-> +1), node:Name, node:Num, range:10, scope:i (-> +1), whole_span:2 (-> +1)
...
# ----------------------------------------------------------------------------------------
def finish(name): # function:finish (-> +1), function_parameter:name, function_parameter_flavor:arg, function_returning_nothing:finish (-> +1), local_scope:name (-> +1), node:FunctionDef (-> +1), node:arg, one_liner_style (-> +1), procedural_style (-> +1), scope:name (-> +1), whole_span:2 (-> +1)
    print("My job here is done. Goodbye " + name) # binary_operator:Add, call_argument:, concatenation_operator:Str, external_free_call:print, free_call:print, free_call_without_result:print, literal:Str, loaded_variable:name, node:BinOp, node:Call, node:Expr, node:Name, node:Str
...
# ----------------------------------------------------------------------------------------
def square(x): # function:square (-> +1), function_parameter:x, function_parameter_flavor:arg, function_returning_something:square (-> +1), functional_style (-> +1), local_scope:x (-> +1), node:FunctionDef (-> +1), node:arg, one_liner_style (-> +1), pure_function:square (-> +1), scope:x (-> +1), whole_span:2 (-> +1)
...
@dataclass # loaded_variable:dataclass, node:ClassDef, node:Name
class Point:
...
# ----------------------------------------------------------------------------------------
for x in items: # for:x (-> +1), for_each:x (-> +1), global_scope:x (-> +1), imperative_style (-> +1), iteration_variable:x, loaded_variable:items, loop:for (-> +1), loop_with_late_exit:for (-> +1), node:For (-> +1), node:Name, scope:x (-> +1), whole_span:2 (-> +1)
    doSomething(x) # call_argument:x, external_free_call:doSomething, free_call:doSomething, free_call_without_result:doSomething, loaded_variable:x, node:Call, node:Expr, node:Name

# ----------------------------------------------------------------------------------------
# 007.0183-iterate-over-list-indexes-and-values.py
# ----------------------------------------------------------------------------------------
for i, x in enumerate(items): # call_argument:items, external_free_call:enumerate, for:i (-> +1), for:x (-> +1), for_indexes_elements:i (-> +1), free_call:enumerate, global_scope:i (-> +1), global_scope:x (-> +1), imperative_style (-> +1), iteration_variable:i, iteration_variable:x, literal:Tuple, loaded_variable:items, loop:for (-> +1), loop_with_late_exit:for (-> +1), node:Call, node:For (-> +1), node:Name, node:Tuple, scope:i (-> +1), scope:x (-> +1), whole_span:2 (-> +1)
    print(i, x) # call_argument:i, call_argument:x, external_free_call:print, free_call:print, free_call_without_result:print, loaded_variable:i, loaded_variable:x, node:Call, node:Expr, node:Name

# ----------------------------------------------------------------------------------------
# 008.0039-initialize-a-new-map-associative-array.py
# ----------------------------------------------------------------------------------------
x = {"one": 1, "two": 2} # assignment, assignment_lhs_identifier:x, assignment_rhs_atom:1, assignment_rhs_atom:2, flat_style, global_scope:x, imperative_style, literal:1, literal:2, literal:Dict, literal:Str, node:Assign, node:Dict, node:Name, node:Num, node:Str, one_liner_style, scope:x, single_assignment:x, whole_span:1

# ----------------------------------------------------------------------------------------
# 009.1410-create-a-binary-tree-data-structure.py
# ----------------------------------------------------------------------------------------
class Node: # node:ClassDef (-> +4), object_oriented_style (-> +4), whole_span:5 (-> +4)
    def __init__(self, data): # function:__init__ (-> +3), function_parameter:data, function_parameter:self, function_parameter_flavor:arg, function_returning_nothing:__init__ (-> +3), instance_method:__init__ (-> +3), local_scope:data (-> +3), local_scope:self (-> +3), method:__init__ (-> +3), node:FunctionDef (-> +3), node:arg, scope:data (-> +3), scope:self (-> +3)
...
shuffle(x) # call_argument:x, external_free_call:shuffle, free_call:shuffle, free_call_without_result:shuffle, loaded_variable:x, node:Call, node:Expr, node:Name
...
random.shuffle(list) # call_argument:list, loaded_variable:list, loaded_variable:random, member_call:random:shuffle, member_call_method:shuffle, member_call_object:random, node:Attribute, node:Call, node:Expr, node:Name

# ----------------------------------------------------------------------------------------
# 011.0047-pick-a-random-element-from-a-list.py
# ----------------------------------------------------------------------------------------
import random # flat_style (-> +1), imperative_style (-> +1), import:random, import_module:random, node:Import, one_liner_style (-> +1), whole_span:2 (-> +1)
random.choice(x) # call_argument:x, loaded_variable:random, loaded_variable:x, member_call:random:choice, member_call_method:choice, member_call_object:random, node:Attribute, node:Call, node:Expr, node:Name

# ----------------------------------------------------------------------------------------
# 012.0181-check-if-list-contains-a-value.py
# ----------------------------------------------------------------------------------------
x in list # comparison_operator:In, flat_style, imperative_style, loaded_variable:list, loaded_variable:x, node:Compare, node:Expr, node:Name, one_liner_style, whole_span:1

# ----------------------------------------------------------------------------------------
# 013.0574-iterate-over-map-keys-and-values.py
# ----------------------------------------------------------------------------------------
for k, v in mymap.items(): # for:k (-> +1), for:v (-> +1), global_scope:k (-> +1), global_scope:v (-> +1), imperative_style (-> +1), iteration_variable:k, iteration_variable:v, literal:Tuple, loaded_variable:mymap, loop:for (-> +1), loop_with_late_exit:for (-> +1), member_call_method:items, node:Attribute, node:Call, node:For (-> +1), node:Name, node:Tuple, scope:k (-> +1), scope:v (-> +1), whole_span:2 (-> +1)
    print(k, v) # call_argument:k, call_argument:v, external_free_call:print, free_call:print, free_call_without_result:print, loaded_variable:k, loaded_variable:v, node:Call, node:Expr, node:Name
...
def pick(a, b): # function:pick (-> +1), function_parameter:a, function_parameter:b, function_parameter_flavor:arg, function_returning_something:pick (-> +1), local_scope:a (-> +1), local_scope:b (-> +1), node:FunctionDef (-> +1), node:arg, pure_function:pick (-> +1), scope:a (-> +1), scope:b (-> +1)
    return random.randrange(a, b) # call_argument:a, call_argument:b, loaded_variable:a, loaded_variable:b, loaded_variable:random, member_call:random:randrange, member_call_method:randrange, member_call_object:random, node:Attribute, node:Call, node:Name, node:Return, return

# ----------------------------------------------------------------------------------------
# 014.3410-pick-uniformly-a-random-floating-point-number-in-ab.py
# ----------------------------------------------------------------------------------------
import random # flat_style (-> +1), imperative_style (-> +1), import:random, import_module:random, node:Import, one_liner_style (-> +1), whole_span:2 (-> +1)
random.uniform(a, b) # call_argument:a, call_argument:b, loaded_variable:a, loaded_variable:b, loaded_variable:random, member_call:random:uniform, member_call_method:uniform, member_call_object:random, node:Attribute, node:Call, node:Expr, node:Name

# ----------------------------------------------------------------------------------------
# 015.0184-pick-uniformly-a-random-integer-in-ab.py
# ----------------------------------------------------------------------------------------
import random # flat_style (-> +1), imperative_style (-> +1), import:random, import_module:random, node:Import, one_liner_style (-> +1), whole_span:2 (-> +1)
random.randint(a, b) # call_argument:a, call_argument:b, loaded_variable:a, loaded_variable:b, loaded_variable:random, member_call:random:randint, member_call_method:randint, member_call_object:random, node:Attribute, node:Call, node:Expr, node:Name

# ----------------------------------------------------------------------------------------
# 016.1530-depth-first-traversing-of-a-binary-tree.py
# ----------------------------------------------------------------------------------------
def dfs(bt): # body_recursive_function:dfs (-> +5), function:dfs (-> +5), function_parameter:bt, function_parameter_flavor:arg, function_returning_nothing:dfs (-> +5), local_scope:bt (-> +5), node:FunctionDef (-> +5), node:arg, procedural_style (-> +5), recursive_call_count:2 (-> +5), recursive_function:dfs (-> +5), scope:bt (-> +5), whole_span:6 (-> +5)
    ...
    dfs(bt.left) # call_argument:, free_call:dfs, free_call_without_result:dfs, internal_free_call:dfs, loaded_variable:bt, node:Attribute, node:Call, node:Expr, node:Name
    f(bt) # call_argument:bt, external_free_call:f, free_call:f, free_call_without_result:f, loaded_variable:bt, node:Call, node:Expr, node:Name
    dfs(bt.right) # call_argument:, free_call:dfs, free_call_without_result:dfs, internal_free_call:dfs, loaded_variable:bt, node:Attribute, node:Call, node:Expr, node:Name

# ----------------------------------------------------------------------------------------
# 017.1103-create-a-tree-data-structure.py
# ----------------------------------------------------------------------------------------
class Node(object): # loaded_variable:object, node:ClassDef (-> +3), node:Name, object_oriented_style (-> +3), whole_span:4 (-> +3)
    def __init__(self, value, *children): # function:__init__ (-> +2), function_parameter:children, function_parameter:self, function_parameter:value, function_parameter_flavor:arg, function_parameter_flavor:vararg, function_returning_nothing:__init__ (-> +2), instance_method:__init__ (-> +2), local_scope:children (-> +2), local_scope:self (-> +2), local_scope:value (-> +2), method:__init__ (-> +2), node:FunctionDef (-> +2), node:arg, scope:children (-> +2), scope:self (-> +2), scope:value (-> +2)
        ...
        self.children = list(children) # assignment:list, assignment_lhs_identifier:self, assignment_rhs_atom:children, call_argument:children, external_free_call:list, free_call:list, loaded_variable:children, loaded_variable:self, node:Assign, node:Attribute, node:Call, node:Name

# ----------------------------------------------------------------------------------------
# 018.2084-depth-first-traversing-of-a-tree.py
# ----------------------------------------------------------------------------------------
def DFS(f, root): # body_recursive_function:DFS (-> +3), function:DFS (-> +3), function_parameter:f, function_parameter:root, function_parameter_flavor:arg, function_returning_nothing:DFS (-> +3), higher_order_function:f (-> +3), local_scope:child (-> +3), local_scope:f (-> +3), local_scope:root (-> +3), node:FunctionDef (-> +3), node:arg, procedural_style (-> +3), recursive_call_count:1 (-> +3), recursive_function:DFS (-> +3), scope:child (-> +3), scope:f (-> +3), scope:root (-> +3), whole_span:4 (-> +3)
    f(root) # call_argument:root, external_free_call:f, free_call:f, free_call_without_result:f, loaded_variable:root, node:Call, node:Expr, node:Name
    for child in root: # for:child (-> +1), for_each:child (-> +1), iteration_variable:child, loaded_variable:root, loop:for (-> +1), loop_with_late_exit:for (-> +1), node:For (-> +1), node:Name
        DFS(f, child) # call_argument:child, call_argument:f, free_call:DFS, free_call_without_result:DFS, internal_free_call:DFS, loaded_variable:child, loaded_variable:f, node:Call, node:Expr, node:Name

# ----------------------------------------------------------------------------------------
# 019.0197-reverse-a-list.py
# ----------------------------------------------------------------------------------------
x = reversed(x) # assignment:reversed, assignment_lhs_identifier:x, assignment_rhs_atom:x, call_argument:x, external_free_call:reversed, flat_style, free_call:reversed, global_scope:x, imperative_style, loaded_variable:x, node:Assign, node:Call, node:Name, one_liner_style, scope:x, single_assignment:x, whole_span:1
...
# ----------------------------------------------------------------------------------------
x.reverse() # flat_style, imperative_style, loaded_variable:x, member_call:x:reverse, member_call_method:reverse, member_call_object:x, node:Attribute, node:Call, node:Expr, node:Name, one_liner_style, whole_span:1

# ----------------------------------------------------------------------------------------
# 020.0573-return-two-values.py
# ----------------------------------------------------------------------------------------
def search(m, x): # function:search (-> +3), function_parameter:m, function_parameter:x, function_parameter_flavor:arg, function_returning_something:search (-> +3), impure_function:search (-> +3), local_scope:idx (-> +3), local_scope:item (-> +3), local_scope:m (-> +3), local_scope:x (-> +3), node:FunctionDef (-> +3), node:arg, procedural_style (-> +3), scope:idx (-> +3), scope:item (-> +3), scope:m (-> +3), scope:x (-> +3), whole_span:4 (-> +3)
    for idx, item in enumerate(m): # call_argument:m, external_free_call:enumerate, for:idx (-> +2), for:item (-> +2), for_indexes_elements:idx (-> +2), free_call:enumerate, iteration_variable:idx, iteration_variable:item, literal:Tuple, loaded_variable:m, loop:for (-> +2), loop_with_early_exit:for:return (-> +2), loop_with_return:for (-> +2), node:Call, node:For (-> +2), node:Name, node:Tuple
        if x in item: # comparison_operator:In, if (-> +1), if_test_atom:item, if_test_atom:x, if_without_else (-> +1), loaded_variable:item, loaded_variable:x, node:Compare, node:If (-> +1), node:Name
            return idx, item.index(x) # call_argument:x, if_then_branch, literal:Tuple, loaded_variable:idx, loaded_variable:item, loaded_variable:x, member_call_method:index, node:Attribute, node:Call, node:Name, node:Return, node:Tuple, return

# ----------------------------------------------------------------------------------------
# 021.0084-swap-values-of-variables-a-and-b.py
# ----------------------------------------------------------------------------------------
a, b = b, a # assignment, assignment_lhs_identifier:a, assignment_lhs_identifier:b, assignment_rhs_atom:a, assignment_rhs_atom:b, flat_style, global_scope:a, global_scope:b, imperative_style, literal:Tuple, loaded_variable:a, loaded_variable:b, node:Assign, node:Name, node:Tuple, one_liner_style, parallel_assignment:2, scope:a, scope:b, swap, update:a:b, update:b:a, update_by_assignment:a:b, update_by_assignment:b:a, update_by_assignment_with, update_with, whole_span:1

# ----------------------------------------------------------------------------------------
# 022.0243-convert-string-to-integer.py
# ----------------------------------------------------------------------------------------
i = int(s) # assignment:int, assignment_lhs_identifier:i, assignment_rhs_atom:s, call_argument:s, external_free_call:int, flat_style, free_call:int, global_scope:i, imperative_style, loaded_variable:s, node:Assign, node:Call, node:Name, one_liner_style, scope:i, single_assignment:i, whole_span:1

# ----------------------------------------------------------------------------------------
# 023.1102-convert-real-number-to-string-with-2-decimal-places.py
# ----------------------------------------------------------------------------------------
s = "{:.2f}".format(x) # assignment:format, assignment_lhs_identifier:s, assignment_rhs_atom:x, call_argument:x, flat_style, global_scope:s, imperative_style, literal:Str, loaded_variable:x, member_call_method:format, node:Assign, node:Attribute, node:Call, node:Name, node:Str, one_liner_style, scope:s, single_assignment:s, whole_span:1

# ----------------------------------------------------------------------------------------
# 024.0664-assign-to-string-the-japanese-word-.py
# ----------------------------------------------------------------------------------------
s = "ネコ" # assignment, assignment_lhs_identifier:s, flat_style, global_scope:s, imperative_style, literal:Str, node:Assign, node:Name, node:Str, one_liner_style, scope:s, single_assignment:s, special_literal_string:ネコ, whole_span:1

# ----------------------------------------------------------------------------------------
# 025.0195-send-a-value-to-another-thread.py
# ----------------------------------------------------------------------------------------
import Queue # flat_style (-> +5), global_scope:q (-> +5), global_scope:t (-> +5), imperative_style (-> +5), import:Queue, import_module:Queue, node:Import, scope:q (-> +5), scope:t (-> +5), whole_span:6 (-> +5)
...
t = Thread(target=worker) # assignment:Thread, assignment_lhs_identifier:t, assignment_rhs_atom:worker, call_argument:worker, call_keyword_argument:target, external_free_call:Thread, free_call:Thread, loaded_variable:worker, node:Assign, node:Call, node:Name, single_assignment:t
...
for i in range(1, 1001): # call_argument:1, call_argument:1001, external_free_call:range, for:i (-> +1), for_range:1:1001 (-> +1), free_call:range, iteration_variable:i, literal:1, literal:1001, loop:for (-> +1), loop_with_late_exit:for (-> +1), magic_number:1001, node:Call, node:For (-> +1), node:Name, node:Num, range:1:1001
    pool.apply_async(f, [i]) # call_argument:, call_argument:f, literal:List, loaded_variable:f, loaded_variable:i, loaded_variable:pool, member_call:pool:apply_async, member_call_method:apply_async, member_call_object:pool, node:Attribute, node:Call, node:Expr, node:List, node:Name

# ----------------------------------------------------------------------------------------
# 031.0188-recursive-factorial-simple.py
# ----------------------------------------------------------------------------------------
def f(i): # body_recursive_function:f (-> +4), function:f (-> +4), function_parameter:i, function_parameter_flavor:arg, function_returning_something:f (-> +4), functional_style (-> +4), local_scope:i (-> +4), node:FunctionDef (-> +4), node:arg, pure_function:f (-> +4), recursive_call_count:1 (-> +4), recursive_function:f (-> +4), scope:i (-> +4), whole_span:5 (-> +4)
    if i == 0: # comparison_operator:Eq, if (-> +3), if_test_atom:0, if_test_atom:i, literal:0, loaded_variable:i, node:Compare, node:If (-> +3), node:Name, node:Num
        ...
        return i * f(i - 1) # binary_operator:Mult, binary_operator:Sub, call_argument:, free_call:f, if_else_branch, internal_free_call:f, literal:1, loaded_variable:i, multiplication_operator, node:BinOp, node:Call, node:Name, node:Num, node:Return, return

# ----------------------------------------------------------------------------------------
# 032.0196-integer-exponentiation-by-squaring.py
# ----------------------------------------------------------------------------------------
def exp(x, n): # function:exp (-> +1), function_parameter:n, function_parameter:x, function_parameter_flavor:arg, function_returning_something:exp (-> +1), functional_style (-> +1), local_scope:n (-> +1), local_scope:x (-> +1), node:FunctionDef (-> +1), node:arg, one_liner_style (-> +1), pure_function:exp (-> +1), scope:n (-> +1), scope:x (-> +1), whole_span:2 (-> +1)
...
# ----------------------------------------------------------------------------------------
import threading # flat_style (-> +6), global_scope:lock (-> +6), global_scope:x (-> +6), imperative_style (-> +6), import:threading, import_module:threading, node:Import, scope:lock (-> +6), scope:x (-> +6), whole_span:7 (-> +6)
...
try: # node:Try (-> +3)
    x = f(x) # assignment:f, assignment_lhs_identifier:x, assignment_rhs_atom:x, call_argument:x, external_free_call:f, free_call:f, loaded_variable:x, node:Assign, node:Call, node:Name, single_assignment:x
finally:
...
# ----------------------------------------------------------------------------------------
class T(object): # global_scope:x (-> +2), loaded_variable:object, node:ClassDef (-> +1), node:Name, object_oriented_style (-> +2), scope:x (-> +2), whole_span:3 (-> +2)
    pass # node:Pass
...
# ----------------------------------------------------------------------------------------
def compose(f, g): # function:compose (-> +1), function_parameter:f, function_parameter:g, function_parameter_flavor:arg, function_returning_something:compose (-> +1), functional_style (-> +1), higher_order_function:f (-> +1), higher_order_function:g (-> +1), local_scope:a (-> +1), local_scope:f (-> +1), local_scope:g (-> +1), node:FunctionDef (-> +1), node:arg, one_liner_style (-> +1), pure_function:compose (-> +1), scope:a (-> +1), scope:f (-> +1), scope:g (-> +1), whole_span:2 (-> +1)
    return lambda a: g(f(a)) # call_argument:, call_argument:a, composition, external_free_call:f, external_free_call:g, free_call:f, free_call:g, function_parameter:a, function_parameter_flavor:arg, loaded_variable:a, node:Call, node:Lambda, node:Name, node:Return, node:arg, return

# ----------------------------------------------------------------------------------------
# 036.0670-first-class-function--generic-composition.py
# ----------------------------------------------------------------------------------------
def compose(f, g): # function:compose (-> +1), function_parameter:f, function_parameter:g, function_parameter_flavor:arg, function_returning_something:compose (-> +1), functional_style (-> +1), higher_order_function:f (-> +1), higher_order_function:g (-> +1), local_scope:f (-> +1), local_scope:g (-> +1), local_scope:x (-> +1), node:FunctionDef (-> +1), node:arg, one_liner_style (-> +1), pure_function:compose (-> +1), scope:f (-> +1), scope:g (-> +1), scope:x (-> +1), whole_span:2 (-> +1)
    return lambda x: g(f(x)) # call_argument:, call_argument:x, composition, external_free_call:f, external_free_call:g, free_call:f, free_call:g, function_parameter:x, function_parameter_flavor:arg, loaded_variable:x, node:Call, node:Lambda, node:Name, node:Return, node:arg, return
...
def f(a): # closure:f (-> +3), function:f (-> +3), function_parameter:a, function_parameter_flavor:arg, function_returning_something:f (-> +3), local_scope:a (-> +3), nested_function:f (-> +3), node:FunctionDef (-> +3), node:arg, pure_function:f (-> +3), scope:a (-> +3)
    def add(b): # access_outer_scope:a (-> +1), function:add (-> +1), function_parameter:b, function_parameter_flavor:arg, function_returning_something:add (-> +1), local_scope:b (-> +1), node:FunctionDef (-> +1), node:arg, pure_function:add (-> +1), scope:b (-> +1)
...
# ----------------------------------------------------------------------------------------
t = s[i:j] # assignment, assignment_lhs_identifier:t, assignment_rhs_atom:i, assignment_rhs_atom:j, assignment_rhs_atom:s, flat_style, global_scope:t, imperative_style, loaded_variable:i, loaded_variable:j, loaded_variable:s, node:Assign, node:Name, node:Subscript, one_liner_style, scope:t, single_assignment:t, slice:i:j:, slice_lower:i, slice_step:, slice_upper:j, whole_span:1

# ----------------------------------------------------------------------------------------
# 039.0571-check-if-string-contains-a-word.py
# ----------------------------------------------------------------------------------------
ok = word in s # assignment, assignment_lhs_identifier:ok, assignment_rhs_atom:s, assignment_rhs_atom:word, comparison_operator:In, flat_style, global_scope:ok, imperative_style, loaded_variable:s, loaded_variable:word, node:Assign, node:Compare, node:Name, one_liner_style, scope:ok, single_assignment:ok, whole_span:1

# ----------------------------------------------------------------------------------------
# 040.2279-graph-with-adjacency-lists.py
# ----------------------------------------------------------------------------------------
from collections import defaultdict # global_scope:G (-> +12), import:collections:defaultdict, import_module:collections, import_name:defaultdict, node:ImportFrom, object_oriented_style (-> +12), scope:G (-> +12), whole_span:13 (-> +12)
class Vertex(set): # loaded_variable:set, node:ClassDef (-> +1), node:Name
    pass # node:Pass
class Graph(defaultdict): # loaded_variable:defaultdict, node:ClassDef (-> +8), node:Name
    def __init__(self, *paths): # function:__init__ (-> +3), function_parameter:paths, function_parameter:self, function_parameter_flavor:arg, function_parameter_flavor:vararg, function_returning_nothing:__init__ (-> +3), instance_method:__init__ (-> +3), local_scope:path (-> +3), local_scope:paths (-> +3), local_scope:self (-> +3), method:__init__ (-> +3), node:FunctionDef (-> +3), node:arg, scope:path (-> +3), scope:paths (-> +3), scope:self (-> +3)
        ...
        for path in paths: # for:path (-> +1), for_each:path (-> +1), iteration_variable:path, loaded_variable:paths, loop:for (-> +1), loop_with_late_exit:for (-> +1), node:For (-> +1), node:Name
            self.make_path(path) # call_argument:path, loaded_variable:path, loaded_variable:self, member_call:self:make_path, member_call_method:make_path, member_call_object:self, node:Attribute, node:Call, node:Expr, node:Name
    def make_path(self, labels): # function:make_path (-> +3), function_parameter:labels, function_parameter:self, function_parameter_flavor:arg, function_returning_nothing:make_path (-> +3), instance_method:make_path (-> +3), local_scope:l1 (-> +3), local_scope:l2 (-> +3), local_scope:labels (-> +3), local_scope:self (-> +3), method:make_path (-> +3), node:FunctionDef (-> +3), node:arg, scope:l1 (-> +3), scope:l2 (-> +3), scope:labels (-> +3), scope:self (-> +3)
        for l1, l2 in zip(labels, labels[1:]): # call_argument:, call_argument:labels, external_free_call:zip, for:l1 (-> +2), for:l2 (-> +2), free_call:zip, iteration_variable:l1, iteration_variable:l2, literal:1, literal:Tuple, loaded_variable:labels, loop:for (-> +2), loop_with_late_exit:for (-> +2), node:Call, node:For (-> +2), node:Name, node:Num, node:Subscript, node:Tuple, slice:1::, slice_lower:1, slice_step:, slice_upper:
            self[l1].add(l2) # call_argument:l2, index:l1, loaded_variable:l1, loaded_variable:l2, loaded_variable:self, member_call_method:add, node:Attribute, node:Call, node:Expr, node:Name, node:Subscript
            self[l2].add(l1) # call_argument:l1, index:l2, loaded_variable:l1, loaded_variable:l2, loaded_variable:self, member_call_method:add, node:Attribute, node:Call, node:Expr, node:Name, node:Subscript
...
# ----------------------------------------------------------------------------------------
for v in a: # for:v (-> +7), for_each:v (-> +7), global_scope:u (-> +7), global_scope:v (-> +7), imperative_style (-> +7), iteration_variable:v, loaded_variable:a, loop:for (-> +7), loop_with_early_exit:for:raise (-> +7), loop_with_raise:for (-> +7), node:For (-> +7), node:Name, scope:u (-> +7), scope:v (-> +7), whole_span:8 (-> +7)
    try: # node:Try (-> +6), try_except:Exception (-> +6), try_raise:Exception (-> +6)
        ...
        print(v) # call_argument:v, external_free_call:print, free_call:print, free_call_without_result:print, loaded_variable:v, node:Call, node:Expr, node:Name
...
# ----------------------------------------------------------------------------------------
for v in a: # for:v (-> +4), for_each:v (-> +4), global_scope:v (-> +4), global_scope:v_ (-> +4), imperative_style (-> +4), iteration_variable:v, loaded_variable:a, loop:for (-> +4), loop_with_late_exit:for (-> +4), node:For (-> +4), node:Name, scope:v (-> +4), scope:v_ (-> +4), whole_span:5 (-> +4)
...
# ----------------------------------------------------------------------------------------
class BreakOuterLoop(Exception): # global_scope:column (-> +10), global_scope:position (-> +10), global_scope:row (-> +10), loaded_variable:Exception, node:ClassDef (-> +1), node:Name, object_oriented_style (-> +10), scope:column (-> +10), scope:position (-> +10), scope:row (-> +10), whole_span:11 (-> +10)
    pass # node:Pass
try: # node:Try (-> +8), try_except:BreakOuterLoop (-> +8), try_raise:BreakOuterLoop (-> +8)
    ...
    for row in m: # for:row (-> +4), for_each:row (-> +4), iteration_variable:row, loaded_variable:m, loop:for (-> +4), loop_with_early_exit:for:raise (-> +4), loop_with_raise:for (-> +4), node:For (-> +4), node:Name
        for column in m[row]: # for:column (-> +3), index:row, iteration_variable:column, loaded_variable:m, loaded_variable:row, loop:for (-> +3), loop_with_early_exit:for:raise (-> +3), loop_with_raise:for (-> +3), nested_for:1 (-> +3), node:For (-> +3), node:Name, node:Subscript
            if m[row][column] == v: # comparison_operator:Eq, if (-> +2), if_test_atom:column, if_test_atom:m, if_test_atom:row, if_test_atom:v, if_without_else (-> +2), index:column, index:row, loaded_variable:column, loaded_variable:m, loaded_variable:row, loaded_variable:v, nested_index:2, node:Compare, node:If (-> +2), node:Name, node:Subscript
...
# ----------------------------------------------------------------------------------------
def loop_breaking(m, v): # function:loop_breaking (-> +5), function_parameter:m, function_parameter:v, function_parameter_flavor:arg, function_returning_something:loop_breaking (-> +5), impure_function:loop_breaking (-> +5), local_scope:i (-> +5), local_scope:j (-> +5), local_scope:m (-> +5), local_scope:row (-> +5), local_scope:v (-> +5), local_scope:value (-> +5), node:FunctionDef (-> +5), node:arg, procedural_style (-> +6), scope:i (-> +5), scope:j (-> +5), scope:m (-> +5), scope:row (-> +5), scope:v (-> +5), scope:value (-> +5), whole_span:7 (-> +6)
    for i, row in enumerate(m): # call_argument:m, external_free_call:enumerate, for:i (-> +3), for:row (-> +3), for_indexes_elements:i (-> +3), free_call:enumerate, iteration_variable:i, iteration_variable:row, literal:Tuple, loaded_variable:m, loop:for (-> +3), loop_with_early_exit:for:return (-> +3), loop_with_return:for (-> +3), node:Call, node:For (-> +3), node:Name, node:Tuple
...
time.sleep(5) # call_argument:5, literal:5, loaded_variable:time, magic_number:5, member_call:time:sleep, member_call_method:sleep, member_call_object:time, node:Attribute, node:Call, node:Expr, node:Name, node:Num

# ----------------------------------------------------------------------------------------
# 046.0191-extract-beginning-of-string-prefix.py
# ----------------------------------------------------------------------------------------
t = s[:5] # assignment, assignment_lhs_identifier:t, assignment_rhs_atom:5, assignment_rhs_atom:s, flat_style, global_scope:t, imperative_style, literal:5, loaded_variable:s, magic_number:5, node:Assign, node:Name, node:Num, node:Subscript, one_liner_style, scope:t, single_assignment:t, slice::5:, slice_lower:, slice_step:, slice_upper:5, whole_span:1

# ----------------------------------------------------------------------------------------
# 047.0198-extract-string-suffix.py
# ----------------------------------------------------------------------------------------
t = s[-5:] # assignment, assignment_lhs_identifier:t, assignment_rhs_atom:-5, assignment_rhs_atom:s, flat_style, global_scope:t, imperative_style, literal:-5, loaded_variable:s, magic_number:-5, node:Assign, node:Name, node:Num, node:Subscript, one_liner_style, scope:t, single_assignment:t, slice:-5::, slice_lower:-5, slice_step:, slice_upper:, whole_span:1

# ----------------------------------------------------------------------------------------
# 048.0210-multi-line-string-literal.py
# ----------------------------------------------------------------------------------------
s = """Huey # assignment, assignment_lhs_identifier:s, flat_style (-> +2), global_scope:s (-> +2), imperative_style (-> +2), node:Assign (-> +2), node:Name, scope:s (-> +2), single_assignment:s, whole_span:3 (-> +2)
Dewey
...
# ----------------------------------------------------------------------------------------
chunks = s.split() # assignment:split, assignment_lhs_identifier:chunks, assignment_rhs_atom:s, flat_style, global_scope:chunks, imperative_style, loaded_variable:s, member_call_method:split, node:Assign, node:Attribute, node:Call, node:Name, one_liner_style, scope:chunks, single_assignment:chunks, whole_span:1
...
# ----------------------------------------------------------------------------------------
k in m # comparison_operator:In, flat_style, imperative_style, loaded_variable:k, loaded_variable:m, node:Compare, node:Expr, node:Name, one_liner_style, whole_span:1

# ----------------------------------------------------------------------------------------
# 052.0666-check-if-map-contains-value.py
# ----------------------------------------------------------------------------------------
v in map.values() # comparison_operator:In, flat_style, imperative_style, loaded_variable:map, loaded_variable:v, member_call_method:values, node:Attribute, node:Call, node:Compare, node:Expr, node:Name, one_liner_style, whole_span:1

# ----------------------------------------------------------------------------------------
# 053.0240-join-a-list-of-strings.py
# ----------------------------------------------------------------------------------------
y = ", ".join(x) # assignment:join, assignment_lhs_identifier:y, assignment_rhs_atom:x, call_argument:x, flat_style, global_scope:y, imperative_style, literal:Str, loaded_variable:x, member_call_method:join, node:Assign, node:Attribute, node:Call, node:Name, node:Str, one_liner_style, scope:y, single_assignment:y, whole_span:1

# ----------------------------------------------------------------------------------------
# 053.1933-join-a-list-of-strings.py
# ----------------------------------------------------------------------------------------
y = ", ".join(map(str, x)) # assignment:join, assignment_lhs_identifier:y, assignment_rhs_atom:str, assignment_rhs_atom:x, call_argument:, call_argument:str, call_argument:x, composition, external_free_call:map, flat_style, free_call:map, global_scope:y, imperative_style, literal:Str, loaded_variable:str, loaded_variable:x, member_call_method:join, node:Assign, node:Attribute, node:Call, node:Name, node:Str, one_liner_style, scope:y, single_assignment:y, whole_span:1

# ----------------------------------------------------------------------------------------
# 054.0241-compute-sum-of-integers.py
# ----------------------------------------------------------------------------------------
s = sum(x) # assignment:sum, assignment_lhs_identifier:s, assignment_rhs_atom:x, call_argument:x, external_free_call:sum, flat_style, free_call:sum, global_scope:s, imperative_style, loaded_variable:x, node:Assign, node:Call, node:Name, one_liner_style, scope:s, single_assignment:s, whole_span:1

# ----------------------------------------------------------------------------------------
# 055.0575-convert-integer-to-string.py
# ----------------------------------------------------------------------------------------
s = str(i) # assignment:str, assignment_lhs_identifier:s, assignment_rhs_atom:i, call_argument:i, external_free_call:str, flat_style, free_call:str, global_scope:s, imperative_style, loaded_variable:i, node:Assign, node:Call, node:Name, one_liner_style, scope:s, single_assignment:s, whole_span:1
...
def f(i): # function:f (-> +1), function_parameter:i, function_parameter_flavor:arg, function_returning_nothing:f (-> +1), local_scope:i (-> +1), node:FunctionDef (-> +1), node:arg, scope:i (-> +1)
    i * i # binary_operator:Mult, loaded_variable:i, multiplication_operator, node:BinOp, node:Expr, node:Name
with Pool(processes) as p: # call_argument:processes, external_free_call:Pool, free_call:Pool, loaded_variable:processes, node:Call, node:Name, node:With (-> +1)
    p.map(func=f, iterable=range(1, 1001)) # call_argument:, call_argument:1, call_argument:1001, call_argument:f, call_keyword_argument:func, call_keyword_argument:iterable, external_free_call:range, free_call:range, literal:1, literal:1001, loaded_variable:f, loaded_variable:p, magic_number:1001, member_call:p:map, member_call_method:map, member_call_object:p, node:Attribute, node:Call, node:Expr, node:Name, node:Num, range:1:1001
...
# ----------------------------------------------------------------------------------------
lines = open(f).read() # assignment:read, assignment_lhs_identifier:lines, assignment_rhs_atom:f, call_argument:f, external_free_call:open, flat_style, free_call:open, global_scope:lines, imperative_style, loaded_variable:f, member_call_method:read, node:Assign, node:Attribute, node:Call, node:Name, one_liner_style, scope:lines, single_assignment:lines, whole_span:1
...
print(x, "is negative", file=sys.stderr) # call_argument:, call_argument:x, call_keyword_argument:file, external_free_call:print, free_call:print, free_call_without_result:print, literal:Str, loaded_variable:sys, loaded_variable:x, node:Attribute, node:Call, node:Expr, node:Name, node:Str, value_attr:stderr

# ----------------------------------------------------------------------------------------
# 060.1084-read-command-line-argument.py
# ----------------------------------------------------------------------------------------
import sys # flat_style (-> +1), global_scope:x (-> +1), imperative_style (-> +1), import:sys, import_module:sys, node:Import, one_liner_style (-> +1), scope:x (-> +1), whole_span:2 (-> +1)
...
# ----------------------------------------------------------------------------------------
import datetime # flat_style (-> +1), global_scope:d (-> +1), imperative_style (-> +1), import:datetime, import_module:datetime, node:Import, one_liner_style (-> +1), scope:d (-> +1), whole_span:2 (-> +1)
...
# ----------------------------------------------------------------------------------------
i = x.find(y) # assignment:find, assignment_lhs_identifier:i, assignment_rhs_atom:x, assignment_rhs_atom:y, call_argument:y, flat_style, global_scope:i, imperative_style, loaded_variable:x, loaded_variable:y, member_call_method:find, node:Assign, node:Attribute, node:Call, node:Name, one_liner_style, scope:i, single_assignment:i, whole_span:1

# ----------------------------------------------------------------------------------------
# 063.1088-replace-fragment-of-a-string.py
# ----------------------------------------------------------------------------------------
x2 = x.replace(y, z) # assignment:replace, assignment_lhs_identifier:x2, assignment_rhs_atom:x, assignment_rhs_atom:y, assignment_rhs_atom:z, call_argument:y, call_argument:z, flat_style, global_scope:x2, imperative_style, loaded_variable:x, loaded_variable:y, loaded_variable:z, member_call_method:replace, node:Assign, node:Attribute, node:Call, node:Name, one_liner_style, scope:x2, single_assignment:x2, whole_span:1

# ----------------------------------------------------------------------------------------
# 064.0274-big-integer--value-3-power-247.py
# ----------------------------------------------------------------------------------------
x = 3 ** 247 # assignment:Pow, assignment_lhs_identifier:x, assignment_rhs_atom:247, assignment_rhs_atom:3, binary_operator:Pow, flat_style, global_scope:x, imperative_style, literal:247, literal:3, magic_number:247, magic_number:3, node:Assign, node:BinOp, node:Name, node:Num, one_liner_style, scope:x, single_assignment:x, whole_span:1

# ----------------------------------------------------------------------------------------
# 065.1085-format-decimal-number.py
# ----------------------------------------------------------------------------------------
s = "{:.1%}".format(x) # assignment:format, assignment_lhs_identifier:s, assignment_rhs_atom:x, call_argument:x, flat_style, global_scope:s, imperative_style, literal:Str, loaded_variable:x, member_call_method:format, node:Assign, node:Attribute, node:Call, node:Name, node:Str, one_liner_style, scope:s, single_assignment:s, whole_span:1

# ----------------------------------------------------------------------------------------
# 066.0672-big-integer-exponentiation.py
# ----------------------------------------------------------------------------------------
z = x ** n # assignment:Pow, assignment_lhs_identifier:z, assignment_rhs_atom:n, assignment_rhs_atom:x, binary_operator:Pow, flat_style, global_scope:z, imperative_style, loaded_variable:n, loaded_variable:x, node:Assign, node:BinOp, node:Name, one_liner_style, scope:z, single_assignment:z, whole_span:1
...
def binom(n, k): # function:binom (-> +1), function_parameter:k, function_parameter:n, function_parameter_flavor:arg, function_returning_something:binom (-> +1), local_scope:k (-> +1), local_scope:n (-> +1), node:FunctionDef (-> +1), node:arg, pure_function:binom (-> +1), scope:k (-> +1), scope:n (-> +1)
    return math.factorial(n) // math.factorial(k) // math.factorial(n - k) # binary_operator:FloorDiv, binary_operator:Sub, call_argument:, call_argument:k, call_argument:n, loaded_variable:k, loaded_variable:math, loaded_variable:n, member_call_method:factorial, node:Attribute, node:BinOp, node:Call, node:Name, node:Return, return

# ----------------------------------------------------------------------------------------
# 068.2271-create-a-bitset.py
# ----------------------------------------------------------------------------------------
from __future__ import division # flat_style (-> +2), global_scope:x (-> +2), imperative_style (-> +2), import:__future__:division, import_module:__future__, import_name:division, node:ImportFrom, one_liner_style (-> +2), scope:x (-> +2), whole_span:3 (-> +2)
import math # import:math, import_module:math, node:Import
x = bytearray(int(math.ceil(n / 8.0))) # assignment:bytearray, assignment_lhs_identifier:x, assignment_rhs_atom:8.0, assignment_rhs_atom:math, assignment_rhs_atom:n, binary_operator:Div, call_argument:, composition, external_free_call:bytearray, external_free_call:int, free_call:bytearray, free_call:int, literal:8.0, loaded_variable:math, loaded_variable:n, magic_number:8.0, member_call_method:ceil, node:Assign, node:Attribute, node:BinOp, node:Call, node:Name, node:Num, single_assignment:x

# ----------------------------------------------------------------------------------------
# 069.1086-seed-random-generator.py
# ----------------------------------------------------------------------------------------
import random # flat_style (-> +1), global_scope:rand (-> +1), imperative_style (-> +1), import:random, import_module:random, node:Import, one_liner_style (-> +1), scope:rand (-> +1), whole_span:2 (-> +1)
rand = random.Random(s) # assignment:Random, assignment_lhs_identifier:rand, assignment_rhs_atom:random, assignment_rhs_atom:s, call_argument:s, loaded_variable:random, loaded_variable:s, member_call_method:Random, node:Assign, node:Attribute, node:Call, node:Name, single_assignment:rand
...
# ----------------------------------------------------------------------------------------
def fact(a_class, str_): # function:fact (-> +2), function_parameter:a_class, function_parameter:str_, function_parameter_flavor:arg, function_returning_something:fact (-> +2), functional_style (-> +2), higher_order_function:a_class (-> +2), local_scope:a_class (-> +2), local_scope:str_ (-> +2), node:FunctionDef (-> +2), node:arg, pure_function:fact (-> +2), scope:a_class (-> +2), scope:str_ (-> +2), whole_span:3 (-> +2)
    if issubclass(a_class, Parent): # call_argument:Parent, call_argument:a_class, external_free_call:issubclass, free_call:issubclass, if (-> +1), if_test_atom:Parent, if_test_atom:a_class, if_without_else (-> +1), loaded_variable:Parent, loaded_variable:a_class, node:Call, node:If (-> +1), node:Name
        return a_class(str_) # call_argument:str_, external_free_call:a_class, free_call:a_class, free_tail_call:a_class, if_then_branch, loaded_variable:str_, node:Call, node:Name, node:Return, return

# ----------------------------------------------------------------------------------------
# 074.0674-compute-gcd.py
# ----------------------------------------------------------------------------------------
from fractions import gcd # flat_style (-> +1), global_scope:x (-> +1), imperative_style (-> +1), import:fractions:gcd, import_module:fractions, import_name:gcd, node:ImportFrom, one_liner_style (-> +1), scope:x (-> +1), whole_span:2 (-> +1)
x = gcd(a, b) # assignment:gcd, assignment_lhs_identifier:x, assignment_rhs_atom:a, assignment_rhs_atom:b, call_argument:a, call_argument:b, external_free_call:gcd, free_call:gcd, loaded_variable:a, loaded_variable:b, node:Assign, node:Call, node:Name, single_assignment:x
...
# ----------------------------------------------------------------------------------------
s = "{:b}".format(x) # assignment:format, assignment_lhs_identifier:s, assignment_rhs_atom:x, call_argument:x, flat_style, global_scope:s, imperative_style, literal:Str, loaded_variable:x, member_call_method:format, node:Assign, node:Attribute, node:Call, node:Name, node:Str, one_liner_style, scope:s, single_assignment:s, whole_span:1

# ----------------------------------------------------------------------------------------
# 077.1093-complex-number.py
# ----------------------------------------------------------------------------------------
x = 3j - 2 # assignment:Sub, assignment_lhs_identifier:x, assignment_rhs_atom:2, assignment_rhs_atom:3j, binary_operator:Sub, flat_style (-> +1), global_scope:x (-> +1), global_scope:y (-> +1), imperative_style (-> +1), literal:2, literal:3j, node:Assign, node:BinOp, node:Name, node:Num, scope:x (-> +1), scope:y (-> +1), single_assignment:x, whole_span:2 (-> +1)
...
# ----------------------------------------------------------------------------------------
y = float(x) # assignment:float, assignment_lhs_identifier:y, assignment_rhs_atom:x, call_argument:x, external_free_call:float, flat_style, free_call:float, global_scope:y, imperative_style, loaded_variable:x, node:Assign, node:Call, node:Name, one_liner_style, scope:y, single_assignment:y, whole_span:1

# ----------------------------------------------------------------------------------------
# 080.1092-truncate-floating-point-number-to-integer.py
# ----------------------------------------------------------------------------------------
y = int(x) # assignment:int, assignment_lhs_identifier:y, assignment_rhs_atom:x, call_argument:x, external_free_call:int, flat_style, free_call:int, global_scope:y, imperative_style, loaded_variable:x, node:Assign, node:Call, node:Name, one_liner_style, scope:y, single_assignment:y, whole_span:1

# ----------------------------------------------------------------------------------------
# 081.2270-round-floating-point-number-to-integer.py
# ----------------------------------------------------------------------------------------
y = int(x + 0.5) # addition_operator, assignment:int, assignment_lhs_identifier:y, assignment_rhs_atom:0.5, assignment_rhs_atom:x, binary_operator:Add, call_argument:, external_free_call:int, flat_style, free_call:int, global_scope:y, imperative_style, literal:0.5, loaded_variable:x, magic_number:0.5, node:Assign, node:BinOp, node:Call, node:Name, node:Num, one_liner_style, scope:y, single_assignment:y, whole_span:1

# ----------------------------------------------------------------------------------------
# 082.1096-count-substring-occurrences.py
# ----------------------------------------------------------------------------------------
count = s.count(t) # assignment:count, assignment_lhs_identifier:count, assignment_rhs_atom:s, assignment_rhs_atom:t, call_argument:t, flat_style, global_scope:count, imperative_style, loaded_variable:s, loaded_variable:t, member_call_method:count, node:Assign, node:Attribute, node:Call, node:Name, one_liner_style, scope:count, single_assignment:count, whole_span:1

# ----------------------------------------------------------------------------------------
# 083.1805-regex-with-character-repetition.py
# ----------------------------------------------------------------------------------------
import re # flat_style (-> +1), global_scope:r (-> +1), imperative_style (-> +1), import:re, import_module:re, node:Import, one_liner_style (-> +1), scope:r (-> +1), whole_span:2 (-> +1)
r = re.compile(r"htt+p") # assignment:compile, assignment_lhs_identifier:r, assignment_rhs_atom:re, call_argument:, literal:Str, loaded_variable:re, member_call_method:compile, node:Assign, node:Attribute, node:Call, node:Name, node:Str, single_assignment:r

# ----------------------------------------------------------------------------------------
# 084.1940-count-bits-set-in-integer-binary-representation.py
# ----------------------------------------------------------------------------------------
c = bin(i).count("1") # assignment:count, assignment_lhs_identifier:c, assignment_rhs_atom:i, call_argument:, call_argument:i, external_free_call:bin, flat_style, free_call:bin, global_scope:c, imperative_style, literal:Str, loaded_variable:i, member_call_method:count, node:Assign, node:Attribute, node:Call, node:Name, node:Str, one_liner_style, scope:c, single_assignment:c, whole_span:1

# ----------------------------------------------------------------------------------------
# 085.1003-check-if-integer-addition-will-overflow.py
# ----------------------------------------------------------------------------------------
def adding_will_overflow(x, y): # function:adding_will_overflow (-> +1), function_parameter:x, function_parameter:y, function_parameter_flavor:arg, function_returning_something:adding_will_overflow (-> +1), functional_style (-> +1), local_scope:x (-> +1), local_scope:y (-> +1), node:FunctionDef (-> +1), node:arg, one_liner_style (-> +1), pure_function:adding_will_overflow (-> +1), scope:x (-> +1), scope:y (-> +1), whole_span:2 (-> +1)
...
# ----------------------------------------------------------------------------------------
def multiplyWillOverflow(x, y): # function:multiplyWillOverflow (-> +1), function_parameter:x, function_parameter:y, function_parameter_flavor:arg, function_returning_something:multiplyWillOverflow (-> +1), functional_style (-> +1), local_scope:x (-> +1), local_scope:y (-> +1), node:FunctionDef (-> +1), node:arg, one_liner_style (-> +1), pure_function:multiplyWillOverflow (-> +1), scope:x (-> +1), scope:y (-> +1), whole_span:2 (-> +1)
    return False # literal:False, node:NameConstant, node:Return, return:False
...
# ----------------------------------------------------------------------------------------
import sys # flat_style (-> +1), imperative_style (-> +1), import:sys, import_module:sys, node:Import, one_liner_style (-> +1), whole_span:2 (-> +1)
sys.exit(1) # call_argument:1, literal:1, loaded_variable:sys, member_call:sys:exit, member_call_method:exit, member_call_object:sys, node:Attribute, node:Call, node:Expr, node:Name, node:Num

# ----------------------------------------------------------------------------------------
# 088.2143-allocate-1m-bytes.py
# ----------------------------------------------------------------------------------------
buf = bytearray(1000000) # assignment:bytearray, assignment_lhs_identifier:buf, assignment_rhs_atom:1000000, call_argument:1000000, external_free_call:bytearray, flat_style, free_call:bytearray, global_scope:buf, imperative_style, literal:1000000, magic_number:1000000, node:Assign, node:Call, node:Name, node:Num, one_liner_style, scope:buf, single_assignment:buf, whole_span:1
...
# ----------------------------------------------------------------------------------------
class Foo(object): # loaded_variable:object, node:ClassDef (-> +5), node:Name, object_oriented_style (-> +5), whole_span:6 (-> +5)
    def __init__(self): # function:__init__ (-> +1), function_parameter:self, function_parameter_flavor:arg, function_returning_nothing:__init__ (-> +1), instance_method:__init__ (-> +1), local_scope:self (-> +1), method:__init__ (-> +1), node:FunctionDef (-> +1), node:arg, scope:self (-> +1)
    ...
    @property # decorated_function:x (-> +2), function:x (-> +2), function_decorator:property (-> +2), function_returning_something:x (-> +2), instance_method:x (-> +2), loaded_variable:property, local_scope:self (-> +2), method:x (-> +2), node:FunctionDef (-> +2), node:Name, scope:self (-> +2)
...
# ----------------------------------------------------------------------------------------
import json # flat_style (-> +2), global_scope:x (-> +2), imperative_style (-> +2), import:json, import_module:json, node:Import, scope:x (-> +2), whole_span:3 (-> +2)
with open("data.json", "r") as input: # call_argument:, external_free_call:open, free_call:open, literal:Str, node:Call, node:Name, node:Str, node:With (-> +1)
    x = json.load(input) # assignment:load, assignment_lhs_identifier:x, assignment_rhs_atom:input, assignment_rhs_atom:json, call_argument:input, loaded_variable:input, loaded_variable:json, member_call_method:load, node:Assign, node:Attribute, node:Call, node:Name, single_assignment:x
...
with open("data.json", "w") as output: # call_argument:, external_free_call:open, free_call:open, literal:Str, node:Call, node:Name, node:Str, node:With (-> +1)
    json.dump(x, output) # call_argument:output, call_argument:x, loaded_variable:json, loaded_variable:output, loaded_variable:x, member_call:json:dump, member_call_method:dump, member_call_object:json, node:Attribute, node:Call, node:Expr, node:Name
...
def control(f): # function:control (-> +1), function_parameter:f, function_parameter_flavor:arg, function_returning_something:control (-> +1), higher_order_function:f (-> +1), local_scope:f (-> +1), node:FunctionDef (-> +1), node:arg, pure_function:control (-> +1), scope:f (-> +1)
...
# ----------------------------------------------------------------------------------------
print(type(x)) # call_argument:, call_argument:x, composition, external_free_call:print, external_free_call:type, flat_style, free_call:print, free_call:type, free_call_without_result:print, imperative_style, loaded_variable:x, node:Call, node:Expr, node:Name, one_liner_style, whole_span:1

# ----------------------------------------------------------------------------------------
# 094.1864-print-type-of-variable.py
# ----------------------------------------------------------------------------------------
print(x.__class__) # call_argument:, external_free_call:print, flat_style, free_call:print, free_call_without_result:print, imperative_style, loaded_variable:x, node:Attribute, node:Call, node:Expr, node:Name, one_liner_style, whole_span:1

# ----------------------------------------------------------------------------------------
# 095.2140-get-file-size.py
# ----------------------------------------------------------------------------------------
import os # flat_style (-> +1), global_scope:x (-> +1), imperative_style (-> +1), import:os, import_module:os, node:Import, one_liner_style (-> +1), scope:x (-> +1), whole_span:2 (-> +1)
x = os.path.getsize(path) # assignment:getsize, assignment_lhs_identifier:x, assignment_rhs_atom:os, assignment_rhs_atom:path, call_argument:path, loaded_variable:os, loaded_variable:path, member_call_method:getsize, node:Assign, node:Attribute, node:Call, node:Name, single_assignment:x, value_attr:path

# ----------------------------------------------------------------------------------------
# 096.1094-check-string-prefix.py
# ----------------------------------------------------------------------------------------
b = s.startswith(prefix) # assignment:startswith, assignment_lhs_identifier:b, assignment_rhs_atom:prefix, assignment_rhs_atom:s, call_argument:prefix, flat_style, global_scope:b, imperative_style, loaded_variable:prefix, loaded_variable:s, member_call_method:startswith, node:Assign, node:Attribute, node:Call, node:Name, one_liner_style, scope:b, single_assignment:b, whole_span:1

# ----------------------------------------------------------------------------------------
# 097.1095-check-string-suffix.py
# ----------------------------------------------------------------------------------------
b = s.endswith(suffix) # assignment:endswith, assignment_lhs_identifier:b, assignment_rhs_atom:s, assignment_rhs_atom:suffix, call_argument:suffix, flat_style, global_scope:b, imperative_style, loaded_variable:s, loaded_variable:suffix, member_call_method:endswith, node:Assign, node:Attribute, node:Call, node:Name, one_liner_style, scope:b, single_assignment:b, whole_span:1

# ----------------------------------------------------------------------------------------
# 098.2142-epoch-seconds-to-date-object.py
# ----------------------------------------------------------------------------------------
import datetime # flat_style (-> +1), global_scope:d (-> +1), imperative_style (-> +1), import:datetime, import_module:datetime, node:Import, one_liner_style (-> +1), scope:d (-> +1), whole_span:2 (-> +1)
d = datetime.date.fromtimestamp(ts) # assignment:fromtimestamp, assignment_lhs_identifier:d, assignment_rhs_atom:datetime, assignment_rhs_atom:ts, call_argument:ts, loaded_variable:datetime, loaded_variable:ts, member_call_method:fromtimestamp, node:Assign, node:Attribute, node:Call, node:Name, single_assignment:d, value_attr:date

# ----------------------------------------------------------------------------------------
# 099.1429-format-date-yyyy-mm-dd.py
# ----------------------------------------------------------------------------------------
from datetime import date # flat_style (-> +2), global_scope:d (-> +2), global_scope:x (-> +2), imperative_style (-> +2), import:datetime:date, import_module:datetime, import_name:date, node:ImportFrom, scope:d (-> +2), scope:x (-> +2), whole_span:3 (-> +2)
...
x = d.strftime("%Y-%m-%d") # assignment:strftime, assignment_lhs_identifier:x, assignment_rhs_atom:d, call_argument:, literal:Str, loaded_variable:d, member_call_method:strftime, node:Assign, node:Attribute, node:Call, node:Name, node:Str, single_assignment:x
...
# ----------------------------------------------------------------------------------------
items.sort(c) # call_argument:c, flat_style, imperative_style, loaded_variable:c, loaded_variable:items, member_call:items:sort, member_call_method:sort, member_call_object:items, node:Attribute, node:Call, node:Expr, node:Name, one_liner_style, whole_span:1

# ----------------------------------------------------------------------------------------
# 101.2172-load-from-http-get-request-into-a-string.py
# ----------------------------------------------------------------------------------------
import urllib.request # flat_style (-> +2), global_scope:s (-> +2), imperative_style (-> +2), import:urllib.request, import_module:urllib.request, node:Import, scope:s (-> +2), whole_span:3 (-> +2)
with urllib.request.urlopen(u) as f: # call_argument:u, loaded_variable:u, loaded_variable:urllib, member_call_method:urlopen, node:Attribute, node:Call, node:Name, node:With (-> +1), value_attr:request
...
# ----------------------------------------------------------------------------------------
import urllib # flat_style (-> +1), global_scope:filename (-> +1), global_scope:headers (-> +1), imperative_style (-> +1), import:urllib, import_module:urllib, node:Import, one_liner_style (-> +1), scope:filename (-> +1), scope:headers (-> +1), whole_span:2 (-> +1)
...
# ----------------------------------------------------------------------------------------
import lxml.etree # flat_style (-> +1), global_scope:x (-> +1), imperative_style (-> +1), import:lxml.etree, import_module:lxml.etree, node:Import, one_liner_style (-> +1), scope:x (-> +1), whole_span:2 (-> +1)
x = lxml.etree.parse("data.xml") # assignment:parse, assignment_lhs_identifier:x, assignment_rhs_atom:lxml, call_argument:, literal:Str, loaded_variable:lxml, member_call_method:parse, node:Assign, node:Attribute, node:Call, node:Name, node:Str, single_assignment:x, value_attr:etree

# ----------------------------------------------------------------------------------------
# 104.3264-save-object-into-xml-file.py
# ----------------------------------------------------------------------------------------
import pyxser as pyx # global_scope:a (-> +11), global_scope:b (-> +11), global_scope:c (-> +11), global_scope:ser (-> +11), global_scope:tst (-> +11), import:pyxser, import_module:pyxser, node:Import, object_oriented_style (-> +11), scope:a (-> +11), scope:b (-> +11), scope:c (-> +11), scope:ser (-> +11), scope:tst (-> +11), whole_span:12 (-> +11)
class TestClass(object): # loaded_variable:object, node:ClassDef (-> +7), node:Name
...
# ----------------------------------------------------------------------------------------
import os # flat_style (-> +1), global_scope:dir (-> +1), imperative_style (-> +1), import:os, import_module:os, node:Import, one_liner_style (-> +1), scope:dir (-> +1), whole_span:2 (-> +1)
...
if "x" in locals(): # comparison_operator:In, external_free_call:locals, free_call:locals, free_call_without_arguments:locals, if (-> +1), if_without_else (-> +1), imperative_style (-> +1), literal:Str, node:Call, node:Compare, node:If (-> +1), node:Name, node:Str, whole_span:2 (-> +1)
    print(x) # call_argument:x, external_free_call:print, free_call:print, free_call_without_result:print, if_then_branch, loaded_variable:x, node:Call, node:Expr, node:Name

# ----------------------------------------------------------------------------------------
# 109.2280-number-of-bytes-of-a-type.py
# ----------------------------------------------------------------------------------------
import pympler.asizeof # flat_style (-> +1), global_scope:n (-> +1), imperative_style (-> +1), import:pympler.asizeof, import_module:pympler.asizeof, node:Import, one_liner_style (-> +1), scope:n (-> +1), whole_span:2 (-> +1)
n = pympler.asizeof.asizeof(t) # assignment:asizeof, assignment_lhs_identifier:n, assignment_rhs_atom:pympler, assignment_rhs_atom:t, call_argument:t, loaded_variable:pympler, loaded_variable:t, member_call_method:asizeof, node:Assign, node:Attribute, node:Call, node:Name, single_assignment:n, value_attr:asizeof
...
subprocess.call(["x", "a", "b"]) # call_argument:, literal:List, literal:Str, loaded_variable:subprocess, member_call:subprocess:call, member_call_method:call, member_call_object:subprocess, node:Attribute, node:Call, node:Expr, node:List, node:Name, node:Str

# ----------------------------------------------------------------------------------------
# 112.2144-iterate-over-map-entries-ordered-by-keys.py
# ----------------------------------------------------------------------------------------
for k in sorted(mymap): # call_argument:mymap, external_free_call:sorted, for:k (-> +1), free_call:sorted, global_scope:k (-> +1), imperative_style (-> +1), iteration_variable:k, loaded_variable:mymap, loop:for (-> +1), loop_with_late_exit:for (-> +1), node:Call, node:For (-> +1), node:Name, scope:k (-> +1), whole_span:2 (-> +1)
    print(mymap[k]) # call_argument:, external_free_call:print, free_call:print, free_call_without_result:print, index:k, loaded_variable:k, loaded_variable:mymap, node:Call, node:Expr, node:Name, node:Subscript

# ----------------------------------------------------------------------------------------
# 113.2157-iterate-over-map-entries-ordered-by-values.py
# ----------------------------------------------------------------------------------------
for x, k in sorted((x, k) for k, x in mymap.items()): # call_argument:, composition, comprehension:Generator, comprehension_for_count:1, external_free_call:sorted, for:k (-> +1), for:x (-> +1), free_call:sorted, global_scope:k (-> +1), global_scope:x (-> +1), imperative_style (-> +1), iteration_variable:k, iteration_variable:x, literal:Tuple, loaded_variable:k, loaded_variable:mymap, loaded_variable:x, local_scope:k, local_scope:x, loop:for (-> +1), loop_with_late_exit:for (-> +1), member_call_method:items, node:Attribute, node:Call, node:For (-> +1), node:GeneratorExp, node:Name, node:Tuple, scope:k, scope:k (-> +1), scope:x, scope:x (-> +1), shadowing_scope:k, shadowing_scope:x, whole_span:2 (-> +1)
    print(k, x) # call_argument:k, call_argument:x, external_free_call:print, free_call:print, free_call_without_result:print, loaded_variable:k, loaded_variable:x, node:Call, node:Expr, node:Name

# ----------------------------------------------------------------------------------------
# 114.2273-test-deep-equality.py
# ----------------------------------------------------------------------------------------
b = x == y # assignment, assignment_lhs_identifier:b, assignment_rhs_atom:x, assignment_rhs_atom:y, comparison_operator:Eq, flat_style, global_scope:b, imperative_style, loaded_variable:x, loaded_variable:y, node:Assign, node:Compare, node:Name, one_liner_style, scope:b, single_assignment:b, whole_span:1

# ----------------------------------------------------------------------------------------
# 115.2138-compare-dates.py
# ----------------------------------------------------------------------------------------
import datetime # flat_style (-> +1), global_scope:b (-> +1), imperative_style (-> +1), import:datetime, import_module:datetime, node:Import, one_liner_style (-> +1), scope:b (-> +1), whole_span:2 (-> +1)
...
# ----------------------------------------------------------------------------------------
s2 = s1.replace(w, "") # assignment:replace, assignment_lhs_identifier:s2, assignment_rhs_atom:s1, assignment_rhs_atom:w, call_argument:, call_argument:w, empty_literal:Str, flat_style, global_scope:s2, imperative_style, literal:Str, loaded_variable:s1, loaded_variable:w, member_call_method:replace, node:Assign, node:Attribute, node:Call, node:Name, node:Str, one_liner_style, scope:s2, single_assignment:s2, whole_span:1

# ----------------------------------------------------------------------------------------
# 117.1297-get-list-size.py
# ----------------------------------------------------------------------------------------
n = len(x) # assignment:len, assignment_lhs_identifier:n, assignment_rhs_atom:x, call_argument:x, external_free_call:len, flat_style, free_call:len, global_scope:n, imperative_style, loaded_variable:x, node:Assign, node:Call, node:Name, one_liner_style, scope:n, single_assignment:n, whole_span:1

# ----------------------------------------------------------------------------------------
# 118.1254-list-to-set.py
# ----------------------------------------------------------------------------------------
y = set(x) # assignment:set, assignment_lhs_identifier:y, assignment_rhs_atom:x, call_argument:x, external_free_call:set, flat_style, free_call:set, global_scope:y, imperative_style, loaded_variable:x, node:Assign, node:Call, node:Name, one_liner_style, scope:y, single_assignment:y, whole_span:1

# ----------------------------------------------------------------------------------------
# 118.3266-list-to-set.py
# ----------------------------------------------------------------------------------------
set(x) # call_argument:x, external_free_call:set, flat_style, free_call:set, free_call_without_result:set, imperative_style, loaded_variable:x, node:Call, node:Expr, node:Name, one_liner_style, whole_span:1

# ----------------------------------------------------------------------------------------
# 119.1253-deduplicate-list.py
# ----------------------------------------------------------------------------------------
x = list(set(x)) # assignment:list, assignment_lhs_identifier:x, assignment_rhs_atom:x, call_argument:, call_argument:x, composition, external_free_call:list, external_free_call:set, flat_style, free_call:list, free_call:set, global_scope:x, imperative_style, loaded_variable:x, node:Assign, node:Call, node:Name, one_liner_style, scope:x, single_assignment:x, whole_span:1

# ----------------------------------------------------------------------------------------
# 119.3263-deduplicate-list.py
# ----------------------------------------------------------------------------------------
elements = ["b", "a", "b", "c"] # assignment, assignment_lhs_identifier:elements, global_scope:elements (-> +7), global_scope:elements_unique (-> +7), global_scope:i (-> +7), global_scope:unique_set (-> +7), imperative_style (-> +7), literal:List, literal:Str, node:Assign, node:List, node:Name, node:Str, scope:elements (-> +7), scope:elements_unique (-> +7), scope:i (-> +7), scope:unique_set (-> +7), single_assignment:elements, whole_span:8 (-> +7)
...
for i in elements: # accumulate_elements:add (-> +3), accumulate_elements:append (-> +3), accumulate_some_elements:add (-> +3), accumulate_some_elements:append (-> +3), for:i (-> +3), for_each:i (-> +3), iteration_variable:i, loaded_variable:elements, loop:for (-> +3), loop_with_late_exit:for (-> +3), node:For (-> +3), node:Name
    if i not in unique_set: # comparison_operator:NotIn, if (-> +2), if_test_atom:i, if_test_atom:unique_set, if_without_else (-> +2), loaded_variable:i, loaded_variable:unique_set, node:Compare, node:If (-> +2), node:Name
        unique_set.add(i) # call_argument:i, if_then_branch (-> +1), loaded_variable:i, loaded_variable:unique_set, member_call:unique_set:add, member_call_method:add, member_call_object:unique_set, node:Attribute, node:Call, node:Expr, node:Name, update:unique_set:i, update_by_member_call:unique_set:i, update_by_member_call_with:add, update_with:add
        elements_unique.append(i) # call_argument:i, loaded_variable:elements_unique, loaded_variable:i, member_call:elements_unique:append, member_call_method:append, member_call_object:elements_unique, node:Attribute, node:Call, node:Expr, node:Name, update:elements_unique:i, update_by_member_call:elements_unique:i, update_by_member_call_with:append, update_with:append
print(elements_unique) # call_argument:elements_unique, external_free_call:print, free_call:print, free_call_without_result:print, loaded_variable:elements_unique, node:Call, node:Expr, node:Name

# ----------------------------------------------------------------------------------------
# 120.1479-read-integer-from-stdin.py
# ----------------------------------------------------------------------------------------
input_var = int(raw_input("Input Prompting String: ")) # assignment:int, assignment_lhs_identifier:input_var, call_argument:, composition, external_free_call:int, external_free_call:raw_input, flat_style, free_call:int, free_call:raw_input, global_scope:input_var, imperative_style, literal:Str, node:Assign, node:Call, node:Name, node:Str, one_liner_style, scope:input_var, single_assignment:input_var, whole_span:1

# ----------------------------------------------------------------------------------------
# 121.3029-udp-listen-and-read.py
# ----------------------------------------------------------------------------------------
import socket # global_scope:UDP_IP (-> +6), global_scope:addr (-> +6), global_scope:data (-> +6), global_scope:sock (-> +6), imperative_style (-> +6), import:socket, import_module:socket, node:Import, scope:UDP_IP (-> +6), scope:addr (-> +6), scope:data (-> +6), scope:sock (-> +6), whole_span:7 (-> +6)
...
sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM) # assignment:socket, assignment_lhs_identifier:sock, assignment_rhs_atom:socket, call_argument:, loaded_variable:socket, member_call_method:socket, node:Assign, node:Attribute, node:Call, node:Name, single_assignment:sock
sock.bind((UDP_IP, p)) # call_argument:, literal:Tuple, loaded_variable:UDP_IP, loaded_variable:p, loaded_variable:sock, member_call:sock:bind, member_call_method:bind, member_call_object:sock, node:Attribute, node:Call, node:Expr, node:Name, node:Tuple
while True: # infinite_while (-> +2), literal:True, loop:while (-> +2), loop_with_late_exit:while (-> +2), node:NameConstant, node:While (-> +2)
    data, addr = sock.recvfrom(1024) # assignment:recvfrom, assignment_lhs_identifier:addr, assignment_lhs_identifier:data, assignment_rhs_atom:1024, assignment_rhs_atom:sock, call_argument:1024, literal:1024, literal:Tuple, loaded_variable:sock, magic_number:1024, member_call_method:recvfrom, node:Assign, node:Attribute, node:Call, node:Name, node:Num, node:Tuple, parallel_assignment:2
    print("received message:", data) # call_argument:, call_argument:data, external_free_call:print, free_call:print, free_call_without_result:print, literal:Str, loaded_variable:data, node:Call, node:Expr, node:Name, node:Str

# ----------------------------------------------------------------------------------------
# 122.1453-declare-enumeration.py
# ----------------------------------------------------------------------------------------
class Suit: # global_scope:CLUBS (-> +1), global_scope:DIAMONDS (-> +1), global_scope:HEARTS (-> +1), global_scope:SPADES (-> +1), node:ClassDef (-> +1), object_oriented_style (-> +1), one_liner_style (-> +1), scope:CLUBS (-> +1), scope:DIAMONDS (-> +1), scope:HEARTS (-> +1), scope:SPADES (-> +1), whole_span:2 (-> +1)
...
# ----------------------------------------------------------------------------------------
from enum import Enum # global_scope:CLUBS (-> +5), global_scope:DIAMONDS (-> +5), global_scope:HEARTS (-> +5), global_scope:SPADES (-> +5), import:enum:Enum, import_module:enum, import_name:Enum, node:ImportFrom, object_oriented_style (-> +5), scope:CLUBS (-> +5), scope:DIAMONDS (-> +5), scope:HEARTS (-> +5), scope:SPADES (-> +5), whole_span:6 (-> +5)
class Suit(Enum): # loaded_variable:Enum, node:ClassDef (-> +4), node:Name
...
# ----------------------------------------------------------------------------------------
assert isConsistent # flat_style, imperative_style, loaded_variable:isConsistent, node:Assert, node:Name, whole_span:1
...
def binarySearch(a, x): # function:binarySearch (-> +2), function_parameter:a, function_parameter:x, function_parameter_flavor:arg, function_returning_something:binarySearch (-> +2), impure_function:binarySearch (-> +2), local_scope:a (-> +2), local_scope:i (-> +2), local_scope:x (-> +2), node:FunctionDef (-> +2), node:arg, scope:a (-> +2), scope:i (-> +2), scope:x (-> +2)
    i = bisect.bisect_left(a, x) # assignment:bisect_left, assignment_lhs_identifier:i, assignment_rhs_atom:a, assignment_rhs_atom:bisect, assignment_rhs_atom:x, call_argument:a, call_argument:x, loaded_variable:a, loaded_variable:bisect, loaded_variable:x, member_call_method:bisect_left, node:Assign, node:Attribute, node:Call, node:Name, single_assignment:i
    return i if i != len(a) and a[i] == x else -1 # boolean_operator:And, call_argument:a, comparison_operator:Eq, comparison_operator:NotEq, external_free_call:len, free_call:len, index:i, literal:-1, loaded_variable:a, loaded_variable:i, loaded_variable:x, node:BoolOp, node:Call, node:Compare, node:IfExp, node:Name, node:Num, node:Return, node:Subscript, return

# ----------------------------------------------------------------------------------------
# 125.2167-measure-function-call-duration.py
# ----------------------------------------------------------------------------------------
import time # flat_style (-> +4), global_scope:t1 (-> +4), global_scope:t2 (-> +4), imperative_style (-> +4), import:time, import_module:time, node:Import, scope:t1 (-> +4), scope:t2 (-> +4), whole_span:5 (-> +4)
...
print("Seconds:", t2 - t1) # binary_operator:Sub, call_argument:, external_free_call:print, free_call:print, free_call_without_result:print, literal:Str, loaded_variable:t1, loaded_variable:t2, node:BinOp, node:Call, node:Expr, node:Name, node:Str
...
def BFS(f, root): # function:BFS (-> +8), function_parameter:f, function_parameter:root, function_parameter_flavor:arg, function_returning_nothing:BFS (-> +8), higher_order_function:f (-> +8), local_scope:Q (-> +8), local_scope:child (-> +8), local_scope:f (-> +8), local_scope:n (-> +8), local_scope:root (-> +8), node:FunctionDef (-> +8), node:arg, procedural_style (-> +8), scope:Q (-> +8), scope:child (-> +8), scope:f (-> +8), scope:n (-> +8), scope:root (-> +8), whole_span:9 (-> +8)
    ...
    while Q: # loaded_variable:Q, loop:while (-> +6), loop_with_late_exit:while (-> +6), node:Name, node:While (-> +6)
        n = Q.pop(0) # assignment:pop, assignment_lhs_identifier:n, assignment_rhs_atom:0, assignment_rhs_atom:Q, call_argument:0, literal:0, loaded_variable:Q, member_call_method:pop, node:Assign, node:Attribute, node:Call, node:Name, node:Num, single_assignment:n
        f(n) # call_argument:n, external_free_call:f, free_call:f, free_call_without_result:f, loaded_variable:n, node:Call, node:Expr, node:Name
        for child in n: # for:child (-> +3), for_each:child (-> +3), iteration_variable:child, loaded_variable:n, loop:for (-> +3), loop_with_late_exit:for (-> +3), node:For (-> +3), node:Name
            if not n.discovered: # if (-> +2), if_test_atom:n, if_without_else (-> +2), loaded_variable:n, node:Attribute, node:If (-> +2), node:Name, node:UnaryOp, unary_operator:Not
                ...
                Q.append(n) # call_argument:n, loaded_variable:Q, loaded_variable:n, member_call:Q:append, member_call_method:append, member_call_object:Q, node:Attribute, node:Call, node:Expr, node:Name, update:Q:n, update_by_member_call:Q:n, update_by_member_call_with:append, update_with:append
...
def breadth_first(start, f): # function:breadth_first (-> +7), function_parameter:f, function_parameter:start, function_parameter_flavor:arg, function_returning_nothing:breadth_first (-> +7), higher_order_function:f (-> +7), local_scope:f (-> +7), local_scope:q (-> +7), local_scope:seen (-> +7), local_scope:start (-> +7), local_scope:vertex (-> +7), node:FunctionDef (-> +7), node:arg, scope:f (-> +7), scope:q (-> +7), scope:seen (-> +7), scope:start (-> +7), scope:vertex (-> +7)
    ...
    q = deque([start]) # assignment:deque, assignment_lhs_identifier:q, assignment_rhs_atom:start, call_argument:, external_free_call:deque, free_call:deque, literal:List, loaded_variable:start, node:Assign, node:Call, node:List, node:Name, single_assignment:q
    while q: # loaded_variable:q, loop:while (-> +4), loop_with_late_exit:while (-> +4), node:Name, node:While (-> +4)
        ...
        f(vertex) # call_argument:vertex, external_free_call:f, free_call:f, free_call_without_result:f, loaded_variable:vertex, node:Call, node:Expr, node:Name
        seen.add(vertex) # call_argument:vertex, loaded_variable:seen, loaded_variable:vertex, member_call:seen:add, member_call_method:add, member_call_object:seen, node:Attribute, node:Call, node:Expr, node:Name, update:seen:vertex, update_by_member_call:seen:vertex, update_by_member_call_with:add, update_with:add
...
# ----------------------------------------------------------------------------------------
f1 if c1 else f2 if c2 else f3 if c3 else None # flat_style, imperative_style, literal:None, loaded_variable:c1, loaded_variable:c2, loaded_variable:c3, loaded_variable:f1, loaded_variable:f2, loaded_variable:f3, node:Expr, node:IfExp, node:Name, node:NameConstant, one_liner_style, whole_span:1

# ----------------------------------------------------------------------------------------
# 131.2766-successive-conditions.py
# ----------------------------------------------------------------------------------------
if c1: # if (-> +5), imperative_style (-> +5), loaded_variable:c1, node:If (-> +5), node:Name, whole_span:6 (-> +5)
...
# ----------------------------------------------------------------------------------------
import timeit # flat_style (-> +1), global_scope:duration (-> +1), imperative_style (-> +1), import:timeit, import_module:timeit, node:Import, one_liner_style (-> +1), scope:duration (-> +1), whole_span:2 (-> +1)
duration = timeit.timeit("f()", setup="from __main__ import f") # assignment:timeit, assignment_lhs_identifier:duration, assignment_rhs_atom:timeit, call_argument:, call_keyword_argument:setup, literal:Str, loaded_variable:timeit, member_call_method:timeit, node:Assign, node:Attribute, node:Call, node:Name, node:Str, single_assignment:duration

# ----------------------------------------------------------------------------------------
# 133.2160-case-insensitive-string-contains.py
# ----------------------------------------------------------------------------------------
ok = word.lower() in s.lower() # assignment, assignment_lhs_identifier:ok, assignment_rhs_atom:s, assignment_rhs_atom:word, comparison_operator:In, flat_style, global_scope:ok, imperative_style, loaded_variable:s, loaded_variable:word, member_call_method:lower, node:Assign, node:Attribute, node:Call, node:Compare, node:Name, one_liner_style, scope:ok, single_assignment:ok, whole_span:1

# ----------------------------------------------------------------------------------------
# 134.1850-create-a-new-list.py
# ----------------------------------------------------------------------------------------
items = [a, b, c] # assignment, assignment_lhs_identifier:items, assignment_rhs_atom:a, assignment_rhs_atom:b, assignment_rhs_atom:c, flat_style, global_scope:items, imperative_style, literal:List, loaded_variable:a, loaded_variable:b, loaded_variable:c, node:Assign, node:List, node:Name, one_liner_style, scope:items, single_assignment:items, whole_span:1

# ----------------------------------------------------------------------------------------
# 135.2158-remove-item-from-list-by-its-value.py
# ----------------------------------------------------------------------------------------
items.remove(x) # call_argument:x, flat_style, imperative_style, loaded_variable:items, loaded_variable:x, member_call:items:remove, member_call_method:remove, member_call_object:items, node:Attribute, node:Call, node:Expr, node:Name, one_liner_style, update:items:x, update_by_member_call:items:x, update_by_member_call_with:remove, update_with:remove, whole_span:1

# ----------------------------------------------------------------------------------------
# 136.2141-remove-all-occurrences-of-a-value-from-a-list.py
# ----------------------------------------------------------------------------------------
newlist = [item for item in items if item != x] # assignment, assignment_lhs_identifier:newlist, assignment_rhs_atom:item, assignment_rhs_atom:items, assignment_rhs_atom:x, comparison_operator:NotEq, comprehension:List, comprehension_for_count:1, filtered_comprehension, flat_style, imperative_style, iteration_variable:item, loaded_variable:item, loaded_variable:items, loaded_variable:x, local_scope:item, local_scope:newlist, node:Assign, node:Compare, node:ListComp, node:Name, one_liner_style, scope:item, scope:newlist, single_assignment:newlist, whole_span:1

# ----------------------------------------------------------------------------------------
# 137.1823-check-if-string-contains-only-digits.py
# ----------------------------------------------------------------------------------------
b = s.isdigit() # assignment:isdigit, assignment_lhs_identifier:b, assignment_rhs_atom:s, flat_style, global_scope:b, imperative_style, loaded_variable:s, member_call_method:isdigit, node:Assign, node:Attribute, node:Call, node:Name, one_liner_style, scope:b, single_assignment:b, whole_span:1

# ----------------------------------------------------------------------------------------
# 138.2161-create-temp-file.py
# ----------------------------------------------------------------------------------------
import tempfile # flat_style (-> +1), global_scope:file (-> +1), imperative_style (-> +1), import:tempfile, import_module:tempfile, node:Import, one_liner_style (-> +1), scope:file (-> +1), whole_span:2 (-> +1)
...
# ----------------------------------------------------------------------------------------
import tempfile # flat_style (-> +1), global_scope:td (-> +1), imperative_style (-> +1), import:tempfile, import_module:tempfile, node:Import, one_liner_style (-> +1), scope:td (-> +1), whole_span:2 (-> +1)
...
# ----------------------------------------------------------------------------------------
for x in items1 + items2: # addition_operator, binary_operator:Add, for:x (-> +1), global_scope:x (-> +1), imperative_style (-> +1), iteration_variable:x, loaded_variable:items1, loaded_variable:items2, loop:for (-> +1), loop_with_late_exit:for (-> +1), node:BinOp, node:For (-> +1), node:Name, scope:x (-> +1), whole_span:2 (-> +1)
    print(x) # call_argument:x, external_free_call:print, free_call:print, free_call_without_result:print, loaded_variable:x, node:Call, node:Expr, node:Name

# ----------------------------------------------------------------------------------------
# 142.2151-hexadecimal-digits-of-an-integer.py
# ----------------------------------------------------------------------------------------
s = hex(x) # assignment:hex, assignment_lhs_identifier:s, assignment_rhs_atom:x, call_argument:x, external_free_call:hex, flat_style, free_call:hex, global_scope:s, imperative_style, loaded_variable:x, node:Assign, node:Call, node:Name, one_liner_style, scope:s, single_assignment:s, whole_span:1

# ----------------------------------------------------------------------------------------
# 143.2256-iterate-alternatively-over-two-lists.py
# ----------------------------------------------------------------------------------------
for pair in zip(item1, item2): # call_argument:item1, call_argument:item2, external_free_call:zip, for:pair (-> +1), free_call:zip, global_scope:pair (-> +1), imperative_style (-> +1), iteration_variable:pair, loaded_variable:item1, loaded_variable:item2, loop:for (-> +1), loop_with_late_exit:for (-> +1), node:Call, node:For (-> +1), node:Name, scope:pair (-> +1), whole_span:2 (-> +1)
    print(pair) # call_argument:pair, external_free_call:print, free_call:print, free_call_without_result:print, loaded_variable:pair, node:Call, node:Expr, node:Name

# ----------------------------------------------------------------------------------------
# 144.2145-check-if-file-exists.py
# ----------------------------------------------------------------------------------------
import os # flat_style (-> +1), global_scope:b (-> +1), imperative_style (-> +1), import:os, import_module:os, node:Import, one_liner_style (-> +1), scope:b (-> +1), whole_span:2 (-> +1)
b = os.path.exists(fp) # assignment:exists, assignment_lhs_identifier:b, assignment_rhs_atom:fp, assignment_rhs_atom:os, call_argument:fp, loaded_variable:fp, loaded_variable:os, member_call_method:exists, node:Assign, node:Attribute, node:Call, node:Name, single_assignment:b, value_attr:path

# ----------------------------------------------------------------------------------------
# 144.2915-check-if-file-exists.py
# ----------------------------------------------------------------------------------------
from pathlib import Path # flat_style (-> +1), global_scope:b (-> +1), imperative_style (-> +1), import:pathlib:Path, import_module:pathlib, import_name:Path, node:ImportFrom, one_liner_style (-> +1), scope:b (-> +1), whole_span:2 (-> +1)
b = Path(fp).exists() # assignment:exists, assignment_lhs_identifier:b, assignment_rhs_atom:fp, call_argument:fp, external_free_call:Path, free_call:Path, loaded_variable:fp, member_call_method:exists, node:Assign, node:Attribute, node:Call, node:Name, single_assignment:b

# ----------------------------------------------------------------------------------------
# 145.1822-print-log-line-with-datetime.py
# ----------------------------------------------------------------------------------------
import sys, logging # flat_style (-> +5), global_scope:logger (-> +5), imperative_style (-> +5), import:logging, import:sys, import_module:logging, import_module:sys, node:Import, scope:logger (-> +5), whole_span:6 (-> +5)
logging.basicConfig( # loaded_variable:logging, member_call:logging:basicConfig, member_call_method:basicConfig, member_call_object:logging, node:Attribute, node:Call (-> +1), node:Expr (-> +1), node:Name
    stream=sys.stdout, level=logging.DEBUG, format="%(asctime)-15s %(message)s" # call_argument:, call_keyword_argument:format, call_keyword_argument:level, call_keyword_argument:stream, literal:Str, loaded_variable:logging, loaded_variable:sys, node:Attribute, node:Name, node:Str, value_attr:DEBUG, value_attr:stdout
)
logger = logging.getLogger("NAME OF LOGGER") # assignment:getLogger, assignment_lhs_identifier:logger, assignment_rhs_atom:logging, call_argument:, literal:Str, loaded_variable:logging, member_call_method:getLogger, node:Assign, node:Attribute, node:Call, node:Name, node:Str, single_assignment:logger
logger.info(msg) # call_argument:msg, loaded_variable:logger, loaded_variable:msg, member_call:logger:info, member_call_method:info, member_call_object:logger, node:Attribute, node:Call, node:Expr, node:Name

# ----------------------------------------------------------------------------------------
# 146.1825-convert-string-to-floating-point-number.py
# ----------------------------------------------------------------------------------------
import locale # flat_style (-> +3), global_scope:f (-> +3), global_scope:s (-> +3), imperative_style (-> +3), import:locale, import_module:locale, node:Import, scope:f (-> +3), scope:s (-> +3), whole_span:4 (-> +3)
s = u"545,2222" # assignment, assignment_lhs_identifier:s, literal:Str, node:Assign, node:Name, node:Str, single_assignment:s
locale.setlocale(locale.LC_ALL, "de") # call_argument:, literal:Str, loaded_variable:locale, member_call:locale:setlocale, member_call_method:setlocale, member_call_object:locale, node:Attribute, node:Call, node:Expr, node:Name, node:Str
f = locale.atof(s) # assignment:atof, assignment_lhs_identifier:f, assignment_rhs_atom:locale, assignment_rhs_atom:s, call_argument:s, loaded_variable:locale, loaded_variable:s, member_call_method:atof, node:Assign, node:Attribute, node:Call, node:Name, single_assignment:f

# ----------------------------------------------------------------------------------------
# 146.1826-convert-string-to-floating-point-number.py
# ----------------------------------------------------------------------------------------
f = float(s) # assignment:float, assignment_lhs_identifier:f, assignment_rhs_atom:s, call_argument:s, external_free_call:float, flat_style, free_call:float, global_scope:f, imperative_style, loaded_variable:s, node:Assign, node:Call, node:Name, one_liner_style, scope:f, single_assignment:f, whole_span:1
...
# ----------------------------------------------------------------------------------------
import re # flat_style (-> +1), global_scope:t (-> +1), imperative_style (-> +1), import:re, import_module:re, node:Import, one_liner_style (-> +1), scope:t (-> +1), whole_span:2 (-> +1)
t = re.sub("[^\u0000-\u007f]", "", s) # assignment:sub, assignment_lhs_identifier:t, assignment_rhs_atom:re, assignment_rhs_atom:s, call_argument:, call_argument:s, empty_literal:Str, literal:Str, loaded_variable:re, loaded_variable:s, member_call_method:sub, node:Assign, node:Attribute, node:Call, node:Name, node:Str, single_assignment:t, special_literal_string:[^\x00-\x7f]

# ----------------------------------------------------------------------------------------
# 148.1829-read-list-of-integer-numbers-from-stdin.py
# ----------------------------------------------------------------------------------------
list(map(int, input().split())) # call_argument:, call_argument:int, composition, external_free_call:input, external_free_call:list, external_free_call:map, flat_style, free_call:input, free_call:list, free_call:map, free_call_without_arguments:input, free_call_without_result:list, imperative_style, loaded_variable:int, member_call_method:split, node:Attribute, node:Call, node:Expr, node:Name, one_liner_style, whole_span:1

# ----------------------------------------------------------------------------------------
# 150.2154-remove-trailing-slash.py
# ----------------------------------------------------------------------------------------
p = p.rstrip("/") # assignment:rstrip, assignment_lhs_identifier:p, assignment_rhs_atom:p, call_argument:, flat_style, global_scope:p, imperative_style, literal:Str, loaded_variable:p, member_call_method:rstrip, node:Assign, node:Attribute, node:Call, node:Name, node:Str, one_liner_style, scope:p, single_assignment:p, whole_span:1

# ----------------------------------------------------------------------------------------
# 151.2166-remove-string-trailing-path-separator.py
# ----------------------------------------------------------------------------------------
import os # global_scope:p (-> +2), imperative_style (-> +2), import:os, import_module:os, node:Import, scope:p (-> +2), whole_span:3 (-> +2)
if p.endswith(os.sep): # call_argument:, if (-> +1), if_test_atom:os, if_test_atom:p, if_without_else (-> +1), loaded_variable:os, loaded_variable:p, member_call_method:endswith, node:Attribute, node:Call, node:If (-> +1), node:Name
...
# ----------------------------------------------------------------------------------------
s = c # assignment, assignment_lhs_identifier:s, assignment_rhs_atom:c, flat_style, global_scope:s, imperative_style, loaded_variable:c, node:Assign, node:Name, one_liner_style, scope:s, single_assignment:s, whole_span:1

# ----------------------------------------------------------------------------------------
# 153.1980-concatenate-string-with-integer.py
# ----------------------------------------------------------------------------------------
t = "{}{}".format(s, i) # assignment:format, assignment_lhs_identifier:t, assignment_rhs_atom:i, assignment_rhs_atom:s, call_argument:i, call_argument:s, flat_style, global_scope:t, imperative_style, literal:Str, loaded_variable:i, loaded_variable:s, member_call_method:format, node:Assign, node:Attribute, node:Call, node:Name, node:Str, one_liner_style, scope:t, single_assignment:t, whole_span:1
...
class RGB(numpy.ndarray): # loaded_variable:numpy, node:Attribute, node:ClassDef (-> +8), node:Name
    ...
    def from_str(cls, rgbstr): # function_parameter:cls, function_parameter:rgbstr, function_parameter_flavor:arg, node:arg
        ...
        ).view(cls) # call_argument:cls, loaded_variable:cls, node:Name
    def __str__(self): # function:__str__ (-> +2), function_parameter:self, function_parameter_flavor:arg, function_returning_something:__str__ (-> +2), instance_method:__str__ (-> +2), local_scope:self (-> +2), method:__str__ (-> +2), node:FunctionDef (-> +2), node:arg, scope:self (-> +2)
        self = self.astype(numpy.uint8) # assignment:astype, assignment_lhs_identifier:self, assignment_rhs_atom:numpy, assignment_rhs_atom:self, call_argument:, loaded_variable:numpy, loaded_variable:self, member_call_method:astype, node:Assign, node:Attribute, node:Call, node:Name, single_assignment:self, update:self:numpy, update_by_assignment:self:numpy, update_by_assignment_with:astype, update_with:astype
        return "#" + "".join(format(n, "x") for n in self) # binary_operator:Add, call_argument:, call_argument:n, composition, comprehension:Generator, comprehension_for_count:1, concatenation_operator:Str, empty_literal:Str, external_free_call:format, free_call:format, iteration_variable:n, literal:Str, loaded_variable:n, loaded_variable:self, local_scope:n, member_call_method:join, node:Attribute, node:BinOp, node:Call, node:GeneratorExp, node:Name, node:Return, node:Str, return, scope:n
c1 = RGB.from_str("#a1b1c1") # assignment:from_str, assignment_lhs_identifier:c1, assignment_rhs_atom:RGB, call_argument:, literal:Str, loaded_variable:RGB, member_call_method:from_str, node:Assign, node:Attribute, node:Call, node:Name, node:Str, single_assignment:c1
print(c1) # call_argument:c1, external_free_call:print, free_call:print, free_call_without_result:print, loaded_variable:c1, node:Call, node:Expr, node:Name
c2 = RGB.from_str("#1A1B1C") # assignment:from_str, assignment_lhs_identifier:c2, assignment_rhs_atom:RGB, call_argument:, literal:Str, loaded_variable:RGB, member_call_method:from_str, node:Assign, node:Attribute, node:Call, node:Name, node:Str, single_assignment:c2
print(c2) # call_argument:c2, external_free_call:print, free_call:print, free_call_without_result:print, loaded_variable:c2, node:Call, node:Expr, node:Name
print((c1 + c2) / 2) # addition_operator, binary_operator:Add, binary_operator:Div, call_argument:, external_free_call:print, free_call:print, free_call_without_result:print, literal:2, loaded_variable:c1, loaded_variable:c2, node:BinOp, node:Call, node:Expr, node:Name, node:Num

# ----------------------------------------------------------------------------------------
# 155.2147-delete-file.py
# ----------------------------------------------------------------------------------------
import pathlib # flat_style (-> +2), global_scope:path (-> +2), imperative_style (-> +2), import:pathlib, import_module:pathlib, node:Import, scope:path (-> +2), whole_span:3 (-> +2)
path = pathlib.Path(_filepath) # assignment:Path, assignment_lhs_identifier:path, assignment_rhs_atom:_filepath, assignment_rhs_atom:pathlib, call_argument:_filepath, loaded_variable:_filepath, loaded_variable:pathlib, member_call_method:Path, node:Assign, node:Attribute, node:Call, node:Name, single_assignment:path
...
# ----------------------------------------------------------------------------------------
s = format("03d", i) # assignment:format, assignment_lhs_identifier:s, assignment_rhs_atom:i, call_argument:, call_argument:i, external_free_call:format, flat_style, free_call:format, global_scope:s, imperative_style, literal:Str, loaded_variable:i, node:Assign, node:Call, node:Name, node:Str, one_liner_style, scope:s, single_assignment:s, whole_span:1

# ----------------------------------------------------------------------------------------
# 157.2150-declare-constant-string.py
# ----------------------------------------------------------------------------------------
PLANET = "Earth" # assignment, assignment_lhs_identifier:PLANET, flat_style, global_scope:PLANET, imperative_style, literal:Str, node:Assign, node:Name, node:Str, one_liner_style, scope:PLANET, single_assignment:PLANET, whole_span:1

# ----------------------------------------------------------------------------------------
# 158.2163-random-sublist.py
# ----------------------------------------------------------------------------------------
import random # flat_style (-> +1), global_scope:y (-> +1), imperative_style (-> +1), import:random, import_module:random, node:Import, one_liner_style (-> +1), scope:y (-> +1), whole_span:2 (-> +1)
y = random.sample(x, k) # assignment:sample, assignment_lhs_identifier:y, assignment_rhs_atom:k, assignment_rhs_atom:random, assignment_rhs_atom:x, call_argument:k, call_argument:x, loaded_variable:k, loaded_variable:random, loaded_variable:x, member_call_method:sample, node:Assign, node:Attribute, node:Call, node:Name, single_assignment:y
...
# ----------------------------------------------------------------------------------------
from itertools import tee # global_scope:a (-> +6), global_scope:b (-> +6), import:itertools:tee, import_module:itertools, import_name:tee, node:ImportFrom, procedural_style (-> +6), scope:a (-> +6), scope:b (-> +6), whole_span:7 (-> +6)
def pairwise(iterable): # function:pairwise (-> +3), function_parameter:iterable, function_parameter_flavor:arg, function_returning_something:pairwise (-> +3), impure_function:pairwise (-> +3), local_scope:a (-> +3), local_scope:b (-> +3), local_scope:iterable (-> +3), node:FunctionDef (-> +3), node:arg, scope:a (-> +3), scope:b (-> +3), scope:iterable (-> +3), shadowing_scope:a (-> +3), shadowing_scope:b (-> +3)
    a, b = tee(iterable) # assignment:tee, assignment_lhs_identifier:a, assignment_lhs_identifier:b, assignment_rhs_atom:iterable, call_argument:iterable, external_free_call:tee, free_call:tee, literal:Tuple, loaded_variable:iterable, node:Assign, node:Call, node:Name, node:Tuple, parallel_assignment:2
    next(b, None) # call_argument:None, call_argument:b, external_free_call:next, free_call:next, free_call_without_result:next, literal:None, loaded_variable:b, node:Call, node:Expr, node:Name, node:NameConstant
    return zip(a, b) # call_argument:a, call_argument:b, external_free_call:zip, free_call:zip, free_tail_call:zip, loaded_variable:a, loaded_variable:b, node:Call, node:Name, node:Return, return
for a, b in pairwise(list): # call_argument:list, for:a (-> +1), for:b (-> +1), free_call:pairwise, internal_free_call:pairwise, iteration_variable:a, iteration_variable:b, literal:Tuple, loaded_variable:list, loop:for (-> +1), loop_with_late_exit:for (-> +1), node:Call, node:For (-> +1), node:Name, node:Tuple
    print(a, b) # call_argument:a, call_argument:b, external_free_call:print, free_call:print, free_call_without_result:print, loaded_variable:a, loaded_variable:b, node:Call, node:Expr, node:Name
...
webbrowser.open(s) # call_argument:s, loaded_variable:s, loaded_variable:webbrowser, member_call:webbrowser:open, member_call_method:open, member_call_object:webbrowser, node:Attribute, node:Call, node:Expr, node:Name

# ----------------------------------------------------------------------------------------
# 165.2149-last-element-of-list.py
# ----------------------------------------------------------------------------------------
x = items[-1] # assignment, assignment_lhs_identifier:x, assignment_rhs_atom:-1, assignment_rhs_atom:items, flat_style, global_scope:x, imperative_style, index:-1, literal:-1, loaded_variable:items, negative_index:-1, node:Assign, node:Name, node:Num, node:Subscript, one_liner_style, scope:x, single_assignment:x, whole_span:1

# ----------------------------------------------------------------------------------------
# 166.2272-concatenate-two-lists.py
# ----------------------------------------------------------------------------------------
ab = a + b # addition_operator, assignment:Add, assignment_lhs_identifier:ab, assignment_rhs_atom:a, assignment_rhs_atom:b, binary_operator:Add, flat_style, global_scope:ab, imperative_style, loaded_variable:a, loaded_variable:b, node:Assign, node:BinOp, node:Name, one_liner_style, scope:ab, single_assignment:ab, whole_span:1

# ----------------------------------------------------------------------------------------
# 167.2611-trim-prefix.py
# ----------------------------------------------------------------------------------------
t = s[s.startswith(p) and len(p) :] # assignment, assignment_lhs_identifier:t, assignment_rhs_atom:p, assignment_rhs_atom:s, boolean_operator:And, call_argument:p, external_free_call:len, flat_style, free_call:len, global_scope:t, imperative_style, loaded_variable:p, loaded_variable:s, member_call_method:startswith, node:Assign, node:Attribute, node:BoolOp, node:Call, node:Name, node:Subscript, one_liner_style, scope:t, single_assignment:t, slice:_::, slice_lower:_, slice_step:, slice_upper:, whole_span:1

# ----------------------------------------------------------------------------------------
# 167.3175-trim-prefix.py
# ----------------------------------------------------------------------------------------
t = s.lstrip(p) # assignment:lstrip, assignment_lhs_identifier:t, assignment_rhs_atom:p, assignment_rhs_atom:s, call_argument:p, flat_style, global_scope:t, imperative_style, loaded_variable:p, loaded_variable:s, member_call_method:lstrip, node:Assign, node:Attribute, node:Call, node:Name, one_liner_style, scope:t, single_assignment:t, whole_span:1

# ----------------------------------------------------------------------------------------
# 168.2277-trim-suffix.py
# ----------------------------------------------------------------------------------------
t = s.rsplit(w, 1)[0] # assignment, assignment_lhs_identifier:t, assignment_rhs_atom:0, assignment_rhs_atom:1, assignment_rhs_atom:s, assignment_rhs_atom:w, call_argument:1, call_argument:w, flat_style, global_scope:t, imperative_style, index:0, literal:0, literal:1, loaded_variable:s, loaded_variable:w, member_call:s:rsplit, member_call_method:rsplit, member_call_object:s, node:Assign, node:Attribute, node:Call, node:Name, node:Num, node:Subscript, one_liner_style, scope:t, single_assignment:t, whole_span:1

# ----------------------------------------------------------------------------------------
# 168.3174-trim-suffix.py
# ----------------------------------------------------------------------------------------
t = s.rstrip(w) # assignment:rstrip, assignment_lhs_identifier:t, assignment_rhs_atom:s, assignment_rhs_atom:w, call_argument:w, flat_style, global_scope:t, imperative_style, loaded_variable:s, loaded_variable:w, member_call_method:rstrip, node:Assign, node:Attribute, node:Call, node:Name, one_liner_style, scope:t, single_assignment:t, whole_span:1

# ----------------------------------------------------------------------------------------
# 169.2233-string-length.py
# ----------------------------------------------------------------------------------------
n = len(s) # assignment:len, assignment_lhs_identifier:n, assignment_rhs_atom:s, call_argument:s, external_free_call:len, flat_style, free_call:len, global_scope:n, imperative_style, loaded_variable:s, node:Assign, node:Call, node:Name, one_liner_style, scope:n, single_assignment:n, whole_span:1

# ----------------------------------------------------------------------------------------
# 170.2275-get-map-size.py
# ----------------------------------------------------------------------------------------
n = len(mymap) # assignment:len, assignment_lhs_identifier:n, assignment_rhs_atom:mymap, call_argument:mymap, external_free_call:len, flat_style, free_call:len, global_scope:n, imperative_style, loaded_variable:mymap, node:Assign, node:Call, node:Name, one_liner_style, scope:n, single_assignment:n, whole_span:1

# ----------------------------------------------------------------------------------------
# 171.2446-add-an-element-at-the-end-of-a-list.py
# ----------------------------------------------------------------------------------------
s.append(x) # call_argument:x, flat_style, imperative_style, loaded_variable:s, loaded_variable:x, member_call:s:append, member_call_method:append, member_call_object:s, node:Attribute, node:Call, node:Expr, node:Name, one_liner_style, update:s:x, update_by_member_call:s:x, update_by_member_call_with:append, update_with:append, whole_span:1

# ----------------------------------------------------------------------------------------
# 172.2442-insert-entry-in-map.py
# ----------------------------------------------------------------------------------------
m[k] = v # assignment, assignment_lhs_identifier:m, assignment_rhs_atom:v, flat_style, imperative_style, index:k, loaded_variable:k, loaded_variable:m, loaded_variable:v, node:Assign, node:Name, node:Subscript, one_liner_style, subscript_assignment:Index, whole_span:1
...
# ----------------------------------------------------------------------------------------
from urllib import request, parse # flat_style (-> +3), global_scope:data (-> +3), global_scope:req (-> +3), global_scope:resp (-> +3), imperative_style (-> +3), import:urllib:parse, import:urllib:request, import_module:urllib, import_name:parse, import_name:request, node:ImportFrom, scope:data (-> +3), scope:req (-> +3), scope:resp (-> +3), whole_span:4 (-> +3)
data = parse.urlencode("<your data dict>").encode() # assignment:encode, assignment_lhs_identifier:data, assignment_rhs_atom:parse, call_argument:, literal:Str, loaded_variable:parse, member_call:parse:encode, member_call:parse:urlencode, member_call_method:encode, member_call_method:urlencode, member_call_object:parse, method_chaining, node:Assign, node:Attribute, node:Call, node:Name, node:Str, single_assignment:data
req = request.Request(u, data=data, method="POST") # assignment:Request, assignment_lhs_identifier:req, assignment_rhs_atom:data, assignment_rhs_atom:request, assignment_rhs_atom:u, call_argument:, call_argument:data, call_argument:u, call_keyword_argument:data, call_keyword_argument:method, literal:Str, loaded_variable:data, loaded_variable:request, loaded_variable:u, member_call_method:Request, node:Assign, node:Attribute, node:Call, node:Name, node:Str, single_assignment:req
resp = request.urlopen(req) # assignment:urlopen, assignment_lhs_identifier:resp, assignment_rhs_atom:req, assignment_rhs_atom:request, call_argument:req, loaded_variable:req, loaded_variable:request, member_call_method:urlopen, node:Assign, node:Attribute, node:Call, node:Name, single_assignment:resp

# ----------------------------------------------------------------------------------------
# 175.2613-bytes-to-hex-string.py
# ----------------------------------------------------------------------------------------
s = a.hex() # assignment:hex, assignment_lhs_identifier:s, assignment_rhs_atom:a, flat_style, global_scope:s, imperative_style, loaded_variable:a, member_call_method:hex, node:Assign, node:Attribute, node:Call, node:Name, one_liner_style, scope:s, single_assignment:s, whole_span:1

# ----------------------------------------------------------------------------------------
# 176.2614-hex-string-to-byte-array.py
# ----------------------------------------------------------------------------------------
a = bytearray.fromhex(s) # assignment:fromhex, assignment_lhs_identifier:a, assignment_rhs_atom:bytearray, assignment_rhs_atom:s, call_argument:s, flat_style, global_scope:a, imperative_style, loaded_variable:bytearray, loaded_variable:s, member_call_method:fromhex, node:Assign, node:Attribute, node:Call, node:Name, one_liner_style, scope:a, single_assignment:a, whole_span:1

# ----------------------------------------------------------------------------------------
# 177.2709-find-files-with-a-given-list-of-filename-extensions.py
# ----------------------------------------------------------------------------------------
import os # flat_style (-> +2), global_scope:L (-> +2), global_scope:extensions (-> +2), imperative_style (-> +2), import:os, import_module:os, node:Import, scope:L (-> +2), scope:extensions (-> +2), whole_span:3 (-> +2)
...
# ----------------------------------------------------------------------------------------
import re # flat_style (-> +6), global_scope:filtered_files (-> +6), imperative_style (-> +6), import:re, import_module:re, node:Import, scope:filtered_files (-> +6), whole_span:7 (-> +6)
...
filtered_files = [ # assignment, assignment_lhs_identifier:filtered_files, node:Assign (-> +4), node:Name, single_assignment:filtered_files
    "{}/{}".format(dirpath, filename) # assignment_rhs_atom:dirpath, assignment_rhs_atom:filename, call_argument:dirpath, call_argument:filename, comprehension:List, comprehension_for_count:2, literal:Str, loaded_variable:dirpath, loaded_variable:filename, local_scope:_, local_scope:dirpath, local_scope:filename, local_scope:filenames, member_call_method:format, node:Attribute, node:Call, node:ListComp (-> +3), node:Name, node:Str, scope:_, scope:dirpath, scope:filename, scope:filenames
    for dirpath, _, filenames in os.walk(D) # assignment_rhs_atom:D, assignment_rhs_atom:_, assignment_rhs_atom:dirpath, assignment_rhs_atom:filenames, assignment_rhs_atom:os, call_argument:D, iteration_variable:_, iteration_variable:dirpath, iteration_variable:filenames, literal:Tuple, loaded_variable:D, loaded_variable:os, member_call_method:walk, node:Attribute, node:Call, node:Name, node:Tuple
    ...
    if re.match(r"^.*\.(?:jpg|jpeg|png)$", filename) # assignment_rhs_atom:filename, assignment_rhs_atom:re, call_argument:, call_argument:filename, filtered_comprehension, literal:Str, loaded_variable:filename, loaded_variable:re, member_call_method:match, node:Attribute, node:Call, node:Name, node:Str
]
...
center = Point((x1 + x2) / 2, (y1 + y2) / 2) # addition_operator, assignment:Point, assignment_lhs_identifier:center, assignment_rhs_atom:2, assignment_rhs_atom:x1, assignment_rhs_atom:x2, assignment_rhs_atom:y1, assignment_rhs_atom:y2, binary_operator:Add, binary_operator:Div, call_argument:, external_free_call:Point, free_call:Point, literal:2, loaded_variable:x1, loaded_variable:x2, loaded_variable:y1, loaded_variable:y2, node:Assign, node:BinOp, node:Call, node:Name, node:Num, single_assignment:center

# ----------------------------------------------------------------------------------------
# 180.2612-list-files-in-directory.py
# ----------------------------------------------------------------------------------------
import os # flat_style (-> +1), global_scope:x (-> +1), imperative_style (-> +1), import:os, import_module:os, node:Import, one_liner_style (-> +1), scope:x (-> +1), whole_span:2 (-> +1)
x = os.listdir(d) # assignment:listdir, assignment_lhs_identifier:x, assignment_rhs_atom:d, assignment_rhs_atom:os, call_argument:d, loaded_variable:d, loaded_variable:os, member_call_method:listdir, node:Assign, node:Attribute, node:Call, node:Name, single_assignment:x
...
print(s % s) # binary_operator:Mod, call_argument:, external_free_call:print, free_call:print, free_call_without_result:print, loaded_variable:s, modulo_operator, node:BinOp, node:Call, node:Expr, node:Name

# ----------------------------------------------------------------------------------------
# 183.3025-make-http-put-request.py
# ----------------------------------------------------------------------------------------
requests # flat_style (-> +6), global_scope:content (-> +6), global_scope:content_type (-> +6), global_scope:data (-> +6), global_scope:headers (-> +6), global_scope:r (-> +6), global_scope:status_code (-> +6), imperative_style (-> +6), loaded_variable:requests, node:Expr, node:Name, scope:content (-> +6), scope:content_type (-> +6), scope:data (-> +6), scope:headers (-> +6), scope:r (-> +6), scope:status_code (-> +6), whole_span:7 (-> +6)
...
r = requests.put(url, headers=headers, data=data) # assignment:put, assignment_lhs_identifier:r, assignment_rhs_atom:data, assignment_rhs_atom:headers, assignment_rhs_atom:requests, assignment_rhs_atom:url, call_argument:data, call_argument:headers, call_argument:url, call_keyword_argument:data, call_keyword_argument:headers, loaded_variable:data, loaded_variable:headers, loaded_variable:requests, loaded_variable:url, member_call_method:put, node:Assign, node:Attribute, node:Call, node:Name, single_assignment:r
...
date.today() + timedelta(days=1) # addition_operator, binary_operator:Add, call_argument:1, call_keyword_argument:days, external_free_call:timedelta, free_call:timedelta, literal:1, loaded_variable:date, member_call_method:today, node:Attribute, node:BinOp, node:Call, node:Expr, node:Name, node:Num

# ----------------------------------------------------------------------------------------
# 185.2820-execute-function-in-30-seconds.py
# ----------------------------------------------------------------------------------------
import threading # flat_style (-> +2), global_scope:timer (-> +2), imperative_style (-> +2), import:threading, import_module:threading, node:Import, scope:timer (-> +2), whole_span:3 (-> +2)
timer = threading.Timer(30.0, f, args=(42,)) # assignment:Timer, assignment_lhs_identifier:timer, assignment_rhs_atom:30.0, assignment_rhs_atom:42, assignment_rhs_atom:f, assignment_rhs_atom:threading, call_argument:, call_argument:30.0, call_argument:f, call_keyword_argument:args, literal:30.0, literal:42, literal:Tuple, loaded_variable:f, loaded_variable:threading, magic_number:30.0, magic_number:42, member_call_method:Timer, node:Assign, node:Attribute, node:Call, node:Name, node:Num, node:Tuple, single_assignment:timer
...
# ----------------------------------------------------------------------------------------
import sys # flat_style (-> +1), imperative_style (-> +1), import:sys, import_module:sys, node:Import, one_liner_style (-> +1), whole_span:2 (-> +1)
sys.exit(0) # call_argument:0, literal:0, loaded_variable:sys, member_call:sys:exit, member_call_method:exit, member_call_object:sys, node:Attribute, node:Call, node:Expr, node:Name, node:Num

# ----------------------------------------------------------------------------------------
# 187.3261-disjoint-set.py
# ----------------------------------------------------------------------------------------
class UnionFind: # node:ClassDef (-> +14), object_oriented_style (-> +14), whole_span:15 (-> +14)
    def __init__(self, size): # function:__init__ (-> +2), function_parameter:self, function_parameter:size, function_parameter_flavor:arg, function_returning_nothing:__init__ (-> +2), instance_method:__init__ (-> +2), local_scope:self (-> +2), local_scope:size (-> +2), method:__init__ (-> +2), node:FunctionDef (-> +2), node:arg, scope:self (-> +2), scope:size (-> +2)
        ...
        self.p = [i for i in range(size)] # assignment, assignment_lhs_identifier:self, assignment_rhs_atom:i, assignment_rhs_atom:size, call_argument:size, comprehension:List, comprehension_for_count:1, external_free_call:range, free_call:range, iteration_variable:i, loaded_variable:i, loaded_variable:self, loaded_variable:size, local_scope:i, node:Assign, node:Attribute, node:Call, node:ListComp, node:Name, range:size, scope:i
    def find_set(self, i): # function:find_set (-> +5), function_parameter:i, function_parameter:self, function_parameter_flavor:arg, function_returning_something:find_set (-> +5), instance_method:find_set (-> +5), local_scope:i (-> +5), local_scope:self (-> +5), method:find_set (-> +5), node:FunctionDef (-> +5), node:arg, scope:i (-> +5), scope:self (-> +5)
        if self.p[i] == i: # comparison_operator:Eq, if (-> +4), if_test_atom:i, if_test_atom:self, index:i, loaded_variable:i, loaded_variable:self, node:Attribute, node:Compare, node:If (-> +4), node:Name, node:Subscript, value_attr:p
            ...
            self.p[i] = self.find_set(self.p[i]) # assignment:find_set, assignment_rhs_atom:i, assignment_rhs_atom:self, call_argument:, if_else_branch (-> +1), index:i, loaded_variable:i, loaded_variable:self, member_call_method:find_set, node:Assign, node:Attribute, node:Call, node:Name, node:Subscript, subscript_assignment:Index, value_attr:p
    ...
    def is_same_set(self, i, j): # function:is_same_set (-> +1), function_parameter:i, function_parameter:j, function_parameter:self, function_parameter_flavor:arg, function_returning_something:is_same_set (-> +1), instance_method:is_same_set (-> +1), local_scope:i (-> +1), local_scope:j (-> +1), local_scope:self (-> +1), method:is_same_set (-> +1), node:FunctionDef (-> +1), node:arg, scope:i (-> +1), scope:j (-> +1), scope:self (-> +1)
        return self.find_set(i) == self.find_set(j) # call_argument:i, call_argument:j, comparison_operator:Eq, loaded_variable:i, loaded_variable:j, loaded_variable:self, member_call_method:find_set, node:Attribute, node:Call, node:Compare, node:Name, node:Return, return
    def union_set(self, i, j): # function:union_set (-> +2), function_parameter:i, function_parameter:j, function_parameter:self, function_parameter_flavor:arg, function_returning_nothing:union_set (-> +2), instance_method:union_set (-> +2), local_scope:i (-> +2), local_scope:j (-> +2), local_scope:self (-> +2), local_scope:x (-> +2), local_scope:y (-> +2), method:union_set (-> +2), node:FunctionDef (-> +2), node:arg, scope:i (-> +2), scope:j (-> +2), scope:self (-> +2), scope:x (-> +2), scope:y (-> +2)
        if not self.is_same_set(i, j): # call_argument:i, call_argument:j, if (-> +1), if_test_atom:i, if_test_atom:j, if_test_atom:self, if_without_else (-> +1), loaded_variable:i, loaded_variable:j, loaded_variable:self, member_call_method:is_same_set, node:Attribute, node:Call, node:If (-> +1), node:Name, node:UnaryOp, unary_operator:Not
            x, y = self.find_set(i), self.find_set(j) # assignment, assignment_lhs_identifier:x, assignment_lhs_identifier:y, assignment_rhs_atom:i, assignment_rhs_atom:j, assignment_rhs_atom:self, call_argument:i, call_argument:j, if_then_branch, literal:Tuple, loaded_variable:i, loaded_variable:j, loaded_variable:self, member_call_method:find_set, node:Assign, node:Attribute, node:Call, node:Name, node:Tuple, parallel_assignment:2

# ----------------------------------------------------------------------------------------
# 188.3171-matrix-multiplication.py
# ----------------------------------------------------------------------------------------
import numpy as np # flat_style (-> +1), global_scope:c (-> +1), imperative_style (-> +1), import:numpy, import_module:numpy, node:Import, one_liner_style (-> +1), scope:c (-> +1), whole_span:2 (-> +1)
...
# ----------------------------------------------------------------------------------------
import numpy as np # flat_style (-> +1), global_scope:c (-> +1), imperative_style (-> +1), import:numpy, import_module:numpy, node:Import, one_liner_style (-> +1), scope:c (-> +1), whole_span:2 (-> +1)
c = np.matmul(a, b) # assignment:matmul, assignment_lhs_identifier:c, assignment_rhs_atom:a, assignment_rhs_atom:b, assignment_rhs_atom:np, call_argument:a, call_argument:b, loaded_variable:a, loaded_variable:b, loaded_variable:np, member_call_method:matmul, node:Assign, node:Attribute, node:Call, node:Name, single_assignment:c
...
# ----------------------------------------------------------------------------------------
with open(path) as f: # call_argument:path, external_free_call:open, flat_style (-> +1), free_call:open, global_scope:lines (-> +1), imperative_style (-> +1), loaded_variable:path, node:Call, node:Name, node:With (-> +1), scope:lines (-> +1), whole_span:2 (-> +1)

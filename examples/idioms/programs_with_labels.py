...
# ----------------------------------------------------------------------------------------
from __future__ import print_function # flat_style (-> +1), import:__future__:print_function, import_module:__future__, import_name:print_function, n_liner_style:1 (-> +1), node:ImportFrom, one_shot_style (-> +1), whole_span:2 (-> +1)
...
# ----------------------------------------------------------------------------------------
print("Hello World") # call_argument:, external_free_call:print, flat_style, free_call:print, free_call_without_result:print, literal:Str, n_liner_style:1, node:Call, node:Expr, node:Name, node:Str, one_shot_style, whole_span:1
...
# ----------------------------------------------------------------------------------------
def finish(name): # function:finish (-> +1), function_argument:name, function_argument_flavor:arg, function_returning_nothing:finish (-> +1), n_liner_style:1 (-> +1), node:FunctionDef (-> +1), node:arg, procedural_style (-> +1), whole_span:2 (-> +1)
...
# ----------------------------------------------------------------------------------------
def a(): # function:a (-> +1), function_returning_nothing:a (-> +1), function_without_arguments:a (-> +1), n_liner_style:1 (-> +1), node:FunctionDef (-> +1), procedural_style (-> +1), whole_span:2 (-> +1)
    pass # node:Pass

# ----------------------------------------------------------------------------------------
# 004.0024-create-a-function-which-returns-the-square-of-an-integer.py
# ----------------------------------------------------------------------------------------
def square(x): # function:square (-> +1), function_argument:x, function_argument_flavor:arg, function_returning_something:square (-> +1), functional_style (-> +1), n_liner_style:1 (-> +1), node:FunctionDef (-> +1), node:arg, whole_span:2 (-> +1)
...
# ----------------------------------------------------------------------------------------
from dataclasses import dataclass # import:dataclasses:dataclass, import_module:dataclasses, import_name:dataclass, n_liner_style:1 (-> +1), node:ImportFrom, object_oriented_style (-> +1), whole_span:2 (-> +1)
...
# ----------------------------------------------------------------------------------------
for x in items: # for:x (-> +1), for_each (-> +1), loop:for (-> +1), loop_with_late_exit:for (-> +1), n_liner_style:2 (-> +1), node:For (-> +1), node:Name, one_shot_style (-> +1), whole_span:2 (-> +1)
...
# ----------------------------------------------------------------------------------------
for i, x in enumerate(items): # call_argument:items, external_free_call:enumerate, for:i (-> +1), for:x (-> +1), for_indexes_elements (-> +1), free_call:enumerate, literal:Tuple, loop:for (-> +1), loop_with_late_exit:for (-> +1), n_liner_style:2 (-> +1), node:Call, node:For (-> +1), node:Name, node:Tuple, one_shot_style (-> +1), whole_span:2 (-> +1)
...
# ----------------------------------------------------------------------------------------
x = {"one": 1, "two": 2} # assignment, assignment_lhs_identifier:x, assignment_rhs_atom:1, assignment_rhs_atom:2, flat_style, literal:1, literal:2, literal:Dict, literal:Str, n_liner_style:1, node:Assign, node:Dict, node:Name, node:Num, node:Str, one_shot_style, single_assignment:x, whole_span:1

# ----------------------------------------------------------------------------------------
# 009.1410-create-a-binary-tree-data-structure.py
# ----------------------------------------------------------------------------------------
class Node: # n_liner_style:4 (-> +4), node:ClassDef (-> +4), object_oriented_style (-> +4), whole_span:5 (-> +4)
...
# ----------------------------------------------------------------------------------------
from random import shuffle # flat_style (-> +1), import:random:shuffle, import_module:random, import_name:shuffle, n_liner_style:1 (-> +1), node:ImportFrom, one_shot_style (-> +1), whole_span:2 (-> +1)
...
# ----------------------------------------------------------------------------------------
import random # flat_style (-> +1), import:random, import_module:random, n_liner_style:1 (-> +1), node:Import, one_shot_style (-> +1), whole_span:2 (-> +1)
...
# ----------------------------------------------------------------------------------------
x in list # comparison_operator:In, flat_style, n_liner_style:1, node:Compare, node:Expr, node:Name, one_shot_style, whole_span:1

# ----------------------------------------------------------------------------------------
# 013.0574-iterate-over-map-keys-and-values.py
# ----------------------------------------------------------------------------------------
for k, v in mymap.items(): # for:k (-> +1), for:v (-> +1), literal:Tuple, loop:for (-> +1), loop_with_late_exit:for (-> +1), member_call_method:items, n_liner_style:2 (-> +1), node:Attribute, node:Call, node:For (-> +1), node:Name, node:Tuple, one_shot_style (-> +1), whole_span:2 (-> +1)
...
# ----------------------------------------------------------------------------------------
import random # functional_style (-> +2), import:random, import_module:random, n_liner_style:1 (-> +2), node:Import, whole_span:3 (-> +2)
...
# ----------------------------------------------------------------------------------------
def dfs(bt): # body_recursive_function:dfs (-> +5), function:dfs (-> +5), function_argument:bt, function_argument_flavor:arg, function_returning_nothing:dfs (-> +5), n_liner_style:5 (-> +5), node:FunctionDef (-> +5), node:arg, procedural_style (-> +5), recursive_call_count:2 (-> +5), recursive_function:dfs (-> +5), whole_span:6 (-> +5)
...
# ----------------------------------------------------------------------------------------
class Node(object): # n_liner_style:3 (-> +3), node:ClassDef (-> +3), node:Name, object_oriented_style (-> +3), whole_span:4 (-> +3)
...
# ----------------------------------------------------------------------------------------
def DFS(f, root): # body_recursive_function:DFS (-> +3), function:DFS (-> +3), function_argument:f, function_argument:root, function_argument_flavor:arg, function_returning_nothing:DFS (-> +3), higher_order_function:f (-> +3), n_liner_style:3 (-> +3), node:FunctionDef (-> +3), node:arg, procedural_style (-> +3), recursive_call_count:1 (-> +3), recursive_function:DFS (-> +3), whole_span:4 (-> +3)
...
# ----------------------------------------------------------------------------------------
x = reversed(x) # assignment:reversed, assignment_lhs_identifier:x, assignment_rhs_atom:x, call_argument:x, external_free_call:reversed, flat_style, free_call:reversed, n_liner_style:1, node:Assign, node:Call, node:Name, one_shot_style, single_assignment:x, whole_span:1

# ----------------------------------------------------------------------------------------
# 019.1983-reverse-a-list.py
# ----------------------------------------------------------------------------------------
y = x[::-1] # assignment, assignment_lhs_identifier:y, assignment_rhs_atom:-1, assignment_rhs_atom:x, flat_style, literal:-1, n_liner_style:1, node:Assign, node:Name, node:Num, node:Subscript, one_shot_style, single_assignment:y, slice:::-1, slice_lower:, slice_step:-1, slice_upper:, whole_span:1

# ----------------------------------------------------------------------------------------
# 019.3164-reverse-a-list.py
# ----------------------------------------------------------------------------------------
x.reverse() # flat_style, member_call:x:reverse, member_call_method:reverse, member_call_object:x, n_liner_style:1, node:Attribute, node:Call, node:Expr, node:Name, one_shot_style, whole_span:1

# ----------------------------------------------------------------------------------------
# 020.0573-return-two-values.py
# ----------------------------------------------------------------------------------------
def search(m, x): # function:search (-> +3), function_argument:m, function_argument:x, function_argument_flavor:arg, function_returning_something:search (-> +3), n_liner_style:3 (-> +3), node:FunctionDef (-> +3), node:arg, whole_span:4 (-> +3)
...
# ----------------------------------------------------------------------------------------
a, b = b, a # assignment, assignment_lhs_identifier:a, assignment_lhs_identifier:b, assignment_rhs_atom:a, assignment_rhs_atom:b, flat_style, literal:Tuple, n_liner_style:1, node:Assign, node:Name, node:Tuple, one_shot_style, parallel_assignment:2, swap, update:a:b, update:b:a, update_by_assignment:a:b, update_by_assignment:b:a, update_by_assignment_with, update_with, whole_span:1

# ----------------------------------------------------------------------------------------
# 022.0243-convert-string-to-integer.py
# ----------------------------------------------------------------------------------------
i = int(s) # assignment:int, assignment_lhs_identifier:i, assignment_rhs_atom:s, call_argument:s, external_free_call:int, flat_style, free_call:int, n_liner_style:1, node:Assign, node:Call, node:Name, one_shot_style, single_assignment:i, whole_span:1

# ----------------------------------------------------------------------------------------
# 023.1102-convert-real-number-to-string-with-2-decimal-places.py
# ----------------------------------------------------------------------------------------
s = "{:.2f}".format(x) # assignment:format, assignment_lhs_identifier:s, assignment_rhs_atom:x, call_argument:x, flat_style, literal:Str, member_call_method:format, n_liner_style:1, node:Assign, node:Attribute, node:Call, node:Name, node:Str, one_shot_style, single_assignment:s, whole_span:1

# ----------------------------------------------------------------------------------------
# 024.0664-assign-to-string-the-japanese-word-.py
# ----------------------------------------------------------------------------------------
s = "ネコ" # assignment, assignment_lhs_identifier:s, flat_style, literal:Str, n_liner_style:1, node:Assign, node:Name, node:Str, one_shot_style, single_assignment:s, whole_span:1

# ----------------------------------------------------------------------------------------
# 025.0195-send-a-value-to-another-thread.py
# ----------------------------------------------------------------------------------------
import Queue # flat_style (-> +5), import:Queue, import_module:Queue, n_liner_style:5 (-> +5), node:Import, one_shot_style (-> +5), whole_span:6 (-> +5)
...
# ----------------------------------------------------------------------------------------
import numpy # flat_style (-> +1), import:numpy, import_module:numpy, n_liner_style:1 (-> +1), node:Import, one_shot_style (-> +1), whole_span:2 (-> +1)
...
# ----------------------------------------------------------------------------------------
items = sorted(items, key=lambda x: x.p) # assignment:sorted, assignment_lhs_identifier:items, assignment_rhs_atom:items, assignment_rhs_atom:x, call_argument:items, external_free_call:sorted, flat_style, free_call:sorted, function_argument:x, function_argument_flavor:arg, n_liner_style:1, node:Assign, node:Attribute, node:Call, node:Lambda, node:Name, node:arg, one_shot_style, single_assignment:items, update:items:x, update_by_assignment:items:x, update_by_assignment_with:sorted, update_with:sorted, whole_span:1

# ----------------------------------------------------------------------------------------
# 029.0199-remove-item-from-list-by-its-index.py
# ----------------------------------------------------------------------------------------
del items[i] # flat_style, index:i, n_liner_style:1, node:Delete, node:Name, node:Subscript, one_shot_style, whole_span:1
...
# ----------------------------------------------------------------------------------------
def f(i): # body_recursive_function:f (-> +4), function:f (-> +4), function_argument:i, function_argument_flavor:arg, function_returning_something:f (-> +4), functional_style (-> +4), n_liner_style:4 (-> +4), node:FunctionDef (-> +4), node:arg, recursive_call_count:1 (-> +4), recursive_function:f (-> +4), whole_span:5 (-> +4)
...
# ----------------------------------------------------------------------------------------
def exp(x, n): # function:exp (-> +1), function_argument:n, function_argument:x, function_argument_flavor:arg, function_returning_something:exp (-> +1), functional_style (-> +1), n_liner_style:1 (-> +1), node:FunctionDef (-> +1), node:arg, whole_span:2 (-> +1)
    return x ** n # binary_operator:Pow, node:BinOp, node:Name, node:Return, return

# ----------------------------------------------------------------------------------------
# 033.1420-atomically-read-and-update-variable.py
# ----------------------------------------------------------------------------------------
import threading # flat_style (-> +6), import:threading, import_module:threading, n_liner_style:6 (-> +6), node:Import, one_shot_style (-> +6), whole_span:7 (-> +6)
...
# ----------------------------------------------------------------------------------------
class T(object): # n_liner_style:3 (-> +2), node:ClassDef (-> +1), node:Name, object_oriented_style (-> +2), whole_span:3 (-> +2)
    pass # node:Pass
...
# ----------------------------------------------------------------------------------------
def compose(f, g): # function:compose (-> +1), function_argument:f, function_argument:g, function_argument_flavor:arg, function_returning_something:compose (-> +1), functional_style (-> +1), higher_order_function:f (-> +1), higher_order_function:g (-> +1), n_liner_style:1 (-> +1), node:FunctionDef (-> +1), node:arg, whole_span:2 (-> +1)
...
# ----------------------------------------------------------------------------------------
from functools import partial # functional_style (-> +5), import:functools:partial, import_module:functools, import_name:partial, n_liner_style:3 (-> +5), node:ImportFrom, whole_span:6 (-> +5)
def f(a): # closure:f (-> +3), function:f (-> +3), function_argument:a, function_argument_flavor:arg, function_returning_something:f (-> +3), nested_function:f (-> +3), node:FunctionDef (-> +3), node:arg
    def add(b): # function:add (-> +1), function_argument:b, function_argument_flavor:arg, function_returning_something:add (-> +1), node:FunctionDef (-> +1), node:arg
...
# ----------------------------------------------------------------------------------------
t = s[i:j] # assignment, assignment_lhs_identifier:t, assignment_rhs_atom:i, assignment_rhs_atom:j, assignment_rhs_atom:s, flat_style, n_liner_style:1, node:Assign, node:Name, node:Subscript, one_shot_style, single_assignment:t, slice:i:j:, slice_lower:i, slice_step:, slice_upper:j, whole_span:1

# ----------------------------------------------------------------------------------------
# 039.0571-check-if-string-contains-a-word.py
# ----------------------------------------------------------------------------------------
ok = word in s # assignment, assignment_lhs_identifier:ok, assignment_rhs_atom:s, assignment_rhs_atom:word, comparison_operator:In, flat_style, n_liner_style:1, node:Assign, node:Compare, node:Name, one_shot_style, single_assignment:ok, whole_span:1

# ----------------------------------------------------------------------------------------
# 040.2279-graph-with-adjacency-lists.py
# ----------------------------------------------------------------------------------------
from collections import defaultdict # import:collections:defaultdict, import_module:collections, import_name:defaultdict, n_liner_style:10 (-> +12), node:ImportFrom, object_oriented_style (-> +12), whole_span:13 (-> +12)
...
# ----------------------------------------------------------------------------------------
t = s.decode("utf8")[::-1].encode("utf8") # assignment:encode, assignment_lhs_identifier:t, assignment_rhs_atom:-1, assignment_rhs_atom:s, call_argument:, flat_style, literal:-1, literal:Str, member_call:s:decode, member_call:s:encode, member_call_method:decode, member_call_method:encode, member_call_object:s, n_liner_style:1, node:Assign, node:Attribute, node:Call, node:Name, node:Num, node:Str, node:Subscript, one_shot_style, single_assignment:t, slice:::-1, slice_lower:, slice_step:-1, slice_upper:, whole_span:1

# ----------------------------------------------------------------------------------------
# 041.2714-reverse-a-string.py
# ----------------------------------------------------------------------------------------
t = s[::-1] # assignment, assignment_lhs_identifier:t, assignment_rhs_atom:-1, assignment_rhs_atom:s, flat_style, literal:-1, n_liner_style:1, node:Assign, node:Name, node:Num, node:Subscript, one_shot_style, single_assignment:t, slice:::-1, slice_lower:, slice_step:-1, slice_upper:, whole_span:1

# ----------------------------------------------------------------------------------------
# 042.1264-continue-outer-loop.py
# ----------------------------------------------------------------------------------------
for v in a: # for:v (-> +7), for_each (-> +7), loop:for (-> +7), loop_with_early_exit:for:raise (-> +7), loop_with_raise:for (-> +7), n_liner_style:8 (-> +7), node:For (-> +7), node:Name, one_shot_style (-> +7), whole_span:8 (-> +7)
...
# ----------------------------------------------------------------------------------------
for v in a: # for:v (-> +4), for_each (-> +4), loop:for (-> +4), loop_with_late_exit:for (-> +4), n_liner_style:5 (-> +4), node:For (-> +4), node:Name, one_shot_style (-> +4), whole_span:5 (-> +4)
...
# ----------------------------------------------------------------------------------------
class BreakOuterLoop(Exception): # n_liner_style:11 (-> +10), node:ClassDef (-> +1), node:Name, object_oriented_style (-> +10), whole_span:11 (-> +10)
    pass # node:Pass
...
# ----------------------------------------------------------------------------------------
def loop_breaking(m, v): # function:loop_breaking (-> +5), function_argument:m, function_argument:v, function_argument_flavor:arg, function_returning_something:loop_breaking (-> +5), n_liner_style:6 (-> +6), node:FunctionDef (-> +5), node:arg, whole_span:7 (-> +6)
...
# ----------------------------------------------------------------------------------------
s.insert(i, x) # call_argument:i, call_argument:x, flat_style, member_call:s:insert, member_call_method:insert, member_call_object:s, n_liner_style:1, node:Attribute, node:Call, node:Expr, node:Name, one_shot_style, update:s:i, update:s:x, update_by_member_call:s:i, update_by_member_call:s:x, update_by_member_call_with:insert, update_with:insert, whole_span:1
...
# ----------------------------------------------------------------------------------------
s = """Huey # assignment, assignment_lhs_identifier:s, flat_style (-> +2), n_liner_style:3 (-> +2), node:Assign (-> +2), node:Name, one_shot_style (-> +2), single_assignment:s, whole_span:3 (-> +2)
Dewey
...
# ----------------------------------------------------------------------------------------
chunks = s.split() # assignment:split, assignment_lhs_identifier:chunks, assignment_rhs_atom:s, flat_style, member_call_method:split, n_liner_style:1, node:Assign, node:Attribute, node:Call, node:Name, one_shot_style, single_assignment:chunks, whole_span:1

# ----------------------------------------------------------------------------------------
# 050.0572-make-an-infinite-loop.py
# ----------------------------------------------------------------------------------------
while True: # infinite_while (-> +1), literal:True, loop:while (-> +1), loop_with_late_exit:while (-> +1), n_liner_style:2 (-> +1), node:NameConstant, node:While (-> +1), one_shot_style (-> +1), whole_span:2 (-> +1)
    pass # node:Pass

# ----------------------------------------------------------------------------------------
# 051.0230-check-if-map-contains-key.py
# ----------------------------------------------------------------------------------------
k in m # comparison_operator:In, flat_style, n_liner_style:1, node:Compare, node:Expr, node:Name, one_shot_style, whole_span:1

# ----------------------------------------------------------------------------------------
# 052.0666-check-if-map-contains-value.py
# ----------------------------------------------------------------------------------------
v in map.values() # comparison_operator:In, flat_style, member_call_method:values, n_liner_style:1, node:Attribute, node:Call, node:Compare, node:Expr, node:Name, one_shot_style, whole_span:1

# ----------------------------------------------------------------------------------------
# 053.0240-join-a-list-of-strings.py
# ----------------------------------------------------------------------------------------
y = ", ".join(x) # assignment:join, assignment_lhs_identifier:y, assignment_rhs_atom:x, call_argument:x, flat_style, literal:Str, member_call_method:join, n_liner_style:1, node:Assign, node:Attribute, node:Call, node:Name, node:Str, one_shot_style, single_assignment:y, whole_span:1

# ----------------------------------------------------------------------------------------
# 053.1933-join-a-list-of-strings.py
# ----------------------------------------------------------------------------------------
y = ", ".join(map(str, x)) # assignment:join, assignment_lhs_identifier:y, assignment_rhs_atom:str, assignment_rhs_atom:x, call_argument:, call_argument:str, call_argument:x, composition, external_free_call:map, flat_style, free_call:map, literal:Str, member_call_method:join, n_liner_style:1, node:Assign, node:Attribute, node:Call, node:Name, node:Str, one_shot_style, single_assignment:y, whole_span:1

# ----------------------------------------------------------------------------------------
# 054.0241-compute-sum-of-integers.py
# ----------------------------------------------------------------------------------------
s = sum(x) # assignment:sum, assignment_lhs_identifier:s, assignment_rhs_atom:x, call_argument:x, external_free_call:sum, flat_style, free_call:sum, n_liner_style:1, node:Assign, node:Call, node:Name, one_shot_style, single_assignment:s, whole_span:1

# ----------------------------------------------------------------------------------------
# 055.0575-convert-integer-to-string.py
# ----------------------------------------------------------------------------------------
s = str(i) # assignment:str, assignment_lhs_identifier:s, assignment_rhs_atom:i, call_argument:i, external_free_call:str, flat_style, free_call:str, n_liner_style:1, node:Assign, node:Call, node:Name, one_shot_style, single_assignment:s, whole_span:1

# ----------------------------------------------------------------------------------------
# 056.1424-launch-1000-parallel-tasks-and-wait-for-completion.py
# ----------------------------------------------------------------------------------------
from multiprocessing import Pool # import:multiprocessing:Pool, import_module:multiprocessing, import_name:Pool, n_liner_style:4 (-> +5), node:ImportFrom, procedural_style (-> +5), whole_span:6 (-> +5)
...
# ----------------------------------------------------------------------------------------
y = filter(p, x) # assignment:filter, assignment_lhs_identifier:y, assignment_rhs_atom:p, assignment_rhs_atom:x, call_argument:p, call_argument:x, external_free_call:filter, flat_style, free_call:filter, n_liner_style:1, node:Assign, node:Call, node:Name, one_shot_style, single_assignment:y, whole_span:1

# ----------------------------------------------------------------------------------------
# 057.3173-filter-list.py
# ----------------------------------------------------------------------------------------
y = [element for element in x if p(element)] # assignment, assignment_lhs_identifier:y, assignment_rhs_atom:element, assignment_rhs_atom:x, call_argument:element, comprehension:List, comprehension_for_count:1, external_free_call:p, filtered_comprehension, flat_style, free_call:p, n_liner_style:1, node:Assign, node:Call, node:ListComp, node:Name, one_shot_style, single_assignment:y, whole_span:1

# ----------------------------------------------------------------------------------------
# 058.0665-extract-file-content-to-a-string.py
# ----------------------------------------------------------------------------------------
lines = open(f).read() # assignment:read, assignment_lhs_identifier:lines, assignment_rhs_atom:f, call_argument:f, external_free_call:open, flat_style, free_call:open, member_call_method:read, n_liner_style:1, node:Assign, node:Attribute, node:Call, node:Name, one_shot_style, single_assignment:lines, whole_span:1

# ----------------------------------------------------------------------------------------
# 059.0668-write-to-standard-error-stream.py
# ----------------------------------------------------------------------------------------
import sys # flat_style (-> +1), import:sys, import_module:sys, n_liner_style:1 (-> +1), node:Import, one_shot_style (-> +1), whole_span:2 (-> +1)
...
# ----------------------------------------------------------------------------------------
import datetime # flat_style (-> +1), import:datetime, import_module:datetime, n_liner_style:1 (-> +1), node:Import, one_shot_style (-> +1), whole_span:2 (-> +1)
...
# ----------------------------------------------------------------------------------------
i = x.find(y) # assignment:find, assignment_lhs_identifier:i, assignment_rhs_atom:x, assignment_rhs_atom:y, call_argument:y, flat_style, member_call_method:find, n_liner_style:1, node:Assign, node:Attribute, node:Call, node:Name, one_shot_style, single_assignment:i, whole_span:1

# ----------------------------------------------------------------------------------------
# 063.1088-replace-fragment-of-a-string.py
# ----------------------------------------------------------------------------------------
x2 = x.replace(y, z) # assignment:replace, assignment_lhs_identifier:x2, assignment_rhs_atom:x, assignment_rhs_atom:y, assignment_rhs_atom:z, call_argument:y, call_argument:z, flat_style, member_call_method:replace, n_liner_style:1, node:Assign, node:Attribute, node:Call, node:Name, one_shot_style, single_assignment:x2, whole_span:1
...
# ----------------------------------------------------------------------------------------
s = "{:.1%}".format(x) # assignment:format, assignment_lhs_identifier:s, assignment_rhs_atom:x, call_argument:x, flat_style, literal:Str, member_call_method:format, n_liner_style:1, node:Assign, node:Attribute, node:Call, node:Name, node:Str, one_shot_style, single_assignment:s, whole_span:1

# ----------------------------------------------------------------------------------------
# 066.0672-big-integer-exponentiation.py
# ----------------------------------------------------------------------------------------
z = x ** n # assignment:Pow, assignment_lhs_identifier:z, assignment_rhs_atom:n, assignment_rhs_atom:x, binary_operator:Pow, flat_style, n_liner_style:1, node:Assign, node:BinOp, node:Name, one_shot_style, single_assignment:z, whole_span:1

# ----------------------------------------------------------------------------------------
# 067.1426-binomial-coefficient-n-choose-k.py
# ----------------------------------------------------------------------------------------
import math # functional_style (-> +2), import:math, import_module:math, n_liner_style:1 (-> +2), node:Import, whole_span:3 (-> +2)
...
# ----------------------------------------------------------------------------------------
def fact(a_class, str_): # function:fact (-> +2), function_argument:a_class, function_argument:str_, function_argument_flavor:arg, function_returning_something:fact (-> +2), functional_style (-> +2), higher_order_function:a_class (-> +2), n_liner_style:2 (-> +2), node:FunctionDef (-> +2), node:arg, whole_span:3 (-> +2)
...
# ----------------------------------------------------------------------------------------
from fractions import gcd # flat_style (-> +1), import:fractions:gcd, import_module:fractions, import_name:gcd, n_liner_style:1 (-> +1), node:ImportFrom, one_shot_style (-> +1), whole_span:2 (-> +1)
...
# ----------------------------------------------------------------------------------------
s = "{:b}".format(x) # assignment:format, assignment_lhs_identifier:s, assignment_rhs_atom:x, call_argument:x, flat_style, literal:Str, member_call_method:format, n_liner_style:1, node:Assign, node:Attribute, node:Call, node:Name, node:Str, one_shot_style, single_assignment:s, whole_span:1

# ----------------------------------------------------------------------------------------
# 077.1093-complex-number.py
# ----------------------------------------------------------------------------------------
x = 3j - 2 # assignment:Sub, assignment_lhs_identifier:x, assignment_rhs_atom:2, assignment_rhs_atom:3j, binary_operator:Sub, flat_style (-> +1), literal:2, literal:3j, n_liner_style:2 (-> +1), node:Assign, node:BinOp, node:Name, node:Num, one_shot_style (-> +1), single_assignment:x, whole_span:2 (-> +1)
...
# ----------------------------------------------------------------------------------------
while True: # infinite_while (-> +3), literal:True, loop:while (-> +3), loop_with_break:while (-> +3), loop_with_early_exit:while:break (-> +3), n_liner_style:4 (-> +3), node:NameConstant, node:While (-> +3), one_shot_style (-> +3), whole_span:4 (-> +3)
...
# ----------------------------------------------------------------------------------------
y = float(x) # assignment:float, assignment_lhs_identifier:y, assignment_rhs_atom:x, call_argument:x, external_free_call:float, flat_style, free_call:float, n_liner_style:1, node:Assign, node:Call, node:Name, one_shot_style, single_assignment:y, whole_span:1

# ----------------------------------------------------------------------------------------
# 080.1092-truncate-floating-point-number-to-integer.py
# ----------------------------------------------------------------------------------------
y = int(x) # assignment:int, assignment_lhs_identifier:y, assignment_rhs_atom:x, call_argument:x, external_free_call:int, flat_style, free_call:int, n_liner_style:1, node:Assign, node:Call, node:Name, one_shot_style, single_assignment:y, whole_span:1
...
# ----------------------------------------------------------------------------------------
count = s.count(t) # assignment:count, assignment_lhs_identifier:count, assignment_rhs_atom:s, assignment_rhs_atom:t, call_argument:t, flat_style, member_call_method:count, n_liner_style:1, node:Assign, node:Attribute, node:Call, node:Name, one_shot_style, single_assignment:count, whole_span:1

# ----------------------------------------------------------------------------------------
# 083.1805-regex-with-character-repetition.py
# ----------------------------------------------------------------------------------------
import re # flat_style (-> +1), import:re, import_module:re, n_liner_style:1 (-> +1), node:Import, one_shot_style (-> +1), whole_span:2 (-> +1)
...
# ----------------------------------------------------------------------------------------
c = bin(i).count("1") # assignment:count, assignment_lhs_identifier:c, assignment_rhs_atom:i, call_argument:, call_argument:i, external_free_call:bin, flat_style, free_call:bin, literal:Str, member_call_method:count, n_liner_style:1, node:Assign, node:Attribute, node:Call, node:Name, node:Str, one_shot_style, single_assignment:c, whole_span:1

# ----------------------------------------------------------------------------------------
# 085.1003-check-if-integer-addition-will-overflow.py
# ----------------------------------------------------------------------------------------
def adding_will_overflow(x, y): # function:adding_will_overflow (-> +1), function_argument:x, function_argument:y, function_argument_flavor:arg, function_returning_something:adding_will_overflow (-> +1), functional_style (-> +1), n_liner_style:1 (-> +1), node:FunctionDef (-> +1), node:arg, whole_span:2 (-> +1)
...
# ----------------------------------------------------------------------------------------
def multiplyWillOverflow(x, y): # function:multiplyWillOverflow (-> +1), function_argument:x, function_argument:y, function_argument_flavor:arg, function_returning_something:multiplyWillOverflow (-> +1), functional_style (-> +1), n_liner_style:1 (-> +1), node:FunctionDef (-> +1), node:arg, whole_span:2 (-> +1)
    return False # literal:False, node:NameConstant, node:Return, return:False
...
# ----------------------------------------------------------------------------------------
raise ValueError("x is invalid") # call_argument:, external_free_call:ValueError, flat_style, free_call:ValueError, literal:Str, n_liner_style:1, node:Call, node:Name, node:Raise, node:Str, one_shot_style, raise:ValueError, whole_span:1

# ----------------------------------------------------------------------------------------
# 090.1099-read-only-outside.py
# ----------------------------------------------------------------------------------------
class Foo(object): # n_liner_style:4 (-> +5), node:ClassDef (-> +5), node:Name, object_oriented_style (-> +5), whole_span:6 (-> +5)
...
# ----------------------------------------------------------------------------------------
import json # flat_style (-> +2), import:json, import_module:json, n_liner_style:2 (-> +2), node:Import, one_shot_style (-> +2), whole_span:3 (-> +2)
...
# ----------------------------------------------------------------------------------------
from __future__ import print_function # functional_style (-> +2), import:__future__:print_function, import_module:__future__, import_name:print_function, n_liner_style:1 (-> +2), node:ImportFrom, whole_span:3 (-> +2)
def control(f): # function:control (-> +1), function_argument:f, function_argument_flavor:arg, function_returning_something:control (-> +1), higher_order_function:f (-> +1), node:FunctionDef (-> +1), node:arg
...
# ----------------------------------------------------------------------------------------
print(type(x)) # call_argument:, call_argument:x, composition, external_free_call:print, external_free_call:type, flat_style, free_call:print, free_call:type, free_call_without_result:print, n_liner_style:1, node:Call, node:Expr, node:Name, one_shot_style, whole_span:1

# ----------------------------------------------------------------------------------------
# 094.1864-print-type-of-variable.py
# ----------------------------------------------------------------------------------------
print(x.__class__) # call_argument:, external_free_call:print, flat_style, free_call:print, free_call_without_result:print, n_liner_style:1, node:Attribute, node:Call, node:Expr, node:Name, one_shot_style, whole_span:1

# ----------------------------------------------------------------------------------------
# 095.2140-get-file-size.py
# ----------------------------------------------------------------------------------------
import os # flat_style (-> +1), import:os, import_module:os, n_liner_style:1 (-> +1), node:Import, one_shot_style (-> +1), whole_span:2 (-> +1)
...
# ----------------------------------------------------------------------------------------
b = s.startswith(prefix) # assignment:startswith, assignment_lhs_identifier:b, assignment_rhs_atom:prefix, assignment_rhs_atom:s, call_argument:prefix, flat_style, member_call_method:startswith, n_liner_style:1, node:Assign, node:Attribute, node:Call, node:Name, one_shot_style, single_assignment:b, whole_span:1

# ----------------------------------------------------------------------------------------
# 097.1095-check-string-suffix.py
# ----------------------------------------------------------------------------------------
b = s.endswith(suffix) # assignment:endswith, assignment_lhs_identifier:b, assignment_rhs_atom:s, assignment_rhs_atom:suffix, call_argument:suffix, flat_style, member_call_method:endswith, n_liner_style:1, node:Assign, node:Attribute, node:Call, node:Name, one_shot_style, single_assignment:b, whole_span:1
...
# ----------------------------------------------------------------------------------------
items.sort(c) # call_argument:c, flat_style, member_call:items:sort, member_call_method:sort, member_call_object:items, n_liner_style:1, node:Attribute, node:Call, node:Expr, node:Name, one_shot_style, whole_span:1

# ----------------------------------------------------------------------------------------
# 101.2172-load-from-http-get-request-into-a-string.py
# ----------------------------------------------------------------------------------------
import urllib.request # flat_style (-> +2), import:urllib.request, import_module:urllib.request, n_liner_style:2 (-> +2), node:Import, one_shot_style (-> +2), whole_span:3 (-> +2)
...
# ----------------------------------------------------------------------------------------
import urllib # flat_style (-> +1), import:urllib, import_module:urllib, n_liner_style:1 (-> +1), node:Import, one_shot_style (-> +1), whole_span:2 (-> +1)
...
# ----------------------------------------------------------------------------------------
import lxml.etree # flat_style (-> +1), import:lxml.etree, import_module:lxml.etree, n_liner_style:1 (-> +1), node:Import, one_shot_style (-> +1), whole_span:2 (-> +1)
...
# ----------------------------------------------------------------------------------------
import pyxser as pyx # import:pyxser, import_module:pyxser, n_liner_style:10 (-> +11), node:Import, object_oriented_style (-> +11), whole_span:12 (-> +11)
...
# ----------------------------------------------------------------------------------------
if "x" in locals(): # comparison_operator:In, external_free_call:locals, free_call:locals, free_call_without_arguments:locals, if (-> +1), if_without_else (-> +1), literal:Str, n_liner_style:2 (-> +1), node:Call, node:Compare, node:If (-> +1), node:Name, node:Str, one_shot_style (-> +1), whole_span:2 (-> +1), yoda_comparison:In
...
# ----------------------------------------------------------------------------------------
import pympler.asizeof # flat_style (-> +1), import:pympler.asizeof, import_module:pympler.asizeof, n_liner_style:1 (-> +1), node:Import, one_shot_style (-> +1), whole_span:2 (-> +1)
...
# ----------------------------------------------------------------------------------------
blank = s.strip() == "" # assignment, assignment_lhs_identifier:blank, assignment_rhs_atom:s, comparison_operator:Eq, empty_literal:Str, flat_style, literal:Str, member_call_method:strip, n_liner_style:1, node:Assign, node:Attribute, node:Call, node:Compare, node:Name, node:Str, one_shot_style, single_assignment:blank, whole_span:1

# ----------------------------------------------------------------------------------------
# 111.2168-launch-other-program.py
# ----------------------------------------------------------------------------------------
import subprocess # flat_style (-> +1), import:subprocess, import_module:subprocess, n_liner_style:1 (-> +1), node:Import, one_shot_style (-> +1), whole_span:2 (-> +1)
...
# ----------------------------------------------------------------------------------------
for k in sorted(mymap): # call_argument:mymap, external_free_call:sorted, for:k (-> +1), free_call:sorted, loop:for (-> +1), loop_with_late_exit:for (-> +1), n_liner_style:2 (-> +1), node:Call, node:For (-> +1), node:Name, one_shot_style (-> +1), whole_span:2 (-> +1)
...
# ----------------------------------------------------------------------------------------
for x, k in sorted((x, k) for k, x in mymap.items()): # call_argument:, composition, comprehension:Generator, comprehension_for_count:1, external_free_call:sorted, for:k (-> +1), for:x (-> +1), free_call:sorted, literal:Tuple, loop:for (-> +1), loop_with_late_exit:for (-> +1), member_call_method:items, n_liner_style:2 (-> +1), node:Attribute, node:Call, node:For (-> +1), node:GeneratorExp, node:Name, node:Tuple, one_shot_style (-> +1), whole_span:2 (-> +1)
...
# ----------------------------------------------------------------------------------------
b = x == y # assignment, assignment_lhs_identifier:b, assignment_rhs_atom:x, assignment_rhs_atom:y, comparison_operator:Eq, flat_style, n_liner_style:1, node:Assign, node:Compare, node:Name, one_shot_style, single_assignment:b, whole_span:1
...
# ----------------------------------------------------------------------------------------
s2 = s1.replace(w, "") # assignment:replace, assignment_lhs_identifier:s2, assignment_rhs_atom:s1, assignment_rhs_atom:w, call_argument:, call_argument:w, empty_literal:Str, flat_style, literal:Str, member_call_method:replace, n_liner_style:1, node:Assign, node:Attribute, node:Call, node:Name, node:Str, one_shot_style, single_assignment:s2, whole_span:1

# ----------------------------------------------------------------------------------------
# 117.1297-get-list-size.py
# ----------------------------------------------------------------------------------------
n = len(x) # assignment:len, assignment_lhs_identifier:n, assignment_rhs_atom:x, call_argument:x, external_free_call:len, flat_style, free_call:len, n_liner_style:1, node:Assign, node:Call, node:Name, one_shot_style, single_assignment:n, whole_span:1

# ----------------------------------------------------------------------------------------
# 118.1254-list-to-set.py
# ----------------------------------------------------------------------------------------
y = set(x) # assignment:set, assignment_lhs_identifier:y, assignment_rhs_atom:x, call_argument:x, external_free_call:set, flat_style, free_call:set, n_liner_style:1, node:Assign, node:Call, node:Name, one_shot_style, single_assignment:y, whole_span:1

# ----------------------------------------------------------------------------------------
# 118.3266-list-to-set.py
# ----------------------------------------------------------------------------------------
set(x) # call_argument:x, external_free_call:set, flat_style, free_call:set, free_call_without_result:set, n_liner_style:1, node:Call, node:Expr, node:Name, one_shot_style, whole_span:1

# ----------------------------------------------------------------------------------------
# 119.1253-deduplicate-list.py
# ----------------------------------------------------------------------------------------
x = list(set(x)) # assignment:list, assignment_lhs_identifier:x, assignment_rhs_atom:x, call_argument:, call_argument:x, composition, external_free_call:list, external_free_call:set, flat_style, free_call:list, free_call:set, n_liner_style:1, node:Assign, node:Call, node:Name, one_shot_style, single_assignment:x, whole_span:1

# ----------------------------------------------------------------------------------------
# 119.3263-deduplicate-list.py
# ----------------------------------------------------------------------------------------
elements = ["b", "a", "b", "c"] # assignment, assignment_lhs_identifier:elements, literal:List, literal:Str, n_liner_style:8 (-> +7), node:Assign, node:List, node:Name, node:Str, one_shot_style (-> +7), single_assignment:elements, whole_span:8 (-> +7)
...
# ----------------------------------------------------------------------------------------
input_var = int(raw_input("Input Prompting String: ")) # assignment:int, assignment_lhs_identifier:input_var, call_argument:, composition, external_free_call:int, external_free_call:raw_input, flat_style, free_call:int, free_call:raw_input, literal:Str, n_liner_style:1, node:Assign, node:Call, node:Name, node:Str, one_shot_style, single_assignment:input_var, whole_span:1

# ----------------------------------------------------------------------------------------
# 121.3029-udp-listen-and-read.py
# ----------------------------------------------------------------------------------------
import socket # import:socket, import_module:socket, n_liner_style:6 (-> +6), node:Import, one_shot_style (-> +6), whole_span:7 (-> +6)
...
# ----------------------------------------------------------------------------------------
from enum import Enum # import:enum:Enum, import_module:enum, import_name:Enum, n_liner_style:5 (-> +5), node:ImportFrom, object_oriented_style (-> +5), whole_span:6 (-> +5)
...
# ----------------------------------------------------------------------------------------
assert isConsistent # flat_style, n_liner_style:0, node:Assert, node:Name, one_shot_style, whole_span:1

# ----------------------------------------------------------------------------------------
# 124.2152-binary-search-for-a-value-in-sorted-array.py
# ----------------------------------------------------------------------------------------
import bisect # import:bisect, import_module:bisect, n_liner_style:2 (-> +3), node:Import, whole_span:4 (-> +3)
...
# ----------------------------------------------------------------------------------------
import time # flat_style (-> +4), import:time, import_module:time, n_liner_style:4 (-> +4), node:Import, one_shot_style (-> +4), whole_span:5 (-> +4)
...
# ----------------------------------------------------------------------------------------
def foo(): # function:foo (-> +1), function_returning_something:foo (-> +1), function_without_arguments:foo (-> +1), functional_style (-> +1), n_liner_style:1 (-> +1), node:FunctionDef (-> +1), whole_span:2 (-> +1)
...
# ----------------------------------------------------------------------------------------
import imp # flat_style (-> +1), import:imp, import_module:imp, n_liner_style:1 (-> +1), node:Import, one_shot_style (-> +1), whole_span:2 (-> +1)
...
# ----------------------------------------------------------------------------------------
def BFS(f, root): # function:BFS (-> +8), function_argument:f, function_argument:root, function_argument_flavor:arg, function_returning_nothing:BFS (-> +8), higher_order_function:f (-> +8), n_liner_style:8 (-> +8), node:FunctionDef (-> +8), node:arg, procedural_style (-> +8), whole_span:9 (-> +8)
...
# ----------------------------------------------------------------------------------------
from collections import deque # import:collections:deque, import_module:collections, import_name:deque, n_liner_style:7 (-> +8), node:ImportFrom, procedural_style (-> +8), whole_span:9 (-> +8)
...
# ----------------------------------------------------------------------------------------
def depth_first(start, f): # function:depth_first (-> +7), function_argument:f, function_argument:start, function_argument_flavor:arg, function_returning_nothing:depth_first (-> +7), higher_order_function:f (-> +7), n_liner_style:7 (-> +7), node:FunctionDef (-> +7), node:arg, procedural_style (-> +7), whole_span:8 (-> +7)
    seen = set() # assignment:set, assignment_lhs_identifier:seen, external_free_call:set, free_call:set, free_call_without_arguments:set, node:Assign, node:Call, node:Name, single_assignment:seen
...
# ----------------------------------------------------------------------------------------
f1 if c1 else f2 if c2 else f3 if c3 else None # flat_style, literal:None, n_liner_style:1, node:Expr, node:IfExp, node:Name, node:NameConstant, one_shot_style, whole_span:1

# ----------------------------------------------------------------------------------------
# 131.2766-successive-conditions.py
# ----------------------------------------------------------------------------------------
if c1: # if (-> +5), n_liner_style:6 (-> +5), node:If (-> +5), node:Name, one_shot_style (-> +5), whole_span:6 (-> +5)
...
# ----------------------------------------------------------------------------------------
import timeit # flat_style (-> +1), import:timeit, import_module:timeit, n_liner_style:1 (-> +1), node:Import, one_shot_style (-> +1), whole_span:2 (-> +1)
...
# ----------------------------------------------------------------------------------------
ok = word.lower() in s.lower() # assignment, assignment_lhs_identifier:ok, assignment_rhs_atom:s, assignment_rhs_atom:word, comparison_operator:In, flat_style, member_call_method:lower, n_liner_style:1, node:Assign, node:Attribute, node:Call, node:Compare, node:Name, one_shot_style, single_assignment:ok, whole_span:1

# ----------------------------------------------------------------------------------------
# 134.1850-create-a-new-list.py
# ----------------------------------------------------------------------------------------
items = [a, b, c] # assignment, assignment_lhs_identifier:items, assignment_rhs_atom:a, assignment_rhs_atom:b, assignment_rhs_atom:c, flat_style, literal:List, n_liner_style:1, node:Assign, node:List, node:Name, one_shot_style, single_assignment:items, whole_span:1

# ----------------------------------------------------------------------------------------
# 135.2158-remove-item-from-list-by-its-value.py
# ----------------------------------------------------------------------------------------
items.remove(x) # call_argument:x, flat_style, member_call:items:remove, member_call_method:remove, member_call_object:items, n_liner_style:1, node:Attribute, node:Call, node:Expr, node:Name, one_shot_style, update:items:x, update_by_member_call:items:x, update_by_member_call_with:remove, update_with:remove, whole_span:1

# ----------------------------------------------------------------------------------------
# 136.2141-remove-all-occurrences-of-a-value-from-a-list.py
# ----------------------------------------------------------------------------------------
newlist = [item for item in items if item != x] # assignment, assignment_lhs_identifier:newlist, assignment_rhs_atom:item, assignment_rhs_atom:items, assignment_rhs_atom:x, comparison_operator:NotEq, comprehension:List, comprehension_for_count:1, filtered_comprehension, flat_style, n_liner_style:1, node:Assign, node:Compare, node:ListComp, node:Name, one_shot_style, single_assignment:newlist, whole_span:1

# ----------------------------------------------------------------------------------------
# 137.1823-check-if-string-contains-only-digits.py
# ----------------------------------------------------------------------------------------
b = s.isdigit() # assignment:isdigit, assignment_lhs_identifier:b, assignment_rhs_atom:s, flat_style, member_call_method:isdigit, n_liner_style:1, node:Assign, node:Attribute, node:Call, node:Name, one_shot_style, single_assignment:b, whole_span:1

# ----------------------------------------------------------------------------------------
# 138.2161-create-temp-file.py
# ----------------------------------------------------------------------------------------
import tempfile # flat_style (-> +1), import:tempfile, import_module:tempfile, n_liner_style:1 (-> +1), node:Import, one_shot_style (-> +1), whole_span:2 (-> +1)
...
# ----------------------------------------------------------------------------------------
m.pop(k, None) # call_argument:None, call_argument:k, flat_style, literal:None, member_call:m:pop, member_call_method:pop, member_call_object:m, n_liner_style:1, node:Attribute, node:Call, node:Expr, node:Name, node:NameConstant, one_shot_style, update:m:None, update:m:k, update_by_member_call:m:None, update_by_member_call:m:k, update_by_member_call_with:pop, update_with:pop, whole_span:1

# ----------------------------------------------------------------------------------------
# 141.2159-iterate-in-sequence-over-two-lists.py
# ----------------------------------------------------------------------------------------
for x in items1 + items2: # addition_operator, binary_operator:Add, for:x (-> +1), loop:for (-> +1), loop_with_late_exit:for (-> +1), n_liner_style:2 (-> +1), node:BinOp, node:For (-> +1), node:Name, one_shot_style (-> +1), whole_span:2 (-> +1)
...
# ----------------------------------------------------------------------------------------
s = hex(x) # assignment:hex, assignment_lhs_identifier:s, assignment_rhs_atom:x, call_argument:x, external_free_call:hex, flat_style, free_call:hex, n_liner_style:1, node:Assign, node:Call, node:Name, one_shot_style, single_assignment:s, whole_span:1

# ----------------------------------------------------------------------------------------
# 143.2256-iterate-alternatively-over-two-lists.py
# ----------------------------------------------------------------------------------------
for pair in zip(item1, item2): # call_argument:item1, call_argument:item2, external_free_call:zip, for:pair (-> +1), free_call:zip, loop:for (-> +1), loop_with_late_exit:for (-> +1), n_liner_style:2 (-> +1), node:Call, node:For (-> +1), node:Name, one_shot_style (-> +1), whole_span:2 (-> +1)
...
# ----------------------------------------------------------------------------------------
from pathlib import Path # flat_style (-> +1), import:pathlib:Path, import_module:pathlib, import_name:Path, n_liner_style:1 (-> +1), node:ImportFrom, one_shot_style (-> +1), whole_span:2 (-> +1)
...
# ----------------------------------------------------------------------------------------
import sys, logging # flat_style (-> +5), import:logging, import:sys, import_module:logging, import_module:sys, n_liner_style:5 (-> +5), node:Import, one_shot_style (-> +5), whole_span:6 (-> +5)
...
# ----------------------------------------------------------------------------------------
import locale # flat_style (-> +3), import:locale, import_module:locale, n_liner_style:3 (-> +3), node:Import, one_shot_style (-> +3), whole_span:4 (-> +3)
...
# ----------------------------------------------------------------------------------------
f = float(s) # assignment:float, assignment_lhs_identifier:f, assignment_rhs_atom:s, call_argument:s, external_free_call:float, flat_style, free_call:float, n_liner_style:1, node:Assign, node:Call, node:Name, one_shot_style, single_assignment:f, whole_span:1

# ----------------------------------------------------------------------------------------
# 146.2739-convert-string-to-floating-point-number.py
# ----------------------------------------------------------------------------------------
float("1.3") # call_argument:, external_free_call:float, flat_style, free_call:float, free_call_without_result:float, literal:Str, n_liner_style:1, node:Call, node:Expr, node:Name, node:Str, one_shot_style, whole_span:1
...
# ----------------------------------------------------------------------------------------
list(map(int, input().split())) # call_argument:, call_argument:int, composition, external_free_call:input, external_free_call:list, external_free_call:map, flat_style, free_call:input, free_call:list, free_call:map, free_call_without_arguments:input, free_call_without_result:list, member_call_method:split, n_liner_style:1, node:Attribute, node:Call, node:Expr, node:Name, one_shot_style, whole_span:1

# ----------------------------------------------------------------------------------------
# 150.2154-remove-trailing-slash.py
# ----------------------------------------------------------------------------------------
p = p.rstrip("/") # assignment:rstrip, assignment_lhs_identifier:p, assignment_rhs_atom:p, call_argument:, flat_style, literal:Str, member_call_method:rstrip, n_liner_style:1, node:Assign, node:Attribute, node:Call, node:Name, node:Str, one_shot_style, single_assignment:p, whole_span:1

# ----------------------------------------------------------------------------------------
# 151.2166-remove-string-trailing-path-separator.py
# ----------------------------------------------------------------------------------------
import os # import:os, import_module:os, n_liner_style:2 (-> +2), node:Import, one_shot_style (-> +2), whole_span:3 (-> +2)
...
# ----------------------------------------------------------------------------------------
s = c # assignment, assignment_lhs_identifier:s, assignment_rhs_atom:c, flat_style, n_liner_style:1, node:Assign, node:Name, one_shot_style, single_assignment:s, whole_span:1

# ----------------------------------------------------------------------------------------
# 153.1980-concatenate-string-with-integer.py
# ----------------------------------------------------------------------------------------
t = "{}{}".format(s, i) # assignment:format, assignment_lhs_identifier:t, assignment_rhs_atom:i, assignment_rhs_atom:s, call_argument:i, call_argument:s, flat_style, literal:Str, member_call_method:format, n_liner_style:1, node:Assign, node:Attribute, node:Call, node:Name, node:Str, one_shot_style, single_assignment:t, whole_span:1
...
# ----------------------------------------------------------------------------------------
import numpy # import:numpy, import_module:numpy, n_liner_style:12 (-> +14), node:Import, object_oriented_style (-> +14), whole_span:15 (-> +14)
...
# ----------------------------------------------------------------------------------------
import pathlib # flat_style (-> +2), import:pathlib, import_module:pathlib, n_liner_style:2 (-> +2), node:Import, one_shot_style (-> +2), whole_span:3 (-> +2)
...
# ----------------------------------------------------------------------------------------
s = format("03d", i) # assignment:format, assignment_lhs_identifier:s, assignment_rhs_atom:i, call_argument:, call_argument:i, external_free_call:format, flat_style, free_call:format, literal:Str, n_liner_style:1, node:Assign, node:Call, node:Name, node:Str, one_shot_style, single_assignment:s, whole_span:1

# ----------------------------------------------------------------------------------------
# 157.2150-declare-constant-string.py
# ----------------------------------------------------------------------------------------
PLANET = "Earth" # assignment, assignment_lhs_identifier:PLANET, flat_style, literal:Str, n_liner_style:1, node:Assign, node:Name, node:Str, one_shot_style, single_assignment:PLANET, whole_span:1
...
# 159.2281-trie.py
# ----------------------------------------------------------------------------------------
class Trie: # n_liner_style:4 (-> +4), node:ClassDef (-> +4), object_oriented_style (-> +4), whole_span:5 (-> +4)
...
# ----------------------------------------------------------------------------------------
for x in zip(list[::2], list[1::2]): # call_argument:, external_free_call:zip, for:x (-> +1), free_call:zip, literal:1, literal:2, loop:for (-> +1), loop_with_late_exit:for (-> +1), n_liner_style:2 (-> +1), node:Call, node:For (-> +1), node:Name, node:Num, node:Subscript, one_shot_style (-> +1), slice:1::2, slice:::2, slice_lower:, slice_lower:1, slice_step:2, slice_upper:, whole_span:2 (-> +1)
    print(x) # call_argument:x, external_free_call:print, free_call:print, free_call_without_result:print, node:Call, node:Expr, node:Name

# ----------------------------------------------------------------------------------------
# 163.3177-print-list-elements-by-group-of-2.py
# ----------------------------------------------------------------------------------------
from itertools import tee # import:itertools:tee, import_module:itertools, import_name:tee, n_liner_style:5 (-> +6), node:ImportFrom, whole_span:7 (-> +6)
def pairwise(iterable): # function:pairwise (-> +3), function_argument:iterable, function_argument_flavor:arg, function_returning_something:pairwise (-> +3), node:FunctionDef (-> +3), node:arg
...
# ----------------------------------------------------------------------------------------
import webbrowser # flat_style (-> +1), import:webbrowser, import_module:webbrowser, n_liner_style:1 (-> +1), node:Import, one_shot_style (-> +1), whole_span:2 (-> +1)
...
# ----------------------------------------------------------------------------------------
x = items[-1] # assignment, assignment_lhs_identifier:x, assignment_rhs_atom:-1, assignment_rhs_atom:items, flat_style, index:-1, literal:-1, n_liner_style:1, negative_index:-1, node:Assign, node:Name, node:Num, node:Subscript, one_shot_style, single_assignment:x, whole_span:1

# ----------------------------------------------------------------------------------------
# 166.2272-concatenate-two-lists.py
# ----------------------------------------------------------------------------------------
ab = a + b # addition_operator, assignment:Add, assignment_lhs_identifier:ab, assignment_rhs_atom:a, assignment_rhs_atom:b, binary_operator:Add, flat_style, n_liner_style:1, node:Assign, node:BinOp, node:Name, one_shot_style, single_assignment:ab, whole_span:1

# ----------------------------------------------------------------------------------------
# 167.2611-trim-prefix.py
# ----------------------------------------------------------------------------------------
t = s[s.startswith(p) and len(p) :] # assignment, assignment_lhs_identifier:t, assignment_rhs_atom:p, assignment_rhs_atom:s, boolean_operator:And, call_argument:p, external_free_call:len, flat_style, free_call:len, member_call_method:startswith, n_liner_style:1, node:Assign, node:Attribute, node:BoolOp, node:Call, node:Name, node:Subscript, one_shot_style, single_assignment:t, slice:_::, slice_lower:_, slice_step:, slice_upper:, whole_span:1

# ----------------------------------------------------------------------------------------
# 167.3175-trim-prefix.py
# ----------------------------------------------------------------------------------------
t = s.lstrip(p) # assignment:lstrip, assignment_lhs_identifier:t, assignment_rhs_atom:p, assignment_rhs_atom:s, call_argument:p, flat_style, member_call_method:lstrip, n_liner_style:1, node:Assign, node:Attribute, node:Call, node:Name, one_shot_style, single_assignment:t, whole_span:1

# ----------------------------------------------------------------------------------------
# 168.2277-trim-suffix.py
# ----------------------------------------------------------------------------------------
t = s.rsplit(w, 1)[0] # assignment, assignment_lhs_identifier:t, assignment_rhs_atom:0, assignment_rhs_atom:1, assignment_rhs_atom:s, assignment_rhs_atom:w, call_argument:1, call_argument:w, flat_style, index:0, literal:0, literal:1, member_call:s:rsplit, member_call_method:rsplit, member_call_object:s, n_liner_style:1, node:Assign, node:Attribute, node:Call, node:Name, node:Num, node:Subscript, one_shot_style, single_assignment:t, whole_span:1

# ----------------------------------------------------------------------------------------
# 168.3174-trim-suffix.py
# ----------------------------------------------------------------------------------------
t = s.rstrip(w) # assignment:rstrip, assignment_lhs_identifier:t, assignment_rhs_atom:s, assignment_rhs_atom:w, call_argument:w, flat_style, member_call_method:rstrip, n_liner_style:1, node:Assign, node:Attribute, node:Call, node:Name, one_shot_style, single_assignment:t, whole_span:1

# ----------------------------------------------------------------------------------------
# 169.2233-string-length.py
# ----------------------------------------------------------------------------------------
n = len(s) # assignment:len, assignment_lhs_identifier:n, assignment_rhs_atom:s, call_argument:s, external_free_call:len, flat_style, free_call:len, n_liner_style:1, node:Assign, node:Call, node:Name, one_shot_style, single_assignment:n, whole_span:1

# ----------------------------------------------------------------------------------------
# 170.2275-get-map-size.py
# ----------------------------------------------------------------------------------------
n = len(mymap) # assignment:len, assignment_lhs_identifier:n, assignment_rhs_atom:mymap, call_argument:mymap, external_free_call:len, flat_style, free_call:len, n_liner_style:1, node:Assign, node:Call, node:Name, one_shot_style, single_assignment:n, whole_span:1

# ----------------------------------------------------------------------------------------
# 171.2446-add-an-element-at-the-end-of-a-list.py
# ----------------------------------------------------------------------------------------
s.append(x) # call_argument:x, flat_style, member_call:s:append, member_call_method:append, member_call_object:s, n_liner_style:1, node:Attribute, node:Call, node:Expr, node:Name, one_shot_style, update:s:x, update_by_member_call:s:x, update_by_member_call_with:append, update_with:append, whole_span:1

# ----------------------------------------------------------------------------------------
# 172.2442-insert-entry-in-map.py
# ----------------------------------------------------------------------------------------
m[k] = v # assignment, assignment_lhs_identifier:m, assignment_rhs_atom:v, flat_style, index:k, n_liner_style:1, node:Assign, node:Name, node:Subscript, one_shot_style, whole_span:1

# ----------------------------------------------------------------------------------------
# 173.2427-format-a-number-with-grouped-thousands.py
# ----------------------------------------------------------------------------------------
print("f'{1000:,}'") # call_argument:, external_free_call:print, flat_style, free_call:print, free_call_without_result:print, literal:Str, n_liner_style:1, node:Call, node:Expr, node:Name, node:Str, one_shot_style, whole_span:1

# ----------------------------------------------------------------------------------------
# 173.2428-format-a-number-with-grouped-thousands.py
# ----------------------------------------------------------------------------------------
print("format(1000, ',')") # call_argument:, external_free_call:print, flat_style, free_call:print, free_call_without_result:print, literal:Str, n_liner_style:1, node:Call, node:Expr, node:Name, node:Str, one_shot_style, whole_span:1
...
# ----------------------------------------------------------------------------------------
from urllib import request, parse # flat_style (-> +3), import:urllib:parse, import:urllib:request, import_module:urllib, import_name:parse, import_name:request, n_liner_style:3 (-> +3), node:ImportFrom, one_shot_style (-> +3), whole_span:4 (-> +3)
...
# ----------------------------------------------------------------------------------------
s = a.hex() # assignment:hex, assignment_lhs_identifier:s, assignment_rhs_atom:a, flat_style, member_call_method:hex, n_liner_style:1, node:Assign, node:Attribute, node:Call, node:Name, one_shot_style, single_assignment:s, whole_span:1

# ----------------------------------------------------------------------------------------
# 176.2614-hex-string-to-byte-array.py
# ----------------------------------------------------------------------------------------
a = bytearray.fromhex(s) # assignment:fromhex, assignment_lhs_identifier:a, assignment_rhs_atom:bytearray, assignment_rhs_atom:s, call_argument:s, flat_style, member_call_method:fromhex, n_liner_style:1, node:Assign, node:Attribute, node:Call, node:Name, one_shot_style, single_assignment:a, whole_span:1

# ----------------------------------------------------------------------------------------
# 177.2709-find-files-with-a-given-list-of-filename-extensions.py
# ----------------------------------------------------------------------------------------
import os # flat_style (-> +2), import:os, import_module:os, n_liner_style:2 (-> +2), node:Import, one_shot_style (-> +2), whole_span:3 (-> +2)
...
# ----------------------------------------------------------------------------------------
import re # flat_style (-> +6), import:re, import_module:re, n_liner_style:5 (-> +6), node:Import, one_shot_style (-> +6), whole_span:7 (-> +6)
...
# ----------------------------------------------------------------------------------------
import glob # flat_style (-> +2), import:glob, import_module:glob, n_liner_style:1 (-> +2), node:Import, one_shot_style (-> +2), whole_span:3 (-> +2)
...
# ----------------------------------------------------------------------------------------
b = (x1 < x < x2) and (y1 < y < y2) # assignment, assignment_lhs_identifier:b, assignment_rhs_atom:x, assignment_rhs_atom:x1, assignment_rhs_atom:x2, assignment_rhs_atom:y, assignment_rhs_atom:y1, assignment_rhs_atom:y2, boolean_operator:And, chained_comparison:2, chained_inequalities:2, comparison_operator:Lt, flat_style, n_liner_style:1, node:Assign, node:BoolOp, node:Compare, node:Name, one_shot_style, single_assignment:b, whole_span:1

# ----------------------------------------------------------------------------------------
# 179.2688-get-center-of-a-rectangle.py
# ----------------------------------------------------------------------------------------
center = ((x1 + x2) / 2, (y1 + y2) / 2) # addition_operator, assignment, assignment_lhs_identifier:center, assignment_rhs_atom:2, assignment_rhs_atom:x1, assignment_rhs_atom:x2, assignment_rhs_atom:y1, assignment_rhs_atom:y2, binary_operator:Add, binary_operator:Div, flat_style, literal:2, literal:Tuple, n_liner_style:1, node:Assign, node:BinOp, node:Name, node:Num, node:Tuple, one_shot_style, single_assignment:center, whole_span:1

# ----------------------------------------------------------------------------------------
# 179.2689-get-center-of-a-rectangle.py
# ----------------------------------------------------------------------------------------
from collections import namedtuple # flat_style (-> +2), import:collections:namedtuple, import_module:collections, import_name:namedtuple, n_liner_style:2 (-> +2), node:ImportFrom, one_shot_style (-> +2), whole_span:3 (-> +2)
...
# ----------------------------------------------------------------------------------------
s = "s = %r\nprint(s%%s)" # assignment, assignment_lhs_identifier:s, flat_style (-> +1), literal:Str, n_liner_style:2 (-> +1), node:Assign, node:Name, node:Str, one_shot_style (-> +1), single_assignment:s, whole_span:2 (-> +1)
...
# ----------------------------------------------------------------------------------------
requests # flat_style (-> +6), n_liner_style:6 (-> +6), node:Expr, node:Name, one_shot_style (-> +6), whole_span:7 (-> +6)
...
# ----------------------------------------------------------------------------------------
from datetime import date, timedelta # flat_style (-> +1), import:datetime:date, import:datetime:timedelta, import_module:datetime, import_name:date, import_name:timedelta, n_liner_style:1 (-> +1), node:ImportFrom, one_shot_style (-> +1), whole_span:2 (-> +1)
...
# ----------------------------------------------------------------------------------------
class UnionFind: # n_liner_style:11 (-> +14), node:ClassDef (-> +14), object_oriented_style (-> +14), whole_span:15 (-> +14)
...
# ----------------------------------------------------------------------------------------
import numpy as np # flat_style (-> +1), import:numpy, import_module:numpy, n_liner_style:1 (-> +1), node:Import, one_shot_style (-> +1), whole_span:2 (-> +1)
...
# ----------------------------------------------------------------------------------------
with open(path) as f: # call_argument:path, external_free_call:open, flat_style (-> +1), free_call:open, n_liner_style:2 (-> +1), node:Call, node:Name, node:With (-> +1), one_shot_style (-> +1), whole_span:2 (-> +1)

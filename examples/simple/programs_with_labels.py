...
# 02_input_name.py
# ----------------------------------------------------------------------------------------
name = input("What is your name?\n") # assignment:input, assignment_lhs_identifier:name, call_argument:, external_free_call:input, flat_style (-> +1), free_call:input, global_scope:name (-> +1), imperative_style (-> +1), literal:Str, node:Assign, node:Call, node:Name, node:Str, scope:name (-> +1), single_assignment:name, special_literal_string:What is your name?\n, whole_span:2 (-> +1)
print("Hi, %s." % name) # binary_operator:Mod, call_argument:, external_free_call:print, free_call:print, free_call_without_result:print, literal:Str, loaded_variable:name, node:BinOp, node:Call, node:Expr, node:Name, node:Str, string_formatting_operator

# ----------------------------------------------------------------------------------------
# 03_friends.py
# ----------------------------------------------------------------------------------------
friends = ["john", "pat", "gary", "michael"] # assignment, assignment_lhs_identifier:friends, global_scope:friends (-> +2), global_scope:i (-> +2), global_scope:name (-> +2), imperative_style (-> +2), literal:List, literal:Str, node:Assign, node:List, node:Name, node:Str, scope:friends (-> +2), scope:i (-> +2), scope:name (-> +2), single_assignment:friends, whole_span:3 (-> +2)
for i, name in enumerate(friends): # call_argument:friends, external_free_call:enumerate, for:i (-> +1), for:name (-> +1), for_indexes_elements:i (-> +1), free_call:enumerate, iteration_variable:i, iteration_variable:name, literal:Tuple, loaded_variable:friends, loop:for (-> +1), loop_with_late_exit:for (-> +1), node:Call, node:For (-> +1), node:Name, node:Tuple
...
while babies < 100: # comparison_operator:Lt, literal:100, loaded_variable:babies, loop:while (-> +2), loop_with_late_exit:while (-> +2), magic_number:100, node:Compare, node:Name, node:Num, node:While (-> +2)
    print("This generation has {} babies".format(babies)) # call_argument:, call_argument:babies, composition, external_free_call:print, free_call:print, free_call_without_result:print, literal:Str, loaded_variable:babies, member_call_method:format, node:Attribute, node:Call, node:Expr, node:Name, node:Str
...
# 05_greet.py
# ----------------------------------------------------------------------------------------
def greet(name): # function:greet (-> +1), function_parameter:name, function_parameter_flavor:arg, function_returning_nothing:greet (-> +1), local_scope:name (-> +1), node:FunctionDef (-> +1), node:arg, procedural_style (-> +4), scope:name (-> +1), whole_span:5 (-> +4)
    print("Hello", name) # call_argument:, call_argument:name, external_free_call:print, free_call:print, free_call_without_result:print, literal:Str, loaded_variable:name, node:Call, node:Expr, node:Name, node:Str
...
# 06_regex.py
# ----------------------------------------------------------------------------------------
import re # global_scope:test_string (-> +5), imperative_style (-> +5), import:re, import_module:re, node:Import, scope:test_string (-> +5), whole_span:6 (-> +5)
for test_string in ["555-1212", "ILL-EGAL"]: # for:test_string (-> +4), iteration_variable:test_string, literal:List, literal:Str, loop:for (-> +4), loop_with_late_exit:for (-> +4), node:For (-> +4), node:List, node:Name, node:Str
    if re.match(r"^\d{3}-\d{4}$", test_string): # call_argument:, call_argument:test_string, if (-> +3), if_test_atom:re, if_test_atom:test_string, literal:Str, loaded_variable:re, loaded_variable:test_string, member_call_method:match, node:Attribute, node:Call, node:If (-> +3), node:Name, node:Str, special_literal_string:^\\d{3}-\\d{4}$
        print(test_string, "is a valid US local phone number") # call_argument:, call_argument:test_string, external_free_call:print, free_call:print, free_call_without_result:print, if_then_branch, literal:Str, loaded_variable:test_string, node:Call, node:Expr, node:Name, node:Str
    else:
        print(test_string, "rejected") # call_argument:, call_argument:test_string, external_free_call:print, free_call:print, free_call_without_result:print, if_else_branch, literal:Str, loaded_variable:test_string, node:Call, node:Expr, node:Name, node:Str

# ----------------------------------------------------------------------------------------
# 07_grocery_bill.py
# ----------------------------------------------------------------------------------------
prices = {"apple": 0.40, "banana": 0.50} # assignment, assignment_lhs_identifier:prices, assignment_rhs_atom:0.4, assignment_rhs_atom:0.5, flat_style (-> +3), global_scope:grocery_bill (-> +3), global_scope:my_purchase (-> +3), global_scope:prices (-> +3), imperative_style (-> +3), literal:0.4, literal:0.5, literal:Dict, literal:Str, magic_number:0.4, magic_number:0.5, node:Assign, node:Dict, node:Name, node:Num, node:Str, scope:grocery_bill (-> +3), scope:my_purchase (-> +3), scope:prices (-> +3), single_assignment:prices, whole_span:4 (-> +3)
...
# 09_indent.py
# ----------------------------------------------------------------------------------------
import glob # global_scope:file_name (-> +7), global_scope:line (-> +7), global_scope:python_files (-> +7), imperative_style (-> +7), import:glob, import_module:glob, node:Import, scope:file_name (-> +7), scope:line (-> +7), scope:python_files (-> +7), whole_span:8 (-> +7)
python_files = glob.glob("*.py") # assignment:glob, assignment_lhs_identifier:python_files, assignment_rhs_atom:glob, call_argument:, literal:Str, loaded_variable:glob, member_call_method:glob, node:Assign, node:Attribute, node:Call, node:Name, node:Str, single_assignment:python_files
for file_name in sorted(python_files): # call_argument:python_files, external_free_call:sorted, for:file_name (-> +5), free_call:sorted, iteration_variable:file_name, loaded_variable:python_files, loop:for (-> +5), loop_with_late_exit:for (-> +5), node:Call, node:For (-> +5), node:Name
    print("    ------" + file_name) # binary_operator:Add, call_argument:, concatenation_operator:Str, external_free_call:print, free_call:print, free_call_without_result:print, literal:Str, loaded_variable:file_name, node:BinOp, node:Call, node:Expr, node:Name, node:Str
    with open(file_name) as f: # call_argument:file_name, external_free_call:open, free_call:open, loaded_variable:file_name, node:Call, node:Name, node:With (-> +2)
        for line in f: # for:line (-> +1), for_each:line (-> +1), iteration_variable:line, loaded_variable:f, loop:for (-> +1), loop_with_late_exit:for (-> +1), nested_for:1 (-> +1), node:For (-> +1), node:Name
            print("    " + line.rstrip()) # binary_operator:Add, call_argument:, composition, concatenation_operator:Str, external_free_call:print, free_call:print, free_call_without_result:print, literal:Str, loaded_variable:line, member_call_method:rstrip, node:Attribute, node:BinOp, node:Call, node:Expr, node:Name, node:Str
...
for activity_time in sorted(activities.keys()): # call_argument:, composition, external_free_call:sorted, for:activity_time (-> +5), free_call:sorted, iteration_variable:activity_time, loaded_variable:activities, loop:for (-> +5), loop_with_break:for (-> +5), loop_with_early_exit:for:break (-> +5), loop_with_else:for (-> +5), member_call_method:keys, node:Attribute, node:Call, node:For (-> +5), node:Name
    if hour < activity_time: # comparison_operator:Lt, if (-> +2), if_test_atom:activity_time, if_test_atom:hour, if_without_else (-> +2), loaded_variable:activity_time, loaded_variable:hour, node:Compare, node:If (-> +2), node:Name
        print(activities[activity_time]) # call_argument:, external_free_call:print, free_call:print, free_call_without_result:print, if_then_branch (-> +1), index:activity_time, loaded_variable:activities, loaded_variable:activity_time, node:Call, node:Expr, node:Name, node:Subscript
        break # node:Break
else:
...
# 11_bottles.py
# ----------------------------------------------------------------------------------------
REFRAIN = """ # assignment, assignment_lhs_identifier:REFRAIN, global_scope:REFRAIN (-> +9), global_scope:bottles_of_beer (-> +9), imperative_style (-> +9), node:Assign (-> +5), node:Name, scope:REFRAIN (-> +9), scope:bottles_of_beer (-> +9), single_assignment:REFRAIN, whole_span:10 (-> +9)
...
# 12_classes.py
# ----------------------------------------------------------------------------------------
class BankAccount(object): # global_scope:my_account (-> +11), loaded_variable:object, node:ClassDef (-> +8), node:Name, object_oriented_style (-> +11), scope:my_account (-> +11), whole_span:12 (-> +11)
    def __init__(self, initial_balance=0): # function:__init__ (-> +1), function_parameter:initial_balance, function_parameter:self, function_parameter_flavor:arg, function_returning_nothing:__init__ (-> +1), instance_method:__init__ (-> +1), literal:0, local_scope:initial_balance (-> +1), local_scope:self (-> +1), method:__init__ (-> +1), node:FunctionDef (-> +1), node:Num, node:arg, scope:initial_balance (-> +1), scope:self (-> +1)
    ...
    def deposit(self, amount): # function:deposit (-> +1), function_parameter:amount, function_parameter:self, function_parameter_flavor:arg, function_returning_nothing:deposit (-> +1), instance_method:deposit (-> +1), local_scope:amount (-> +1), local_scope:self (-> +1), method:deposit (-> +1), node:FunctionDef (-> +1), node:arg, scope:amount (-> +1), scope:self (-> +1)
    ...
    def overdrawn(self): # function:overdrawn (-> +1), function_parameter:self, function_parameter_flavor:arg, function_returning_something:overdrawn (-> +1), instance_method:overdrawn (-> +1), local_scope:self (-> +1), method:overdrawn (-> +1), node:FunctionDef (-> +1), node:arg, scope:self (-> +1)
...
my_account.withdraw(50) # call_argument:50, literal:50, loaded_variable:my_account, magic_number:50, member_call:my_account:withdraw, member_call_method:withdraw, member_call_object:my_account, node:Attribute, node:Call, node:Expr, node:Name, node:Num
print(my_account.balance, my_account.overdrawn()) # call_argument:, composition, external_free_call:print, free_call:print, free_call_without_result:print, loaded_variable:my_account, member_call_method:overdrawn, node:Attribute, node:Call, node:Expr, node:Name

# ----------------------------------------------------------------------------------------
# 13_unit_testing.py
# ----------------------------------------------------------------------------------------
import unittest # import:unittest, import_module:unittest, node:Import, object_oriented_style (-> +10), whole_span:11 (-> +10)
def median(pool): # function:median (-> +6), function_parameter:pool, function_parameter_flavor:arg, function_returning_something:median (-> +6), impure_function:median (-> +6), local_scope:copy (-> +6), local_scope:pool (-> +6), local_scope:size (-> +6), node:FunctionDef (-> +6), node:arg, scope:copy (-> +6), scope:pool (-> +6), scope:size (-> +6)
    copy = sorted(pool) # assignment:sorted, assignment_lhs_identifier:copy, assignment_rhs_atom:pool, call_argument:pool, external_free_call:sorted, free_call:sorted, loaded_variable:pool, node:Assign, node:Call, node:Name, single_assignment:copy
    size = len(copy) # assignment:len, assignment_lhs_identifier:size, assignment_rhs_atom:copy, call_argument:copy, external_free_call:len, free_call:len, loaded_variable:copy, node:Assign, node:Call, node:Name, single_assignment:size
    if size % 2 == 1: # binary_operator:Mod, comparison_operator:Eq, divisibility_test:2, if (-> +3), if_test_atom:1, if_test_atom:2, if_test_atom:size, literal:1, literal:2, loaded_variable:size, modulo_operator, node:BinOp, node:Compare, node:If (-> +3), node:Name, node:Num
        return copy[int((size - 1) / 2)] # binary_operator:Div, binary_operator:Sub, call_argument:, external_free_call:int, free_call:int, if_then_branch, index:_, literal:1, literal:2, loaded_variable:copy, loaded_variable:size, node:BinOp, node:Call, node:Name, node:Num, node:Return, node:Subscript, return
    else:
        return (copy[int(size / 2 - 1)] + copy[int(size / 2)]) / 2 # addition_operator, binary_operator:Add, binary_operator:Div, binary_operator:Sub, call_argument:, external_free_call:int, free_call:int, if_else_branch, index:_, literal:1, literal:2, loaded_variable:copy, loaded_variable:size, node:BinOp, node:Call, node:Name, node:Num, node:Return, node:Subscript, return
class TestMedian(unittest.TestCase): # loaded_variable:unittest, node:Attribute, node:ClassDef (-> +2), node:Name
    def testMedian(self): # function:testMedian (-> +1), function_parameter:self, function_parameter_flavor:arg, function_returning_nothing:testMedian (-> +1), instance_method:testMedian (-> +1), local_scope:self (-> +1), method:testMedian (-> +1), node:FunctionDef (-> +1), node:arg, scope:self (-> +1)
...
# ----------------------------------------------------------------------------------------
from itertools import groupby # global_scope:frags (-> +8), global_scope:has_chars (-> +8), global_scope:lines (-> +8), imperative_style (-> +8), import:itertools:groupby, import_module:itertools, import_name:groupby, node:ImportFrom, scope:frags (-> +8), scope:has_chars (-> +8), scope:lines (-> +8), whole_span:9 (-> +8)
...
for has_chars, frags in groupby(lines, bool): # call_argument:bool, call_argument:lines, external_free_call:groupby, for:frags (-> +2), for:has_chars (-> +2), free_call:groupby, iteration_variable:frags, iteration_variable:has_chars, literal:Tuple, loaded_variable:bool, loaded_variable:lines, loop:for (-> +2), loop_with_late_exit:for (-> +2), node:Call, node:For (-> +2), node:Name, node:Tuple
    if has_chars: # if (-> +1), if_without_else (-> +1), loaded_variable:has_chars, node:If (-> +1), node:Name
        print(" ".join(frags)) # call_argument:, call_argument:frags, composition, external_free_call:print, free_call:print, free_call_without_result:print, if_then_branch, literal:Str, loaded_variable:frags, member_call_method:join, node:Attribute, node:Call, node:Expr, node:Name, node:Str

# ----------------------------------------------------------------------------------------
# 16_csv.py
# ----------------------------------------------------------------------------------------
import csv # global_scope:change (-> +17), global_scope:name (-> +17), global_scope:pct (-> +17), global_scope:price (-> +17), global_scope:status (-> +17), global_scope:status_labels (-> +17), global_scope:stocks (-> +17), global_scope:ticker (-> +17), global_scope:writer (-> +17), import:csv, import_module:csv, node:Import, procedural_style (-> +17), scope:change (-> +17), scope:name (-> +17), scope:pct (-> +17), scope:price (-> +17), scope:status (-> +17), scope:status_labels (-> +17), scope:stocks (-> +17), scope:ticker (-> +17), scope:writer (-> +17), whole_span:18 (-> +17)
def cmp(a, b): # function:cmp (-> +1), function_parameter:a, function_parameter:b, function_parameter_flavor:arg, function_returning_something:cmp (-> +1), local_scope:a (-> +1), local_scope:b (-> +1), node:FunctionDef (-> +1), node:arg, pure_function:cmp (-> +1), scope:a (-> +1), scope:b (-> +1)
...
with open("stocks.csv", "w", newline="") as stocksFileW: # call_argument:, call_keyword_argument:newline, empty_literal:Str, external_free_call:open, free_call:open, literal:Str, node:Call, node:Name, node:Str, node:With (-> +6)
    writer = csv.writer(stocksFileW) # assignment:writer, assignment_lhs_identifier:writer, assignment_rhs_atom:csv, assignment_rhs_atom:stocksFileW, call_argument:stocksFileW, loaded_variable:csv, loaded_variable:stocksFileW, member_call_method:writer, node:Assign, node:Attribute, node:Call, node:Name, single_assignment:writer
...
with open("stocks.csv", "r") as stocksFile: # call_argument:, external_free_call:open, free_call:open, literal:Str, node:Call, node:Name, node:Str, node:With (-> +5)
    stocks = csv.reader(stocksFile) # assignment:reader, assignment_lhs_identifier:stocks, assignment_rhs_atom:csv, assignment_rhs_atom:stocksFile, call_argument:stocksFile, loaded_variable:csv, loaded_variable:stocksFile, member_call_method:reader, node:Assign, node:Attribute, node:Call, node:Name, single_assignment:stocks
...
# 18_queens.py
# ----------------------------------------------------------------------------------------
BOARD_SIZE = 8 # assignment:8, assignment_lhs_identifier:BOARD_SIZE, assignment_rhs_atom:8, global_scope:BOARD_SIZE (-> +19), global_scope:answer (-> +19), literal:8, node:Assign, node:Name, node:Num, procedural_style (-> +19), scope:BOARD_SIZE (-> +19), scope:answer (-> +19), single_assignment:BOARD_SIZE, whole_span:20 (-> +19)
def under_attack(col, queens): # function:under_attack (-> +6), function_parameter:col, function_parameter:queens, function_parameter_flavor:arg, function_returning_something:under_attack (-> +6), impure_function:under_attack (-> +6), local_scope:c (-> +6), local_scope:col (-> +6), local_scope:left (-> +6), local_scope:queens (-> +6), local_scope:r (-> +6), local_scope:right (-> +6), node:FunctionDef (-> +6), node:arg, scope:c (-> +6), scope:col (-> +6), scope:left (-> +6), scope:queens (-> +6), scope:r (-> +6), scope:right (-> +6)
...
def solve(n): # access_outer_scope:BOARD_SIZE (-> +8), body_recursive_function:solve (-> +8), function:solve (-> +8), function_parameter:n, function_parameter_flavor:arg, function_returning_something:solve (-> +8), impure_function:solve (-> +8), local_scope:n (-> +8), local_scope:smaller_solutions (-> +8), node:FunctionDef (-> +8), node:arg, recursive_call_count:1 (-> +8), recursive_function:solve (-> +8), scope:n (-> +8), scope:smaller_solutions (-> +8)
    ...
    return [ # node:Return (-> +4), return (-> +4)
        ...
        for i in range(BOARD_SIZE) # call_argument:BOARD_SIZE, external_free_call:range, free_call:range, iteration_variable:i, loaded_variable:BOARD_SIZE, node:Call, node:Name, range:BOARD_SIZE
        for solution in smaller_solutions # iteration_variable:solution, loaded_variable:smaller_solutions, node:Name
        if not under_attack(i + 1, solution) # addition_operator, binary_operator:Add, call_argument:, call_argument:solution, filtered_comprehension, free_call:under_attack, internal_free_call:under_attack, literal:1, loaded_variable:i, loaded_variable:solution, node:BinOp, node:Call, node:Name, node:Num, node:UnaryOp, unary_operator:Not
    ]
for answer in solve(BOARD_SIZE): # call_argument:BOARD_SIZE, for:answer (-> +1), free_call:solve, internal_free_call:solve, iteration_variable:answer, loaded_variable:BOARD_SIZE, loop:for (-> +1), loop_with_late_exit:for (-> +1), node:Call, node:For (-> +1), node:Name
    print(answer) # call_argument:answer, external_free_call:print, free_call:print, free_call_without_result:print, loaded_variable:answer, node:Call, node:Expr, node:Name

# ----------------------------------------------------------------------------------------
# 20_prime_numbers.py
# ----------------------------------------------------------------------------------------
import itertools # global_scope:p (-> +10), import:itertools, import_module:itertools, node:Import, procedural_style (-> +10), scope:p (-> +10), whole_span:11 (-> +10)
def iter_primes(): # function:iter_primes (-> +5), function_without_parameters:iter_primes (-> +5), generator:iter_primes (-> +5), local_scope:numbers (-> +5), local_scope:prime (-> +5), node:FunctionDef (-> +5), scope:numbers (-> +5), scope:prime (-> +5)
    numbers = itertools.count(2) # assignment:count, assignment_lhs_identifier:numbers, assignment_rhs_atom:2, assignment_rhs_atom:itertools, call_argument:2, literal:2, loaded_variable:itertools, member_call_method:count, node:Assign, node:Attribute, node:Call, node:Name, node:Num, single_assignment:numbers
    while True: # infinite_while (-> +3), literal:True, loop:while (-> +3), loop_with_late_exit:while (-> +3), node:NameConstant, node:While (-> +3)
        prime = next(numbers) # assignment:next, assignment_lhs_identifier:prime, assignment_rhs_atom:numbers, call_argument:numbers, external_free_call:next, free_call:next, loaded_variable:numbers, node:Assign, node:Call, node:Name, single_assignment:prime
...
tree = etree.fromstring(dinner_recipe) # assignment:fromstring, assignment_lhs_identifier:tree, assignment_rhs_atom:dinner_recipe, assignment_rhs_atom:etree, call_argument:dinner_recipe, loaded_variable:dinner_recipe, loaded_variable:etree, member_call_method:fromstring, node:Assign, node:Attribute, node:Call, node:Name, single_assignment:tree
...
for ingredient in tree.getiterator("tr"): # call_argument:, for:ingredient (-> +3), iteration_variable:ingredient, literal:Str, loaded_variable:tree, loop:for (-> +3), loop_with_late_exit:for (-> +3), member_call_method:getiterator, node:Attribute, node:Call, node:For (-> +3), node:Name, node:Str
    ...
    if item.tag == "td" and item.text not in pantry: # boolean_operator:And, comparison_operator:Eq, comparison_operator:NotIn, if (-> +1), if_test_atom:item, if_test_atom:pantry, if_without_else (-> +1), literal:Str, loaded_variable:item, loaded_variable:pantry, node:Attribute, node:BoolOp, node:Compare, node:If (-> +1), node:Name, node:Str
        print("{}: {} {}".format(item.text, amt.text, unit.text)) # call_argument:, composition, external_free_call:print, free_call:print, free_call_without_result:print, if_then_branch, literal:Str, loaded_variable:amt, loaded_variable:item, loaded_variable:unit, member_call_method:format, node:Attribute, node:Call, node:Expr, node:Name, node:Str

# ----------------------------------------------------------------------------------------
# 28_queens.py
# ----------------------------------------------------------------------------------------
BOARD_SIZE = 8 # assignment:8, assignment_lhs_identifier:BOARD_SIZE, assignment_rhs_atom:8, global_scope:BOARD_SIZE (-> +23), global_scope:queens (-> +23), literal:8, node:Assign, node:Name, node:Num, object_oriented_style (-> +23), scope:BOARD_SIZE (-> +23), scope:queens (-> +23), single_assignment:BOARD_SIZE, whole_span:24 (-> +23)
class BailOut(Exception): # loaded_variable:Exception, node:ClassDef (-> +1), node:Name
    pass # node:Pass
def validate(queens): # function:validate (-> +5), function_parameter:queens, function_parameter_flavor:arg, function_returning_nothing:validate (-> +5), local_scope:col (-> +5), local_scope:left (-> +5), local_scope:queens (-> +5), local_scope:r (-> +5), local_scope:right (-> +5), node:FunctionDef (-> +5), node:arg, scope:col (-> +5), scope:left (-> +5), scope:queens (-> +5), scope:r (-> +5), scope:right (-> +5), shadowing_scope:queens (-> +5)
    ...
    for r in reversed(queens[:-1]): # call_argument:, external_free_call:reversed, for:r (-> +3), free_call:reversed, iteration_variable:r, literal:-1, loaded_variable:queens, loop:for (-> +3), loop_with_early_exit:for:raise (-> +3), loop_with_raise:for (-> +3), node:Call, node:For (-> +3), node:Name, node:Num, node:Subscript, slice::-1:, slice_lower:, slice_step:, slice_upper:-1
        left, right = left - 1, right + 1 # addition_operator, assignment, assignment_lhs_identifier:left, assignment_lhs_identifier:right, assignment_rhs_atom:1, assignment_rhs_atom:left, assignment_rhs_atom:right, binary_operator:Add, binary_operator:Sub, literal:1, literal:Tuple, loaded_variable:left, loaded_variable:right, node:Assign, node:BinOp, node:Name, node:Num, node:Tuple, parallel_assignment:2, update:left:1, update:left:right, update:right:1, update:right:left, update_by_assignment:left:1, update_by_assignment:left:right, update_by_assignment:right:1, update_by_assignment:right:left, update_by_assignment_with, update_with
...
def add_queen(queens): # access_outer_scope:BOARD_SIZE (-> +11), function:add_queen (-> +11), function_parameter:queens, function_parameter_flavor:arg, function_returning_something:add_queen (-> +11), impure_function:add_queen (-> +11), local_scope:i (-> +11), local_scope:queens (-> +11), local_scope:test_queens (-> +11), node:FunctionDef (-> +11), node:arg, recursive_call_count:1 (-> +11), recursive_function:add_queen (-> +11), scope:i (-> +11), scope:queens (-> +11), scope:test_queens (-> +11), shadowing_scope:queens (-> +11), tail_recursive_function:add_queen (-> +11)
    for i in range(BOARD_SIZE): # call_argument:BOARD_SIZE, external_free_call:range, for:i (-> +9), for_range:BOARD_SIZE (-> +9), free_call:range, iteration_variable:i, loaded_variable:BOARD_SIZE, loop:for (-> +9), loop_with_early_exit:for:return (-> +9), loop_with_return:for (-> +9), node:Call, node:For (-> +9), node:Name, range:BOARD_SIZE
        ...
        try: # node:Try (-> +7), try_except:BailOut (-> +7)
            validate(test_queens) # call_argument:test_queens, free_call:validate, free_call_without_result:validate, internal_free_call:validate, loaded_variable:test_queens, node:Call, node:Expr, node:Name
            if len(test_queens) == BOARD_SIZE: # call_argument:test_queens, comparison_operator:Eq, external_free_call:len, free_call:len, if (-> +3), if_test_atom:BOARD_SIZE, if_test_atom:test_queens, loaded_variable:BOARD_SIZE, loaded_variable:test_queens, node:Call, node:Compare, node:If (-> +3), node:Name
                return test_queens # if_then_branch, loaded_variable:test_queens, node:Name, node:Return, return:test_queens
            else:
                return add_queen(test_queens) # call_argument:test_queens, free_call:add_queen, free_tail_call:add_queen, if_else_branch, internal_free_call:add_queen, loaded_variable:test_queens, node:Call, node:Name, node:Return, return
...
number = random.randint(1, 20) # assignment:randint, assignment_lhs_identifier:number, assignment_rhs_atom:1, assignment_rhs_atom:20, assignment_rhs_atom:random, call_argument:1, call_argument:20, literal:1, literal:20, loaded_variable:random, magic_number:20, member_call_method:randint, node:Assign, node:Attribute, node:Call, node:Name, node:Num, single_assignment:number
print("Well, {}, I am thinking of a number between 1 and 20.".format(name)) # call_argument:, call_argument:name, composition, external_free_call:print, free_call:print, free_call_without_result:print, literal:Str, loaded_variable:name, member_call_method:format, node:Attribute, node:Call, node:Expr, node:Name, node:Str
...
if guess == number: # comparison_operator:Eq, if (-> +3), if_test_atom:guess, if_test_atom:number, loaded_variable:guess, loaded_variable:number, node:Compare, node:If (-> +3), node:Name
    ...
    print("Nope. The number I was thinking of was {}".format(number)) # call_argument:, call_argument:number, composition, external_free_call:print, free_call:print, free_call_without_result:print, if_else_branch, literal:Str, loaded_variable:number, member_call_method:format, node:Attribute, node:Call, node:Expr, node:Name, node:Str

...
for i, name in enumerate(friends): # call_argument:friends, external_free_call:enumerate, for:i (-> +1), for:name (-> +1), for_indexes_elements:i (-> +1), free_call:enumerate, literal:Tuple, loop:for (-> +1), loop_with_late_exit:for (-> +1), node:Call, node:For (-> +1), node:Name, node:Tuple
...
for file_name in sorted(python_files): # call_argument:python_files, external_free_call:sorted, for:file_name (-> +5), free_call:sorted, loop:for (-> +5), loop_with_late_exit:for (-> +5), node:Call, node:For (-> +5), node:Name
    ...
    with open(file_name) as f: # call_argument:file_name, external_free_call:open, free_call:open, node:Call, node:Name, node:With (-> +2)
        for line in f: # for:line (-> +1), for_each:line (-> +1), loop:for (-> +1), loop_with_late_exit:for (-> +1), nested_for:1 (-> +1), node:For (-> +1), node:Name
...
def median(pool): # function:median (-> +6), function_argument:pool, function_argument_flavor:arg, function_returning_something:median (-> +6), impure_function:median (-> +6), node:FunctionDef (-> +6), node:arg
...
# 14_median.py
# ----------------------------------------------------------------------------------------
def median(pool): # function:median (-> +6), function_argument:pool, function_argument_flavor:arg, function_returning_something:median (-> +6), impure_function:median (-> +6), node:FunctionDef (-> +6), node:arg, procedural_style (-> +6), whole_span:7 (-> +6)
    copy = sorted(pool) # assignment:sorted, assignment_lhs_identifier:copy, assignment_rhs_atom:pool, call_argument:pool, external_free_call:sorted, free_call:sorted, node:Assign, node:Call, node:Name, single_assignment:copy
    size = len(copy) # assignment:len, assignment_lhs_identifier:size, assignment_rhs_atom:copy, call_argument:copy, external_free_call:len, free_call:len, node:Assign, node:Call, node:Name, single_assignment:size
    if size % 2 == 1: # binary_operator:Mod, comparison_operator:Eq, divisibility_test:2, if (-> +3), if_test_atom:1, if_test_atom:2, if_test_atom:size, literal:1, literal:2, modulo_operator, node:BinOp, node:Compare, node:If (-> +3), node:Name, node:Num
        return copy[int((size - 1) / 2)] # binary_operator:Div, binary_operator:Sub, call_argument:, external_free_call:int, free_call:int, if_then_branch, index:_, literal:1, literal:2, node:BinOp, node:Call, node:Name, node:Num, node:Return, node:Subscript, return
...
with open("stocks.csv", "r") as stocksFile: # call_argument:, external_free_call:open, free_call:open, literal:Str, node:Call, node:Name, node:Str, node:With (-> +5)
    ...
    for ticker, name, price, change, pct in stocks: # for:change (-> +2), for:name (-> +2), for:pct (-> +2), for:price (-> +2), for:ticker (-> +2), for_each:change (-> +2), for_each:name (-> +2), for_each:pct (-> +2), for_each:price (-> +2), for_each:ticker (-> +2), literal:Tuple, loop:for (-> +2), loop_with_late_exit:for (-> +2), node:For (-> +2), node:Name, node:Tuple
...
def under_attack(col, queens): # function:under_attack (-> +6), function_argument:col, function_argument:queens, function_argument_flavor:arg, function_returning_something:under_attack (-> +6), impure_function:under_attack (-> +6), node:FunctionDef (-> +6), node:arg
...
def solve(n): # body_recursive_function:solve (-> +8), function:solve (-> +8), function_argument:n, function_argument_flavor:arg, function_returning_something:solve (-> +8), impure_function:solve (-> +8), node:FunctionDef (-> +8), node:arg, recursive_call_count:1 (-> +8), recursive_function:solve (-> +8)
...
def add_queen(queens): # function:add_queen (-> +11), function_argument:queens, function_argument_flavor:arg, function_returning_something:add_queen (-> +11), impure_function:add_queen (-> +11), node:FunctionDef (-> +11), node:arg, recursive_call_count:1 (-> +11), recursive_function:add_queen (-> +11), tail_recursive_function:add_queen (-> +11)

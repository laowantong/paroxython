...
# 05_greet.py
# ----------------------------------------------------------------------------------------
def greet(name): # function:greet (-> +1), function_line_count:2 (-> +1), function_parameter:name, function_parameter_flavor:arg, function_returning_nothing:greet (-> +1), local_scope:name (-> +1), node:FunctionDef (-> +1), node:arg, procedural_style (-> +4), scope:name (-> +1), variety:★★ (-> +4), whole_span:5 (-> +4)
...
# 06_regex.py
# ----------------------------------------------------------------------------------------
import re # global_scope:test_string (-> +5), imperative_style (-> +5), import:re, import_module:re, node:Import, scope:test_string (-> +5), variety:★★ (-> +5), whole_span:6 (-> +5)
...
# 08_arguments.py
# ----------------------------------------------------------------------------------------
import sys # global_scope:total (-> +5), imperative_style (-> +5), import:sys, import_module:sys, node:Import, scope:total (-> +5), variety:★★★ (-> +5), whole_span:6 (-> +5)
...
# 09_indent.py
# ----------------------------------------------------------------------------------------
import glob # global_scope:file_name (-> +7), global_scope:line (-> +7), global_scope:python_files (-> +7), imperative_style (-> +7), import:glob, import_module:glob, node:Import, scope:file_name (-> +7), scope:line (-> +7), scope:python_files (-> +7), variety:★★★ (-> +7), whole_span:8 (-> +7)
...
# 12_classes.py
# ----------------------------------------------------------------------------------------
class BankAccount(object): # class:BankAccount (-> +8), class_method_count:4 (-> +8), global_scope:my_account (-> +11), loaded_variable:object, node:ClassDef (-> +8), node:Name, object_oriented_style (-> +11), scope:my_account (-> +11), variety:★★★★ (-> +11), whole_span:12 (-> +11)
...
# 13_unit_testing.py
# ----------------------------------------------------------------------------------------
import unittest # import:unittest, import_module:unittest, node:Import, object_oriented_style (-> +10), variety:★★★ (-> +10), whole_span:11 (-> +10)
...
# 16_csv.py
# ----------------------------------------------------------------------------------------
import csv # global_scope:change (-> +17), global_scope:name (-> +17), global_scope:pct (-> +17), global_scope:price (-> +17), global_scope:status (-> +17), global_scope:status_labels (-> +17), global_scope:stocks (-> +17), global_scope:ticker (-> +17), global_scope:writer (-> +17), import:csv, import_module:csv, node:Import, procedural_style (-> +17), scope:change (-> +17), scope:name (-> +17), scope:pct (-> +17), scope:price (-> +17), scope:status (-> +17), scope:status_labels (-> +17), scope:stocks (-> +17), scope:ticker (-> +17), scope:writer (-> +17), variety:★★★ (-> +17), whole_span:18 (-> +17)
...
# 18_queens.py
# ----------------------------------------------------------------------------------------
BOARD_SIZE = 8 # assignment:8, assignment_lhs_identifier:BOARD_SIZE, assignment_rhs_atom:8, global_scope:BOARD_SIZE (-> +19), global_scope:answer (-> +19), literal:8, node:Assign, node:Name, node:Num, procedural_style (-> +19), scope:BOARD_SIZE (-> +19), scope:answer (-> +19), single_assignment:BOARD_SIZE, variety:★★★★★ (-> +19), whole_span:20 (-> +19)
...
# 20_prime_numbers.py
# ----------------------------------------------------------------------------------------
import itertools # global_scope:p (-> +10), import:itertools, import_module:itertools, node:Import, procedural_style (-> +10), scope:p (-> +10), variety:★★★★★★ (-> +10), whole_span:11 (-> +10)
...
# ----------------------------------------------------------------------------------------
import random # global_scope:guess (-> +17), global_scope:guesses_made (-> +17), global_scope:name (-> +17), global_scope:number (-> +17), imperative_style (-> +17), import:random, import_module:random, node:Import, scope:guess (-> +17), scope:guesses_made (-> +17), scope:name (-> +17), scope:number (-> +17), variety:★★★★ (-> +17), whole_span:18 (-> +17)

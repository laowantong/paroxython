...
# 05_greet.py
# ----------------------------------------------------------------------------------------
def greet(name): # function:greet (-> +1), function_line_count:2 (-> +1), function_parameter:name, function_parameter_flavor:arg, function_returning_nothing:greet (-> +1), local_scope:name (-> +1), node:FunctionDef (-> +1), node:arg, procedural_style (-> +4), scope:name (-> +1), whole_span:5 (-> +4)
...
# 12_classes.py
# ----------------------------------------------------------------------------------------
class BankAccount(object): # class:BankAccount (-> +8), class_method_count:4 (-> +8), global_scope:my_account (-> +11), loaded_variable:object, node:ClassDef (-> +8), node:Name, object_oriented_style (-> +11), scope:my_account (-> +11), whole_span:12 (-> +11)
    def __init__(self, initial_balance=0): # function:__init__ (-> +1), function_line_count:2 (-> +1), function_parameter:initial_balance, function_parameter:self, function_parameter_flavor:arg, function_returning_nothing:__init__ (-> +1), instance_method:__init__ (-> +1), literal:0, local_scope:initial_balance (-> +1), local_scope:self (-> +1), method:__init__ (-> +1), node:FunctionDef (-> +1), node:Num, node:arg, scope:initial_balance (-> +1), scope:self (-> +1)

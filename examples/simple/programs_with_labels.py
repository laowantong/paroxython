...
for file_name in sorted(python_files): # argument:python_files, external_free_call:sorted, for:file_name (-> +5), free_call:sorted, iteration_variable:file_name, loaded_variable:python_files, loop:for (-> +5), loop_with_late_exit:for (-> +5), node:Call, node:For (-> +5), node:Name
    ...
    with open(file_name) as f: # argument:file_name, external_free_call:open, free_call:open, loaded_variable:file_name, node:Call, node:Name, node:With (-> +2)
...
with open("stocks.csv", "w", newline="") as stocksFileW: # argument:, empty_literal:Str, external_free_call:open, free_call:open, free_call_with_keyword_argument:open:newline, keyword_argument:newline, literal:Str, node:Call, node:Name, node:Str, node:With (-> +6)
...
with open("stocks.csv", "r") as stocksFile: # argument:, external_free_call:open, free_call:open, literal:Str, node:Call, node:Name, node:Str, node:With (-> +5)

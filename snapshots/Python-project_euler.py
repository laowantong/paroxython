...
def solution(n): # function:solution (-> +22), function_returning_a_value:solution (-> +22)
    ...
    while True: # literal:True
        ...
        if result < n: # comparison_operator:Lt, if (-> +5)
            ...
            temp = 1 # assignment, if_else_branch (-> +1), int_literal, literal:Num
            break
    while True: # literal:True
...
def solution(n): # function:solution (-> +24), function_returning_a_value:solution (-> +24)
    ...
    if isprime(n): # function_call:isprime, if (-> +16)
        ...
        if isprime(n): # function_call:isprime, if (-> +11), nested_if:2 (-> +11)
            ...
            n1 = int(math.sqrt(n)) + 1 # assignment, binary_operator:Add, composition, function_call:int, if_else_branch (-> +8), int_literal, literal:Num, method_call:sqrt
            for i in range(3, n1, 2): # for_range_step:2 (-> +6), function_call:range, int_literal, literal:Num, suggest_constant_definition
                if n % i == 0: # binary_operator:Mod, comparison_operator:Eq, divisibility_test, if (-> +5), int_literal, literal:Num, nested_if:3 (-> +5)
                    if isprime(n / i): # binary_operator:Div, function_call:isprime, if (-> +4), if_then_branch (-> +4), nested_if:4 (-> +4)
                        maxNumber = n / i # assignment, binary_operator:Div, if_then_branch (-> +1)
                        break
                    elif isprime(i): # function_call:isprime, if (-> +1), nested_if:4 (-> +1)
                        maxNumber = i # assignment, if_elif_branch
            return maxNumber
...
def solution(n): # function:solution (-> +16), function_returning_a_value:solution (-> +16)
    ...
    if n > 1: # comparison_operator:Gt, if (-> +1), int_literal, literal:Num
        prime = n # assignment, if_then_branch
...
def solution(n): # function:solution (-> +8), function_returning_a_value:solution (-> +8)
    for number in range(n - 1, 10000, -1): # binary_operator:Sub, find_first_element (-> +6), for_range_step:-1 (-> +7), function_call:range, int_literal, literal:Num, suggest_constant_definition
        ...
        if strNumber == strNumber[::-1]: # comparison_operator:Eq, if (-> +5), int_literal, literal:Num, slice_step
            divisor = 999 # assignment, if_then_branch (-> +4), int_literal, literal:Num, suggest_constant_definition
    ...
    for i in range(999, 99, -1): # accumulate_elements:Assign (-> +4), for_range_step:-1 (-> +4), function_call:range, int_literal, literal:Num, nested_for (-> +4), square_nested_for (-> +4), suggest_constant_definition
        for j in range(999, 99, -1): # accumulate_elements:Assign (-> +3), for_range_step:-1 (-> +3), function_call:range, int_literal, literal:Num, suggest_constant_definition
            ...
            if t == t[::-1] and i * j < n: # binary_operator:Mult, boolean_operator:And, comparison_operator:Eq, comparison_operator:Lt, if (-> +1), int_literal, literal:Num, slice_step
                answer = max(answer, i * j) # assignment, binary_operator:Mult, function_call:max, if_then_branch
    return answer
    ...
    while 1: # int_literal, literal:Num
        ...
        for j in range(2, n): # for_range_start (-> +3), function_call:range, int_literal, literal:Num
            if i % j != 0: # binary_operator:Mod, comparison_operator:NotEq, divisibility_test, if (-> +2), int_literal, literal:Num
                nfound = 1 # assignment, if_then_branch (-> +1), int_literal, literal:Num
                break
        if nfound == 0: # comparison_operator:Eq, if (-> +3), int_literal, literal:Num
            if i == 0: # comparison_operator:Eq, if (-> +1), if_then_branch (-> +2), int_literal, literal:Num, nested_if:2 (-> +1)
                i = 1 # assignment, if_then_branch, int_literal, literal:Num
            return i
...
def isprime(n): # function:isprime (-> +10), function_returning_a_value:isprime (-> +10)
    if n == 2: # comparison_operator:Eq, if (-> +8), int_literal, literal:Num
        ...
        sq = int(sqrt(n)) + 1 # assignment, binary_operator:Add, composition, function_call:int, function_call:sqrt, if_else_branch (-> +3), int_literal, literal:Num
...
def solution(n): # function:solution (-> +8), function_returning_a_value:solution (-> +8)
    ...
    for i in range(len(n) - 12): # accumulate_elements:AugAssign (-> +5), binary_operator:Sub, composition, for_range_stop (-> +5), function_call:len, function_call:range, int_literal, literal:Num, nested_for (-> +5), suggest_constant_definition
        ...
        if product > LargestProduct: # comparison_operator:Gt, if (-> +1)
            LargestProduct = product # assignment, if_then_branch
...
def solution(n: str) -> int: # function:solution, function_returning_a_value:solution
    ...
    while cur_index < len(n) - 13: # binary_operator:Sub, comparison_operator:Lt, function_call:len, int_literal, literal:Num, suggest_constant_definition
        if int(n[cur_index]) >= int(substr[0]): # comparison_operator:GtE, function_call:int, if (-> +6), index, int_literal, literal:Num
            substr = substr[1:] + n[cur_index] # assignment, binary_operator:Add, if_then_branch (-> +1), index, int_literal, literal:Num, slice
            cur_index += 1 # augmented_assignment, int_literal, literal:Num
        else:
            LargestProduct = max(LargestProduct, streval(substr)) # assignment, composition, function_call:max, function_call:streval, if_else_branch (-> +2)
...
def solution(n): # function:solution (-> +10), function_returning_a_value:solution (-> +10)
    ...
    for a in range(1, n // 3): # binary_operator:FloorDiv, for_range_start (-> +6), function_call:range, int_literal, literal:Num, suggest_constant_definition
        ...
        if c * c == (a * a + b * b): # binary_operator:Add, binary_operator:Mult, comparison_operator:Eq, if (-> +3)
            d = a * b * c # assignment, binary_operator:Mult, if_then_branch (-> +2)
            if d >= product: # comparison_operator:GtE, if (-> +1), nested_if:2 (-> +1)
                product = d # assignment, if_then_branch
    return product
...
def sum_of_primes(n): # function:sum_of_primes (-> +8), function_returning_a_value:sum_of_primes (-> +8)
    if n > 2: # comparison_operator:Gt, if (-> +3), int_literal, literal:Num
        sumOfPrimes = 2 # assignment, if_then_branch, int_literal, literal:Num
    else:
...
def prime_sum(n: int) -> int: # function:prime_sum, function_returning_a_value:prime_sum
    ...
    for i in range(2, int(n ** 0.5) + 1): # binary_operator:Add, binary_operator:Pow, composition, float_literal, for_range_start (-> +3), function_call:int, function_call:range, int_literal, literal:Num, suggest_constant_definition
        if list_[i] == 0: # comparison_operator:Eq, if (-> +2), index, int_literal, literal:Num
            for j in range(i * i, n + 1, i): # binary_operator:Add, binary_operator:Mult, for_range_step (-> +1), function_call:range, if_then_branch (-> +1), int_literal, literal:Num
    ...
    for i in range(nColumns): # for_range_stop (-> +20), function_call:range, nested_for (-> +20)
        for j in range(nRows - 3): # binary_operator:Sub, for_range_stop (-> +19), function_call:range, int_literal, literal:Num, suggest_constant_definition
            ...
            if i < nColumns - 3: # binary_operator:Sub, comparison_operator:Lt, if (-> +5), int_literal, literal:Num, suggest_constant_definition
                lrDiagProduct = ( # assignment, if_then_branch (-> +4)
            ...
            if i > 2: # comparison_operator:Gt, if (-> +5), int_literal, literal:Num
                rlDiagProduct = ( # assignment, if_then_branch (-> +4)
                    grid[i][j] # binary_operator:Mult, index
            ...
            if maxProduct > largest: # comparison_operator:Gt, if (-> +1)
                largest = maxProduct # assignment, if_then_branch
    return largest
        ...
        for i in range(20): # for_range_stop (-> +4), function_call:range, int_literal, literal:Num, nested_for (-> +4), suggest_constant_definition
            for j in range(17): # for_range_stop (-> +3), function_call:range, int_literal, literal:Num, suggest_constant_definition
                ...
                if temp > maximum: # comparison_operator:Gt, if (-> +1)
                    maximum = temp # assignment, if_then_branch
    ...
    for input1 in range(n): # for_range_stop (-> +12), function_call:range
        ...
        while number > 1: # comparison_operator:Gt, evolve_state (-> +6), int_literal, literal:Num
            if number % 2 == 0: # binary_operator:Mod, comparison_operator:Eq, divisibility_test:2, if (-> +5), int_literal, literal:Num
                ...
                number = (3 * number) + 1 # assignment, binary_operator:Add, binary_operator:Mult, if_else_branch (-> +1), int_literal, literal:Num, suggest_constant_definition
                counter += 1 # augmented_assignment, int_literal, literal:Num
        if counter > pre_counter: # comparison_operator:Gt, if (-> +2)
            largest_number = input1 # assignment, if_then_branch (-> +1)
    ...
    for i in range(1, len(a)): # composition, for_range_start (-> +10), function_call:len, function_call:range, int_literal, literal:Num, nested_for (-> +10)
        for j in range(len(a[i])): # composition, for_indexes (-> +9), for_range_stop (-> +9), function_call:len, function_call:range, index
            if j != len(a[i - 1]): # binary_operator:Sub, comparison_operator:NotEq, function_call:len, if (-> +3), index, index_arithmetic, int_literal, literal:Num, suggest_conditional_expression (-> +3)
                number1 = a[i - 1][j] # assignment, binary_operator:Sub, if_then_branch, index, index_arithmetic, int_literal, literal:Num
            else:
                number1 = 0 # assignment, if_else_branch, int_literal, literal:Num
            if j > 0: # comparison_operator:Gt, if (-> +3), int_literal, literal:Num, suggest_conditional_expression (-> +3)
                number2 = a[i - 1][j - 1] # assignment, binary_operator:Sub, if_then_branch, index, index_arithmetic, int_literal, literal:Num
            else:
                number2 = 0 # assignment, if_else_branch, int_literal, literal:Num
...
def fibonacci(n): # function:fibonacci (-> +9), function_returning_a_value:fibonacci (-> +9)
    if n == 1 or type(n) is not int: # boolean_operator:Or, comparison_operator:Eq, comparison_operator:IsNot, function_call:type, if (-> +8), int_literal, literal:Num
        ...
        sequence = [0, 1] # assignment, if_else_branch (-> +3), int_literal, literal:List, literal:Num
...
def is_prime(k: int) -> bool: # function:is_prime, function_returning_a_value:is_prime
    if k < 2 or k % 2 == 0: # binary_operator:Mod, boolean_operator:Or, comparison_operator:Eq, comparison_operator:Lt, divisibility_test:2, if (-> +7), int_literal, literal:Num
        ...
        for x in range(3, int(math.sqrt(k) + 1), 2): # binary_operator:Add, composition, for_range_step:2 (-> +2), function_call:int, function_call:range, if_else_branch (-> +2), int_literal, literal:Num, method_call:sqrt, suggest_constant_definition
    ...
    for a in range((a_limit * -1) + 1, a_limit): # binary_operator:Add, binary_operator:Mult, for_range_start (-> +9), function_call:range, int_literal, literal:Num, nested_for (-> +9)
        for b in range(2, b_limit): # for_range_start (-> +8), function_call:range, int_literal, literal:Num
            if is_prime(b): # function_call:is_prime, if (-> +7)
                count = 0 # assignment, if_then_branch (-> +6), int_literal, literal:Num
                ...
                if count > longest[0]: # comparison_operator:Gt, if (-> +1), index, int_literal, literal:Num, nested_if:2 (-> +1)
                    longest = [count, a, b] # assignment, if_then_branch
...
def next_term(a_i, k, i, n): # function:next_term (-> +50), function_returning_a_value:next_term (-> +50), recursive_function:next_term (-> +50)
    ...
    if sub_memo != None: # comparison_operator:NotEq, if (-> +19), literal:None
        jumps = sub_memo.get(c) # assignment, if_then_branch (-> +15), method_call:get
        if jumps != None and len(jumps) > 0: # boolean_operator:And, comparison_operator:Gt, comparison_operator:NotEq, function_call:len, if (-> +14), int_literal, literal:None, literal:Num, nested_if:2 (-> +14)
            max_jump = -1 # assignment, if_then_branch (-> +11), int_literal, literal:Num
            for _k in range(len(jumps) - 1, -1, -1): # binary_operator:Sub, composition, for_range_step:-1 (-> +3), function_call:len, function_call:range, int_literal, literal:Num
                if jumps[_k][2] <= k and jumps[_k][1] <= max_dn: # boolean_operator:And, comparison_operator:LtE, if (-> +2), index, int_literal, literal:Num, nested_if:3 (-> +2)
                    max_jump = _k # assignment, if_then_branch (-> +1)
                    break
            if max_jump >= 0: # comparison_operator:GtE, if (-> +6), int_literal, literal:Num, nested_if:3 (-> +6)
                diff, dn, _kk = jumps[max_jump] # assignment, if_then_branch (-> +5), index
        ...
        sub_memo = {c: []} # assignment, if_else_branch (-> +1), literal:List
    ...
    if k > ks[0]: # comparison_operator:Gt, if (-> +10), index, int_literal, literal:Num
        ...
        _diff, terms_jumped = compute(a_i, k, i + dn, n) # assignment, binary_operator:Add, function_call:compute, if_else_branch (-> +2)
        diff += _diff # augmented_assignment
        dn += terms_jumped # augmented_assignment
...
def add(digits, k, addend): # function:add (-> +13), procedure:add (-> +13)
    for j in range(k, len(digits)): # composition, for_range_start (-> +9), function_call:len, function_call:range
        ...
        if s >= 10: # comparison_operator:GtE, if (-> +5), int_literal, literal:Num, suggest_constant_definition
            quotient, digits[j] = divmod(s, 10) # assignment, function_call:divmod, if_then_branch (-> +1), index, int_literal, literal:Num, suggest_constant_definition
...
def find_largest(data_file: str = "base_exp.txt") -> int: # function:find_largest, function_returning_a_value:find_largest, function_with_default_positional_arguments:find_largest, literal:Str
    ...
    for i, line in enumerate(open(os.path.join(os.path.dirname(__file__), data_file))): # composition, for_indexes_elements (-> +3), function_call:enumerate, function_call:open, method_call:dirname, method_call:join
        ...
        if x * log10(a) > largest[0]: # binary_operator:Mult, comparison_operator:Gt, function_call:log10, if (-> +1), index, int_literal, literal:Num
            largest = [x * log10(a), i + 1] # assignment, binary_operator:Add, binary_operator:Mult, function_call:log10, if_then_branch, int_literal, literal:Num

...
def solution(n): # function:solution (-> +32), function_returning_something:solution (-> +32)
    ...
    while 1: # int_literal, literal:Num, while (-> +28)
        num += 3 # assignment_lhs_identifier:num, assignment_rhs_atom:3, augmented_assignment, int_literal, literal:Num, suggest_constant_definition
        if num >= n: # comparison_operator:GtE, if (-> +1), if_test_id:n, if_test_id:num
            break # if_then_branch
        sum += num # assignment_lhs_identifier:sum, assignment_rhs_atom:num, augmented_assignment
        num += 2 # assignment_lhs_identifier:num, assignment_rhs_atom:2, augmented_assignment, int_literal, literal:Num
        if num >= n: # comparison_operator:GtE, if (-> +1), if_test_id:n, if_test_id:num
            break # if_then_branch
        sum += num # assignment_lhs_identifier:sum, assignment_rhs_atom:num, augmented_assignment
        num += 1 # assignment_lhs_identifier:num, assignment_rhs_atom:1, augmented_assignment, int_literal, literal:Num
        if num >= n: # comparison_operator:GtE, if (-> +1), if_test_id:n, if_test_id:num
            break # if_then_branch
        sum += num # assignment_lhs_identifier:sum, assignment_rhs_atom:num, augmented_assignment
        num += 3 # assignment_lhs_identifier:num, assignment_rhs_atom:3, augmented_assignment, int_literal, literal:Num, suggest_constant_definition
...
def solution(n): # function:solution (-> +22), function_returning_something:solution (-> +22)
    ...
    collection = list(set(xmulti + zmulti)) # assignment, assignment_lhs_identifier:collection, assignment_rhs_atom:list, assignment_rhs_atom:set, assignment_rhs_atom:xmulti, assignment_rhs_atom:zmulti, binary_operator:Add, call_argument:, composition, function_call:list, function_call:set
...
def solution(n): # function:solution (-> +9), function_returning_something:solution (-> +9)
    ...
    while a < n: # comparison_operator:Lt, while (-> +5)
        if a % 3 == 0 or a % 5 == 0: # binary_operator:Mod, boolean_operator:Or, comparison_operator:Eq, divisibility_test:3, divisibility_test:5, if (-> +3), if_test_id:a, int_literal, literal:Num, suggest_constant_definition
            result += a # assignment_lhs_identifier:result, assignment_rhs_atom:a, augmented_assignment, if_then_branch
            ...
            result -= a # assignment_lhs_identifier:result, assignment_rhs_atom:a, augmented_assignment, if_elif_branch
        a += 1 # assignment_lhs_identifier:a, assignment_rhs_atom:1, augmented_assignment, int_literal, literal:Num
...
def solution(n): # function:solution (-> +7), function_returning_something:solution (-> +7)
    ...
    for i in range(n): # accumulate_elements:1 (-> +4), call_argument:n, for:i (-> +4), for_range:n (-> +4), function_call:range, range:n
        if i % 3 == 0: # binary_operator:Mod, comparison_operator:Eq, divisibility_test:3, if (-> +3), if_test_id:i, int_literal, literal:Num, suggest_constant_definition
            result += i # assignment_lhs_identifier:result, assignment_rhs_atom:i, augmented_assignment, if_then_branch
            ...
            result += i # assignment_lhs_identifier:result, assignment_rhs_atom:i, augmented_assignment, if_elif_branch
    return result # return:result
...
def solution(n): # function:solution (-> +8), function_returning_something:solution (-> +8)
    ...
    while j <= n: # comparison_operator:LtE, while (-> +3)
        if j % 2 == 0: # binary_operator:Mod, comparison_operator:Eq, divisibility_test:2, if (-> +1), if_test_id:j, int_literal, literal:Num
            sum += j # assignment_lhs_identifier:sum, assignment_rhs_atom:j, augmented_assignment, if_then_branch
        i, j = j, i + j # assignment, assignment_lhs_identifier:i, assignment_lhs_identifier:j, assignment_rhs_atom:i, assignment_rhs_atom:j, binary_operator:Add
    return sum # return:sum
...
def solution(n): # function:solution (-> +9), function_returning_something:solution (-> +9)
    ...
    while 4 * b + a <= n: # binary_operator:Add, binary_operator:Mult, comparison_operator:LtE, int_literal, literal:Num, suggest_constant_definition, while (-> +2)
        a, b = b, 4 * b + a # assignment, assignment_lhs_identifier:a, assignment_lhs_identifier:b, assignment_rhs_atom:4, assignment_rhs_atom:a, assignment_rhs_atom:b, binary_operator:Add, binary_operator:Mult, int_literal, literal:Num, suggest_constant_definition
        count += a # assignment_lhs_identifier:count, assignment_rhs_atom:a, augmented_assignment
...
def solution(n): # function:solution (-> +12), function_returning_something:solution (-> +12)
    ...
    phi = (Decimal(5) ** Decimal(0.5) + 1) / Decimal(2) # assignment, assignment_lhs_identifier:phi, assignment_rhs_atom:0.5, assignment_rhs_atom:1, assignment_rhs_atom:2, assignment_rhs_atom:5, assignment_rhs_atom:Decimal, binary_operator:Add, binary_operator:Div, binary_operator:Pow, call_argument:0.5, call_argument:2, call_argument:5, float_literal, function_call:Decimal, int_literal, literal:Num, suggest_constant_definition
...
def solution(n): # function:solution (-> +12), function_returning_something:solution (-> +12)
    ...
    while a[i] <= n: # comparison_operator:LtE, index, while (-> +4)
        ...
        if a[i + 2] > n: # binary_operator:Add, comparison_operator:Gt, if (-> +1), if_test_id:a, if_test_id:i, if_test_id:n, index, index_arithmetic, int_literal, literal:Num
            break # if_then_branch
        i += 1 # assignment_lhs_identifier:i, assignment_rhs_atom:1, augmented_assignment, int_literal, literal:Num
    sum = 0 # assignment, assignment_lhs_identifier:sum, assignment_rhs_atom:0, int_literal, literal:Num
    for j in range(len(a) - 1): # accumulate_elements:1 (-> +2), binary_operator:Sub, call_argument:, call_argument:a, composition, for:j (-> +2), for_range:_ (-> +2), function_call:len, function_call:range, int_literal, literal:Num, range:_
        if a[j] % 2 == 0: # binary_operator:Mod, comparison_operator:Eq, divisibility_test:2, if (-> +1), if_test_id:a, if_test_id:j, index, int_literal, literal:Num
            sum += a[j] # assignment_lhs_identifier:sum, assignment_rhs_atom:a, assignment_rhs_atom:j, augmented_assignment, if_then_branch, index
    return sum # return:sum
...
def isprime(no): # function:isprime (-> +9), function_returning_something:isprime (-> +9)
    ...
    sq = int(math.sqrt(no)) + 1 # assignment, assignment_lhs_identifier:sq, assignment_rhs_atom:1, assignment_rhs_atom:int, assignment_rhs_atom:math, assignment_rhs_atom:no, binary_operator:Add, call_argument:, call_argument:no, composition, function_call:int, int_literal, literal:Num, method_call:sqrt
...
def solution(n): # function:solution (-> +24), function_returning_something:solution (-> +24)
    ...
    if isprime(n): # call_argument:n, function_call:isprime, if (-> +16), if_test_id:isprime, if_test_id:n
        ...
        while n % 2 == 0: # binary_operator:Mod, comparison_operator:Eq, divisibility_test:2, evolve_state (-> +1), if_else_branch (-> +13), int_literal, literal:Num, while (-> +1)
            n = n / 2 # assignment, assignment_lhs_identifier:n, assignment_rhs_atom:2, assignment_rhs_atom:n, binary_operator:Div, int_literal, literal:Num, suggest_augmented_assignment
        if isprime(n): # call_argument:n, function_call:isprime, if (-> +11), if_test_id:isprime, if_test_id:n, nested_if:1 (-> +11)
            ...
            n1 = int(math.sqrt(n)) + 1 # assignment, assignment_lhs_identifier:n1, assignment_rhs_atom:1, assignment_rhs_atom:int, assignment_rhs_atom:math, assignment_rhs_atom:n, binary_operator:Add, call_argument:, call_argument:n, composition, function_call:int, if_else_branch (-> +8), int_literal, literal:Num, method_call:sqrt
...
def solution(n): # function:solution (-> +16), function_returning_something:solution (-> +16)
    ...
    while i * i <= n: # binary_operator:Mult, comparison_operator:LtE, evolve_state (-> +4), while (-> +4)
        while n % i == 0: # binary_operator:Mod, comparison_operator:Eq, divisibility_test, int_literal, literal:Num, while (-> +2)
            ...
            n //= i # assignment_lhs_identifier:n, assignment_rhs_atom:i, augmented_assignment
        i += 1 # assignment_lhs_identifier:i, assignment_rhs_atom:1, augmented_assignment, int_literal, literal:Num
...
def solution(n: int) -> int: # function:solution (-> +18), function_returning_something:solution (-> +18)
    ...
    while n > 2: # comparison_operator:Gt, evolve_state (-> +6), int_literal, literal:Num, while (-> +6)
        ...
        while n % i == 0: # binary_operator:Mod, comparison_operator:Eq, divisibility_test, int_literal, literal:Num, while (-> +1)
            n = n / i # assignment, assignment_lhs_identifier:n, assignment_rhs_atom:i, assignment_rhs_atom:n, binary_operator:Div, suggest_augmented_assignment
        i += 1 # assignment_lhs_identifier:i, assignment_rhs_atom:1, augmented_assignment, int_literal, literal:Num
...
def solution(n): # function:solution (-> +8), function_returning_something:solution (-> +8)
    for number in range(n - 1, 10000, -1): # binary_operator:Sub, call_argument:, call_argument:-1, call_argument:10000, find_first_element (-> +6), for:number (-> +7), for_range:_:10000:-1 (-> +7), function_call:range, int_literal, literal:Num, range:_:10000:-1, suggest_constant_definition
        strNumber = str(number) # assignment, assignment_lhs_identifier:strNumber, assignment_rhs_atom:number, assignment_rhs_atom:str, call_argument:number, function_call:str
        if strNumber == strNumber[::-1]: # comparison_operator:Eq, if (-> +5), if_test_id:strNumber, int_literal, literal:Num, slice_step
            ...
            while divisor != 99: # comparison_operator:NotEq, evolve_state (-> +3), int_literal, literal:Num, suggest_constant_definition, while (-> +3)
                ...
                divisor -= 1 # assignment_lhs_identifier:divisor, assignment_rhs_atom:1, augmented_assignment, int_literal, literal:Num
...
def solution(n): # function:solution (-> +7), function_returning_something:solution (-> +7)
    ...
    for i in range(999, 99, -1): # accumulate_elements:1 (-> +4), call_argument:-1, call_argument:99, call_argument:999, for:i (-> +4), for_range:999:99:-1 (-> +4), function_call:range, int_literal, literal:Num, range:999:99:-1, square_nested_for (-> +4), suggest_constant_definition
        for j in range(999, 99, -1): # accumulate_elements:1 (-> +3), call_argument:-1, call_argument:99, call_argument:999, for:j (-> +3), for_range:999:99:-1 (-> +3), function_call:range, int_literal, literal:Num, nested_for:1 (-> +3), range:999:99:-1, suggest_constant_definition
            t = str(i * j) # assignment, assignment_lhs_identifier:t, assignment_rhs_atom:i, assignment_rhs_atom:j, assignment_rhs_atom:str, binary_operator:Mult, call_argument:, function_call:str
            if t == t[::-1] and i * j < n: # binary_operator:Mult, boolean_operator:And, comparison_operator:Eq, comparison_operator:Lt, if (-> +1), if_test_id:i, if_test_id:j, if_test_id:n, if_test_id:t, int_literal, literal:Num, slice_step
                answer = max(answer, i * j) # assignment, assignment_lhs_identifier:answer, assignment_rhs_atom:answer, assignment_rhs_atom:i, assignment_rhs_atom:j, assignment_rhs_atom:max, binary_operator:Mult, call_argument:, call_argument:answer, function_call:max, if_then_branch
...
def solution(n): # function:solution (-> +18), function_returning_something:solution (-> +18)
    ...
    while 1: # int_literal, literal:Num, while (-> +10)
        i += n * (n - 1) # assignment_lhs_identifier:i, assignment_rhs_atom:1, assignment_rhs_atom:n, augmented_assignment, binary_operator:Mult, binary_operator:Sub, int_literal, literal:Num
...
def solution(n): # function:solution (-> +4), function_returning_something:solution (-> +4)
    ...
    for i in range(1, n + 1): # accumulate_elements:1 (-> +1), binary_operator:Add, call_argument:, call_argument:1, for:i (-> +1), for_range:1:_ (-> +1), function_call:range, int_literal, literal:Num, range:1:_
        g = lcm(g, i) # assignment, assignment_lhs_identifier:g, assignment_rhs_atom:g, assignment_rhs_atom:i, assignment_rhs_atom:lcm, call_argument:g, call_argument:i, function_call:lcm
    return g # return:g
...
def solution(n): # function:solution (-> +7), function_returning_something:solution (-> +7)
    ...
    for i in range(1, n + 1): # accumulate_elements:1 (-> +2), binary_operator:Add, call_argument:, call_argument:1, for:i (-> +2), for_range:1:_ (-> +2), function_call:range, int_literal, literal:Num, range:1:_
        suma += i ** 2 # assignment_lhs_identifier:suma, assignment_rhs_atom:2, assignment_rhs_atom:i, augmented_assignment, binary_operator:Pow, int_literal, literal:Num
        sumb += i # assignment_lhs_identifier:sumb, assignment_rhs_atom:i, augmented_assignment
...
def solution(n): # function:solution (-> +4), function_returning_something:solution (-> +4)
    ...
    suma **= 2 # assignment_lhs_identifier:suma, assignment_rhs_atom:2, augmented_assignment, int_literal, literal:Num
...
def isprime(n): # function:isprime (-> +10), function_returning_something:isprime (-> +10)
    if n == 2: # comparison_operator:Eq, if (-> +8), if_test_id:n, int_literal, literal:Num
        ...
        sq = int(sqrt(n)) + 1 # assignment, assignment_lhs_identifier:sq, assignment_rhs_atom:1, assignment_rhs_atom:int, assignment_rhs_atom:n, assignment_rhs_atom:sqrt, binary_operator:Add, call_argument:, call_argument:n, composition, function_call:int, function_call:sqrt, if_else_branch (-> +3), int_literal, literal:Num
...
def solution(n): # function:solution (-> +11), function_returning_something:solution (-> +11)
    ...
    while i != n: # comparison_operator:NotEq, while (-> +3)
        j += 2 # assignment_lhs_identifier:j, assignment_rhs_atom:2, augmented_assignment, int_literal, literal:Num
        if isprime(j): # call_argument:j, function_call:isprime, if (-> +1), if_test_id:isprime, if_test_id:j
            i += 1 # assignment_lhs_identifier:i, assignment_rhs_atom:1, augmented_assignment, if_then_branch, int_literal, literal:Num
    return j # return:j
...
def solution(n): # function:solution (-> +15), function_returning_something:solution (-> +15)
    ...
    while len(primes) < n: # call_argument:primes, comparison_operator:Lt, evolve_state (-> +5), function_call:len, while (-> +5)
        if isprime(num): # call_argument:num, function_call:isprime, if (-> +4), if_test_id:isprime, if_test_id:num
            ...
            num += 1 # assignment_lhs_identifier:num, assignment_rhs_atom:1, augmented_assignment, if_else_branch, int_literal, literal:Num
...
def solution(n): # function:solution (-> +8), function_returning_something:solution (-> +8)
    ...
    for i in range(len(n) - 12): # accumulate_elements:1 (-> +5), binary_operator:Sub, call_argument:, call_argument:n, composition, for:i (-> +5), for_range:13 (-> +5), for_range:_ (-> +5), function_call:len, function_call:range, int_literal, literal:Num, range:_, suggest_constant_definition
        ...
        for j in range(13): # accumulate_elements:1 (-> +1), call_argument:13, for:j (-> +1), for_range:13 (-> +1), function_call:range, int_literal, literal:Num, nested_for:1 (-> +1), range:13, suggest_constant_definition
            product *= int(n[i + j]) # assignment_lhs_identifier:product, assignment_rhs_atom:i, assignment_rhs_atom:int, assignment_rhs_atom:j, assignment_rhs_atom:n, augmented_assignment, binary_operator:Add, call_argument:, function_call:int, index, index_arithmetic
...
def streval(s: str) -> int: # function:streval (-> +4), function_returning_something:streval (-> +4)
    ...
    for it in s: # accumulate_elements:1 (-> +1), for:it (-> +1), for_each (-> +1)
        ret *= int(it) # assignment_lhs_identifier:ret, assignment_rhs_atom:int, assignment_rhs_atom:it, augmented_assignment, call_argument:it, function_call:int
...
def solution(n: str) -> int: # function:solution (-> +12), function_returning_something:solution (-> +12)
    ...
    while cur_index < len(n) - 13: # binary_operator:Sub, call_argument:n, comparison_operator:Lt, function_call:len, int_literal, literal:Num, suggest_constant_definition, while (-> +7)
        if int(n[cur_index]) >= int(substr[0]): # call_argument:, comparison_operator:GtE, function_call:int, if (-> +6), if_test_id:cur_index, if_test_id:int, if_test_id:n, if_test_id:substr, index, int_literal, literal:Num
            substr = substr[1:] + n[cur_index] # assignment, assignment_lhs_identifier:substr, assignment_rhs_atom:1, assignment_rhs_atom:cur_index, assignment_rhs_atom:n, assignment_rhs_atom:substr, binary_operator:Add, if_then_branch (-> +1), index, int_literal, literal:Num, slice
            cur_index += 1 # assignment_lhs_identifier:cur_index, assignment_rhs_atom:1, augmented_assignment, int_literal, literal:Num
        else:
            LargestProduct = max(LargestProduct, streval(substr)) # assignment, assignment_lhs_identifier:LargestProduct, assignment_rhs_atom:LargestProduct, assignment_rhs_atom:max, assignment_rhs_atom:streval, assignment_rhs_atom:substr, call_argument:, call_argument:LargestProduct, call_argument:substr, composition, function_call:max, function_call:streval, if_else_branch (-> +2)
            ...
            cur_index += 13 # assignment_lhs_identifier:cur_index, assignment_rhs_atom:13, augmented_assignment, int_literal, literal:Num, suggest_constant_definition
    return LargestProduct # return:LargestProduct
...
def sum_of_primes(n): # function:sum_of_primes (-> +8), function_returning_something:sum_of_primes (-> +8)
    ...
    for i in range(3, n, 2): # accumulate_elements:1 (-> +2), call_argument:2, call_argument:3, call_argument:n, for:i (-> +2), for_range:3:n:2 (-> +2), function_call:range, int_literal, literal:Num, range:3:n:2, suggest_constant_definition
        if is_prime(i): # call_argument:i, function_call:is_prime, if (-> +1), if_test_id:i, if_test_id:is_prime
            sumOfPrimes += i # assignment_lhs_identifier:sumOfPrimes, assignment_rhs_atom:i, augmented_assignment, if_then_branch
...
def prime_sum(n: int) -> int: # function:prime_sum (-> +12), function_returning_something:prime_sum (-> +12)
    ...
    for i in range(n): # accumulate_elements:1 (-> +2), call_argument:n, for:i (-> +2), for_range:n (-> +2), function_call:range, range:n
        if list_[i] == 0: # comparison_operator:Eq, if (-> +1), if_test_id:i, if_test_id:list_, index, int_literal, literal:Num
            s += i # assignment_lhs_identifier:s, assignment_rhs_atom:i, augmented_assignment, if_then_branch
    return s # return:s
...
def largest_product(grid): # function:largest_product (-> +27), function_returning_something:largest_product (-> +27)
    nColumns = len(grid[0]) # assignment, assignment_lhs_identifier:nColumns, assignment_rhs_atom:0, assignment_rhs_atom:grid, assignment_rhs_atom:len, call_argument:, function_call:len, index, int_literal, literal:Num
    nRows = len(grid) # assignment, assignment_lhs_identifier:nRows, assignment_rhs_atom:grid, assignment_rhs_atom:len, call_argument:grid, function_call:len
    ...
    for i in range(nColumns): # call_argument:nColumns, for:i (-> +20), for_range:_ (-> +20), for_range:nColumns (-> +20), function_call:range, range:nColumns
        for j in range(nRows - 3): # binary_operator:Sub, call_argument:, for:j (-> +19), for_range:_ (-> +19), function_call:range, int_literal, literal:Num, nested_for:1 (-> +19), range:_, suggest_constant_definition
            ...
            maxProduct = max(vertProduct, horzProduct, lrDiagProduct, rlDiagProduct) # assignment, assignment_lhs_identifier:maxProduct, assignment_rhs_atom:horzProduct, assignment_rhs_atom:lrDiagProduct, assignment_rhs_atom:max, assignment_rhs_atom:rlDiagProduct, assignment_rhs_atom:vertProduct, call_argument:horzProduct, call_argument:lrDiagProduct, call_argument:rlDiagProduct, call_argument:vertProduct, function_call:max
...
def solution(): # function:solution (-> +6), function_returning_something:solution (-> +6)
    ...
    grid = [[int(i) for i in grid[j]] for j in range(len(grid))] # assignment, assignment_lhs_identifier:grid, assignment_rhs_atom:grid, assignment_rhs_atom:i, assignment_rhs_atom:int, assignment_rhs_atom:j, assignment_rhs_atom:len, assignment_rhs_atom:range, call_argument:, call_argument:grid, call_argument:i, composition, comprehension:List, comprehension_for_count:1, function_call:int, function_call:len, function_call:range, index, range:_
...
def count_divisors(n): # function:count_divisors (-> +7), function_returning_something:count_divisors (-> +7)
    ...
    for i in range(1, int(sqrt(n)) + 1): # binary_operator:Add, call_argument:, call_argument:1, call_argument:n, composition, for:i (-> +2), for_range:1:_ (-> +2), function_call:int, function_call:range, function_call:sqrt, int_literal, literal:Num, range:1:_
        if n % i == 0: # binary_operator:Mod, comparison_operator:Eq, divisibility_test, if (-> +1), if_test_id:i, if_test_id:n, int_literal, literal:Num
            nDivisors += 2 # assignment_lhs_identifier:nDivisors, assignment_rhs_atom:2, augmented_assignment, if_then_branch, int_literal, literal:Num
    if n ** 0.5 == int(n ** 0.5): # binary_operator:Pow, call_argument:, comparison_operator:Eq, float_literal, function_call:int, if (-> +1), if_test_id:int, if_test_id:n, literal:Num, suggest_constant_definition
        nDivisors -= 1 # assignment_lhs_identifier:nDivisors, assignment_rhs_atom:1, augmented_assignment, if_then_branch, int_literal, literal:Num
...
def solution(): # function:solution (-> +8), function_returning_something:solution (-> +8)
    ...
    while True: # literal:True, while (-> +4)
        i += 1 # assignment_lhs_identifier:i, assignment_rhs_atom:1, augmented_assignment, int_literal, literal:Num
        tNum += i # assignment_lhs_identifier:tNum, assignment_rhs_atom:i, augmented_assignment
    ...
    for input1 in range(n): # call_argument:n, for:input1 (-> +12), for_range:n (-> +12), function_call:range, range:n
        ...
        while number > 1: # comparison_operator:Gt, evolve_state (-> +6), int_literal, literal:Num, while (-> +6)
            if number % 2 == 0: # binary_operator:Mod, comparison_operator:Eq, divisibility_test:2, if (-> +5), if_test_id:number, int_literal, literal:Num
                number /= 2 # assignment_lhs_identifier:number, assignment_rhs_atom:2, augmented_assignment, if_then_branch (-> +1), int_literal, literal:Num
                counter += 1 # assignment_lhs_identifier:counter, assignment_rhs_atom:1, augmented_assignment, int_literal, literal:Num
            else:
                number = (3 * number) + 1 # assignment, assignment_lhs_identifier:number, assignment_rhs_atom:1, assignment_rhs_atom:3, assignment_rhs_atom:number, binary_operator:Add, binary_operator:Mult, if_else_branch (-> +1), int_literal, literal:Num, suggest_constant_definition
                counter += 1 # assignment_lhs_identifier:counter, assignment_rhs_atom:1, augmented_assignment, int_literal, literal:Num
...
def solution(n): # function:solution (-> +2), function_returning_something:solution (-> +2)
    result = max([(len(collatz_sequence(i)), i) for i in range(1, n)]) # assignment, assignment_lhs_identifier:result, assignment_rhs_atom:1, assignment_rhs_atom:collatz_sequence, assignment_rhs_atom:i, assignment_rhs_atom:len, assignment_rhs_atom:max, assignment_rhs_atom:n, assignment_rhs_atom:range, call_argument:, call_argument:1, call_argument:i, call_argument:n, composition, comprehension:List, comprehension_for_count:1, function_call:collatz_sequence, function_call:len, function_call:max, function_call:range, int_literal, literal:Num, range:1:n
...
def lattice_paths(n): # function:lattice_paths (-> +3), function_returning_something:lattice_paths (-> +3)
    n = 2 * n # assignment, assignment_lhs_identifier:n, assignment_rhs_atom:2, assignment_rhs_atom:n, binary_operator:Mult, int_literal, literal:Num
...
def solution(power): # function:solution (-> +7), function_returning_something:solution (-> +7)
    ...
    string_num = str(num) # assignment, assignment_lhs_identifier:string_num, assignment_rhs_atom:num, assignment_rhs_atom:str, call_argument:num, function_call:str
    list_num = list(string_num) # assignment, assignment_lhs_identifier:list_num, assignment_rhs_atom:list, assignment_rhs_atom:string_num, call_argument:string_num, function_call:list
    ...
    for i in list_num: # accumulate_elements:1 (-> +1), for:i (-> +1), for_each (-> +1)
        sum_of_num += int(i) # assignment_lhs_identifier:sum_of_num, assignment_rhs_atom:i, assignment_rhs_atom:int, augmented_assignment, call_argument:i, function_call:int
...
def solution(power): # function:solution (-> +5), function_returning_something:solution (-> +5)
    ...
    while n: # while (-> +1)
        r, n = r + n % 10, n // 10 # assignment, assignment_lhs_identifier:n, assignment_lhs_identifier:r, assignment_rhs_atom:10, assignment_rhs_atom:n, assignment_rhs_atom:r, binary_operator:Add, binary_operator:FloorDiv, binary_operator:Mod, int_literal, literal:Num, suggest_constant_definition
    return r # return:r
    ...
    for i in range(1, n + 1): # accumulate_elements:1 (-> +12), binary_operator:Add, call_argument:, call_argument:1, for:i (-> +12), for_range:1:_ (-> +12), function_call:range, int_literal, literal:Num, range:1:_
        if i < 1000: # comparison_operator:Lt, if (-> +11), if_test_id:i, int_literal, literal:Num, suggest_constant_definition
            if i >= 100: # comparison_operator:GtE, if (-> +3), if_test_id:i, if_then_branch (-> +8), int_literal, literal:Num, nested_if:1 (-> +3), suggest_constant_definition
                count += ones_counts[i // 100] + 7 # assignment_lhs_identifier:count, assignment_rhs_atom:100, assignment_rhs_atom:7, assignment_rhs_atom:i, assignment_rhs_atom:ones_counts, augmented_assignment, binary_operator:Add, binary_operator:FloorDiv, if_then_branch (-> +2), index, index_arithmetic, int_literal, literal:Num, suggest_constant_definition
                if i % 100 != 0: # binary_operator:Mod, comparison_operator:NotEq, divisibility_test:100, if (-> +1), if_test_id:i, int_literal, literal:Num, nested_if:2 (-> +1), suggest_constant_definition
                    count += 3 # assignment_lhs_identifier:count, assignment_rhs_atom:3, augmented_assignment, if_then_branch, int_literal, literal:Num, suggest_constant_definition
            if 0 < i % 100 < 20: # binary_operator:Mod, chained_comparison:2, chained_inequalities:2, comparison_operator:Lt, if (-> +4), if_test_id:i, int_literal, literal:Num, nested_if:1 (-> +4), suggest_constant_definition
                count += ones_counts[i % 100] # assignment_lhs_identifier:count, assignment_rhs_atom:100, assignment_rhs_atom:i, assignment_rhs_atom:ones_counts, augmented_assignment, binary_operator:Mod, if_then_branch, index, index_arithmetic, int_literal, literal:Num, suggest_constant_definition
            else:
                count += ones_counts[i % 10] # assignment_lhs_identifier:count, assignment_rhs_atom:10, assignment_rhs_atom:i, assignment_rhs_atom:ones_counts, augmented_assignment, binary_operator:Mod, if_else_branch (-> +1), index, index_arithmetic, int_literal, literal:Num, suggest_constant_definition
                count += tens_counts[(i % 100 - i % 10) // 10] # assignment_lhs_identifier:count, assignment_rhs_atom:10, assignment_rhs_atom:100, assignment_rhs_atom:i, assignment_rhs_atom:tens_counts, augmented_assignment, binary_operator:FloorDiv, binary_operator:Mod, binary_operator:Sub, index, index_arithmetic, int_literal, literal:Num, suggest_constant_definition
        else:
            count += ones_counts[i // 1000] + 8 # assignment_lhs_identifier:count, assignment_rhs_atom:1000, assignment_rhs_atom:8, assignment_rhs_atom:i, assignment_rhs_atom:ones_counts, augmented_assignment, binary_operator:Add, binary_operator:FloorDiv, if_else_branch, index, index_arithmetic, int_literal, literal:Num, suggest_constant_definition
...
def solution(): # function:solution (-> +17), function_returning_something:solution (-> +17)
    script_dir = os.path.dirname(os.path.realpath(__file__)) # assignment, assignment_lhs_identifier:script_dir, assignment_rhs_atom:__file__, assignment_rhs_atom:os, call_argument:, call_argument:__file__, composition, method_call:dirname, method_call:realpath
    triangle = os.path.join(script_dir, "triangle.txt") # assignment, assignment_lhs_identifier:triangle, assignment_rhs_atom:os, assignment_rhs_atom:script_dir, call_argument:, call_argument:script_dir, literal:Str, method_call:join
    with open(triangle, "r") as f: # call_argument:, call_argument:triangle, function_call:open, literal:Str
        triangle = f.readlines() # assignment, assignment_lhs_identifier:triangle, assignment_rhs_atom:f, method_call:readlines
    a = [[int(y) for y in x.rstrip("\r\n").split(" ")] for x in triangle] # assignment, assignment_lhs_identifier:a, assignment_rhs_atom:int, assignment_rhs_atom:triangle, assignment_rhs_atom:x, assignment_rhs_atom:y, call_argument:, call_argument:y, comprehension:List, comprehension_for_count:1, function_call:int, literal:Str, method_call:rstrip, method_call:split, method_call_object:x, method_chaining
    for i in range(1, len(a)): # call_argument:, call_argument:1, call_argument:a, composition, for:i (-> +10), for_range:1:_ (-> +10), for_range:_ (-> +10), function_call:len, function_call:range, int_literal, literal:Num, range:1:_
        for j in range(len(a[i])): # call_argument:, composition, for:j (-> +9), for_indexes (-> +9), for_range:_ (-> +9), function_call:len, function_call:range, index, nested_for:1 (-> +9), range:_
            ...
            a[i][j] += max(number1, number2) # assignment_rhs_atom:max, assignment_rhs_atom:number1, assignment_rhs_atom:number2, augmented_assignment, call_argument:number1, call_argument:number2, function_call:max, index
...
def solution(): # function:solution (-> +24), function_returning_something:solution (-> +24)
    ...
    while year < 2001: # comparison_operator:Lt, evolve_state (-> +17), int_literal, literal:Num, suggest_constant_definition, while (-> +17)
        day += 7 # assignment_lhs_identifier:day, assignment_rhs_atom:7, augmented_assignment, int_literal, literal:Num, suggest_constant_definition
        if (year % 4 == 0 and not year % 100 == 0) or (year % 400 == 0): # binary_operator:Mod, boolean_operator:And, boolean_operator:Or, comparison_operator:Eq, divisibility_test:100, divisibility_test:4, divisibility_test:400, if (-> +10), if_test_id:year, int_literal, literal:Num, suggest_constant_definition, unary_operator:Not
            if day > days_per_month[month - 1] and month != 2: # binary_operator:Sub, boolean_operator:And, comparison_operator:Gt, comparison_operator:NotEq, if (-> +5), if_test_id:day, if_test_id:days_per_month, if_test_id:month, if_then_branch (-> +5), index, index_arithmetic, int_literal, literal:Num, nested_if:1 (-> +5)
                month += 1 # assignment_lhs_identifier:month, assignment_rhs_atom:1, augmented_assignment, if_then_branch (-> +1), int_literal, literal:Num
                day = day - days_per_month[month - 2] # assignment, assignment_lhs_identifier:day, assignment_rhs_atom:2, assignment_rhs_atom:day, assignment_rhs_atom:days_per_month, assignment_rhs_atom:month, binary_operator:Sub, index, index_arithmetic, int_literal, literal:Num, suggest_augmented_assignment
                ...
                month += 1 # assignment_lhs_identifier:month, assignment_rhs_atom:1, augmented_assignment, if_elif_branch (-> +1), int_literal, literal:Num
                day = day - 29 # assignment, assignment_lhs_identifier:day, assignment_rhs_atom:29, assignment_rhs_atom:day, binary_operator:Sub, int_literal, literal:Num, suggest_augmented_assignment, suggest_constant_definition
        else:
        ...
        if month > 12: # comparison_operator:Gt, if (-> +2), if_test_id:month, int_literal, literal:Num, suggest_constant_definition
            year += 1 # assignment_lhs_identifier:year, assignment_rhs_atom:1, augmented_assignment, if_then_branch (-> +1), int_literal, literal:Num
            month = 1 # assignment, assignment_lhs_identifier:month, assignment_rhs_atom:1, int_literal, literal:Num
        if year < 2001 and day == 1: # boolean_operator:And, comparison_operator:Eq, comparison_operator:Lt, if (-> +1), if_test_id:day, if_test_id:year, int_literal, literal:Num, suggest_constant_definition
            sundays += 1 # assignment_lhs_identifier:sundays, assignment_rhs_atom:1, augmented_assignment, if_then_branch, int_literal, literal:Num
...
def factorial(n): # function:factorial (-> +4), function_returning_something:factorial (-> +4)
    ...
    for i in range(1, n + 1): # accumulate_elements:1 (-> +1), binary_operator:Add, call_argument:, call_argument:1, for:i (-> +1), for_range:1:_ (-> +1), function_call:range, int_literal, literal:Num, range:1:_
        fact *= i # assignment_lhs_identifier:fact, assignment_rhs_atom:i, augmented_assignment
...
def split_and_add(number): # function:split_and_add (-> +6), function_returning_something:split_and_add (-> +6)
    ...
    while number > 0: # comparison_operator:Gt, evolve_state (-> +3), int_literal, literal:Num, while (-> +3)
        ...
        sum_of_digits += last_digit # assignment_lhs_identifier:sum_of_digits, assignment_rhs_atom:last_digit, augmented_assignment
        number = number // 10 # assignment, assignment_lhs_identifier:number, assignment_rhs_atom:10, assignment_rhs_atom:number, binary_operator:FloorDiv, int_literal, literal:Num, suggest_augmented_assignment, suggest_constant_definition
    return sum_of_digits # return:sum_of_digits
def solution(n): # function:solution (-> +3), function_returning_something:solution (-> +3)
    f = factorial(n) # assignment, assignment_lhs_identifier:f, assignment_rhs_atom:factorial, assignment_rhs_atom:n, call_argument:n, function_call:factorial
    result = split_and_add(f) # assignment, assignment_lhs_identifier:result, assignment_rhs_atom:f, assignment_rhs_atom:split_and_add, call_argument:f, function_call:split_and_add
    return result # return:result
...
def solution(n): # function:solution (-> +7), function_returning_something:solution (-> +7)
    ...
    for j in str(fact): # accumulate_elements:1 (-> +1), call_argument:fact, for:j (-> +1), function_call:str
        result += int(j) # assignment_lhs_identifier:result, assignment_rhs_atom:int, assignment_rhs_atom:j, augmented_assignment, call_argument:j, function_call:int
    return result # return:result
...
def sum_of_divisors(n): # function:sum_of_divisors (-> +7), function_returning_something:sum_of_divisors (-> +7)
    ...
    for i in range(1, int(sqrt(n) + 1)): # accumulate_elements:1 (-> +4), binary_operator:Add, call_argument:, call_argument:1, call_argument:n, composition, for:i (-> +4), for_range:1:_ (-> +4), function_call:int, function_call:range, function_call:sqrt, int_literal, literal:Num, range:1:_
        if n % i == 0 and i != sqrt(n): # binary_operator:Mod, boolean_operator:And, call_argument:n, comparison_operator:Eq, comparison_operator:NotEq, divisibility_test, function_call:sqrt, if (-> +3), if_test_id:i, if_test_id:n, if_test_id:sqrt, int_literal, literal:Num
            total += i + n // i # assignment_lhs_identifier:total, assignment_rhs_atom:i, assignment_rhs_atom:n, augmented_assignment, binary_operator:Add, binary_operator:FloorDiv, if_then_branch
        elif i == sqrt(n): # call_argument:n, comparison_operator:Eq, function_call:sqrt, if (-> +1), if_test_id:i, if_test_id:n, if_test_id:sqrt
            total += i # assignment_lhs_identifier:total, assignment_rhs_atom:i, augmented_assignment, if_elif_branch
    return total - n # binary_operator:Sub, return
def solution(n): # function:solution (-> +8), function_returning_something:solution (-> +8)
    total = sum( # assignment, assignment_lhs_identifier:total, assignment_rhs_atom:sum, composition, function_call:sum
        [
            i # assignment_rhs_atom:i, call_argument:, comprehension:List, comprehension_for_count:1
            for i in range(1, n) # assignment_rhs_atom:1, assignment_rhs_atom:i, assignment_rhs_atom:n, assignment_rhs_atom:range, call_argument:1, call_argument:n, function_call:range, int_literal, literal:Num, range:1:n
            if sum_of_divisors(sum_of_divisors(i)) == i and sum_of_divisors(i) != i # assignment_rhs_atom:i, assignment_rhs_atom:sum_of_divisors, boolean_operator:And, call_argument:, call_argument:i, comparison_operator:Eq, comparison_operator:NotEq, composition, filtered_comprehension, function_call:sum_of_divisors
        ]
    )
...
def solution(): # function:solution (-> +12), function_returning_something:solution (-> +12)
    with open(os.path.dirname(__file__) + "/p022_names.txt") as file: # binary_operator:Add, call_argument:, call_argument:__file__, composition, function_call:open, literal:Str, method_call:dirname
        names = str(file.readlines()[0]) # assignment, assignment_lhs_identifier:names, assignment_rhs_atom:0, assignment_rhs_atom:file, assignment_rhs_atom:str, call_argument:, composition, function_call:str, index, int_literal, literal:Num, method_call:readlines, method_call_object:file
        names = names.replace('"', "").split(",") # assignment, assignment_lhs_identifier:names, assignment_rhs_atom:names, call_argument:, literal:Str, method_call:replace, method_call:split, method_call_object:names, method_chaining
    ...
    for i, name in enumerate(names): # call_argument:names, for:i:name (-> +4), for_indexes_elements (-> +4), function_call:enumerate
        for letter in name: # accumulate_elements:1 (-> +1), for:letter (-> +1), for_each (-> +1), nested_for:1 (-> +1)
            name_score += ord(letter) - 64 # assignment_lhs_identifier:name_score, assignment_rhs_atom:64, assignment_rhs_atom:letter, assignment_rhs_atom:ord, augmented_assignment, binary_operator:Sub, call_argument:letter, function_call:ord, int_literal, literal:Num, suggest_constant_definition
        total_score += (i + 1) * name_score # assignment_lhs_identifier:total_score, assignment_rhs_atom:1, assignment_rhs_atom:i, assignment_rhs_atom:name_score, augmented_assignment, binary_operator:Add, binary_operator:Mult, int_literal, literal:Num
        name_score = 0 # assignment, assignment_lhs_identifier:name_score, assignment_rhs_atom:0, int_literal, literal:Num
...
def solution(): # function:solution (-> +12), function_returning_something:solution (-> +12)
    ...
    with open(os.path.dirname(__file__) + "/p022_names.txt") as file: # binary_operator:Add, call_argument:, call_argument:__file__, composition, function_call:open, literal:Str, method_call:dirname
        name = str(file.readlines()[0]) # assignment, assignment_lhs_identifier:name, assignment_rhs_atom:0, assignment_rhs_atom:file, assignment_rhs_atom:str, call_argument:, composition, function_call:str, index, int_literal, literal:Num, method_call:readlines, method_call_object:file
        name = name.replace('"', "").split(",") # assignment, assignment_lhs_identifier:name, assignment_rhs_atom:name, call_argument:, literal:Str, method_call:replace, method_call:split, method_call_object:name, method_chaining
    ...
    for i in range(len(name)): # accumulate_elements:1 (-> +4), call_argument:, call_argument:name, composition, for:i (-> +4), for_indexes (-> +4), for_range:_ (-> +4), function_call:len, function_call:range, range:_
        for j in name[i]: # accumulate_elements:1 (-> +1), for:j (-> +1), index, nested_for:1 (-> +1)
            temp_sum += ord(j) - ord("A") + 1 # assignment_lhs_identifier:temp_sum, assignment_rhs_atom:1, assignment_rhs_atom:j, assignment_rhs_atom:ord, augmented_assignment, binary_operator:Add, binary_operator:Sub, call_argument:, call_argument:j, function_call:ord, int_literal, literal:Num, literal:Str
        total_sum += (i + 1) * temp_sum # assignment_lhs_identifier:total_sum, assignment_rhs_atom:1, assignment_rhs_atom:i, assignment_rhs_atom:temp_sum, augmented_assignment, binary_operator:Add, binary_operator:Mult, int_literal, literal:Num
        temp_sum = 0 # assignment, assignment_lhs_identifier:temp_sum, assignment_rhs_atom:0, int_literal, literal:Num
...
def solution(limit=28123): # function:solution (-> +13), function_returning_something:solution (-> +13), function_with_default_positional_arguments:solution (-> +13), int_literal, literal:Num
    ...
    for i in range(2, int(limit ** 0.5) + 1): # accumulate_elements:1 (-> +3), binary_operator:Add, binary_operator:Pow, call_argument:, call_argument:2, composition, float_literal, for:i (-> +3), for_range:2:_ (-> +3), for_range:_:_ (-> +3), function_call:int, function_call:range, int_literal, literal:Num, range:2:_, suggest_constant_definition
        sumDivs[i * i] += i # assignment_lhs_identifier:sumDivs, assignment_rhs_atom:i, augmented_assignment, binary_operator:Mult, index, index_arithmetic
        for k in range(i + 1, limit // i + 1): # accumulate_elements:1 (-> +1), binary_operator:Add, binary_operator:FloorDiv, call_argument:, for:k (-> +1), for_range:_:_ (-> +1), function_call:range, int_literal, literal:Num, nested_for:1 (-> +1), range:_:_
            sumDivs[k * i] += k + i # assignment_lhs_identifier:sumDivs, assignment_rhs_atom:i, assignment_rhs_atom:k, augmented_assignment, binary_operator:Add, binary_operator:Mult, index, index_arithmetic
    abundants = set() # assignment, assignment_lhs_identifier:abundants, assignment_rhs_atom:set, function_call:set
    ...
    for n in range(1, limit + 1): # accumulate_elements:2 (-> +4), binary_operator:Add, call_argument:, call_argument:1, for:n (-> +4), for_range:1:_ (-> +4), function_call:range, int_literal, literal:Num, range:1:_
        ...
        if not any((n - a in abundants) for a in abundants): # binary_operator:Sub, call_argument:, comparison_operator:In, comprehension:Generator, comprehension_for_count:1, function_call:any, if (-> +1), if_test_id:a, if_test_id:abundants, if_test_id:any, if_test_id:n, unary_operator:Not
            res += n # assignment_lhs_identifier:res, assignment_rhs_atom:n, augmented_assignment, if_then_branch
...
def fib(a, b, n): # function:fib (-> +13), function_returning_something:fib (-> +13)
    ...
    for x in range(2, n): # call_argument:2, call_argument:n, for:x (-> +4), for_range:2:n (-> +4), function_call:range, int_literal, literal:Num, range:2:n
        c = str(a) + str(b) # assignment, assignment_lhs_identifier:c, assignment_rhs_atom:a, assignment_rhs_atom:b, assignment_rhs_atom:str, binary_operator:Add, call_argument:a, call_argument:b, function_call:str
...
def solution(n): # function:solution (-> +11), function_returning_something:solution (-> +11)
    ...
    for x in range(n): # call_argument:n, for:x (-> +8), for_range:n (-> +8), function_call:range, range:n
        l = [i for i in input().split()] # assignment, assignment_lhs_identifier:l, assignment_rhs_atom:i, assignment_rhs_atom:input, comprehension:List, comprehension_for_count:1, function_call:input, method_call:split
        ...
        while 1: # int_literal, literal:Num, while (-> +4)
            if len(fib(l[0], l[1], c2)) < int(l[2]): # call_argument:, call_argument:c2, comparison_operator:Lt, composition, function_call:fib, function_call:int, function_call:len, if (-> +3), if_test_id:c2, if_test_id:fib, if_test_id:int, if_test_id:l, if_test_id:len, index, int_literal, literal:Num
                c2 += 1 # assignment_lhs_identifier:c2, assignment_rhs_atom:1, augmented_assignment, if_then_branch, int_literal, literal:Num
            else:
                break # if_else_branch
...
def solution(): # function:solution (-> +2), function_returning_something:solution (-> +2)
    result = list(map("".join, permutations("0123456789"))) # assignment, assignment_lhs_identifier:result, assignment_rhs_atom:list, assignment_rhs_atom:map, assignment_rhs_atom:permutations, call_argument:, composition, function_call:list, function_call:map, function_call:permutations, literal:Str
...
def fibonacci_digits_index(n): # function:fibonacci_digits_index (-> +6), function_returning_something:fibonacci_digits_index (-> +6)
    ...
    while digits < n: # comparison_operator:Lt, while (-> +2)
        index += 1 # assignment_lhs_identifier:index, assignment_rhs_atom:1, augmented_assignment, int_literal, literal:Num
        digits = len(str(fibonacci(index))) # assignment, assignment_lhs_identifier:digits, assignment_rhs_atom:fibonacci, assignment_rhs_atom:index, assignment_rhs_atom:len, assignment_rhs_atom:str, call_argument:, call_argument:index, composition, function_call:fibonacci, function_call:len, function_call:str
...
def solution(n): # function:solution (-> +5), function_returning_something:solution (-> +5)
    ...
    gen = fibonacci_generator() # assignment, assignment_lhs_identifier:gen, assignment_rhs_atom:fibonacci_generator, function_call:fibonacci_generator
    while len(str(next(gen))) < n: # call_argument:, call_argument:gen, comparison_operator:Lt, composition, function_call:len, function_call:next, function_call:str, while (-> +1)
        answer += 1 # assignment_lhs_identifier:answer, assignment_rhs_atom:1, augmented_assignment, int_literal, literal:Num
...
def solution(n): # function:solution (-> +12), function_returning_something:solution (-> +12)
    ...
    while True: # literal:True, while (-> +8)
        ...
        f1, f2 = f2, f # assignment, assignment_lhs_identifier:f1, assignment_lhs_identifier:f2, assignment_rhs_atom:f, assignment_rhs_atom:f2
        index += 1 # assignment_lhs_identifier:index, assignment_rhs_atom:1, augmented_assignment, int_literal, literal:Num
        ...
        for b in range(2, b_limit): # call_argument:2, call_argument:b_limit, for:b (-> +8), for_range:2:b_limit (-> +8), function_call:range, int_literal, literal:Num, nested_for:1 (-> +8), range:2:b_limit
            if is_prime(b): # call_argument:b, function_call:is_prime, if (-> +7), if_test_id:b, if_test_id:is_prime
                ...
                while is_prime((n ** 2) + (a * n) + b): # binary_operator:Add, binary_operator:Mult, binary_operator:Pow, call_argument:, function_call:is_prime, int_literal, literal:Num, while (-> +2)
                    count += 1 # assignment_lhs_identifier:count, assignment_rhs_atom:1, augmented_assignment, int_literal, literal:Num
...
# ----------------------------------------------------------------------------------------
def solution(n): # function:solution (-> +8), function_returning_something:solution (-> +8)
    collectPowers = set() # assignment, assignment_lhs_identifier:collectPowers, assignment_rhs_atom:set, function_call:set
...
def solve(digit_len: int) -> str: # function:solve (-> +13), function_returning_something:solve (-> +13)
    ...
    last_digit = int("1" + "0" * digit_len) # assignment, assignment_lhs_identifier:last_digit, assignment_rhs_atom:digit_len, assignment_rhs_atom:int, binary_operator:Add, binary_operator:Mult, call_argument:, function_call:int, literal:Str
    for num in range(den, last_digit): # accumulate_elements:1 (-> +7), call_argument:den, call_argument:last_digit, for:num (-> +7), for_range:den:last_digit (-> +7), function_call:range, range:den:last_digit
        while den <= 99: # comparison_operator:LtE, evolve_state (-> +4), int_literal, literal:Num, suggest_constant_definition, while (-> +4)
            ...
            den += 1 # assignment_lhs_identifier:den, assignment_rhs_atom:1, augmented_assignment, int_literal, literal:Num
        num += 1 # assignment_lhs_identifier:num, assignment_rhs_atom:1, augmented_assignment, int_literal, literal:Num
    ...
    solutions = " , ".join(solutions) # assignment, assignment_lhs_identifier:solutions, assignment_rhs_atom:solutions, call_argument:solutions, literal:Str, method_call:join
...
def is_palindrome(n): # function:is_palindrome (-> +5), function_returning_something:is_palindrome (-> +5)
    n = str(n) # assignment, assignment_lhs_identifier:n, assignment_rhs_atom:n, assignment_rhs_atom:str, call_argument:n, function_call:str
...
def solution(n): # function:solution (-> +5), function_returning_something:solution (-> +5)
    ...
    for i in range(1, n): # accumulate_elements:1 (-> +2), call_argument:1, call_argument:n, for:i (-> +2), for_range:1:n (-> +2), function_call:range, int_literal, literal:Num, range:1:n
        if is_palindrome(i) and is_palindrome(bin(i).split("b")[1]): # boolean_operator:And, call_argument:, call_argument:i, composition, function_call:bin, function_call:is_palindrome, if (-> +1), if_test_id:bin, if_test_id:i, if_test_id:is_palindrome, index, int_literal, literal:Num, literal:Str, method_call:split
            total += i # assignment_lhs_identifier:total, assignment_rhs_atom:i, augmented_assignment, if_then_branch
    return total # return:total
...
def solution(): # function:solution (-> +14), function_returning_something:solution (-> +14)
    ...
    while len(constant) < 1e6: # call_argument:constant, comparison_operator:Lt, evolve_state (-> +2), float_literal, function_call:len, literal:Num, suggest_constant_definition, while (-> +2)
        ...
        i += 1 # assignment_lhs_identifier:i, assignment_rhs_atom:1, augmented_assignment, int_literal, literal:Num
    constant = "".join(constant) # assignment, assignment_lhs_identifier:constant, assignment_rhs_atom:constant, call_argument:constant, literal:Str, method_call:join
    return ( # return
...
def solution(): # function:solution (-> +13), function_returning_something:solution (-> +13)
    ...
    with open(wordsFilePath, "r") as f: # call_argument:, call_argument:wordsFilePath, function_call:open, literal:Str
        words = f.readline() # assignment, assignment_lhs_identifier:words, assignment_rhs_atom:f, method_call:readline
    words = list(map(lambda word: word.strip('"'), words.strip("\r\n").split(","))) # assignment, assignment_lhs_identifier:words, assignment_rhs_atom:list, assignment_rhs_atom:map, assignment_rhs_atom:word, assignment_rhs_atom:words, call_argument:, composition, function_call:list, function_call:map, lambda_function, literal:Str, method_call:split, method_call:strip, method_call_object:words, method_chaining
    words = list( # assignment, assignment_lhs_identifier:words, assignment_rhs_atom:list, composition, function_call:list
        filter( # assignment_rhs_atom:filter, call_argument:, composition, function_call:filter
            ...
            map(lambda word: sum(map(lambda x: ord(x) - 64, word)), words), # assignment_rhs_atom:64, assignment_rhs_atom:map, assignment_rhs_atom:ord, assignment_rhs_atom:sum, assignment_rhs_atom:word, assignment_rhs_atom:words, assignment_rhs_atom:x, binary_operator:Sub, call_argument:, call_argument:word, call_argument:words, call_argument:x, composition, function_call:map, function_call:ord, function_call:sum, int_literal, lambda_function, literal:Num, suggest_constant_definition
        )
    )
...
def solution(): # function:solution (-> +4), function_returning_something:solution (-> +4)
    ...
    for i in range(1, 1001): # accumulate_elements:1 (-> +1), call_argument:1, call_argument:1001, for:i (-> +1), for_range:1:1001 (-> +1), function_call:range, int_literal, literal:Num, range:1:1001, suggest_constant_definition
        total += i ** i # assignment_lhs_identifier:total, assignment_rhs_atom:i, augmented_assignment, binary_operator:Pow
    ...
    for i in range(1, 101): # call_argument:1, call_argument:101, for:i (-> +3), for_range:1:101 (-> +3), for_range:1:_ (-> +3), function_call:range, int_literal, literal:Num, range:1:101, suggest_constant_definition
        for j in range(1, i + 1): # binary_operator:Add, call_argument:, call_argument:1, for:j (-> +2), for_range:1:_ (-> +2), function_call:range, int_literal, literal:Num, nested_for:1 (-> +2), range:1:_
            if combinations(i, j) > 1e6: # call_argument:i, call_argument:j, comparison_operator:Gt, float_literal, function_call:combinations, if (-> +1), if_test_id:combinations, if_test_id:i, if_test_id:j, literal:Num, suggest_constant_definition
                total += 1 # assignment_lhs_identifier:total, assignment_rhs_atom:1, augmented_assignment, if_then_branch, int_literal, literal:Num
    return total # return:total
...
base = [10 ** k for k in range(ks[-1] + 1)] # assignment, assignment_lhs_identifier:base, assignment_rhs_atom:-1, assignment_rhs_atom:1, assignment_rhs_atom:10, assignment_rhs_atom:k, assignment_rhs_atom:ks, assignment_rhs_atom:range, binary_operator:Add, binary_operator:Pow, call_argument:, comprehension:List, comprehension_for_count:1, function_call:range, index, int_literal, literal:Num, negative_index:-1, range:_
...
def next_term(a_i, k, i, n): # body_recursive_function:next_term (-> +50), function:next_term (-> +50), function_returning_something:next_term (-> +50), recursive_function:next_term (-> +50)
    ...
    for j in range(k, len(a_i)): # accumulate_elements:1 (-> +1), call_argument:, call_argument:a_i, call_argument:k, composition, for:j (-> +1), for_range:k:_ (-> +1), function_call:len, function_call:range, range:k:_
        ds_b += a_i[j] # assignment_lhs_identifier:ds_b, assignment_rhs_atom:a_i, assignment_rhs_atom:j, augmented_assignment, index
    ...
    for j in range(min(len(a_i), k)): # accumulate_elements:1 (-> +1), call_argument:, call_argument:a_i, call_argument:k, composition, for:j (-> +1), for_range:_ (-> +1), function_call:len, function_call:min, function_call:range, range:_
        c += a_i[j] * base[j] # assignment_lhs_identifier:c, assignment_rhs_atom:a_i, assignment_rhs_atom:base, assignment_rhs_atom:j, augmented_assignment, binary_operator:Mult, index
    ...
    sub_memo = memo.get(ds_b) # assignment, assignment_lhs_identifier:sub_memo, assignment_rhs_atom:ds_b, assignment_rhs_atom:memo, call_argument:ds_b, method_call:get
    if sub_memo != None: # comparison_operator:NotEq, if (-> +19), if_test_id:sub_memo, literal:None
        jumps = sub_memo.get(c) # assignment, assignment_lhs_identifier:jumps, assignment_rhs_atom:c, assignment_rhs_atom:sub_memo, call_argument:c, if_then_branch (-> +15), method_call:get
        if jumps != None and len(jumps) > 0: # boolean_operator:And, call_argument:jumps, comparison_operator:Gt, comparison_operator:NotEq, function_call:len, if (-> +14), if_test_id:jumps, if_test_id:len, int_literal, literal:None, literal:Num, nested_if:1 (-> +14)
            ...
            if max_jump >= 0: # comparison_operator:GtE, if (-> +6), if_test_id:max_jump, int_literal, literal:Num, nested_if:2 (-> +6)
                ...
                for j in range(min(k, len(a_i))): # call_argument:, call_argument:a_i, call_argument:k, composition, for:j (-> +1), for_range:_ (-> +1), function_call:len, function_call:min, function_call:range, range:_
                    new_c, a_i[j] = divmod(new_c, 10) # assignment, assignment_lhs_identifier:a_i, assignment_lhs_identifier:new_c, assignment_rhs_atom:10, assignment_rhs_atom:divmod, assignment_rhs_atom:new_c, call_argument:10, call_argument:new_c, function_call:divmod, index, int_literal, literal:Num, suggest_constant_definition
    ...
    if k > ks[0]: # comparison_operator:Gt, if (-> +10), if_test_id:k, if_test_id:ks, index, int_literal, literal:Num
        ...
        _diff, terms_jumped = compute(a_i, k, i + dn, n) # assignment, assignment_lhs_identifier:_diff, assignment_lhs_identifier:terms_jumped, assignment_rhs_atom:a_i, assignment_rhs_atom:compute, assignment_rhs_atom:dn, assignment_rhs_atom:i, assignment_rhs_atom:k, assignment_rhs_atom:n, binary_operator:Add, call_argument:, call_argument:a_i, call_argument:k, call_argument:n, function_call:compute, if_else_branch (-> +2)
        diff += _diff # assignment_lhs_identifier:diff, assignment_rhs_atom:_diff, augmented_assignment
        dn += terms_jumped # assignment_lhs_identifier:dn, assignment_rhs_atom:terms_jumped, augmented_assignment
...
def compute(a_i, k, i, n): # function:compute (-> +25), function_returning_something:compute (-> +25)
    ...
    for j in range(len(a_i)): # accumulate_elements:1 (-> +4), call_argument:, call_argument:a_i, composition, for:j (-> +4), for_indexes (-> +4), for_range:_ (-> +4), function_call:len, function_call:range, range:_
        if j >= k: # comparison_operator:GtE, if (-> +3), if_test_id:j, if_test_id:k
            ds_b += a_i[j] # assignment_lhs_identifier:ds_b, assignment_rhs_atom:a_i, assignment_rhs_atom:j, augmented_assignment, if_then_branch, index
        else:
            ds_c += a_i[j] # assignment_lhs_identifier:ds_c, assignment_rhs_atom:a_i, assignment_rhs_atom:j, augmented_assignment, if_else_branch, index
    while i < n: # comparison_operator:Lt, while (-> +10)
        ...
        diff += addend # assignment_lhs_identifier:diff, assignment_rhs_atom:addend, augmented_assignment
        ...
        for j in range(k): # accumulate_elements:1 (-> +3), call_argument:k, for:j (-> +3), for_range:k (-> +3), function_call:range, range:k
            ...
            addend, a_i[j] = divmod(s, 10) # assignment, assignment_lhs_identifier:a_i, assignment_lhs_identifier:addend, assignment_rhs_atom:10, assignment_rhs_atom:divmod, assignment_rhs_atom:s, call_argument:10, call_argument:s, function_call:divmod, index, int_literal, literal:Num, suggest_constant_definition
            ds_c += a_i[j] # assignment_lhs_identifier:ds_c, assignment_rhs_atom:a_i, assignment_rhs_atom:j, augmented_assignment, index
...
def add(digits, k, addend): # function:add (-> +13), function_returning_nothing:add (-> +13)
    for j in range(k, len(digits)): # call_argument:, call_argument:digits, call_argument:k, composition, for:j (-> +9), for_range:k:_ (-> +9), function_call:len, function_call:range, range:k:_
        ...
        if s >= 10: # comparison_operator:GtE, if (-> +5), if_test_id:s, int_literal, literal:Num, suggest_constant_definition
            quotient, digits[j] = divmod(s, 10) # assignment, assignment_lhs_identifier:digits, assignment_lhs_identifier:quotient, assignment_rhs_atom:10, assignment_rhs_atom:divmod, assignment_rhs_atom:s, call_argument:10, call_argument:s, function_call:divmod, if_then_branch (-> +1), index, int_literal, literal:Num, suggest_constant_definition
            addend = addend // 10 + quotient # assignment, assignment_lhs_identifier:addend, assignment_rhs_atom:10, assignment_rhs_atom:addend, assignment_rhs_atom:quotient, binary_operator:Add, binary_operator:FloorDiv, int_literal, literal:Num, suggest_constant_definition
        else:
            digits[j] = s # assignment, assignment_lhs_identifier:digits, assignment_rhs_atom:s, if_else_branch (-> +1), index
            addend = addend // 10 # assignment, assignment_lhs_identifier:addend, assignment_rhs_atom:10, assignment_rhs_atom:addend, binary_operator:FloorDiv, int_literal, literal:Num, suggest_augmented_assignment, suggest_constant_definition
    ...
    while addend > 0: # comparison_operator:Gt, int_literal, literal:Num, while (-> +2)
        addend, digit = divmod(addend, 10) # assignment, assignment_lhs_identifier:addend, assignment_lhs_identifier:digit, assignment_rhs_atom:10, assignment_rhs_atom:addend, assignment_rhs_atom:divmod, call_argument:10, call_argument:addend, function_call:divmod, int_literal, literal:Num, suggest_constant_definition
...
def solution(n): # function:solution (-> +12), function_returning_something:solution (-> +12)
    ...
    for j in range(len(digits)): # accumulate_elements:1 (-> +1), call_argument:, call_argument:digits, composition, for:j (-> +1), for_indexes (-> +1), for_range:_ (-> +1), function_call:len, function_call:range, range:_
        a_n += digits[j] * 10 ** j # assignment_lhs_identifier:a_n, assignment_rhs_atom:10, assignment_rhs_atom:digits, assignment_rhs_atom:j, augmented_assignment, binary_operator:Mult, binary_operator:Pow, index, int_literal, literal:Num, suggest_constant_definition
...
def solution(): # function:solution (-> +18), function_returning_something:solution (-> +18)
    ...
    with open(triangle, "r") as f: # call_argument:, call_argument:triangle, function_call:open, literal:Str
        triangle = f.readlines() # assignment, assignment_lhs_identifier:triangle, assignment_rhs_atom:f, method_call:readlines
    a = map(lambda x: x.rstrip("\r\n").split(" "), triangle) # assignment, assignment_lhs_identifier:a, assignment_rhs_atom:map, assignment_rhs_atom:triangle, assignment_rhs_atom:x, call_argument:, call_argument:triangle, composition, function_call:map, lambda_function, literal:Str, method_call:rstrip, method_call:split, method_call_object:x, method_chaining
    a = list(map(lambda x: list(map(lambda y: int(y), x)), a)) # assignment, assignment_lhs_identifier:a, assignment_rhs_atom:a, assignment_rhs_atom:int, assignment_rhs_atom:list, assignment_rhs_atom:map, assignment_rhs_atom:x, assignment_rhs_atom:y, call_argument:, call_argument:a, call_argument:x, call_argument:y, composition, function_call:int, function_call:list, function_call:map, lambda_function
    for i in range(1, len(a)): # call_argument:, call_argument:1, call_argument:a, composition, for:i (-> +10), for_range:1:_ (-> +10), for_range:_ (-> +10), function_call:len, function_call:range, int_literal, literal:Num, range:1:_
        for j in range(len(a[i])): # call_argument:, composition, for:j (-> +9), for_indexes (-> +9), for_range:_ (-> +9), function_call:len, function_call:range, index, nested_for:1 (-> +9), range:_
            if j != len(a[i - 1]): # binary_operator:Sub, call_argument:, comparison_operator:NotEq, function_call:len, if (-> +3), if_test_id:a, if_test_id:i, if_test_id:j, if_test_id:len, index, index_arithmetic, int_literal, literal:Num, suggest_conditional_expression (-> +3)
                number1 = a[i - 1][j] # assignment, assignment_lhs_identifier:number1, assignment_rhs_atom:1, assignment_rhs_atom:a, assignment_rhs_atom:i, assignment_rhs_atom:j, binary_operator:Sub, if_then_branch, index, index_arithmetic, int_literal, literal:Num
...
def find_largest(data_file: str = "base_exp.txt") -> int: # function:find_largest (-> +6), function_returning_something:find_largest (-> +6), function_with_default_positional_arguments:find_largest (-> +6), literal:Str
    ...
    for i, line in enumerate(open(os.path.join(os.path.dirname(__file__), data_file))): # call_argument:, call_argument:__file__, call_argument:data_file, composition, for:i:line (-> +3), for_indexes_elements (-> +3), function_call:enumerate, function_call:open, method_call:dirname, method_call:join
        a, x = list(map(int, line.split(","))) # assignment, assignment_lhs_identifier:a, assignment_lhs_identifier:x, assignment_rhs_atom:int, assignment_rhs_atom:line, assignment_rhs_atom:list, assignment_rhs_atom:map, call_argument:, call_argument:int, composition, function_call:list, function_call:map, literal:Str, method_call:split
        if x * log10(a) > largest[0]: # binary_operator:Mult, call_argument:a, comparison_operator:Gt, function_call:log10, if (-> +1), if_test_id:a, if_test_id:largest, if_test_id:log10, if_test_id:x, index, int_literal, literal:Num
            largest = [x * log10(a), i + 1] # assignment, assignment_lhs_identifier:largest, assignment_rhs_atom:1, assignment_rhs_atom:a, assignment_rhs_atom:i, assignment_rhs_atom:log10, assignment_rhs_atom:x, binary_operator:Add, binary_operator:Mult, call_argument:a, function_call:log10, if_then_branch, int_literal, literal:Num

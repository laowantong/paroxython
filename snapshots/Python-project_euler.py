...
def solution(n): # function:solution (-> +1), function_returning_something:solution (-> +1)
    return sum([e for e in range(3, n) if e % 3 == 0 or e % 5 == 0]) # binary_operator:Mod, boolean_operator:Or, call_parameter:, call_parameter:3, call_parameter:n, comparison_operator:Eq, composition, comprehension:List, comprehension_for_count:1, divisibility_test:3, divisibility_test:5, filtered_comprehension, function_call:range, function_call:sum, int_literal, literal:Num, return, suggest_constant_definition
...
def solution(n): # function:solution (-> +22), function_returning_something:solution (-> +22)
    ...
    collection = list(set(xmulti + zmulti)) # assignment, assignment_lhs_identifier:collection, assignment_rhs_identifier:list, assignment_rhs_identifier:set, assignment_rhs_identifier:xmulti, assignment_rhs_identifier:zmulti, binary_operator:Add, call_parameter:, composition, function_call:list, function_call:set
    return sum(collection) # call_parameter:collection, function_call:sum, return

# ----------------------------------------------------------------------------------------
# ../Python/project_euler/problem_01/sol5.py
# ----------------------------------------------------------------------------------------
def solution(n): # function:solution (-> +1), function_returning_something:solution (-> +1)
    return sum([i for i in range(n) if i % 3 == 0 or i % 5 == 0]) # binary_operator:Mod, boolean_operator:Or, call_parameter:, call_parameter:n, comparison_operator:Eq, composition, comprehension:List, comprehension_for_count:1, divisibility_test:3, divisibility_test:5, filtered_comprehension, function_call:range, function_call:sum, int_literal, literal:Num, return, suggest_constant_definition
...
def solution(n): # function:solution (-> +12), function_returning_something:solution (-> +12)
    try: # try (-> +3), try_except:TypeError (-> +3), try_except:ValueError (-> +3), try_raise:TypeError (-> +3)
        ...
        raise TypeError("Parameter n must be int or passive of cast to int.") # call_parameter:, function_call:TypeError, literal:Str, raise:TypeError
    if n <= 0: # comparison_operator:LtE, if (-> +1), if_test_id:n, int_literal, literal:Num
        raise ValueError("Parameter n must be greater or equal to one.") # call_parameter:, function_call:ValueError, if_then_branch, literal:Str, raise:ValueError
    getcontext().prec = 100 # assignment, function_call:getcontext, int_literal, literal:Num, suggest_constant_definition
    phi = (Decimal(5) ** Decimal(0.5) + 1) / Decimal(2) # assignment, assignment_lhs_identifier:phi, assignment_rhs_identifier:Decimal, binary_operator:Add, binary_operator:Div, binary_operator:Pow, call_parameter:0.5, call_parameter:2, call_parameter:5, float_literal, function_call:Decimal, int_literal, literal:Num, suggest_constant_definition
    index = (math.floor(math.log(n * (phi + 2), phi) - 1) // 3) * 3 + 2 # assignment, assignment_lhs_identifier:index, assignment_rhs_identifier:math, assignment_rhs_identifier:n, assignment_rhs_identifier:phi, binary_operator:Add, binary_operator:FloorDiv, binary_operator:Mult, binary_operator:Sub, call_parameter:, call_parameter:phi, composition, int_literal, literal:Num, method_call:floor, method_call:log, suggest_constant_definition
    num = Decimal(round(phi ** Decimal(index + 1))) / (phi + 2) # assignment, assignment_lhs_identifier:num, assignment_rhs_identifier:Decimal, assignment_rhs_identifier:index, assignment_rhs_identifier:phi, assignment_rhs_identifier:round, binary_operator:Add, binary_operator:Div, binary_operator:Pow, call_parameter:, composition, function_call:Decimal, function_call:round, int_literal, literal:Num
    ...
    return int(sum) # call_parameter:sum, function_call:int, return
...
def solution(n): # function:solution (-> +12), function_returning_something:solution (-> +12)
    ...
    while a[i] <= n: # comparison_operator:LtE, index, while (-> +4)
        a.append(a[i] + a[i + 1]) # binary_operator:Add, call_parameter:, index, index_arithmetic, int_literal, literal:Num, method_call:append, method_call_object:a
    ...
    sum = 0 # assignment, assignment_lhs_identifier:sum, int_literal, literal:Num
    for j in range(len(a) - 1): # accumulate_elements:1 (-> +2), binary_operator:Sub, call_parameter:, call_parameter:a, composition, for:j (-> +2), for_range_stop (-> +2), function_call:len, function_call:range, int_literal, literal:Num
...
def isprime(no): # function:isprime (-> +9), function_returning_something:isprime (-> +9)
    ...
    sq = int(math.sqrt(no)) + 1 # assignment, assignment_lhs_identifier:sq, assignment_rhs_identifier:int, assignment_rhs_identifier:math, assignment_rhs_identifier:no, binary_operator:Add, call_parameter:, call_parameter:no, composition, function_call:int, int_literal, literal:Num, method_call:sqrt
    for i in range(3, sq, 2): # call_parameter:2, call_parameter:3, call_parameter:sq, for:i (-> +2), for_range_step:2 (-> +2), function_call:range, int_literal, literal:Num, suggest_constant_definition, universal_quantifier (-> +3)
...
def solution(n): # function:solution (-> +24), function_returning_something:solution (-> +24)
    ...
    if isprime(n): # call_parameter:n, function_call:isprime, if (-> +16), if_test_id:isprime, if_test_id:n
        ...
        if isprime(n): # call_parameter:n, function_call:isprime, if (-> +11), if_test_id:isprime, if_test_id:n, nested_if:1 (-> +11)
            return int(n) # call_parameter:n, function_call:int, if_then_branch, return
        else:
            n1 = int(math.sqrt(n)) + 1 # assignment, assignment_lhs_identifier:n1, assignment_rhs_identifier:int, assignment_rhs_identifier:math, assignment_rhs_identifier:n, binary_operator:Add, call_parameter:, call_parameter:n, composition, function_call:int, if_else_branch (-> +8), int_literal, literal:Num, method_call:sqrt
            for i in range(3, n1, 2): # call_parameter:2, call_parameter:3, call_parameter:n1, for:i (-> +6), for_range_step:2 (-> +6), function_call:range, int_literal, literal:Num, suggest_constant_definition
                if n % i == 0: # binary_operator:Mod, comparison_operator:Eq, divisibility_test, if (-> +5), if_test_id:i, if_test_id:n, int_literal, literal:Num, nested_if:2 (-> +5)
                    if isprime(n / i): # binary_operator:Div, call_parameter:, function_call:isprime, if (-> +4), if_test_id:i, if_test_id:isprime, if_test_id:n, if_then_branch (-> +4), nested_if:3 (-> +4)
                        ...
                        break
                    elif isprime(i): # call_parameter:i, function_call:isprime, if (-> +1), if_test_id:i, if_test_id:isprime, nested_if:3 (-> +1)
...
def solution(n): # function:solution (-> +16), function_returning_something:solution (-> +16)
    ...
    return int(prime) # call_parameter:prime, function_call:int, return
...
def solution(n: int) -> int: # function:solution (-> +18), function_returning_something:solution (-> +18)
    ...
    while n > 2: # comparison_operator:Gt, evolve_state (-> +6), int_literal, literal:Num, while (-> +6)
        ...
        i += 1 # assignment_lhs_identifier:i, augmented_assignment, int_literal, literal:Num
    return int(ans) # call_parameter:ans, function_call:int, return

# ----------------------------------------------------------------------------------------
# ../Python/project_euler/problem_04/sol1.py
# ----------------------------------------------------------------------------------------
def solution(n): # function:solution (-> +8), function_returning_something:solution (-> +8)
    for number in range(n - 1, 10000, -1): # binary_operator:Sub, call_parameter:, call_parameter:-1, call_parameter:10000, find_first_element (-> +6), for:number (-> +7), for_range_step:-1 (-> +7), function_call:range, int_literal, literal:Num, suggest_constant_definition
        ...
        if strNumber == strNumber[::-1]: # comparison_operator:Eq, if (-> +5), if_test_id:strNumber, int_literal, literal:Num, slice_step
            ...
            while divisor != 99: # comparison_operator:NotEq, evolve_state (-> +3), int_literal, literal:Num, suggest_constant_definition, while (-> +3)
                if (number % divisor == 0) and (len(str(int(number / divisor))) == 3): # binary_operator:Div, binary_operator:Mod, boolean_operator:And, call_parameter:, comparison_operator:Eq, composition, divisibility_test, function_call:int, function_call:len, function_call:str, if (-> +1), if_test_id:divisor, if_test_id:int, if_test_id:len, if_test_id:number, if_test_id:str, int_literal, literal:Num, nested_if:1 (-> +1), suggest_constant_definition
...
def solution(n): # function:solution (-> +7), function_returning_something:solution (-> +7)
    ...
    for i in range(999, 99, -1): # accumulate_elements:1 (-> +4), call_parameter:-1, call_parameter:99, call_parameter:999, for:i (-> +4), for_range_step:-1 (-> +4), function_call:range, int_literal, literal:Num, square_nested_for (-> +4), suggest_constant_definition
        for j in range(999, 99, -1): # accumulate_elements:1 (-> +3), call_parameter:-1, call_parameter:99, call_parameter:999, for:j (-> +3), for_range_step:-1 (-> +3), function_call:range, int_literal, literal:Num, nested_for:1 (-> +3), suggest_constant_definition
            t = str(i * j) # assignment, assignment_lhs_identifier:t, assignment_rhs_identifier:i, assignment_rhs_identifier:j, assignment_rhs_identifier:str, binary_operator:Mult, call_parameter:, function_call:str
            if t == t[::-1] and i * j < n: # binary_operator:Mult, boolean_operator:And, comparison_operator:Eq, comparison_operator:Lt, if (-> +1), if_test_id:i, if_test_id:j, if_test_id:n, if_test_id:t, int_literal, literal:Num, slice_step
                answer = max(answer, i * j) # assignment, assignment_lhs_identifier:answer, assignment_rhs_identifier:answer, assignment_rhs_identifier:i, assignment_rhs_identifier:j, assignment_rhs_identifier:max, binary_operator:Mult, call_parameter:, call_parameter:answer, function_call:max, if_then_branch
...
def solution(n): # function:solution (-> +18), function_returning_something:solution (-> +18)
    ...
    while 1: # int_literal, literal:Num, while (-> +10)
        ...
        for j in range(2, n): # call_parameter:2, call_parameter:n, for:j (-> +3), for_range_start (-> +3), function_call:range, int_literal, literal:Num
...
def gcd(x, y): # function:gcd (-> +1), function_returning_something:gcd (-> +1), recursive_function:gcd (-> +1)
    return x if y == 0 else gcd(y, x % y) # binary_operator:Mod, call_parameter:, call_parameter:y, comparison_operator:Eq, conditional_expression, function_call:gcd, int_literal, literal:Num, return
def lcm(x, y): # function:lcm (-> +1), function_returning_something:lcm (-> +1)
    return (x * y) // gcd(x, y) # binary_operator:FloorDiv, binary_operator:Mult, call_parameter:x, call_parameter:y, function_call:gcd, return
def solution(n): # function:solution (-> +4), function_returning_something:solution (-> +4)
    ...
    for i in range(1, n + 1): # accumulate_elements:1 (-> +1), binary_operator:Add, call_parameter:, call_parameter:1, for:i (-> +1), for_range_start (-> +1), function_call:range, int_literal, literal:Num
...
def solution(n): # function:solution (-> +7), function_returning_something:solution (-> +7)
    ...
    for i in range(1, n + 1): # accumulate_elements:1 (-> +2), binary_operator:Add, call_parameter:, call_parameter:1, for:i (-> +2), for_range_start (-> +2), function_call:range, int_literal, literal:Num
...
def solution(n): # function:solution (-> +4), function_returning_something:solution (-> +4)
    ...
    return int(suma - sumb) # binary_operator:Sub, call_parameter:, function_call:int, return
...
def solution(n): # function:solution (-> +3), function_returning_something:solution (-> +3)
    ...
    return int(square_of_sum - sum_of_squares) # binary_operator:Sub, call_parameter:, function_call:int, return
...
def isprime(n): # function:isprime (-> +10), function_returning_something:isprime (-> +10)
    if n == 2: # comparison_operator:Eq, if (-> +8), if_test_id:n, int_literal, literal:Num
        ...
        sq = int(sqrt(n)) + 1 # assignment, assignment_lhs_identifier:sq, assignment_rhs_identifier:int, assignment_rhs_identifier:n, assignment_rhs_identifier:sqrt, binary_operator:Add, call_parameter:, call_parameter:n, composition, function_call:int, function_call:sqrt, if_else_branch (-> +3), int_literal, literal:Num
        for i in range(3, sq, 2): # call_parameter:2, call_parameter:3, call_parameter:sq, for:i (-> +2), for_range_step:2 (-> +2), function_call:range, int_literal, literal:Num, suggest_constant_definition
...
def solution(n): # function:solution (-> +11), function_returning_something:solution (-> +11)
    ...
    while i != n and j < 3: # boolean_operator:And, comparison_operator:Lt, comparison_operator:NotEq, evolve_state (-> +3), int_literal, literal:Num, suggest_constant_definition, while (-> +3)
        ...
        if isprime(j): # call_parameter:j, function_call:isprime, if (-> +1), if_test_id:isprime, if_test_id:j
...
def isprime(number): # function:isprime (-> +4), function_returning_something:isprime (-> +4)
    for i in range(2, int(number ** 0.5) + 1): # binary_operator:Add, binary_operator:Pow, call_parameter:, call_parameter:2, composition, float_literal, for:i (-> +2), for_range_start (-> +2), function_call:int, function_call:range, int_literal, literal:Num, suggest_constant_definition, universal_quantifier (-> +3)
...
def solution(n): # function:solution (-> +15), function_returning_something:solution (-> +15)
    ...
    while len(primes) < n: # call_parameter:primes, comparison_operator:Lt, function_call:len, while (-> +5)
        if isprime(num): # call_parameter:num, function_call:isprime, if (-> +4), if_test_id:isprime, if_test_id:num
    ...
    return primes[len(primes) - 1] # binary_operator:Sub, call_parameter:primes, function_call:len, index, index_arithmetic, int_literal, literal:Num, return
...
def primeCheck(number): # function:primeCheck (-> +3), function_returning_something:primeCheck (-> +3)
    if number % 2 == 0 and number > 2: # binary_operator:Mod, boolean_operator:And, comparison_operator:Eq, comparison_operator:Gt, divisibility_test:2, if (-> +1), if_test_id:number, int_literal, literal:Num
        return False # if_then_branch, literal:False, return:False
    return all(number % i for i in range(3, int(math.sqrt(number)) + 1, 2)) # binary_operator:Add, binary_operator:Mod, call_parameter:, call_parameter:2, call_parameter:3, call_parameter:number, composition, comprehension:Generator, comprehension_for_count:1, function_call:all, function_call:int, function_call:range, int_literal, literal:Num, method_call:sqrt, return, suggest_constant_definition
def prime_generator(): # function:prime_generator (-> +5), generator:prime_generator (-> +5)
    ...
    while True: # literal:True, while (-> +3)
        ...
        num += 1 # assignment_lhs_identifier:num, augmented_assignment, int_literal, literal:Num
def solution(n): # function:solution (-> +1), function_returning_something:solution (-> +1)
    return next(itertools.islice(prime_generator(), n - 1, n)) # binary_operator:Sub, call_parameter:, call_parameter:n, composition, function_call:next, function_call:prime_generator, int_literal, literal:Num, method_call:islice, return
...
def solution(n): # function:solution (-> +8), function_returning_something:solution (-> +8)
    ...
    for i in range(len(n) - 12): # accumulate_elements:1 (-> +5), binary_operator:Sub, call_parameter:, call_parameter:n, composition, for:i (-> +5), for_range_stop (-> +5), function_call:len, function_call:range, int_literal, literal:Num, suggest_constant_definition
        product = 1 # assignment, assignment_lhs_identifier:product, int_literal, literal:Num
        for j in range(13): # accumulate_elements:1 (-> +1), call_parameter:13, for:j (-> +1), for_range_stop (-> +1), function_call:range, int_literal, literal:Num, nested_for:1 (-> +1), suggest_constant_definition
            product *= int(n[i + j]) # assignment_lhs_identifier:product, assignment_rhs_identifier:i, assignment_rhs_identifier:int, assignment_rhs_identifier:j, assignment_rhs_identifier:n, augmented_assignment, binary_operator:Add, call_parameter:, function_call:int, index, index_arithmetic
...
def solution(n): # function:solution (-> +4), function_returning_something:solution (-> +4)
    return max( # composition, function_call:max, return
        [
            reduce(lambda x, y: int(x) * int(y), n[i : i + 13]) # binary_operator:Add, binary_operator:Mult, call_parameter:, call_parameter:x, call_parameter:y, composition, comprehension:List, comprehension_for_count:1, function_call:int, function_call:reduce, int_literal, lambda_function, literal:Num, slice, suggest_constant_definition
            for i in range(len(n) - 12) # binary_operator:Sub, call_parameter:, call_parameter:n, composition, function_call:len, function_call:range, int_literal, literal:Num, suggest_constant_definition
        ]
    )
...
def solution(n: str) -> int: # function:solution (-> +12), function_returning_something:solution (-> +12)
    ...
    while cur_index < len(n) - 13: # binary_operator:Sub, call_parameter:n, comparison_operator:Lt, function_call:len, int_literal, literal:Num, suggest_constant_definition, while (-> +7)
        if int(n[cur_index]) >= int(substr[0]): # call_parameter:, comparison_operator:GtE, function_call:int, if (-> +6), if_test_id:cur_index, if_test_id:int, if_test_id:n, if_test_id:substr, index, int_literal, literal:Num
            ...
            LargestProduct = max(LargestProduct, streval(substr)) # assignment, assignment_lhs_identifier:LargestProduct, assignment_rhs_identifier:LargestProduct, assignment_rhs_identifier:max, assignment_rhs_identifier:streval, assignment_rhs_identifier:substr, call_parameter:, call_parameter:LargestProduct, call_parameter:substr, composition, function_call:max, function_call:streval, if_else_branch (-> +2)
...
def solution(): # function:solution (-> +7), function_returning_something:solution (-> +7)
    for a in range(300): # call_parameter:300, for:a (-> +6), for_range_stop (-> +6), function_call:range, int_literal, literal:Num, suggest_constant_definition
        for b in range(400): # call_parameter:400, for:b (-> +5), for_range_stop (-> +5), function_call:range, int_literal, literal:Num, nested_for:1 (-> +5), suggest_constant_definition
            for c in range(500): # call_parameter:500, for:c (-> +4), for_range_stop (-> +4), function_call:range, int_literal, literal:Num, nested_for:2 (-> +4), suggest_constant_definition
...
def solution(n): # function:solution (-> +10), function_returning_something:solution (-> +10)
    ...
    for a in range(1, n // 3): # binary_operator:FloorDiv, call_parameter:, call_parameter:1, for:a (-> +6), for_range_start (-> +6), function_call:range, int_literal, literal:Num, suggest_constant_definition
...
def solution(): # function:solution (-> +7), function_returning_something:solution (-> +7)
    return [ # return
        ...
        for a in range(1, 999) # call_parameter:1, call_parameter:999, function_call:range, int_literal, literal:Num, suggest_constant_definition
        for b in range(a, 999) # call_parameter:999, call_parameter:a, function_call:range, int_literal, literal:Num, suggest_constant_definition
        for c in range(b, 999) # call_parameter:999, call_parameter:b, function_call:range, int_literal, literal:Num, suggest_constant_definition
...
def is_prime(n): # function:is_prime (-> +4), function_returning_something:is_prime (-> +4)
    for i in range(2, int(sqrt(n)) + 1): # binary_operator:Add, call_parameter:, call_parameter:2, call_parameter:n, composition, for:i (-> +2), for_range_start (-> +2), function_call:int, function_call:range, function_call:sqrt, int_literal, literal:Num, universal_quantifier (-> +3)
...
def sum_of_primes(n): # function:sum_of_primes (-> +8), function_returning_something:sum_of_primes (-> +8)
    ...
    for i in range(3, n, 2): # accumulate_elements:1 (-> +2), call_parameter:2, call_parameter:3, call_parameter:n, for:i (-> +2), for_range_step:2 (-> +2), function_call:range, int_literal, literal:Num, suggest_constant_definition
        if is_prime(i): # call_parameter:i, function_call:is_prime, if (-> +1), if_test_id:i, if_test_id:is_prime
...
def solution(n): # function:solution (-> +1), function_returning_something:solution (-> +1)
    return sum_of_primes(n) # call_parameter:n, function_call:sum_of_primes, return
...
def prime_generator(): # function:prime_generator (-> +5), generator:prime_generator (-> +5)
    ...
    while True: # literal:True, while (-> +3)
        ...
        num += 1 # assignment_lhs_identifier:num, augmented_assignment, int_literal, literal:Num
def solution(n): # function:solution (-> +1), function_returning_something:solution (-> +1)
    return sum(takewhile(lambda x: x < n, prime_generator())) # call_parameter:, comparison_operator:Lt, composition, function_call:prime_generator, function_call:sum, function_call:takewhile, lambda_function, return
...
def prime_sum(n: int) -> int: # function:prime_sum (-> +12), function_returning_something:prime_sum (-> +12)
    list_ = [0 for i in range(n + 1)] # assignment, assignment_lhs_identifier:list_, assignment_rhs_identifier:i, assignment_rhs_identifier:n, assignment_rhs_identifier:range, binary_operator:Add, call_parameter:, comprehension:List, comprehension_for_count:1, function_call:range, int_literal, literal:Num
    ...
    for i in range(2, int(n ** 0.5) + 1): # binary_operator:Add, binary_operator:Pow, call_parameter:, call_parameter:2, composition, float_literal, for:i (-> +3), for_range_start (-> +3), function_call:int, function_call:range, int_literal, literal:Num, suggest_constant_definition
        if list_[i] == 0: # comparison_operator:Eq, if (-> +2), if_test_id:i, if_test_id:list_, index, int_literal, literal:Num
            for j in range(i * i, n + 1, i): # binary_operator:Add, binary_operator:Mult, call_parameter:, call_parameter:i, for:j (-> +1), for_range_step (-> +1), function_call:range, if_then_branch (-> +1), int_literal, literal:Num, nested_for:1 (-> +1)
...
def largest_product(grid): # function:largest_product (-> +27), function_returning_something:largest_product (-> +27)
    nColumns = len(grid[0]) # assignment, assignment_lhs_identifier:nColumns, assignment_rhs_identifier:grid, assignment_rhs_identifier:len, call_parameter:, function_call:len, index, int_literal, literal:Num
    ...
    for i in range(nColumns): # call_parameter:nColumns, for:i (-> +20), for_range_stop (-> +20), function_call:range
        for j in range(nRows - 3): # binary_operator:Sub, call_parameter:, for:j (-> +19), for_range_stop (-> +19), function_call:range, int_literal, literal:Num, nested_for:1 (-> +19), suggest_constant_definition
...
def solution(): # function:solution (-> +6), function_returning_something:solution (-> +6)
    ...
    with open(os.path.dirname(__file__) + "/grid.txt") as file: # binary_operator:Add, call_parameter:, call_parameter:__file__, composition, function_call:open, literal:Str, method_call:dirname
        for line in file: # for:line (-> +1), for_each (-> +1)
            grid.append(line.strip("\n").split(" ")) # call_parameter:, composition, literal:Str, method_call:append, method_call:split, method_call:strip, method_call_object:grid, method_call_object:line, method_chaining
    grid = [[int(i) for i in grid[j]] for j in range(len(grid))] # assignment, assignment_lhs_identifier:grid, assignment_rhs_identifier:grid, assignment_rhs_identifier:i, assignment_rhs_identifier:int, assignment_rhs_identifier:j, assignment_rhs_identifier:len, assignment_rhs_identifier:range, call_parameter:, call_parameter:grid, call_parameter:i, composition, comprehension:List, comprehension_for_count:1, function_call:int, function_call:len, function_call:range, index
    return largest_product(grid) # call_parameter:grid, function_call:largest_product, return
...
def solution(): # function:solution (-> +26), function_returning_something:solution (-> +26)
    with open(os.path.dirname(__file__) + "/grid.txt") as f: # binary_operator:Add, call_parameter:, call_parameter:__file__, composition, function_call:open, literal:Str, method_call:dirname
        l = [] # assignment, assignment_lhs_identifier:l, literal:List
        for i in range(20): # call_parameter:20, for:i (-> +1), for_range_stop (-> +1), function_call:range, int_literal, literal:Num, suggest_constant_definition
            l.append([int(x) for x in f.readline().split()]) # call_parameter:, call_parameter:x, composition, comprehension:List, comprehension_for_count:1, function_call:int, method_call:append, method_call:readline, method_call:split, method_call_object:f, method_call_object:l, method_chaining
        maximum = 0 # assignment, assignment_lhs_identifier:maximum, int_literal, literal:Num
        for i in range(20): # call_parameter:20, for:i (-> +4), for_range_stop (-> +4), function_call:range, int_literal, literal:Num, suggest_constant_definition
            for j in range(17): # call_parameter:17, for:j (-> +3), for_range_stop (-> +3), function_call:range, int_literal, literal:Num, nested_for:1 (-> +3), suggest_constant_definition
        ...
        for i in range(17): # call_parameter:17, for:i (-> +4), for_range_stop (-> +4), function_call:range, int_literal, literal:Num, suggest_constant_definition
            for j in range(20): # call_parameter:20, for:j (-> +3), for_range_stop (-> +3), function_call:range, int_literal, literal:Num, nested_for:1 (-> +3), suggest_constant_definition
                ...
                if temp > maximum: # comparison_operator:Gt, if (-> +1), if_test_id:maximum, if_test_id:temp
                    maximum = temp # assignment, assignment_lhs_identifier:maximum, assignment_rhs_identifier:temp, if_then_branch
        for i in range(17): # call_parameter:17, for:i (-> +4), for_range_stop (-> +4), function_call:range, int_literal, literal:Num, square_nested_for (-> +4), suggest_constant_definition
            for j in range(17): # call_parameter:17, for:j (-> +3), for_range_stop (-> +3), function_call:range, int_literal, literal:Num, nested_for:1 (-> +3), suggest_constant_definition
                ...
                if temp > maximum: # comparison_operator:Gt, if (-> +1), if_test_id:maximum, if_test_id:temp
                    maximum = temp # assignment, assignment_lhs_identifier:maximum, assignment_rhs_identifier:temp, if_then_branch
        for i in range(17): # call_parameter:17, for:i (-> +4), for_range_stop (-> +4), function_call:range, int_literal, literal:Num, suggest_constant_definition
            for j in range(3, 20): # call_parameter:20, call_parameter:3, for:j (-> +3), for_range_start (-> +3), function_call:range, int_literal, literal:Num, nested_for:1 (-> +3), suggest_constant_definition
...
def count_divisors(n): # function:count_divisors (-> +7), function_returning_something:count_divisors (-> +7)
    ...
    for i in range(1, int(sqrt(n)) + 1): # binary_operator:Add, call_parameter:, call_parameter:1, call_parameter:n, composition, for:i (-> +2), for_range_start (-> +2), function_call:int, function_call:range, function_call:sqrt, int_literal, literal:Num
        if n % i == 0: # binary_operator:Mod, comparison_operator:Eq, divisibility_test, if (-> +1), if_test_id:i, if_test_id:n, int_literal, literal:Num
            nDivisors += 2 # assignment_lhs_identifier:nDivisors, augmented_assignment, if_then_branch, int_literal, literal:Num
    if n ** 0.5 == int(n ** 0.5): # binary_operator:Pow, call_parameter:, comparison_operator:Eq, float_literal, function_call:int, if (-> +1), if_test_id:int, if_test_id:n, literal:Num, suggest_constant_definition
...
def solution(): # function:solution (-> +8), function_returning_something:solution (-> +8)
    ...
    while True: # literal:True, while (-> +4)
        ...
        if count_divisors(tNum) > 500: # call_parameter:tNum, comparison_operator:Gt, function_call:count_divisors, if (-> +1), if_test_id:count_divisors, if_test_id:tNum, int_literal, literal:Num, suggest_constant_definition
            break # if_then_branch
...
def triangle_number_generator(): # function:triangle_number_generator (-> +2), generator:triangle_number_generator (-> +2)
    for n in range(1, 1000000): # call_parameter:1, call_parameter:1000000, for:n (-> +1), for_range_start (-> +1), function_call:range, int_literal, literal:Num, suggest_constant_definition
...
def lattice_paths(n): # function:lattice_paths (-> +3), function_returning_something:lattice_paths (-> +3)
    ...
    return int(factorial(n) / (factorial(k) * factorial(n - k))) # binary_operator:Div, binary_operator:Mult, binary_operator:Sub, call_parameter:, call_parameter:k, call_parameter:n, composition, function_call:factorial, function_call:int, return
...
def solution(n): # function:solution (-> +17), function_returning_something:solution (-> +17)
    ...
    count = 0 # assignment, assignment_lhs_identifier:count, int_literal, literal:Num
    for i in range(1, n + 1): # accumulate_elements:1 (-> +12), binary_operator:Add, call_parameter:, call_parameter:1, for:i (-> +12), for_range_start (-> +12), function_call:range, int_literal, literal:Num
...
def solution(): # function:solution (-> +17), function_returning_something:solution (-> +17)
    script_dir = os.path.dirname(os.path.realpath(__file__)) # assignment, assignment_lhs_identifier:script_dir, assignment_rhs_identifier:__file__, assignment_rhs_identifier:os, call_parameter:, call_parameter:__file__, composition, method_call:dirname, method_call:realpath
    triangle = os.path.join(script_dir, "triangle.txt") # assignment, assignment_lhs_identifier:triangle, assignment_rhs_identifier:os, assignment_rhs_identifier:script_dir, call_parameter:, call_parameter:script_dir, literal:Str, method_call:join
    with open(triangle, "r") as f: # call_parameter:, call_parameter:triangle, function_call:open, literal:Str
        triangle = f.readlines() # assignment, assignment_lhs_identifier:triangle, assignment_rhs_identifier:f, method_call:readlines
    a = [[int(y) for y in x.rstrip("\r\n").split(" ")] for x in triangle] # assignment, assignment_lhs_identifier:a, assignment_rhs_identifier:int, assignment_rhs_identifier:triangle, assignment_rhs_identifier:x, assignment_rhs_identifier:y, call_parameter:, call_parameter:y, comprehension:List, comprehension_for_count:1, function_call:int, literal:Str, method_call:rstrip, method_call:split, method_call_object:x, method_chaining
    for i in range(1, len(a)): # call_parameter:, call_parameter:1, call_parameter:a, composition, for:i (-> +10), for_range_start (-> +10), function_call:len, function_call:range, int_literal, literal:Num
        for j in range(len(a[i])): # call_parameter:, composition, for:j (-> +9), for_indexes (-> +9), for_range_stop (-> +9), function_call:len, function_call:range, index, nested_for:1 (-> +9)
            if j != len(a[i - 1]): # binary_operator:Sub, call_parameter:, comparison_operator:NotEq, function_call:len, if (-> +3), if_test_id:a, if_test_id:i, if_test_id:j, if_test_id:len, index, index_arithmetic, int_literal, literal:Num, suggest_conditional_expression (-> +3)
    ...
    return max(a[-1]) # call_parameter:, function_call:max, index, int_literal, literal:Num, negative_index:-1, return
...
from math import factorial # import:math:factorial, import_module:math, import_name:factorial
def solution(n): # function:solution (-> +1), function_returning_something:solution (-> +1)
    return sum([int(x) for x in str(factorial(n))]) # call_parameter:, call_parameter:n, call_parameter:x, composition, comprehension:List, comprehension_for_count:1, function_call:factorial, function_call:int, function_call:str, function_call:sum, return
...
from math import factorial # import:math:factorial, import_module:math, import_name:factorial
def solution(n): # function:solution (-> +1), function_returning_something:solution (-> +1)
    return sum(map(int, str(factorial(n)))) # call_parameter:, call_parameter:int, call_parameter:n, composition, function_call:factorial, function_call:map, function_call:str, function_call:sum, return
...
def sum_of_divisors(n): # function:sum_of_divisors (-> +7), function_returning_something:sum_of_divisors (-> +7)
    ...
    for i in range(1, int(sqrt(n) + 1)): # accumulate_elements:1 (-> +4), binary_operator:Add, call_parameter:, call_parameter:1, call_parameter:n, composition, for:i (-> +4), for_range_start (-> +4), function_call:int, function_call:range, function_call:sqrt, int_literal, literal:Num
        if n % i == 0 and i != sqrt(n): # binary_operator:Mod, boolean_operator:And, call_parameter:n, comparison_operator:Eq, comparison_operator:NotEq, divisibility_test, function_call:sqrt, if (-> +3), if_test_id:i, if_test_id:n, if_test_id:sqrt, int_literal, literal:Num
        ...
        elif i == sqrt(n): # call_parameter:n, comparison_operator:Eq, function_call:sqrt, if (-> +1), if_test_id:i, if_test_id:n, if_test_id:sqrt
...
def solution(n): # function:solution (-> +8), function_returning_something:solution (-> +8)
    total = sum( # assignment, assignment_lhs_identifier:total, assignment_rhs_identifier:sum, composition, function_call:sum
        [
            i # assignment_rhs_identifier:i, call_parameter:, comprehension:List, comprehension_for_count:1
            for i in range(1, n) # assignment_rhs_identifier:i, assignment_rhs_identifier:n, assignment_rhs_identifier:range, call_parameter:1, call_parameter:n, function_call:range, int_literal, literal:Num
            if sum_of_divisors(sum_of_divisors(i)) == i and sum_of_divisors(i) != i # assignment_rhs_identifier:i, assignment_rhs_identifier:sum_of_divisors, boolean_operator:And, call_parameter:, call_parameter:i, comparison_operator:Eq, comparison_operator:NotEq, composition, filtered_comprehension, function_call:sum_of_divisors
        ]
    )
...
def solution(): # function:solution (-> +12), function_returning_something:solution (-> +12)
    with open(os.path.dirname(__file__) + "/p022_names.txt") as file: # binary_operator:Add, call_parameter:, call_parameter:__file__, composition, function_call:open, literal:Str, method_call:dirname
        names = str(file.readlines()[0]) # assignment, assignment_lhs_identifier:names, assignment_rhs_identifier:file, assignment_rhs_identifier:str, call_parameter:, composition, function_call:str, index, int_literal, literal:Num, method_call:readlines, method_call_object:file
        names = names.replace('"', "").split(",") # assignment, assignment_lhs_identifier:names, assignment_rhs_identifier:names, call_parameter:, literal:Str, method_call:replace, method_call:split, method_call_object:names, method_chaining
...
def solution(): # function:solution (-> +12), function_returning_something:solution (-> +12)
    ...
    with open(os.path.dirname(__file__) + "/p022_names.txt") as file: # binary_operator:Add, call_parameter:, call_parameter:__file__, composition, function_call:open, literal:Str, method_call:dirname
        name = str(file.readlines()[0]) # assignment, assignment_lhs_identifier:name, assignment_rhs_identifier:file, assignment_rhs_identifier:str, call_parameter:, composition, function_call:str, index, int_literal, literal:Num, method_call:readlines, method_call_object:file
        name = name.replace('"', "").split(",") # assignment, assignment_lhs_identifier:name, assignment_rhs_identifier:name, call_parameter:, literal:Str, method_call:replace, method_call:split, method_call_object:name, method_chaining
    name.sort() # method_call:sort, method_call_object:name
    for i in range(len(name)): # accumulate_elements:1 (-> +4), call_parameter:, call_parameter:name, composition, for:i (-> +4), for_indexes (-> +4), for_range_stop (-> +4), function_call:len, function_call:range
        for j in name[i]: # accumulate_elements:1 (-> +1), for:j (-> +1), index, nested_for:1 (-> +1)
            temp_sum += ord(j) - ord("A") + 1 # assignment_lhs_identifier:temp_sum, assignment_rhs_identifier:j, assignment_rhs_identifier:ord, augmented_assignment, binary_operator:Add, binary_operator:Sub, call_parameter:, call_parameter:j, function_call:ord, int_literal, literal:Num, literal:Str
...
def solution(limit=28123): # function:solution (-> +13), function_returning_something:solution (-> +13), function_with_default_positional_arguments:solution (-> +13), int_literal, literal:Num
    ...
    for i in range(2, int(limit ** 0.5) + 1): # accumulate_elements:1 (-> +3), binary_operator:Add, binary_operator:Pow, call_parameter:, call_parameter:2, composition, float_literal, for:i (-> +3), for_range_start (-> +3), function_call:int, function_call:range, int_literal, literal:Num, suggest_constant_definition
        ...
        for k in range(i + 1, limit // i + 1): # accumulate_elements:1 (-> +1), binary_operator:Add, binary_operator:FloorDiv, call_parameter:, for:k (-> +1), for_range_start (-> +1), function_call:range, int_literal, literal:Num, nested_for:1 (-> +1)
    ...
    for n in range(1, limit + 1): # accumulate_elements:2 (-> +4), binary_operator:Add, call_parameter:, call_parameter:1, for:n (-> +4), for_range_start (-> +4), function_call:range, int_literal, literal:Num
        ...
        if not any((n - a in abundants) for a in abundants): # binary_operator:Sub, call_parameter:, comparison_operator:In, comprehension:Generator, comprehension_for_count:1, function_call:any, if (-> +1), if_test_id:a, if_test_id:abundants, if_test_id:any, if_test_id:n, unary_operator:Not
...
def fib(a, b, n): # function:fib (-> +13), function_returning_something:fib (-> +13)
    if n == 1: # comparison_operator:Eq, if (-> +5), if_test_id:n, int_literal, literal:Num
        ...
        return str(a) + str(b) # binary_operator:Add, call_parameter:a, call_parameter:b, function_call:str, if_elif_branch, return
    temp = 0 # assignment, assignment_lhs_identifier:temp, int_literal, literal:Num
    for x in range(2, n): # call_parameter:2, call_parameter:n, for:x (-> +4), for_range_start (-> +4), function_call:range, int_literal, literal:Num
...
def solution(n): # function:solution (-> +11), function_returning_something:solution (-> +11)
    ...
    for x in range(n): # call_parameter:n, for:x (-> +8), for_range_stop (-> +8), function_call:range
        ...
        while 1: # int_literal, literal:Num, while (-> +4)
            if len(fib(l[0], l[1], c2)) < int(l[2]): # call_parameter:, call_parameter:c2, comparison_operator:Lt, composition, function_call:fib, function_call:int, function_call:len, if (-> +3), if_test_id:c2, if_test_id:fib, if_test_id:int, if_test_id:l, if_test_id:len, index, int_literal, literal:Num
                c2 += 1 # assignment_lhs_identifier:c2, augmented_assignment, if_then_branch, int_literal, literal:Num
            else:
                break # if_else_branch
        semidivisible.append(fib(l[0], l[1], c2 + 1)[int(l[2]) - 1]) # binary_operator:Add, binary_operator:Sub, call_parameter:, composition, function_call:fib, function_call:int, index, index_arithmetic, int_literal, literal:Num, method_call:append, method_call_object:semidivisible
...
def solution(): # function:solution (-> +2), function_returning_something:solution (-> +2)
    result = list(map("".join, permutations("0123456789"))) # assignment, assignment_lhs_identifier:result, assignment_rhs_identifier:list, assignment_rhs_identifier:map, assignment_rhs_identifier:permutations, call_parameter:, composition, function_call:list, function_call:map, function_call:permutations, literal:Str
...
def fibonacci(n): # function:fibonacci (-> +9), function_returning_something:fibonacci (-> +9)
    if n == 1 or type(n) is not int: # boolean_operator:Or, call_parameter:n, comparison_operator:Eq, comparison_operator:IsNot, function_call:type, if (-> +8), if_test_id:int, if_test_id:n, if_test_id:type, int_literal, literal:Num
        return 0 # if_then_branch, int_literal, literal:Num, return:0
        ...
        for i in range(2, n + 1): # binary_operator:Add, call_parameter:, call_parameter:2, for:i (-> +1), for_range_start (-> +1), function_call:range, int_literal, literal:Num
            sequence.append(sequence[i - 1] + sequence[i - 2]) # binary_operator:Add, binary_operator:Sub, call_parameter:, index, index_arithmetic, int_literal, literal:Num, method_call:append, method_call_object:sequence
...
def fibonacci_digits_index(n): # function:fibonacci_digits_index (-> +6), function_returning_something:fibonacci_digits_index (-> +6)
    ...
    while digits < n: # comparison_operator:Lt, while (-> +2)
        ...
        digits = len(str(fibonacci(index))) # assignment, assignment_lhs_identifier:digits, assignment_rhs_identifier:fibonacci, assignment_rhs_identifier:index, assignment_rhs_identifier:len, assignment_rhs_identifier:str, call_parameter:, call_parameter:index, composition, function_call:fibonacci, function_call:len, function_call:str
    return index # return:index
def solution(n): # function:solution (-> +1), function_returning_something:solution (-> +1)
    return fibonacci_digits_index(n) # call_parameter:n, function_call:fibonacci_digits_index, return
...
def solution(n): # function:solution (-> +5), function_returning_something:solution (-> +5)
    ...
    while len(str(next(gen))) < n: # call_parameter:, call_parameter:gen, comparison_operator:Lt, composition, function_call:len, function_call:next, function_call:str, while (-> +1)
...
def is_prime(k: int) -> bool: # function:is_prime (-> +9), function_returning_something:is_prime (-> +9)
    if k < 2 or k % 2 == 0: # binary_operator:Mod, boolean_operator:Or, comparison_operator:Eq, comparison_operator:Lt, divisibility_test:2, if (-> +7), if_test_id:k, int_literal, literal:Num
        ...
        for x in range(3, int(math.sqrt(k) + 1), 2): # binary_operator:Add, call_parameter:, call_parameter:2, call_parameter:3, call_parameter:k, composition, for:x (-> +2), for_range_step:2 (-> +2), function_call:int, function_call:range, if_else_branch (-> +2), int_literal, literal:Num, method_call:sqrt, suggest_constant_definition
...
def solution(a_limit: int, b_limit: int) -> int: # function:solution (-> +13), function_returning_something:solution (-> +13)
    ...
    for a in range((a_limit * -1) + 1, a_limit): # binary_operator:Add, binary_operator:Mult, call_parameter:, call_parameter:a_limit, for:a (-> +9), for_range_start (-> +9), function_call:range, int_literal, literal:Num
        for b in range(2, b_limit): # call_parameter:2, call_parameter:b_limit, for:b (-> +8), for_range_start (-> +8), function_call:range, int_literal, literal:Num, nested_for:1 (-> +8)
            if is_prime(b): # call_parameter:b, function_call:is_prime, if (-> +7), if_test_id:b, if_test_id:is_prime
                ...
                while is_prime((n ** 2) + (a * n) + b): # binary_operator:Add, binary_operator:Mult, binary_operator:Pow, call_parameter:, function_call:is_prime, int_literal, literal:Num, while (-> +2)
...
def diagonal_sum(n): # function:diagonal_sum (-> +6), function_returning_something:diagonal_sum (-> +6)
    ...
    for i in range(1, int(ceil(n / 2.0))): # binary_operator:Div, call_parameter:, call_parameter:1, composition, float_literal, for:i (-> +3), for_range_start (-> +3), function_call:ceil, function_call:int, function_call:range, int_literal, literal:Num, suggest_constant_definition
...
def solution(n): # function:solution (-> +8), function_returning_something:solution (-> +8)
    ...
    for a in range(2, N): # call_parameter:2, call_parameter:N, for:a (-> +3), for_range_start (-> +3), function_call:range, int_literal, literal:Num, square_nested_for (-> +3)
        for b in range(2, N): # call_parameter:2, call_parameter:N, for:b (-> +2), for_range_start (-> +2), function_call:range, int_literal, literal:Num, nested_for:1 (-> +2)
    ...
    return len(collectPowers) # call_parameter:collectPowers, function_call:len, return
...
def two_pence(x): # body_recursive_function:two_pence (-> +1), function:two_pence (-> +1), function_returning_something:two_pence (-> +1), recursive_function:two_pence (-> +1)
    return 0 if x < 0 else two_pence(x - 2) + one_pence() # binary_operator:Add, binary_operator:Sub, call_parameter:, comparison_operator:Lt, conditional_expression, function_call:one_pence, function_call:two_pence, int_literal, literal:Num, return
def five_pence(x): # body_recursive_function:five_pence (-> +1), function:five_pence (-> +1), function_returning_something:five_pence (-> +1), recursive_function:five_pence (-> +1)
    return 0 if x < 0 else five_pence(x - 5) + two_pence(x) # binary_operator:Add, binary_operator:Sub, call_parameter:, call_parameter:x, comparison_operator:Lt, conditional_expression, function_call:five_pence, function_call:two_pence, int_literal, literal:Num, return, suggest_constant_definition
def ten_pence(x): # body_recursive_function:ten_pence (-> +1), function:ten_pence (-> +1), function_returning_something:ten_pence (-> +1), recursive_function:ten_pence (-> +1)
    return 0 if x < 0 else ten_pence(x - 10) + five_pence(x) # binary_operator:Add, binary_operator:Sub, call_parameter:, call_parameter:x, comparison_operator:Lt, conditional_expression, function_call:five_pence, function_call:ten_pence, int_literal, literal:Num, return, suggest_constant_definition
def twenty_pence(x): # body_recursive_function:twenty_pence (-> +1), function:twenty_pence (-> +1), function_returning_something:twenty_pence (-> +1), recursive_function:twenty_pence (-> +1)
    return 0 if x < 0 else twenty_pence(x - 20) + ten_pence(x) # binary_operator:Add, binary_operator:Sub, call_parameter:, call_parameter:x, comparison_operator:Lt, conditional_expression, function_call:ten_pence, function_call:twenty_pence, int_literal, literal:Num, return, suggest_constant_definition
def fifty_pence(x): # body_recursive_function:fifty_pence (-> +1), function:fifty_pence (-> +1), function_returning_something:fifty_pence (-> +1), recursive_function:fifty_pence (-> +1)
    return 0 if x < 0 else fifty_pence(x - 50) + twenty_pence(x) # binary_operator:Add, binary_operator:Sub, call_parameter:, call_parameter:x, comparison_operator:Lt, conditional_expression, function_call:fifty_pence, function_call:twenty_pence, int_literal, literal:Num, return, suggest_constant_definition
def one_pound(x): # body_recursive_function:one_pound (-> +1), function:one_pound (-> +1), function_returning_something:one_pound (-> +1), recursive_function:one_pound (-> +1)
    return 0 if x < 0 else one_pound(x - 100) + fifty_pence(x) # binary_operator:Add, binary_operator:Sub, call_parameter:, call_parameter:x, comparison_operator:Lt, conditional_expression, function_call:fifty_pence, function_call:one_pound, int_literal, literal:Num, return, suggest_constant_definition
def two_pound(x): # body_recursive_function:two_pound (-> +1), function:two_pound (-> +1), function_returning_something:two_pound (-> +1), recursive_function:two_pound (-> +1)
    return 0 if x < 0 else two_pound(x - 200) + one_pound(x) # binary_operator:Add, binary_operator:Sub, call_parameter:, call_parameter:x, comparison_operator:Lt, conditional_expression, function_call:one_pound, function_call:two_pound, int_literal, literal:Num, return, suggest_constant_definition
def solution(n): # function:solution (-> +1), function_returning_something:solution (-> +1)
    return two_pound(n) # call_parameter:n, function_call:two_pound, return
...
def isCombinationValid(combination): # function:isCombinationValid (-> +6), function_returning_something:isCombinationValid (-> +6)
    return ( # boolean_operator:Or, return
        int("".join(combination[0:2])) * int("".join(combination[2:5])) # binary_operator:Mult, call_parameter:, composition, function_call:int, int_literal, literal:Num, literal:Str, method_call:join, slice, suggest_constant_definition
        == int("".join(combination[5:9])) # call_parameter:, comparison_operator:Eq, composition, function_call:int, int_literal, literal:Num, literal:Str, method_call:join, slice, suggest_constant_definition
    ) or (
        int("".join(combination[0])) * int("".join(combination[1:5])) # binary_operator:Mult, call_parameter:, composition, function_call:int, index, int_literal, literal:Num, literal:Str, method_call:join, slice, suggest_constant_definition
        == int("".join(combination[5:9])) # call_parameter:, comparison_operator:Eq, composition, function_call:int, int_literal, literal:Num, literal:Str, method_call:join, slice, suggest_constant_definition
    )
def solution(): # function:solution (-> +6), function_returning_something:solution (-> +6)
    return sum( # composition, function_call:sum, return
        set( # call_parameter:, composition, function_call:set
            [
                int("".join(pandigital[5:9])) # call_parameter:, composition, comprehension:List, comprehension_for_count:1, function_call:int, int_literal, literal:Num, literal:Str, method_call:join, slice, suggest_constant_definition
                for pandigital in itertools.permutations("123456789") # call_parameter:, literal:Str, method_call:permutations
...
def solve(digit_len: int) -> str: # function:solve (-> +13), function_returning_something:solve (-> +13)
    ...
    last_digit = int("1" + "0" * digit_len) # assignment, assignment_lhs_identifier:last_digit, assignment_rhs_identifier:digit_len, assignment_rhs_identifier:int, binary_operator:Add, binary_operator:Mult, call_parameter:, function_call:int, literal:Str
    for num in range(den, last_digit): # accumulate_elements:1 (-> +7), call_parameter:den, call_parameter:last_digit, for:num (-> +7), for_range_start (-> +7), function_call:range
        while den <= 99: # comparison_operator:LtE, evolve_state (-> +4), int_literal, literal:Num, suggest_constant_definition, while (-> +4)
            if (num != den) and (num % 10 == den // 10) and (den % 10 != 0): # binary_operator:FloorDiv, binary_operator:Mod, boolean_operator:And, comparison_operator:Eq, comparison_operator:NotEq, divisibility_test:10, if (-> +2), if_test_id:den, if_test_id:num, int_literal, literal:Num, suggest_constant_definition
                if isDigitCancelling(num, den): # call_parameter:den, call_parameter:num, function_call:isDigitCancelling, if (-> +1), if_test_id:den, if_test_id:isDigitCancelling, if_test_id:num, if_then_branch (-> +1), nested_if:1 (-> +1)
                    solutions.append("{}/{}".format(num, den)) # call_parameter:, call_parameter:den, call_parameter:num, composition, if_then_branch, literal:Str, method_call:append, method_call:format, method_call_object:solutions
...
def solution(n): # function:solution (-> +5), function_returning_something:solution (-> +5)
    total = 0 # assignment, assignment_lhs_identifier:total, int_literal, literal:Num
    for i in range(1, n): # accumulate_elements:1 (-> +2), call_parameter:1, call_parameter:n, for:i (-> +2), for_range_start (-> +2), function_call:range, int_literal, literal:Num
        if is_palindrome(i) and is_palindrome(bin(i).split("b")[1]): # boolean_operator:And, call_parameter:, call_parameter:i, composition, function_call:bin, function_call:is_palindrome, if (-> +1), if_test_id:bin, if_test_id:i, if_test_id:is_palindrome, index, int_literal, literal:Num, literal:Str, method_call:split
...
def solution(): # function:solution (-> +14), function_returning_something:solution (-> +14)
    ...
    while len(constant) < 1e6: # call_parameter:constant, comparison_operator:Lt, evolve_state (-> +2), float_literal, function_call:len, literal:Num, suggest_constant_definition, while (-> +2)
        constant.append(str(i)) # call_parameter:, call_parameter:i, composition, function_call:str, method_call:append, method_call_object:constant
        i += 1 # assignment_lhs_identifier:i, augmented_assignment, int_literal, literal:Num
    ...
    return ( # return
        int(constant[0]) # binary_operator:Mult, call_parameter:, function_call:int, index, int_literal, literal:Num
        * int(constant[9]) # call_parameter:, function_call:int, index, int_literal, literal:Num, suggest_constant_definition
        * int(constant[99]) # binary_operator:Mult, call_parameter:, function_call:int, index, int_literal, literal:Num, suggest_constant_definition
        * int(constant[999]) # binary_operator:Mult, call_parameter:, function_call:int, index, int_literal, literal:Num, suggest_constant_definition
        * int(constant[9999]) # binary_operator:Mult, call_parameter:, function_call:int, index, int_literal, literal:Num, suggest_constant_definition
        * int(constant[99999]) # binary_operator:Mult, call_parameter:, function_call:int, index, int_literal, literal:Num, suggest_constant_definition
        * int(constant[999999]) # binary_operator:Mult, call_parameter:, function_call:int, index, int_literal, literal:Num, suggest_constant_definition
    )
...
def solution(): # function:solution (-> +4), function_returning_something:solution (-> +4)
    total = 0 # assignment, assignment_lhs_identifier:total, int_literal, literal:Num
    for i in range(1, 1001): # accumulate_elements:1 (-> +1), call_parameter:1, call_parameter:1001, for:i (-> +1), for_range_start (-> +1), function_call:range, int_literal, literal:Num, suggest_constant_definition
        total += i ** i # assignment_lhs_identifier:total, assignment_rhs_identifier:i, augmented_assignment, binary_operator:Pow
    return str(total)[-10:] # call_parameter:total, function_call:str, int_literal, literal:Num, return, slice, suggest_constant_definition
...
def solution(): # function:solution (-> +12), function_returning_something:solution (-> +12)
    ...
    while True: # literal:True, while (-> +10)
        if ( # if (-> +8), if_test_id:i, if_test_id:list, if_test_id:sorted, if_test_id:str
            sorted(list(str(i))) # call_parameter:, call_parameter:i, chained_comparison:5, chained_equalities:5, composition, function_call:list, function_call:sorted, function_call:str
            == sorted(list(str(2 * i))) # binary_operator:Mult, call_parameter:, comparison_operator:Eq, composition, function_call:list, function_call:sorted, function_call:str, int_literal, literal:Num
            == sorted(list(str(3 * i))) # binary_operator:Mult, call_parameter:, comparison_operator:Eq, composition, function_call:list, function_call:sorted, function_call:str, int_literal, literal:Num, suggest_constant_definition
            == sorted(list(str(4 * i))) # binary_operator:Mult, call_parameter:, comparison_operator:Eq, composition, function_call:list, function_call:sorted, function_call:str, int_literal, literal:Num, suggest_constant_definition
            == sorted(list(str(5 * i))) # binary_operator:Mult, call_parameter:, comparison_operator:Eq, composition, function_call:list, function_call:sorted, function_call:str, int_literal, literal:Num, suggest_constant_definition
            == sorted(list(str(6 * i))) # binary_operator:Mult, call_parameter:, comparison_operator:Eq, composition, function_call:list, function_call:sorted, function_call:str, int_literal, literal:Num, suggest_constant_definition
        ):
...
def combinations(n, r): # function:combinations (-> +1), function_returning_something:combinations (-> +1)
    return factorial(n) / (factorial(r) * factorial(n - r)) # binary_operator:Div, binary_operator:Mult, binary_operator:Sub, call_parameter:, call_parameter:n, call_parameter:r, function_call:factorial, return
def solution(): # function:solution (-> +6), function_returning_something:solution (-> +6)
    total = 0 # assignment, assignment_lhs_identifier:total, int_literal, literal:Num
    for i in range(1, 101): # call_parameter:1, call_parameter:101, for:i (-> +3), for_range_start (-> +3), function_call:range, int_literal, literal:Num, suggest_constant_definition
        for j in range(1, i + 1): # binary_operator:Add, call_parameter:, call_parameter:1, for:j (-> +2), for_range_start (-> +2), function_call:range, int_literal, literal:Num, nested_for:1 (-> +2)
            if combinations(i, j) > 1e6: # call_parameter:i, call_parameter:j, comparison_operator:Gt, float_literal, function_call:combinations, if (-> +1), if_test_id:combinations, if_test_id:i, if_test_id:j, literal:Num, suggest_constant_definition
...
# ----------------------------------------------------------------------------------------
ks = [k for k in range(2, 20 + 1)] # assignment, assignment_lhs_identifier:ks, assignment_rhs_identifier:k, assignment_rhs_identifier:range, binary_operator:Add, call_parameter:, call_parameter:2, comprehension:List, comprehension_for_count:1, function_call:range, int_literal, literal:Num
base = [10 ** k for k in range(ks[-1] + 1)] # assignment, assignment_lhs_identifier:base, assignment_rhs_identifier:k, assignment_rhs_identifier:ks, assignment_rhs_identifier:range, binary_operator:Add, binary_operator:Pow, call_parameter:, comprehension:List, comprehension_for_count:1, function_call:range, index, int_literal, literal:Num, negative_index:-1
...
def next_term(a_i, k, i, n): # function:next_term (-> +50), function_returning_something:next_term (-> +50), recursive_function:next_term (-> +50)
    ...
    for j in range(k, len(a_i)): # accumulate_elements:1 (-> +1), call_parameter:, call_parameter:a_i, call_parameter:k, composition, for:j (-> +1), for_range_start (-> +1), function_call:len, function_call:range
    ...
    for j in range(min(len(a_i), k)): # accumulate_elements:1 (-> +1), call_parameter:, call_parameter:a_i, call_parameter:k, composition, for:j (-> +1), for_range_stop (-> +1), function_call:len, function_call:min, function_call:range
    ...
    if sub_memo != None: # comparison_operator:NotEq, if (-> +19), if_test_id:sub_memo, literal:None
        ...
        if jumps != None and len(jumps) > 0: # boolean_operator:And, call_parameter:jumps, comparison_operator:Gt, comparison_operator:NotEq, function_call:len, if (-> +14), if_test_id:jumps, if_test_id:len, int_literal, literal:None, literal:Num, nested_if:1 (-> +14)
            max_jump = -1 # assignment, assignment_lhs_identifier:max_jump, if_then_branch (-> +11), int_literal, literal:Num
            for _k in range(len(jumps) - 1, -1, -1): # binary_operator:Sub, call_parameter:, call_parameter:-1, call_parameter:jumps, composition, for:_k (-> +3), for_range_step:-1 (-> +3), function_call:len, function_call:range, int_literal, literal:Num
            ...
            if max_jump >= 0: # comparison_operator:GtE, if (-> +6), if_test_id:max_jump, int_literal, literal:Num, nested_if:2 (-> +6)
                ...
                for j in range(min(k, len(a_i))): # call_parameter:, call_parameter:a_i, call_parameter:k, composition, for:j (-> +1), for_range_stop (-> +1), function_call:len, function_call:min, function_call:range
                    new_c, a_i[j] = divmod(new_c, 10) # assignment, assignment_lhs_identifier:a_i, assignment_lhs_identifier:new_c, assignment_rhs_identifier:divmod, assignment_rhs_identifier:new_c, call_parameter:10, call_parameter:new_c, function_call:divmod, index, int_literal, literal:Num, suggest_constant_definition
    ...
    if k > ks[0]: # comparison_operator:Gt, if (-> +10), if_test_id:k, if_test_id:ks, index, int_literal, literal:Num
        while True: # if_then_branch (-> +5), literal:True, while (-> +5)
            _diff, terms_jumped = next_term(a_i, k - 1, i + dn, n) # assignment, assignment_lhs_identifier:_diff, assignment_lhs_identifier:terms_jumped, assignment_rhs_identifier:a_i, assignment_rhs_identifier:dn, assignment_rhs_identifier:i, assignment_rhs_identifier:k, assignment_rhs_identifier:n, assignment_rhs_identifier:next_term, binary_operator:Add, binary_operator:Sub, call_parameter:, call_parameter:a_i, call_parameter:n, function_call:next_term, int_literal, literal:Num
    ...
    while j < len(jumps): # call_parameter:jumps, comparison_operator:Lt, function_call:len, while (-> +3)
        if jumps[j][1] > dn: # comparison_operator:Gt, if (-> +1), if_test_id:dn, if_test_id:j, if_test_id:jumps, index, int_literal, literal:Num
            break # if_then_branch
        j += 1 # assignment_lhs_identifier:j, augmented_assignment, int_literal, literal:Num
    sub_memo[c].insert(j, (diff, dn, k)) # call_parameter:, call_parameter:j, index, method_call:insert
...
def compute(a_i, k, i, n): # function:compute (-> +25), function_returning_something:compute (-> +25)
    ...
    if k > len(a_i): # call_parameter:a_i, comparison_operator:Gt, function_call:len, if (-> +1), if_test_id:a_i, if_test_id:k, if_test_id:len
        a_i.extend([0 for _ in range(k - len(a_i))]) # binary_operator:Sub, call_parameter:, call_parameter:a_i, composition, comprehension:List, comprehension_for_count:1, function_call:len, function_call:range, if_then_branch, int_literal, literal:Num, method_call:extend, method_call_object:a_i
    ...
    for j in range(len(a_i)): # accumulate_elements:1 (-> +4), call_parameter:, call_parameter:a_i, composition, for:j (-> +4), for_indexes (-> +4), for_range_stop (-> +4), function_call:len, function_call:range
    ...
    while i < n: # comparison_operator:Lt, while (-> +10)
        ...
        for j in range(k): # accumulate_elements:1 (-> +3), call_parameter:k, for:j (-> +3), for_range_stop (-> +3), function_call:range
            ...
            addend, a_i[j] = divmod(s, 10) # assignment, assignment_lhs_identifier:a_i, assignment_lhs_identifier:addend, assignment_rhs_identifier:divmod, assignment_rhs_identifier:s, call_parameter:10, call_parameter:s, function_call:divmod, index, int_literal, literal:Num, suggest_constant_definition
...
def add(digits, k, addend): # function:add (-> +13), function_returning_nothing:add (-> +13)
    for j in range(k, len(digits)): # call_parameter:, call_parameter:digits, call_parameter:k, composition, for:j (-> +9), for_range_start (-> +9), function_call:len, function_call:range
        ...
        if s >= 10: # comparison_operator:GtE, if (-> +5), if_test_id:s, int_literal, literal:Num, suggest_constant_definition
            quotient, digits[j] = divmod(s, 10) # assignment, assignment_lhs_identifier:digits, assignment_lhs_identifier:quotient, assignment_rhs_identifier:divmod, assignment_rhs_identifier:s, call_parameter:10, call_parameter:s, function_call:divmod, if_then_branch (-> +1), index, int_literal, literal:Num, suggest_constant_definition
    ...
    while addend > 0: # comparison_operator:Gt, int_literal, literal:Num, while (-> +2)
        addend, digit = divmod(addend, 10) # assignment, assignment_lhs_identifier:addend, assignment_lhs_identifier:digit, assignment_rhs_identifier:addend, assignment_rhs_identifier:divmod, call_parameter:10, call_parameter:addend, function_call:divmod, int_literal, literal:Num, suggest_constant_definition
...
def solution(n): # function:solution (-> +12), function_returning_something:solution (-> +12)
    ...
    for j in range(len(digits)): # accumulate_elements:1 (-> +1), call_parameter:, call_parameter:digits, composition, for:j (-> +1), for_indexes (-> +1), for_range_stop (-> +1), function_call:len, function_call:range
...
def maximum_digital_sum(a: int, b: int) -> int: # function:maximum_digital_sum (-> +5), function_returning_something:maximum_digital_sum (-> +5)
    return max( # composition, function_call:max, return
        [
            sum([int(x) for x in str(base ** power)]) # binary_operator:Pow, call_parameter:, call_parameter:x, composition, comprehension:List, comprehension_for_count:1, comprehension_for_count:2, function_call:int, function_call:str, function_call:sum
...
def partition(m): # function:partition (-> +9), function_returning_something:partition (-> +9)
    memo = [[0 for _ in range(m)] for _ in range(m + 1)] # assignment, assignment_lhs_identifier:memo, assignment_rhs_identifier:_, assignment_rhs_identifier:m, assignment_rhs_identifier:range, binary_operator:Add, call_parameter:, call_parameter:m, comprehension:List, comprehension_for_count:1, function_call:range, int_literal, literal:Num
    for i in range(m + 1): # binary_operator:Add, call_parameter:, for:i (-> +1), for_range_stop (-> +1), function_call:range, int_literal, literal:Num
        memo[i][0] = 1 # assignment, index, int_literal, literal:Num
    for n in range(m + 1): # accumulate_elements:1 (-> +4), binary_operator:Add, call_parameter:, for:n (-> +4), for_range_stop (-> +4), function_call:range, int_literal, literal:Num
        for k in range(1, m): # accumulate_elements:1 (-> +3), call_parameter:1, call_parameter:m, for:k (-> +3), for_range_start (-> +3), function_call:range, int_literal, literal:Num, nested_for:1 (-> +3)
...
def find_largest(data_file: str = "base_exp.txt") -> int: # function:find_largest (-> +6), function_returning_something:find_largest (-> +6), function_with_default_positional_arguments:find_largest (-> +6), literal:Str
    ...
    for i, line in enumerate(open(os.path.join(os.path.dirname(__file__), data_file))): # call_parameter:, call_parameter:__file__, call_parameter:data_file, composition, for:i:line (-> +3), for_indexes_elements (-> +3), function_call:enumerate, function_call:open, method_call:dirname, method_call:join
        a, x = list(map(int, line.split(","))) # assignment, assignment_lhs_identifier:a, assignment_lhs_identifier:x, assignment_rhs_identifier:int, assignment_rhs_identifier:line, assignment_rhs_identifier:list, assignment_rhs_identifier:map, call_parameter:, call_parameter:int, composition, function_call:list, function_call:map, literal:Str, method_call:split
        if x * log10(a) > largest[0]: # binary_operator:Mult, call_parameter:a, comparison_operator:Gt, function_call:log10, if (-> +1), if_test_id:a, if_test_id:largest, if_test_id:log10, if_test_id:x, index, int_literal, literal:Num

...
def solution(n): # function:solution (-> +8)
    sum = 0 # assignment, int_literal, literal:Num
    terms = (n - 1) // 3 # assignment, binary_operator:FloorDiv, binary_operator:Sub, int_literal, literal:Num, suggest_constant_definition
    ...
    terms = (n - 1) // 5 # assignment, binary_operator:FloorDiv, binary_operator:Sub, int_literal, literal:Num, suggest_constant_definition
    ...
    terms = (n - 1) // 15 # assignment, binary_operator:FloorDiv, binary_operator:Sub, int_literal, literal:Num, suggest_constant_definition
...
def solution(n): # function:solution (-> +32)
    sum = 0 # assignment, int_literal, literal:Num
    num = 0 # assignment, int_literal, literal:Num
...
def solution(n): # function:solution (-> +22)
    ...
    z = 3 # assignment, int_literal, literal:Num, suggest_constant_definition
    x = 5 # assignment, int_literal, literal:Num, suggest_constant_definition
    temp = 1 # assignment, int_literal, literal:Num
...
def solution(n): # function:solution (-> +9)
    a = 3 # assignment, int_literal, literal:Num, suggest_constant_definition
    result = 0 # assignment, int_literal, literal:Num
    while a < n: # comparison_operator:Lt
        if a % 3 == 0 or a % 5 == 0: # binary_operator:Mod, boolean_operator:Or, comparison_operator:Eq, divisibility_test:3, divisibility_test:5, if (-> +3), if_elif (-> +3), int_literal, literal:Num, suggest_constant_definition
            result += a # augmented_assignment
        elif a % 15 == 0: # binary_operator:Mod, comparison_operator:Eq, divisibility_test:15, int_literal, literal:Num, suggest_constant_definition
            result -= a # augmented_assignment
...
def solution(n): # function:solution (-> +7)
    ...
    for i in range(n): # accumulate_elements:AugAssign (-> +4), for_range_stop (-> +4), function_call:range
        if i % 3 == 0: # binary_operator:Mod, comparison_operator:Eq, divisibility_test:3, if (-> +3), if_elif (-> +3), int_literal, literal:Num, suggest_constant_definition
            result += i # augmented_assignment
        elif i % 5 == 0: # binary_operator:Mod, comparison_operator:Eq, divisibility_test:5, int_literal, literal:Num, suggest_constant_definition
            result += i # augmented_assignment
    return result
...
# ----------------------------------------------------------------------------------------
def solution(n): # function:solution (-> +8)
    i = 1 # assignment, int_literal, literal:Num
    j = 2 # assignment, int_literal, literal:Num
    sum = 0 # assignment, int_literal, literal:Num
...
def solution(n): # function:solution (-> +7)
    ...
    a, b = 0, 1 # assignment, int_literal, literal:Num, literal:Tuple
...
def solution(n): # function:solution (-> +9)
    ...
    a = 0 # assignment, int_literal, literal:Num
    b = 2 # assignment, int_literal, literal:Num
    count = 0 # assignment, int_literal, literal:Num
    while 4 * b + a <= n: # binary_operator:Add, binary_operator:Mult, comparison_operator:LtE, int_literal, literal:Num, suggest_constant_definition
        a, b = b, 4 * b + a # assignment, binary_operator:Add, binary_operator:Mult, int_literal, literal:Num, suggest_constant_definition
...
def solution(n): # function:solution (-> +12)
    ...
    phi = (Decimal(5) ** Decimal(0.5) + 1) / Decimal(2) # assignment, binary_operator:Add, binary_operator:Div, binary_operator:Pow, float_literal, function_call:Decimal, int_literal, literal:Num, suggest_constant_definition
    index = (math.floor(math.log(n * (phi + 2), phi) - 1) // 3) * 3 + 2 # assignment, binary_operator:Add, binary_operator:FloorDiv, binary_operator:Mult, binary_operator:Sub, composition, int_literal, literal:Num, method_call:floor, method_call:log, suggest_constant_definition
    num = Decimal(round(phi ** Decimal(index + 1))) / (phi + 2) # assignment, binary_operator:Add, binary_operator:Div, binary_operator:Pow, composition, function_call:Decimal, function_call:round, int_literal, literal:Num
    sum = num // 2 # assignment, binary_operator:FloorDiv, int_literal, literal:Num
...
# ----------------------------------------------------------------------------------------
def solution(n): # function:solution (-> +12)
    a = [0, 1] # assignment, int_literal, literal:List, literal:Num
    i = 0 # assignment, int_literal, literal:Num
    ...
    sum = 0 # assignment, int_literal, literal:Num
    for j in range(len(a) - 1): # accumulate_elements:AugAssign (-> +2), binary_operator:Sub, composition, for_range_stop (-> +2), function_call:len, function_call:range, int_literal, literal:Num
...
def isprime(no): # function:isprime (-> +9)
    if no == 2: # comparison_operator:Eq, if (-> +3), if_elif (-> +3), int_literal, literal:Num
        return True # literal:True
    elif no % 2 == 0: # binary_operator:Mod, comparison_operator:Eq, divisibility_test:2, int_literal, literal:Num
        return False # literal:False
    sq = int(math.sqrt(no)) + 1 # assignment, binary_operator:Add, composition, function_call:int, int_literal, literal:Num, method_call:sqrt
    for i in range(3, sq, 2): # for_range_step:2 (-> +2), function_call:range, int_literal, literal:Num, suggest_constant_definition, universal_quantifier (-> +3)
...
def solution(n): # function:solution (-> +24)
    ...
    if n <= 0: # comparison_operator:LtE, if (-> +1), int_literal, literal:Num
        raise ValueError("Parameter n must be greater or equal to one.") # function_call:ValueError, literal:Str, raise_exception:ValueError
    maxNumber = 0 # assignment, int_literal, literal:Num
    if isprime(n): # function_call:isprime, if (-> +16), if_else (-> +16), nested_ifs (-> +16)
        return n
    else:
        while n % 2 == 0: # binary_operator:Mod, comparison_operator:Eq, divisibility_test:2, evolve_state (-> +1), int_literal, literal:Num
            n = n / 2 # assignment, binary_operator:Div, int_literal, literal:Num, suggest_augmented_assignment
        if isprime(n): # function_call:isprime, if_else (-> +11)
            return int(n) # function_call:int
        else:
            n1 = int(math.sqrt(n)) + 1 # assignment, binary_operator:Add, composition, function_call:int, int_literal, literal:Num, method_call:sqrt
            for i in range(3, n1, 2): # for_range_step:2 (-> +6), function_call:range, int_literal, literal:Num, suggest_constant_definition
                if n % i == 0: # binary_operator:Mod, comparison_operator:Eq, divisibility_test, if (-> +5), int_literal, literal:Num, nested_ifs (-> +5)
                    if isprime(n / i): # binary_operator:Div, function_call:isprime, if (-> +4), if_elif (-> +4)
                        maxNumber = n / i # assignment, binary_operator:Div
                        break
                    elif isprime(i): # function_call:isprime
...
def solution(n): # function:solution (-> +16)
    ...
    if n <= 0: # comparison_operator:LtE, if (-> +1), int_literal, literal:Num
        raise ValueError("Parameter n must be greater or equal to one.") # function_call:ValueError, literal:Str, raise_exception:ValueError
    prime = 1 # assignment, int_literal, literal:Num
    i = 2 # assignment, int_literal, literal:Num
...
# ----------------------------------------------------------------------------------------
def solution(n): # function:solution (-> +8)
    for number in range(n - 1, 10000, -1): # binary_operator:Sub, find_first_element (-> +6), for_range_step:-1 (-> +7), function_call:range, int_literal, literal:Num, suggest_constant_definition
        strNumber = str(number) # assignment, function_call:str
        if strNumber == strNumber[::-1]: # comparison_operator:Eq, if (-> +5), int_literal, literal:Num, nested_ifs (-> +5), slice_step
            divisor = 999 # assignment, int_literal, literal:Num, suggest_constant_definition
            while divisor != 99: # comparison_operator:NotEq, evolve_state (-> +3), int_literal, literal:Num, suggest_constant_definition
                if (number % divisor == 0) and (len(str(int(number / divisor))) == 3): # binary_operator:Div, binary_operator:Mod, boolean_operator:And, comparison_operator:Eq, composition, divisibility_test, function_call:int, function_call:len, function_call:str, if (-> +1), int_literal, literal:Num, suggest_constant_definition
                    return number
...
# ----------------------------------------------------------------------------------------
def solution(n): # function:solution (-> +7)
    answer = 0 # assignment, int_literal, literal:Num
    for i in range(999, 99, -1): # accumulate_elements:Assign (-> +4), for_range_step:-1 (-> +4), function_call:range, int_literal, literal:Num, nested_for (-> +4), square_nested_for (-> +4), suggest_constant_definition
        for j in range(999, 99, -1): # accumulate_elements:Assign (-> +3), for_range_step:-1 (-> +3), function_call:range, int_literal, literal:Num, suggest_constant_definition
...
def solution(n): # function:solution (-> +18)
    ...
    while 1: # int_literal, literal:Num
        ...
        nfound = 0 # assignment, int_literal, literal:Num
        for j in range(2, n): # for_range_start (-> +3), function_call:range, int_literal, literal:Num
            if i % j != 0: # binary_operator:Mod, comparison_operator:NotEq, divisibility_test, if (-> +2), int_literal, literal:Num
                nfound = 1 # assignment, int_literal, literal:Num
                break
        if nfound == 0: # comparison_operator:Eq, if (-> +3), int_literal, literal:Num, nested_ifs (-> +3)
            if i == 0: # comparison_operator:Eq, if (-> +1), int_literal, literal:Num
                i = 1 # assignment, int_literal, literal:Num
            return i
...
def solution(n): # function:solution (-> +4)
    g = 1 # assignment, int_literal, literal:Num
    for i in range(1, n + 1): # accumulate_elements:Assign (-> +1), binary_operator:Add, for_range_start (-> +1), function_call:range, int_literal, literal:Num
...
# ----------------------------------------------------------------------------------------
def solution(n): # function:solution (-> +7)
    suma = 0 # assignment, int_literal, literal:Num
    sumb = 0 # assignment, int_literal, literal:Num
    for i in range(1, n + 1): # accumulate_elements:AugAssign (-> +2), binary_operator:Add, for_range_start (-> +2), function_call:range, int_literal, literal:Num
        suma += i ** 2 # augmented_assignment, binary_operator:Pow, int_literal, literal:Num
        sumb += i # augmented_assignment
    sum = sumb ** 2 - suma # assignment, binary_operator:Pow, binary_operator:Sub, int_literal, literal:Num
    return sum
...
# ----------------------------------------------------------------------------------------
def solution(n): # function:solution (-> +4)
    suma = n * (n + 1) / 2 # assignment, binary_operator:Add, binary_operator:Div, binary_operator:Mult, int_literal, literal:Num
    suma **= 2 # augmented_assignment, int_literal, literal:Num
    sumb = n * (n + 1) * (2 * n + 1) / 6 # assignment, binary_operator:Add, binary_operator:Div, binary_operator:Mult, int_literal, literal:Num, suggest_constant_definition
...
def solution(n): # function:solution (-> +3)
    sum_of_squares = sum([i * i for i in range(1, n + 1)]) # assignment, binary_operator:Add, binary_operator:Mult, composition, comprehension:List, comprehension_for_count:1, function_call:range, function_call:sum, int_literal, literal:Num
    square_of_sum = int(math.pow(sum(range(1, n + 1)), 2)) # assignment, binary_operator:Add, composition, function_call:int, function_call:range, function_call:sum, int_literal, literal:Num, method_call:pow
...
# ----------------------------------------------------------------------------------------
def solution(n): # function:solution (-> +3)
    sum_of_squares = n * (n + 1) * (2 * n + 1) / 6 # assignment, binary_operator:Add, binary_operator:Div, binary_operator:Mult, int_literal, literal:Num, suggest_constant_definition
    square_of_sum = (n * (n + 1) / 2) ** 2 # assignment, binary_operator:Add, binary_operator:Div, binary_operator:Mult, binary_operator:Pow, int_literal, literal:Num
...
def isprime(n): # function:isprime (-> +10)
    if n == 2: # comparison_operator:Eq, if (-> +8), if_elif (-> +8), int_literal, literal:Num, nested_ifs (-> +8)
        return True # literal:True
    elif n % 2 == 0: # binary_operator:Mod, comparison_operator:Eq, divisibility_test:2, if_else (-> +6), int_literal, literal:Num
        return False # literal:False
    else:
        sq = int(sqrt(n)) + 1 # assignment, binary_operator:Add, composition, function_call:int, function_call:sqrt, int_literal, literal:Num
        for i in range(3, sq, 2): # for_range_step:2 (-> +2), function_call:range, int_literal, literal:Num, suggest_constant_definition
...
def solution(n): # function:solution (-> +11)
    i = 0 # assignment, int_literal, literal:Num
    j = 1 # assignment, int_literal, literal:Num
...
def isprime(number): # function:isprime (-> +4)
    for i in range(2, int(number ** 0.5) + 1): # binary_operator:Add, binary_operator:Pow, composition, float_literal, for_range_start (-> +2), function_call:int, function_call:range, int_literal, literal:Num, suggest_constant_definition, universal_quantifier (-> +3)
...
def solution(n): # function:solution (-> +15)
    ...
    num = 2 # assignment, int_literal, literal:Num
...
def solution(n): # function:solution (-> +8)
    LargestProduct = -sys.maxsize - 1 # assignment, binary_operator:Sub, int_literal, literal:Num, unary_operator:USub
    for i in range(len(n) - 12): # accumulate_elements:AugAssign (-> +5), binary_operator:Sub, composition, for_range_stop (-> +5), function_call:len, function_call:range, int_literal, literal:Num, nested_for (-> +5), suggest_constant_definition
        product = 1 # assignment, int_literal, literal:Num
        for j in range(13): # accumulate_elements:AugAssign (-> +1), for_range_stop (-> +1), function_call:range, int_literal, literal:Num, suggest_constant_definition
...
def streval(s: str) -> int: # function:streval
    ret = 1 # assignment, int_literal, literal:Num
...
def solution(n: str) -> int: # function:solution
    LargestProduct = -sys.maxsize - 1 # assignment, binary_operator:Sub, int_literal, literal:Num, unary_operator:USub
    substr = n[:13] # assignment, int_literal, literal:Num, slice, suggest_constant_definition
    cur_index = 13 # assignment, int_literal, literal:Num, suggest_constant_definition
    while cur_index < len(n) - 13: # binary_operator:Sub, comparison_operator:Lt, function_call:len, int_literal, literal:Num, suggest_constant_definition
        if int(n[cur_index]) >= int(substr[0]): # comparison_operator:GtE, function_call:int, if (-> +6), if_else (-> +6), index, int_literal, literal:Num
            substr = substr[1:] + n[cur_index] # assignment, binary_operator:Add, index, int_literal, literal:Num, slice
            ...
            substr = n[cur_index : cur_index + 13] # assignment, binary_operator:Add, int_literal, literal:Num, slice, suggest_constant_definition
...
def solution(): # function:solution (-> +7)
    for a in range(300): # for_range_stop (-> +6), function_call:range, int_literal, literal:Num, nested_for (-> +6), suggest_constant_definition
        for b in range(400): # for_range_stop (-> +5), function_call:range, int_literal, literal:Num, nested_for (-> +5), suggest_constant_definition
            for c in range(500): # for_range_stop (-> +4), function_call:range, int_literal, literal:Num, suggest_constant_definition
                if a < b < c: # chained_comparison:2, comparison_operator:Lt, if (-> +3), nested_ifs (-> +3)
                    if (a ** 2) + (b ** 2) == (c ** 2): # binary_operator:Add, binary_operator:Pow, comparison_operator:Eq, if (-> +2), int_literal, literal:Num, nested_ifs (-> +2)
                        if (a + b + c) == 1000: # binary_operator:Add, comparison_operator:Eq, if (-> +1), int_literal, literal:Num, suggest_constant_definition
...
def solution(n): # function:solution (-> +10)
    product = -1 # assignment, int_literal, literal:Num
    d = 0 # assignment, int_literal, literal:Num
    for a in range(1, n // 3): # binary_operator:FloorDiv, for_range_start (-> +6), function_call:range, int_literal, literal:Num, suggest_constant_definition
        b = (n * n - 2 * a * n) // (2 * n - 2 * a) # assignment, binary_operator:FloorDiv, binary_operator:Mult, binary_operator:Sub, int_literal, literal:Num
        c = n - a - b # assignment, binary_operator:Sub
        if c * c == (a * a + b * b): # binary_operator:Add, binary_operator:Mult, comparison_operator:Eq, if (-> +3), nested_ifs (-> +3)
...
def is_prime(n): # function:is_prime (-> +4)
    for i in range(2, int(sqrt(n)) + 1): # binary_operator:Add, composition, for_range_start (-> +2), function_call:int, function_call:range, function_call:sqrt, int_literal, literal:Num, universal_quantifier (-> +3)
        if n % i == 0: # binary_operator:Mod, comparison_operator:Eq, divisibility_test, if (-> +1), int_literal, literal:Num
            return False # literal:False
    return True # literal:True
def sum_of_primes(n): # function:sum_of_primes (-> +8)
    if n > 2: # comparison_operator:Gt, if (-> +3), if_else (-> +3), int_literal, literal:Num
        sumOfPrimes = 2 # assignment, int_literal, literal:Num
    else:
        return 0 # int_literal, literal:Num
    for i in range(3, n, 2): # accumulate_elements:AugAssign (-> +2), for_range_step:2 (-> +2), function_call:range, int_literal, literal:Num, suggest_constant_definition
...
def prime_sum(n: int) -> int: # function:prime_sum
    list_ = [0 for i in range(n + 1)] # assignment, binary_operator:Add, comprehension:List, comprehension_for_count:1, function_call:range, int_literal, literal:Num
    list_[0] = 1 # assignment, index, int_literal, literal:Num
    list_[1] = 1 # assignment, index, int_literal, literal:Num
    for i in range(2, int(n ** 0.5) + 1): # binary_operator:Add, binary_operator:Pow, composition, float_literal, for_range_start (-> +3), function_call:int, function_call:range, int_literal, literal:Num, suggest_constant_definition
        if list_[i] == 0: # comparison_operator:Eq, if (-> +2), index, int_literal, literal:Num
            for j in range(i * i, n + 1, i): # binary_operator:Add, binary_operator:Mult, for_range_step (-> +1), function_call:range, int_literal, literal:Num
                list_[j] = 1 # assignment, index, int_literal, literal:Num
    s = 0 # assignment, int_literal, literal:Num
...
def largest_product(grid): # function:largest_product (-> +27)
    nColumns = len(grid[0]) # assignment, function_call:len, index, int_literal, literal:Num
    nRows = len(grid) # assignment, function_call:len
    largest = 0 # assignment, int_literal, literal:Num
    lrDiagProduct = 0 # assignment, int_literal, literal:Num
    rlDiagProduct = 0 # assignment, int_literal, literal:Num
    for i in range(nColumns): # for_range_stop (-> +20), function_call:range, nested_for (-> +20)
        for j in range(nRows - 3): # binary_operator:Sub, for_range_stop (-> +19), function_call:range, int_literal, literal:Num, suggest_constant_definition
            vertProduct = grid[j][i] * grid[j + 1][i] * grid[j + 2][i] * grid[j + 3][i] # assignment, binary_operator:Add, binary_operator:Mult, index, index_arithmetic, int_literal, literal:Num, suggest_constant_definition
            horzProduct = grid[i][j] * grid[i][j + 1] * grid[i][j + 2] * grid[i][j + 3] # assignment, binary_operator:Add, binary_operator:Mult, index, index_arithmetic, int_literal, literal:Num, suggest_constant_definition
...
def solution(): # function:solution (-> +26)
    with open(os.path.dirname(__file__) + "/grid.txt") as f: # binary_operator:Add, composition, function_call:open, literal:Str, method_call:dirname
        ...
        for i in range(20): # for_range_stop (-> +1), function_call:range, int_literal, literal:Num, suggest_constant_definition
        ...
        maximum = 0 # assignment, int_literal, literal:Num
        for i in range(20): # for_range_stop (-> +4), function_call:range, int_literal, literal:Num, nested_for (-> +4), suggest_constant_definition
            for j in range(17): # for_range_stop (-> +3), function_call:range, int_literal, literal:Num, suggest_constant_definition
                temp = l[i][j] * l[i][j + 1] * l[i][j + 2] * l[i][j + 3] # assignment, binary_operator:Add, binary_operator:Mult, index, index_arithmetic, int_literal, literal:Num, suggest_constant_definition
                if temp > maximum: # comparison_operator:Gt, if (-> +1)
                    maximum = temp # assignment
        for i in range(17): # for_range_stop (-> +4), function_call:range, int_literal, literal:Num, nested_for (-> +4), suggest_constant_definition
            for j in range(20): # for_range_stop (-> +3), function_call:range, int_literal, literal:Num, suggest_constant_definition
                temp = l[i][j] * l[i + 1][j] * l[i + 2][j] * l[i + 3][j] # assignment, binary_operator:Add, binary_operator:Mult, index, index_arithmetic, int_literal, literal:Num, suggest_constant_definition
                if temp > maximum: # comparison_operator:Gt, if (-> +1)
                    maximum = temp # assignment
        for i in range(17): # for_range_stop (-> +4), function_call:range, int_literal, literal:Num, nested_for (-> +4), square_nested_for (-> +4), suggest_constant_definition
            for j in range(17): # for_range_stop (-> +3), function_call:range, int_literal, literal:Num, suggest_constant_definition
                temp = l[i][j] * l[i + 1][j + 1] * l[i + 2][j + 2] * l[i + 3][j + 3] # assignment, binary_operator:Add, binary_operator:Mult, index, index_arithmetic, int_literal, literal:Num, suggest_constant_definition
                if temp > maximum: # comparison_operator:Gt, if (-> +1)
                    maximum = temp # assignment
        for i in range(17): # for_range_stop (-> +4), function_call:range, int_literal, literal:Num, nested_for (-> +4), suggest_constant_definition
            for j in range(3, 20): # for_range_start (-> +3), function_call:range, int_literal, literal:Num, suggest_constant_definition
                temp = l[i][j] * l[i + 1][j - 1] * l[i + 2][j - 2] * l[i + 3][j - 3] # assignment, binary_operator:Add, binary_operator:Mult, binary_operator:Sub, index, index_arithmetic, int_literal, literal:Num, suggest_constant_definition
                if temp > maximum: # comparison_operator:Gt, if (-> +1)
                    maximum = temp # assignment
        return maximum
...
def count_divisors(n): # function:count_divisors (-> +7)
    nDivisors = 0 # assignment, int_literal, literal:Num
    for i in range(1, int(sqrt(n)) + 1): # binary_operator:Add, composition, for_range_start (-> +2), function_call:int, function_call:range, function_call:sqrt, int_literal, literal:Num
        if n % i == 0: # binary_operator:Mod, comparison_operator:Eq, divisibility_test, if (-> +1), int_literal, literal:Num
            nDivisors += 2 # augmented_assignment, int_literal, literal:Num
    if n ** 0.5 == int(n ** 0.5): # binary_operator:Pow, comparison_operator:Eq, float_literal, function_call:int, if (-> +1), literal:Num, suggest_constant_definition
        nDivisors -= 1 # augmented_assignment, int_literal, literal:Num
    return nDivisors
def solution(): # function:solution (-> +8)
    tNum = 1 # assignment, int_literal, literal:Num
    i = 1 # assignment, int_literal, literal:Num
    while True: # literal:True
        i += 1 # augmented_assignment, int_literal, literal:Num
...
def triangle_number_generator(): # function:triangle_number_generator (-> +2), generator:triangle_number_generator (-> +2)
    for n in range(1, 1000000): # for_range_start (-> +1), function_call:range, int_literal, literal:Num, suggest_constant_definition
...
# ----------------------------------------------------------------------------------------
def solution(n): # function:solution (-> +16)
    largest_number = 0 # assignment, int_literal, literal:Num
    pre_counter = 0 # assignment, int_literal, literal:Num
    for input1 in range(n): # for_range_stop (-> +12), function_call:range
        counter = 1 # assignment, int_literal, literal:Num
        number = input1 # assignment
        while number > 1: # comparison_operator:Gt, evolve_state (-> +6), int_literal, literal:Num
            if number % 2 == 0: # binary_operator:Mod, comparison_operator:Eq, divisibility_test:2, if (-> +5), if_else (-> +5), int_literal, literal:Num
                number /= 2 # augmented_assignment, int_literal, literal:Num
                counter += 1 # augmented_assignment, int_literal, literal:Num
            else:
                number = (3 * number) + 1 # assignment, binary_operator:Add, binary_operator:Mult, int_literal, literal:Num, suggest_constant_definition
                counter += 1 # augmented_assignment, int_literal, literal:Num
...
def collatz_sequence(n): # function:collatz_sequence (-> +8)
    ...
    while n != 1: # comparison_operator:NotEq, evolve_state (-> +5), int_literal, literal:Num
        if n % 2 == 0: # binary_operator:Mod, comparison_operator:Eq, divisibility_test:2, if (-> +3), if_else (-> +3), int_literal, literal:Num
...
def solution(n): # function:solution (-> +2)
    result = max([(len(collatz_sequence(i)), i) for i in range(1, n)]) # assignment, composition, comprehension:List, comprehension_for_count:1, function_call:collatz_sequence, function_call:len, function_call:max, function_call:range, int_literal, literal:Num
...
def lattice_paths(n): # function:lattice_paths (-> +3)
    n = 2 * n # assignment, binary_operator:Mult, int_literal, literal:Num
    k = n / 2 # assignment, binary_operator:Div, int_literal, literal:Num
...
def solution(power): # function:solution (-> +7)
    num = 2 ** power # assignment, binary_operator:Pow, int_literal, literal:Num
    string_num = str(num) # assignment, function_call:str
    list_num = list(string_num) # assignment, function_call:list
    sum_of_num = 0 # assignment, int_literal, literal:Num
...
def solution(power): # function:solution (-> +5)
    n = 2 ** power # assignment, binary_operator:Pow, int_literal, literal:Num
    r = 0 # assignment, int_literal, literal:Num
    while n:
        r, n = r + n % 10, n // 10 # assignment, binary_operator:Add, binary_operator:FloorDiv, binary_operator:Mod, int_literal, literal:Num, suggest_constant_definition
    return r
...
def solution(n): # function:solution (-> +17)
    ones_counts = [0, 3, 3, 5, 4, 4, 3, 5, 5, 4, 3, 6, 6, 8, 8, 7, 7, 9, 8, 8] # assignment, int_literal, literal:List, literal:Num, suggest_constant_definition
    tens_counts = [0, 0, 6, 6, 5, 5, 5, 7, 6, 6] # assignment, int_literal, literal:List, literal:Num, suggest_constant_definition
    count = 0 # assignment, int_literal, literal:Num
    for i in range(1, n + 1): # accumulate_elements:AugAssign (-> +12), binary_operator:Add, for_range_start (-> +12), function_call:range, int_literal, literal:Num
        if i < 1000: # comparison_operator:Lt, if (-> +11), if_else (-> +11), int_literal, literal:Num, nested_ifs (-> +11), suggest_constant_definition
            if i >= 100: # comparison_operator:GtE, if (-> +3), int_literal, literal:Num, nested_ifs (-> +3), suggest_constant_definition
                count += ones_counts[i // 100] + 7 # augmented_assignment, binary_operator:Add, binary_operator:FloorDiv, index, index_arithmetic, int_literal, literal:Num, suggest_constant_definition
                if i % 100 != 0: # binary_operator:Mod, comparison_operator:NotEq, divisibility_test:100, if (-> +1), int_literal, literal:Num, suggest_constant_definition
                    count += 3 # augmented_assignment, int_literal, literal:Num, suggest_constant_definition
            if 0 < i % 100 < 20: # binary_operator:Mod, chained_comparison:2, comparison_operator:Lt, if (-> +4), if_else (-> +4), int_literal, literal:Num, suggest_constant_definition
                count += ones_counts[i % 100] # augmented_assignment, binary_operator:Mod, index, index_arithmetic, int_literal, literal:Num, suggest_constant_definition
            else:
                count += ones_counts[i % 10] # augmented_assignment, binary_operator:Mod, index, index_arithmetic, int_literal, literal:Num, suggest_constant_definition
            ...
            count += ones_counts[i // 1000] + 8 # augmented_assignment, binary_operator:Add, binary_operator:FloorDiv, index, index_arithmetic, int_literal, literal:Num, suggest_constant_definition
    return count
...
def solution(): # function:solution (-> +17)
    ...
    for i in range(1, len(a)): # composition, for_range_start (-> +10), function_call:len, function_call:range, int_literal, literal:Num, nested_for (-> +10)
        for j in range(len(a[i])): # composition, for_indexes (-> +9), for_range_stop (-> +9), function_call:len, function_call:range, index
            if j != len(a[i - 1]): # binary_operator:Sub, comparison_operator:NotEq, function_call:len, if (-> +3), if_else (-> +3), index, index_arithmetic, int_literal, literal:Num, suggest_conditional_expression (-> +3)
                number1 = a[i - 1][j] # assignment, binary_operator:Sub, index, index_arithmetic, int_literal, literal:Num
            else:
                number1 = 0 # assignment, int_literal, literal:Num
            if j > 0: # comparison_operator:Gt, if (-> +3), if_else (-> +3), int_literal, literal:Num, suggest_conditional_expression (-> +3)
                number2 = a[i - 1][j - 1] # assignment, binary_operator:Sub, index, index_arithmetic, int_literal, literal:Num
            else:
                number2 = 0 # assignment, int_literal, literal:Num
...
def solution(): # function:solution (-> +24)
    days_per_month = [31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31] # assignment, int_literal, literal:List, literal:Num, suggest_constant_definition
    day = 6 # assignment, int_literal, literal:Num, suggest_constant_definition
    month = 1 # assignment, int_literal, literal:Num
    year = 1901 # assignment, int_literal, literal:Num, suggest_constant_definition
    sundays = 0 # assignment, int_literal, literal:Num
    while year < 2001: # comparison_operator:Lt, evolve_state (-> +17), int_literal, literal:Num, suggest_constant_definition
        ...
        if (year % 4 == 0 and not year % 100 == 0) or (year % 400 == 0): # binary_operator:Mod, boolean_operator:And, boolean_operator:Or, comparison_operator:Eq, divisibility_test:100, divisibility_test:4, divisibility_test:400, if (-> +10), if_elif (-> +10), int_literal, literal:Num, nested_ifs (-> +10), suggest_constant_definition, unary_operator:Not
            if day > days_per_month[month - 1] and month != 2: # binary_operator:Sub, boolean_operator:And, comparison_operator:Gt, comparison_operator:NotEq, if (-> +5), if_elif (-> +5), index, index_arithmetic, int_literal, literal:Num
                month += 1 # augmented_assignment, int_literal, literal:Num
                day = day - days_per_month[month - 2] # assignment, binary_operator:Sub, index, index_arithmetic, int_literal, literal:Num, suggest_augmented_assignment
            elif day > 29 and month == 2: # boolean_operator:And, comparison_operator:Eq, comparison_operator:Gt, int_literal, literal:Num, suggest_constant_definition
                month += 1 # augmented_assignment, int_literal, literal:Num
                day = day - 29 # assignment, binary_operator:Sub, int_literal, literal:Num, suggest_augmented_assignment, suggest_constant_definition
        else:
            if day > days_per_month[month - 1]: # binary_operator:Sub, comparison_operator:Gt, index, index_arithmetic, int_literal, literal:Num
                month += 1 # augmented_assignment, int_literal, literal:Num
                day = day - days_per_month[month - 2] # assignment, binary_operator:Sub, index, index_arithmetic, int_literal, literal:Num, suggest_augmented_assignment
        if month > 12: # comparison_operator:Gt, if (-> +2), int_literal, literal:Num, suggest_constant_definition
            year += 1 # augmented_assignment, int_literal, literal:Num
            month = 1 # assignment, int_literal, literal:Num
        if year < 2001 and day == 1: # boolean_operator:And, comparison_operator:Eq, comparison_operator:Lt, if (-> +1), int_literal, literal:Num, suggest_constant_definition
            sundays += 1 # augmented_assignment, int_literal, literal:Num
    return sundays
...
def factorial(n): # function:factorial (-> +4)
    fact = 1 # assignment, int_literal, literal:Num
    for i in range(1, n + 1): # accumulate_elements:AugAssign (-> +1), binary_operator:Add, for_range_start (-> +1), function_call:range, int_literal, literal:Num
        fact *= i # augmented_assignment
    return fact
def split_and_add(number): # function:split_and_add (-> +6)
    sum_of_digits = 0 # assignment, int_literal, literal:Num
    while number > 0: # comparison_operator:Gt, evolve_state (-> +3), int_literal, literal:Num
        last_digit = number % 10 # assignment, binary_operator:Mod, int_literal, literal:Num, suggest_constant_definition
        sum_of_digits += last_digit # augmented_assignment
        number = number // 10 # assignment, binary_operator:FloorDiv, int_literal, literal:Num, suggest_augmented_assignment, suggest_constant_definition
...
def sum_of_divisors(n): # function:sum_of_divisors (-> +7)
    total = 0 # assignment, int_literal, literal:Num
    for i in range(1, int(sqrt(n) + 1)): # accumulate_elements:AugAssign (-> +4), binary_operator:Add, composition, for_range_start (-> +4), function_call:int, function_call:range, function_call:sqrt, int_literal, literal:Num
        if n % i == 0 and i != sqrt(n): # binary_operator:Mod, boolean_operator:And, comparison_operator:Eq, comparison_operator:NotEq, divisibility_test, function_call:sqrt, if (-> +3), if_elif (-> +3), int_literal, literal:Num
            total += i + n // i # augmented_assignment, binary_operator:Add, binary_operator:FloorDiv
        elif i == sqrt(n): # comparison_operator:Eq, function_call:sqrt
...
def solution(): # function:solution (-> +12)
    with open(os.path.dirname(__file__) + "/p022_names.txt") as file: # binary_operator:Add, composition, function_call:open, literal:Str, method_call:dirname
        names = str(file.readlines()[0]) # assignment, composition, function_call:str, index, int_literal, literal:Num, method_call:readlines
    ...
    name_score = 0 # assignment, int_literal, literal:Num
    total_score = 0 # assignment, int_literal, literal:Num
...
import os # import:os
def solution(): # function:solution (-> +12)
    total_sum = 0 # assignment, int_literal, literal:Num
    temp_sum = 0 # assignment, int_literal, literal:Num
    with open(os.path.dirname(__file__) + "/p022_names.txt") as file: # binary_operator:Add, composition, function_call:open, literal:Str, method_call:dirname
        name = str(file.readlines()[0]) # assignment, composition, function_call:str, index, int_literal, literal:Num, method_call:readlines
...
# ----------------------------------------------------------------------------------------
def solution(limit=28123): # function:solution (-> +13), function_with_default_positional_arguments:solution (-> +13), int_literal, literal:Num
    sumDivs = [1] * (limit + 1) # assignment, binary_operator:Add, binary_operator:Mult, int_literal, literal:List, literal:Num
    for i in range(2, int(limit ** 0.5) + 1): # accumulate_elements:AugAssign (-> +3), binary_operator:Add, binary_operator:Pow, composition, float_literal, for_range_start (-> +3), function_call:int, function_call:range, int_literal, literal:Num, nested_for (-> +3), suggest_constant_definition
        sumDivs[i * i] += i # augmented_assignment, binary_operator:Mult, index, index_arithmetic
        for k in range(i + 1, limit // i + 1): # accumulate_elements:AugAssign (-> +1), binary_operator:Add, binary_operator:FloorDiv, for_range_start (-> +1), function_call:range, int_literal, literal:Num
    ...
    res = 0 # assignment, int_literal, literal:Num
    for n in range(1, limit + 1): # accumulate_elements:AugAssign (-> +4), binary_operator:Add, for_range_start (-> +4), function_call:range, int_literal, literal:Num
...
def fib(a, b, n): # function:fib (-> +13)
    if n == 1: # comparison_operator:Eq, if (-> +5), if_elif (-> +5), int_literal, literal:Num
        return a
    elif n == 2: # comparison_operator:Eq, if_elif (-> +3), int_literal, literal:Num
        return b
    elif n == 3: # comparison_operator:Eq, int_literal, literal:Num, suggest_constant_definition
        return str(a) + str(b) # binary_operator:Add, function_call:str
    temp = 0 # assignment, int_literal, literal:Num
    for x in range(2, n): # for_range_start (-> +4), function_call:range, int_literal, literal:Num
...
def solution(n): # function:solution (-> +11)
    ...
    for x in range(n): # for_range_stop (-> +8), function_call:range
        ...
        c2 = 1 # assignment, int_literal, literal:Num
        while 1: # int_literal, literal:Num
            if len(fib(l[0], l[1], c2)) < int(l[2]): # comparison_operator:Lt, composition, function_call:fib, function_call:int, function_call:len, if (-> +3), if_else (-> +3), index, int_literal, literal:Num
                c2 += 1 # augmented_assignment, int_literal, literal:Num
            else:
                break
...
def fibonacci(n): # function:fibonacci (-> +9)
    if n == 1 or type(n) is not int: # boolean_operator:Or, comparison_operator:Eq, comparison_operator:IsNot, function_call:type, if (-> +8), if_elif (-> +8), int_literal, literal:Num
        return 0 # int_literal, literal:Num
    elif n == 2: # comparison_operator:Eq, if_else (-> +6), int_literal, literal:Num
        return 1 # int_literal, literal:Num
    else:
        sequence = [0, 1] # assignment, int_literal, literal:List, literal:Num
        for i in range(2, n + 1): # binary_operator:Add, for_range_start (-> +1), function_call:range, int_literal, literal:Num
...
def fibonacci_digits_index(n): # function:fibonacci_digits_index (-> +6)
    digits = 0 # assignment, int_literal, literal:Num
    index = 2 # assignment, int_literal, literal:Num
...
def solution(n): # function:solution (-> +5)
    answer = 1 # assignment, int_literal, literal:Num
...
# ----------------------------------------------------------------------------------------
def solution(n): # function:solution (-> +12)
    f1, f2 = 1, 1 # assignment, int_literal, literal:Num, literal:Tuple
    index = 2 # assignment, int_literal, literal:Num
    while True: # literal:True
        i = 0 # assignment, int_literal, literal:Num
...
def is_prime(k: int) -> bool: # function:is_prime
    if k < 2 or k % 2 == 0: # binary_operator:Mod, boolean_operator:Or, comparison_operator:Eq, comparison_operator:Lt, divisibility_test:2, if (-> +7), if_elif (-> +7), int_literal, literal:Num, nested_ifs (-> +7)
        return False # literal:False
    elif k == 2: # comparison_operator:Eq, if_else (-> +5), int_literal, literal:Num
        return True # literal:True
    else:
        for x in range(3, int(math.sqrt(k) + 1), 2): # binary_operator:Add, composition, for_range_step:2 (-> +2), function_call:int, function_call:range, int_literal, literal:Num, method_call:sqrt, suggest_constant_definition
            if k % x == 0: # binary_operator:Mod, comparison_operator:Eq, divisibility_test, if (-> +1), int_literal, literal:Num
                return False # literal:False
    return True # literal:True
def solution(a_limit: int, b_limit: int) -> int: # function:solution
    longest = [0, 0, 0] # assignment, int_literal, literal:List, literal:Num
    for a in range((a_limit * -1) + 1, a_limit): # binary_operator:Add, binary_operator:Mult, for_range_start (-> +9), function_call:range, int_literal, literal:Num, nested_for (-> +9)
        for b in range(2, b_limit): # for_range_start (-> +8), function_call:range, int_literal, literal:Num
            if is_prime(b): # function_call:is_prime, if (-> +7), nested_ifs (-> +7)
                count = 0 # assignment, int_literal, literal:Num
                n = 0 # assignment, int_literal, literal:Num
                ...
                if count > longest[0]: # comparison_operator:Gt, if (-> +1), index, int_literal, literal:Num
                    longest = [count, a, b] # assignment
    ans = longest[1] * longest[2] # assignment, binary_operator:Mult, index, int_literal, literal:Num
    return ans
...
def diagonal_sum(n): # function:diagonal_sum (-> +6)
    total = 1 # assignment, int_literal, literal:Num
    for i in range(1, int(ceil(n / 2.0))): # binary_operator:Div, composition, float_literal, for_range_start (-> +3), function_call:ceil, function_call:int, function_call:range, int_literal, literal:Num, suggest_constant_definition
        odd = 2 * i + 1 # assignment, binary_operator:Add, binary_operator:Mult, int_literal, literal:Num
        even = 2 * i # assignment, binary_operator:Mult, int_literal, literal:Num
        total = total + 4 * odd ** 2 - 6 * even # assignment, binary_operator:Add, binary_operator:Mult, binary_operator:Pow, binary_operator:Sub, int_literal, literal:Num, suggest_constant_definition
    return total
...
def solution(n): # function:solution (-> +8)
    ...
    currentPow = 0 # assignment, int_literal, literal:Num
    N = n + 1 # assignment, binary_operator:Add, int_literal, literal:Num
    for a in range(2, N): # for_range_start (-> +3), function_call:range, int_literal, literal:Num, nested_for (-> +3), square_nested_for (-> +3)
        for b in range(2, N): # for_range_start (-> +2), function_call:range, int_literal, literal:Num
...
def isDigitCancelling(num, den): # function:isDigitCancelling (-> +4)
    if num != den: # comparison_operator:NotEq, if (-> +3), nested_ifs (-> +3)
        if num % 10 == den // 10: # binary_operator:FloorDiv, binary_operator:Mod, comparison_operator:Eq, divisibility_test:10, if (-> +2), int_literal, literal:Num, nested_ifs (-> +2), suggest_constant_definition
            if (num // 10) / (den % 10) == num / den: # binary_operator:Div, binary_operator:FloorDiv, binary_operator:Mod, comparison_operator:Eq, if (-> +1), int_literal, literal:Num, suggest_constant_definition
                return True # literal:True
def solve(digit_len: int) -> str: # function:solve
    solutions = [] # assignment, literal:List
    den = 11 # assignment, int_literal, literal:Num, suggest_constant_definition
    ...
    for num in range(den, last_digit): # for_range_start (-> +7), function_call:range
        while den <= 99: # comparison_operator:LtE, evolve_state (-> +4), int_literal, literal:Num, suggest_constant_definition
            if (num != den) and (num % 10 == den // 10) and (den % 10 != 0): # binary_operator:FloorDiv, binary_operator:Mod, boolean_operator:And, comparison_operator:Eq, comparison_operator:NotEq, divisibility_test:10, if (-> +2), int_literal, literal:Num, nested_ifs (-> +2), suggest_constant_definition
        ...
        num += 1 # augmented_assignment, int_literal, literal:Num
        den = 10 # assignment, int_literal, literal:Num, suggest_constant_definition
...
def is_palindrome(n): # function:is_palindrome (-> +5)
    ...
    if n == n[::-1]: # comparison_operator:Eq, if (-> +3), if_else (-> +3), int_literal, literal:Num, slice_step, suggest_condition_return (-> +3)
        return True # literal:True
    else:
        return False # literal:False
def solution(n): # function:solution (-> +5)
    total = 0 # assignment, int_literal, literal:Num
    for i in range(1, n): # accumulate_elements:AugAssign (-> +2), for_range_start (-> +2), function_call:range, int_literal, literal:Num
...
# ----------------------------------------------------------------------------------------
import os # import:os
TRIANGULAR_NUMBERS = [int(0.5 * n * (n + 1)) for n in range(1, 101)] # assignment, binary_operator:Add, binary_operator:Mult, comprehension:List, comprehension_for_count:1, float_literal, function_call:int, function_call:range, global_constant_definition, int_literal, literal:Num
...
def solution(): # function:solution (-> +4)
    total = 0 # assignment, int_literal, literal:Num
    for i in range(1, 1001): # accumulate_elements:AugAssign (-> +1), for_range_start (-> +1), function_call:range, int_literal, literal:Num, suggest_constant_definition
...
def solution(): # function:solution (-> +6)
    total = 0 # assignment, int_literal, literal:Num
    for i in range(1, 101): # for_range_start (-> +3), function_call:range, int_literal, literal:Num, nested_for (-> +3), suggest_constant_definition
        for j in range(1, i + 1): # binary_operator:Add, for_range_start (-> +2), function_call:range, int_literal, literal:Num
            if combinations(i, j) > 1e6: # comparison_operator:Gt, float_literal, function_call:combinations, if (-> +1), literal:Num, suggest_constant_definition
                total += 1 # augmented_assignment, int_literal, literal:Num
    return total

# ----------------------------------------------------------------------------------------
# ../Python/project_euler/problem_551/sol1.py
# ----------------------------------------------------------------------------------------
ks = [k for k in range(2, 20 + 1)] # assignment, binary_operator:Add, comprehension:List, comprehension_for_count:1, function_call:range, global_variable_definition, int_literal, literal:Num
base = [10 ** k for k in range(ks[-1] + 1)] # assignment, binary_operator:Add, binary_operator:Pow, comprehension:List, comprehension_for_count:1, function_call:range, global_variable_definition, index, int_literal, literal:Num, negative_index:-1
...
def next_term(a_i, k, i, n): # function:next_term (-> +50), recursive_function:next_term (-> +50)
    ds_b = 0 # assignment, int_literal, literal:Num
    ...
    c = 0 # assignment, int_literal, literal:Num
    ...
    diff, dn = 0, 0 # assignment, int_literal, literal:Num, literal:Tuple
    max_dn = n - i # assignment, binary_operator:Sub
    sub_memo = memo.get(ds_b) # assignment, method_call:get
    if sub_memo != None: # comparison_operator:NotEq, if (-> +19), if_else (-> +19), literal:None, nested_ifs (-> +19)
        jumps = sub_memo.get(c) # assignment, method_call:get
        if jumps != None and len(jumps) > 0: # boolean_operator:And, comparison_operator:Gt, comparison_operator:NotEq, function_call:len, if (-> +14), if_else (-> +14), int_literal, literal:None, literal:Num, nested_ifs (-> +14)
            max_jump = -1 # assignment, int_literal, literal:Num
            for _k in range(len(jumps) - 1, -1, -1): # binary_operator:Sub, composition, for_range_step:-1 (-> +3), function_call:len, function_call:range, int_literal, literal:Num
                if jumps[_k][2] <= k and jumps[_k][1] <= max_dn: # boolean_operator:And, comparison_operator:LtE, if (-> +2), index, int_literal, literal:Num
                    max_jump = _k # assignment
                    break
            if max_jump >= 0: # comparison_operator:GtE, if (-> +6), int_literal, literal:Num, nested_ifs (-> +6)
                ...
                for j in range(min(k, len(a_i))): # composition, for_range_stop (-> +1), function_call:len, function_call:min, function_call:range
                    new_c, a_i[j] = divmod(new_c, 10) # assignment, function_call:divmod, index, int_literal, literal:Num, suggest_constant_definition
                if new_c > 0: # comparison_operator:Gt, if (-> +1), int_literal, literal:Num
    ...
    if dn >= max_dn or c + diff >= base[k]: # binary_operator:Add, boolean_operator:Or, comparison_operator:GtE, if (-> +1), index
        return diff, dn
    if k > ks[0]: # comparison_operator:Gt, if (-> +10), if_else (-> +10), index, int_literal, literal:Num, nested_ifs (-> +10)
        while True: # literal:True
            _diff, terms_jumped = next_term(a_i, k - 1, i + dn, n) # assignment, binary_operator:Add, binary_operator:Sub, function_call:next_term, int_literal, literal:Num
    ...
    j = 0 # assignment, int_literal, literal:Num
...
def compute(a_i, k, i, n): # function:compute (-> +25)
    if i >= n: # comparison_operator:GtE, if (-> +1)
        return 0, i # int_literal, literal:Num
    if k > len(a_i): # comparison_operator:Gt, function_call:len, if (-> +1)
        a_i.extend([0 for _ in range(k - len(a_i))]) # binary_operator:Sub, composition, comprehension:List, comprehension_for_count:1, function_call:len, function_call:range, int_literal, literal:Num, method_call:extend
    start_i = i # assignment
    ds_b, ds_c, diff = 0, 0, 0 # assignment, int_literal, literal:Num, literal:Tuple
    ...
    while i < n: # comparison_operator:Lt
        ...
        ds_c = 0 # assignment, int_literal, literal:Num
        for j in range(k): # accumulate_elements:AugAssign (-> +3), for_range_stop (-> +3), function_call:range
            ...
            addend, a_i[j] = divmod(s, 10) # assignment, function_call:divmod, index, int_literal, literal:Num, suggest_constant_definition
            ds_c += a_i[j] # augmented_assignment, index
...
def add(digits, k, addend): # function:add (-> +13)
    for j in range(k, len(digits)): # composition, for_range_start (-> +9), function_call:len, function_call:range
        ...
        if s >= 10: # comparison_operator:GtE, if (-> +5), if_else (-> +5), int_literal, literal:Num, suggest_constant_definition
            quotient, digits[j] = divmod(s, 10) # assignment, function_call:divmod, index, int_literal, literal:Num, suggest_constant_definition
            addend = addend // 10 + quotient # assignment, binary_operator:Add, binary_operator:FloorDiv, int_literal, literal:Num, suggest_constant_definition
        else:
            digits[j] = s # assignment, index
            addend = addend // 10 # assignment, binary_operator:FloorDiv, int_literal, literal:Num, suggest_augmented_assignment, suggest_constant_definition
    ...
    while addend > 0: # comparison_operator:Gt, int_literal, literal:Num
        addend, digit = divmod(addend, 10) # assignment, function_call:divmod, int_literal, literal:Num, suggest_constant_definition
        digits.append(digit) # method_call:append
def solution(n): # function:solution (-> +12)
    digits = [1] # assignment, int_literal, literal:List, literal:Num
    i = 1 # assignment, int_literal, literal:Num
    dn = 0 # assignment, int_literal, literal:Num
    while True: # literal:True
        diff, terms_jumped = next_term(digits, 20, i + dn, n) # assignment, binary_operator:Add, function_call:next_term, int_literal, literal:Num, suggest_constant_definition
        dn += terms_jumped # augmented_assignment
        if dn == n - i: # binary_operator:Sub, comparison_operator:Eq, if (-> +1)
            break
    a_n = 0 # assignment, int_literal, literal:Num
...
def partition(m): # function:partition (-> +9)
    memo = [[0 for _ in range(m)] for _ in range(m + 1)] # assignment, binary_operator:Add, comprehension:List, comprehension_for_count:1, function_call:range, int_literal, literal:Num
    for i in range(m + 1): # binary_operator:Add, for_range_stop (-> +1), function_call:range, int_literal, literal:Num
        memo[i][0] = 1 # assignment, index, int_literal, literal:Num
    for n in range(m + 1): # accumulate_elements:AugAssign (-> +4), binary_operator:Add, for_range_stop (-> +4), function_call:range, int_literal, literal:Num, nested_for (-> +4)
        for k in range(1, m): # accumulate_elements:AugAssign (-> +3), for_range_start (-> +3), function_call:range, int_literal, literal:Num
            ...
            if n > k: # comparison_operator:Gt, if (-> +1)
                memo[n][k] += memo[n - k - 1][k] # augmented_assignment, binary_operator:Sub, index, index_arithmetic, int_literal, literal:Num
...
def find_largest(data_file: str = "base_exp.txt") -> int: # function:find_largest, function_with_default_positional_arguments:find_largest, literal:Str
    largest = [0, 0] # assignment, int_literal, literal:List, literal:Num
    for i, line in enumerate(open(os.path.join(os.path.dirname(__file__), data_file))): # composition, for_indexes_elements (-> +3), function_call:enumerate, function_call:open, method_call:dirname, method_call:join
        ...
        if x * log10(a) > largest[0]: # binary_operator:Mult, comparison_operator:Gt, function_call:log10, if (-> +1), index, int_literal, literal:Num
            largest = [x * log10(a), i + 1] # assignment, binary_operator:Add, binary_operator:Mult, function_call:log10, int_literal, literal:Num

...
def median(nums): # function:median (-> +10), function_returning_something:median (-> +10)
    sorted_list = sorted(nums) # assignment, assignment_lhs_identifier:sorted_list, assignment_rhs_identifier:nums, assignment_rhs_identifier:sorted, call_argument:nums, function_call:sorted
    ...
    if len(sorted_list) % 2 == 0: # binary_operator:Mod, call_argument:sorted_list, comparison_operator:Eq, divisibility_test:2, function_call:len, if (-> +6), if_test_id:len, if_test_id:sorted_list, int_literal, literal:Num
        mid_index_1 = len(sorted_list) // 2 # assignment, assignment_lhs_identifier:mid_index_1, assignment_rhs_identifier:len, assignment_rhs_identifier:sorted_list, binary_operator:FloorDiv, call_argument:sorted_list, function_call:len, if_then_branch (-> +2), int_literal, literal:Num
        mid_index_2 = (len(sorted_list) // 2) - 1 # assignment, assignment_lhs_identifier:mid_index_2, assignment_rhs_identifier:len, assignment_rhs_identifier:sorted_list, binary_operator:FloorDiv, binary_operator:Sub, call_argument:sorted_list, function_call:len, int_literal, literal:Num
        ...
        mid_index = (len(sorted_list) - 1) // 2 # assignment, assignment_lhs_identifier:mid_index, assignment_rhs_identifier:len, assignment_rhs_identifier:sorted_list, binary_operator:FloorDiv, binary_operator:Sub, call_argument:sorted_list, function_call:len, if_else_branch (-> +1), int_literal, literal:Num
...
def mode(input_list): # function:mode (-> +7), function_returning_something:mode (-> +7)
    ...
    for x in input_list: # accumulate_elements:1 (-> +4), for:x (-> +4), for_each (-> +4)
        ...
        y = max(result) # assignment, assignment_lhs_identifier:y, assignment_rhs_identifier:max, assignment_rhs_identifier:result, call_argument:result, function_call:max
...
def prime_factors(n: int) -> list: # function:prime_factors (-> +11), function_returning_something:prime_factors (-> +11)
    ...
    while n % 2 == 0: # binary_operator:Mod, comparison_operator:Eq, divisibility_test:2, evolve_state (-> +2), int_literal, literal:Num, while (-> +2)
        ...
        n = int(n / 2) # assignment, assignment_lhs_identifier:n, assignment_rhs_identifier:int, assignment_rhs_identifier:n, binary_operator:Div, call_argument:, function_call:int, int_literal, literal:Num
    for i in range(3, int(math.sqrt(n)) + 1, 2): # accumulate_elements:2 (-> +3), binary_operator:Add, call_argument:, call_argument:2, call_argument:3, call_argument:n, composition, for:i (-> +3), for_range_step:2 (-> +3), function_call:int, function_call:range, int_literal, literal:Num, method_call:sqrt, suggest_constant_definition
...
def number_of_divisors(n: int) -> int: # function:number_of_divisors (-> +13), function_returning_something:number_of_divisors (-> +13)
    ...
    for i in range(3, int(math.sqrt(n)) + 1, 2): # accumulate_elements:1 (-> +5), binary_operator:Add, call_argument:, call_argument:2, call_argument:3, call_argument:n, composition, for:i (-> +5), for_range_step:2 (-> +5), function_call:int, function_call:range, int_literal, literal:Num, method_call:sqrt, suggest_constant_definition
        temp = 1 # assignment, assignment_lhs_identifier:temp, int_literal, literal:Num
...
def sum_of_divisors(n: int) -> int: # function:sum_of_divisors (-> +15), function_returning_something:sum_of_divisors (-> +15)
    ...
    for i in range(3, int(math.sqrt(n)) + 1, 2): # accumulate_elements:2 (-> +6), binary_operator:Add, call_argument:, call_argument:2, call_argument:3, call_argument:n, composition, for:i (-> +6), for_range_step:2 (-> +6), function_call:int, function_call:range, int_literal, literal:Num, method_call:sqrt, suggest_constant_definition
        temp = 1 # assignment, assignment_lhs_identifier:temp, int_literal, literal:Num
        while n % i == 0: # binary_operator:Mod, comparison_operator:Eq, divisibility_test, int_literal, literal:Num, while (-> +2)
            temp += 1 # assignment_lhs_identifier:temp, augmented_assignment, int_literal, literal:Num
            n = int(n / i) # assignment, assignment_lhs_identifier:n, assignment_rhs_identifier:i, assignment_rhs_identifier:int, assignment_rhs_identifier:n, binary_operator:Div, call_argument:, function_call:int
...
def binary_exponentiation(a, n): # body_recursive_function:binary_exponentiation (-> +7), function:binary_exponentiation (-> +7), function_returning_something:binary_exponentiation (-> +7), recursive_function:binary_exponentiation (-> +7)
    if n == 0: # comparison_operator:Eq, if (-> +6), if_test_id:n, int_literal, literal:Num
        ...
        b = binary_exponentiation(a, n / 2) # assignment, assignment_lhs_identifier:b, assignment_rhs_identifier:a, assignment_rhs_identifier:binary_exponentiation, assignment_rhs_identifier:n, binary_operator:Div, call_argument:, call_argument:a, function_call:binary_exponentiation, if_else_branch (-> +1), int_literal, literal:Num
...
def binomial_coefficient(n, r): # function:binomial_coefficient (-> +8), function_returning_something:binomial_coefficient (-> +8)
    C = [0 for i in range(r + 1)] # assignment, assignment_lhs_identifier:C, assignment_rhs_identifier:i, assignment_rhs_identifier:r, assignment_rhs_identifier:range, binary_operator:Add, call_argument:, comprehension:List, comprehension_for_count:1, constant_assignment:C, function_call:range, int_literal, literal:Num
    C[0] = 1 # assignment, assignment_lhs_identifier:C, constant_assignment:C, index, int_literal, literal:Num
    for i in range(1, n + 1): # binary_operator:Add, call_argument:, call_argument:1, for:i (-> +4), for_range_start (-> +4), function_call:range, int_literal, literal:Num
...
def main(): # function:main (-> +4), function_returning_nothing:main (-> +4)
    ...
    sequence = collatz_sequence(n) # assignment, assignment_lhs_identifier:sequence, assignment_rhs_identifier:collatz_sequence, assignment_rhs_identifier:n, call_argument:n, function_call:collatz_sequence
...
def explicit_euler(ode_func, y0, x0, stepsize, x_end): # function:explicit_euler (-> +8), function_returning_something:explicit_euler (-> +8)
    ...
    for k in range(N): # accumulate_elements:1 (-> +2), call_argument:N, for:k (-> +2), for_range_stop (-> +2), function_call:range
...
def extended_euclidean_algorithm(m, n): # function:extended_euclidean_algorithm (-> +31), function_returning_something:extended_euclidean_algorithm (-> +31)
    ...
    while True: # literal:True, while (-> +12)
        q = int(c / d) # assignment, assignment_lhs_identifier:q, assignment_rhs_identifier:c, assignment_rhs_identifier:d, assignment_rhs_identifier:int, binary_operator:Div, call_argument:, function_call:int
...
def main(): # function:main (-> +6), function_returning_nothing:main (-> +6)
    ...
    m = int(sys.argv[1]) # assignment, assignment_lhs_identifier:m, assignment_rhs_identifier:int, assignment_rhs_identifier:sys, call_argument:, function_call:int, index, int_literal, literal:Num
    n = int(sys.argv[2]) # assignment, assignment_lhs_identifier:n, assignment_rhs_identifier:int, assignment_rhs_identifier:sys, call_argument:, function_call:int, index, int_literal, literal:Num
...
def factorial(input_number: int) -> int: # function:factorial (-> +8), function_returning_something:factorial (-> +8)
    ...
    for i in range(1, input_number): # accumulate_elements:1 (-> +1), call_argument:1, call_argument:input_number, for:i (-> +1), for_range_start (-> +1), function_call:range, int_literal, literal:Num
...
def factors_of_a_number(num: int) -> list: # function:factors_of_a_number (-> +1), function_returning_something:factors_of_a_number (-> +1)
    return [i for i in range(1, num + 1) if num % i == 0] # binary_operator:Add, binary_operator:Mod, call_argument:, call_argument:1, comparison_operator:Eq, comprehension:List, comprehension_for_count:1, divisibility_test, filtered_comprehension, function_call:range, int_literal, literal:Num, return
...
def fib_iterative(n): # function:fib_iterative (-> +8), function_returning_something:fib_iterative (-> +8)
    ...
    if _check_number_input(n, 2): # call_argument:2, call_argument:n, function_call:_check_number_input, if (-> +6), if_test_id:_check_number_input, if_test_id:n, int_literal, literal:Num
        ...
        for _ in range(n - len(seq_out)): # binary_operator:Sub, call_argument:, call_argument:seq_out, composition, for:_ (-> +2), for_range_stop (-> +2), function_call:len, function_call:range
...
def fib_formula(n): # function:fib_formula (-> +12), function_returning_something:fib_formula (-> +12)
    ...
    if _check_number_input(n, 2, 1000000): # call_argument:1000000, call_argument:2, call_argument:n, function_call:_check_number_input, if (-> +9), if_test_id:_check_number_input, if_test_id:n, int_literal, literal:Num, suggest_constant_definition
        sqrt = Decimal(math.sqrt(5)) # assignment, assignment_lhs_identifier:sqrt, assignment_rhs_identifier:Decimal, assignment_rhs_identifier:math, call_argument:, call_argument:5, composition, function_call:Decimal, if_then_branch (-> +8), int_literal, literal:Num, method_call:sqrt, suggest_constant_definition
        phi_1 = Decimal(1 + sqrt) / Decimal(2) # assignment, assignment_lhs_identifier:phi_1, assignment_rhs_identifier:Decimal, assignment_rhs_identifier:sqrt, binary_operator:Add, binary_operator:Div, call_argument:, call_argument:2, function_call:Decimal, int_literal, literal:Num
        phi_2 = Decimal(1 - sqrt) / Decimal(2) # assignment, assignment_lhs_identifier:phi_2, assignment_rhs_identifier:Decimal, assignment_rhs_identifier:sqrt, binary_operator:Div, binary_operator:Sub, call_argument:, call_argument:2, function_call:Decimal, int_literal, literal:Num
        for i in range(2, n): # call_argument:2, call_argument:n, for:i (-> +4), for_range_start (-> +4), function_call:range, int_literal, literal:Num
            temp_out = ((phi_1 ** Decimal(i)) - (phi_2 ** Decimal(i))) * ( # assignment, assignment_lhs_identifier:temp_out, assignment_rhs_identifier:Decimal, assignment_rhs_identifier:i, assignment_rhs_identifier:phi_1, assignment_rhs_identifier:phi_2, binary_operator:Mult, binary_operator:Pow, binary_operator:Sub, call_argument:i, function_call:Decimal
                Decimal(sqrt) ** Decimal(-1) # assignment_rhs_identifier:Decimal, assignment_rhs_identifier:sqrt, binary_operator:Pow, call_argument:-1, call_argument:sqrt, function_call:Decimal, int_literal, literal:Num
            )
...
def main(): # function:main (-> +6), function_returning_nothing:main (-> +6)
    limit = int(input("How many terms to include in fibonacci series: ")) # assignment, assignment_lhs_identifier:limit, assignment_rhs_identifier:input, assignment_rhs_identifier:int, call_argument:, composition, function_call:input, function_call:int, literal:Str
    if limit > 0: # comparison_operator:Gt, if (-> +4), if_test_id:limit, int_literal, literal:Num
        ...
        print([recur_fibo(n) for n in range(limit)]) # call_argument:, call_argument:limit, call_argument:n, composition, comprehension:List, comprehension_for_count:1, function_call:print, function_call:range, function_call:recur_fibo
    else:
...
def find_max(nums, left, right): # function:find_max (-> +6), function_returning_something:find_max (-> +6), recursive_function:find_max (-> +6)
    ...
    left_max = find_max(nums, left, mid) # assignment, assignment_lhs_identifier:left_max, assignment_rhs_identifier:find_max, assignment_rhs_identifier:left, assignment_rhs_identifier:mid, assignment_rhs_identifier:nums, call_argument:left, call_argument:mid, call_argument:nums, function_call:find_max
    right_max = find_max(nums, mid + 1, right) # assignment, assignment_lhs_identifier:right_max, assignment_rhs_identifier:find_max, assignment_rhs_identifier:mid, assignment_rhs_identifier:nums, assignment_rhs_identifier:right, binary_operator:Add, call_argument:, call_argument:nums, call_argument:right, function_call:find_max, int_literal, literal:Num
...
def find_min(nums, left, right): # function:find_min (-> +6), function_returning_something:find_min (-> +6), recursive_function:find_min (-> +6)
    if left == right: # comparison_operator:Eq, if (-> +1), if_test_id:left, if_test_id:right
        return nums[left] # if_then_branch, index, return
    mid = (left + right) >> 1 # assignment, assignment_lhs_identifier:mid, assignment_rhs_identifier:left, assignment_rhs_identifier:right, binary_operator:Add, binary_operator:RShift, int_literal, literal:Num
    left_min = find_min(nums, left, mid) # assignment, assignment_lhs_identifier:left_min, assignment_rhs_identifier:find_min, assignment_rhs_identifier:left, assignment_rhs_identifier:mid, assignment_rhs_identifier:nums, call_argument:left, call_argument:mid, call_argument:nums, function_call:find_min
    right_min = find_min(nums, mid + 1, right) # assignment, assignment_lhs_identifier:right_min, assignment_rhs_identifier:find_min, assignment_rhs_identifier:mid, assignment_rhs_identifier:nums, assignment_rhs_identifier:right, binary_operator:Add, call_argument:, call_argument:nums, call_argument:right, function_call:find_min, int_literal, literal:Num
...
def main(): # function:main (-> +10), function_returning_nothing:main (-> +10)
    try: # try (-> +9), try_except:IndexError (-> +9), try_except:UnboundLocalError (-> +9), try_except:ValueError (-> +9)
        nums = input("Enter two integers separated by comma (,): ").split(",") # assignment, assignment_lhs_identifier:nums, assignment_rhs_identifier:input, call_argument:, function_call:input, literal:Str, method_call:split
        num_1 = int(nums[0]) # assignment, assignment_lhs_identifier:num_1, assignment_rhs_identifier:int, assignment_rhs_identifier:nums, call_argument:, function_call:int, index, int_literal, literal:Num
        num_2 = int(nums[1]) # assignment, assignment_lhs_identifier:num_2, assignment_rhs_identifier:int, assignment_rhs_identifier:nums, call_argument:, function_call:int, index, int_literal, literal:Num
...
def jaccard_similariy(setA, setB, alternativeUnion=False): # function:jaccard_similariy (-> +14), function_returning_something:jaccard_similariy (-> +14), function_with_default_positional_arguments:jaccard_similariy (-> +14), literal:False
    if isinstance(setA, set) and isinstance(setB, set): # boolean_operator:And, call_argument:set, call_argument:setA, call_argument:setB, function_call:isinstance, if (-> +6), if_test_id:isinstance, if_test_id:set, if_test_id:setA, if_test_id:setB
        intersection = len(setA.intersection(setB)) # assignment, assignment_lhs_identifier:intersection, assignment_rhs_identifier:len, assignment_rhs_identifier:setA, assignment_rhs_identifier:setB, call_argument:, call_argument:setB, composition, function_call:len, if_then_branch (-> +5), method_call:intersection
        if alternativeUnion: # if (-> +3), nested_if:1 (-> +3), suggest_conditional_expression (-> +3)
            union = len(setA) + len(setB) # assignment, assignment_lhs_identifier:union, assignment_rhs_identifier:len, assignment_rhs_identifier:setA, assignment_rhs_identifier:setB, binary_operator:Add, call_argument:setA, call_argument:setB, function_call:len, if_then_branch
        else:
            union = len(setA.union(setB)) # assignment, assignment_lhs_identifier:union, assignment_rhs_identifier:len, assignment_rhs_identifier:setA, assignment_rhs_identifier:setB, call_argument:, call_argument:setB, composition, function_call:len, if_else_branch, method_call:union
...
def karatsuba(a, b): # body_recursive_function:karatsuba (-> +11), function:karatsuba (-> +11), function_returning_something:karatsuba (-> +11), recursive_function:karatsuba (-> +11)
    if len(str(a)) == 1 or len(str(b)) == 1: # boolean_operator:Or, call_argument:, call_argument:a, call_argument:b, comparison_operator:Eq, composition, function_call:len, function_call:str, if (-> +10), if_test_id:a, if_test_id:b, if_test_id:len, if_test_id:str, int_literal, literal:Num
        ...
        m1 = max(len(str(a)), len(str(b))) # assignment, assignment_lhs_identifier:m1, assignment_rhs_identifier:a, assignment_rhs_identifier:b, assignment_rhs_identifier:len, assignment_rhs_identifier:max, assignment_rhs_identifier:str, call_argument:, call_argument:a, call_argument:b, composition, function_call:len, function_call:max, function_call:str, if_else_branch (-> +7)
        ...
        a1, a2 = divmod(a, 10 ** m2) # assignment, assignment_lhs_identifier:a1, assignment_lhs_identifier:a2, assignment_rhs_identifier:a, assignment_rhs_identifier:divmod, assignment_rhs_identifier:m2, binary_operator:Pow, call_argument:, call_argument:a, function_call:divmod, int_literal, literal:Num, suggest_constant_definition
        b1, b2 = divmod(b, 10 ** m2) # assignment, assignment_lhs_identifier:b1, assignment_lhs_identifier:b2, assignment_rhs_identifier:b, assignment_rhs_identifier:divmod, assignment_rhs_identifier:m2, binary_operator:Pow, call_argument:, call_argument:b, function_call:divmod, int_literal, literal:Num, suggest_constant_definition
        x = karatsuba(a2, b2) # assignment, assignment_lhs_identifier:x, assignment_rhs_identifier:a2, assignment_rhs_identifier:b2, assignment_rhs_identifier:karatsuba, call_argument:a2, call_argument:b2, function_call:karatsuba
        y = karatsuba((a1 + a2), (b1 + b2)) # assignment, assignment_lhs_identifier:y, assignment_rhs_identifier:a1, assignment_rhs_identifier:a2, assignment_rhs_identifier:b1, assignment_rhs_identifier:b2, assignment_rhs_identifier:karatsuba, binary_operator:Add, call_argument:, function_call:karatsuba
        z = karatsuba(a1, b1) # assignment, assignment_lhs_identifier:z, assignment_rhs_identifier:a1, assignment_rhs_identifier:b1, assignment_rhs_identifier:karatsuba, call_argument:a1, call_argument:b1, function_call:karatsuba
...
def kthPermutation(k, n): # function:kthPermutation (-> +13), function_returning_something:kthPermutation (-> +13)
    ...
    for i in range(2, n): # call_argument:2, call_argument:n, for:i (-> +1), for_range_start (-> +1), function_call:range, int_literal, literal:Num
    ...
    elements = list(range(n)) # assignment, assignment_lhs_identifier:elements, assignment_rhs_identifier:list, assignment_rhs_identifier:n, assignment_rhs_identifier:range, call_argument:, call_argument:n, composition, function_call:list, function_call:range
    while factorials: # while (-> +4)
        ...
        number, k = divmod(k, factorial) # assignment, assignment_lhs_identifier:k, assignment_lhs_identifier:number, assignment_rhs_identifier:divmod, assignment_rhs_identifier:factorial, assignment_rhs_identifier:k, call_argument:factorial, call_argument:k, function_call:divmod
...
class TestLeastCommonMultiple(unittest.TestCase):
    ...
    def test_lcm_function(self): # function:test_lcm_function (-> +4), function_returning_nothing:test_lcm_function (-> +4)
        for i, (first_num, second_num) in enumerate(self.test_inputs): # call_argument:, for:i:first_num:second_num (-> +3), for_indexes_elements (-> +3), function_call:enumerate
            actual_result = find_lcm(first_num, second_num) # assignment, assignment_lhs_identifier:actual_result, assignment_rhs_identifier:find_lcm, assignment_rhs_identifier:first_num, assignment_rhs_identifier:second_num, call_argument:first_num, call_argument:second_num, function_call:find_lcm
...
class Matrix(object):
    def __init__(self, arg): # function:__init__ (-> +6), function_returning_nothing:__init__ (-> +6)
        if isinstance(arg, list): # call_argument:arg, call_argument:list, function_call:isinstance, if (-> +5), if_test_id:arg, if_test_id:isinstance, if_test_id:list
            ...
            self.n = len(arg) # assignment, assignment_lhs_identifier:self, assignment_rhs_identifier:arg, assignment_rhs_identifier:len, call_argument:arg, function_call:len
        else:
            self.n = arg # assignment, assignment_lhs_identifier:self, assignment_rhs_identifier:arg, if_else_branch (-> +1)
            self.t = [[0 for _ in range(self.n)] for _ in range(self.n)] # assignment, assignment_lhs_identifier:self, assignment_rhs_identifier:_, assignment_rhs_identifier:range, assignment_rhs_identifier:self, call_argument:, comprehension:List, comprehension_for_count:1, function_call:range, int_literal, literal:Num
    def __mul__(self, b): # function:__mul__ (-> +6), function_returning_something:__mul__ (-> +6)
        matrix = Matrix(self.n) # assignment, assignment_lhs_identifier:matrix, assignment_rhs_identifier:Matrix, assignment_rhs_identifier:self, call_argument:, function_call:Matrix
        for i in range(self.n): # accumulate_elements:1 (-> +3), call_argument:, for:i (-> +3), for_range_stop (-> +3), function_call:range, square_nested_for (-> +3)
            for j in range(self.n): # accumulate_elements:1 (-> +2), call_argument:, for:j (-> +2), for_range_stop (-> +2), function_call:range, nested_for:1 (-> +2), square_nested_for (-> +2)
                for k in range(self.n): # accumulate_elements:1 (-> +1), call_argument:, for:k (-> +1), for_range_stop (-> +1), function_call:range, nested_for:2 (-> +1)
...
def modular_exponentiation(a, b): # function:modular_exponentiation (-> +7), function_returning_something:modular_exponentiation (-> +7)
    matrix = Matrix([[1, 0], [0, 1]]) # assignment, assignment_lhs_identifier:matrix, assignment_rhs_identifier:Matrix, call_argument:, function_call:Matrix, int_literal, literal:List, literal:Num
...
def fibonacci_with_matrix_exponentiation(n, f1, f2): # function:fibonacci_with_matrix_exponentiation (-> +7), function_returning_something:fibonacci_with_matrix_exponentiation (-> +7)
    ...
    matrix = Matrix([[1, 1], [1, 0]]) # assignment, assignment_lhs_identifier:matrix, assignment_rhs_identifier:Matrix, call_argument:, function_call:Matrix, int_literal, literal:List, literal:Num
    matrix = modular_exponentiation(matrix, n - 2) # assignment, assignment_lhs_identifier:matrix, assignment_rhs_identifier:matrix, assignment_rhs_identifier:modular_exponentiation, assignment_rhs_identifier:n, binary_operator:Sub, call_argument:, call_argument:matrix, function_call:modular_exponentiation, int_literal, literal:Num
...
def mobius(n: int) -> int: # function:mobius (-> +4), function_returning_something:mobius (-> +4)
    factors = prime_factors(n) # assignment, assignment_lhs_identifier:factors, assignment_rhs_identifier:n, assignment_rhs_identifier:prime_factors, call_argument:n, function_call:prime_factors
...
def newton_raphson(f, x0=0, maxiter=100, step=0.0001, maxerror=1e-6, logsteps=False): # float_literal, function:newton_raphson (-> +17), function_returning_something:newton_raphson (-> +17), function_with_default_positional_arguments:newton_raphson (-> +17), int_literal, literal:False, literal:Num
    ...
    error = abs(f(a)) # assignment, assignment_lhs_identifier:error, assignment_rhs_identifier:a, assignment_rhs_identifier:abs, assignment_rhs_identifier:f, call_argument:, call_argument:a, composition, function_call:abs, function_call:f
    f1 = lambda x: calc_derivative(f, x, h=step) # assignment, assignment_lhs_identifier:f1, assignment_rhs_identifier:calc_derivative, assignment_rhs_identifier:f, assignment_rhs_identifier:step, assignment_rhs_identifier:x, call_argument:f, call_argument:x, function_call:calc_derivative, lambda_function
    for _ in range(maxiter): # call_argument:maxiter, for:_ (-> +9), for_range_stop (-> +9), function_call:range
        ...
        a = a - f(a) / f1(a) # assignment, assignment_lhs_identifier:a, assignment_rhs_identifier:a, assignment_rhs_identifier:f, assignment_rhs_identifier:f1, binary_operator:Div, binary_operator:Sub, call_argument:a, function_call:f, function_call:f1, suggest_augmented_assignment
        if logsteps: # if (-> +1)
...
def primes(max: int) -> List[int]: # function:primes (-> +9), function_returning_something:primes (-> +9), index
    ...
    for i in range(2, max): # accumulate_elements:1 (-> +4), call_argument:2, call_argument:max, for:i (-> +4), for_range_start (-> +4), function_call:range, int_literal, literal:Num
        if not numbers[i]: # if (-> +3), if_test_id:i, if_test_id:numbers, index, unary_operator:Not
            for j in range(i, max, i): # call_argument:i, call_argument:max, for:j (-> +1), for_range_step (-> +1), function_call:range, if_then_branch (-> +2), nested_for:1 (-> +1)
...
def prime_sieve_eratosthenes(num): # function:prime_sieve_eratosthenes (-> +10), function_returning_nothing:prime_sieve_eratosthenes (-> +10)
    ...
    while p * p <= num: # binary_operator:Mult, comparison_operator:LtE, while (-> +4)
        if primes[p] == True: # comparison_operator:Eq, if (-> +2), if_test_id:p, if_test_id:primes, index, literal:True
            for i in range(p * p, num + 1, p): # binary_operator:Add, binary_operator:Mult, call_argument:, call_argument:p, for:i (-> +1), for_range_step (-> +1), function_call:range, if_then_branch (-> +1), int_literal, literal:Num
    ...
    for prime in range(2, num + 1): # binary_operator:Add, call_argument:, call_argument:2, for:prime (-> +2), for_range_start (-> +2), function_call:range, int_literal, literal:Num
...
def qr_householder(A): # function:qr_householder (-> +16), function_returning_something:qr_householder (-> +16)
    ...
    for k in range(t - 1): # accumulate_elements:1 (-> +10), binary_operator:Sub, call_argument:, for:k (-> +10), for_range_stop (-> +10), function_call:range, int_literal, literal:Num
        ...
        alpha = np.linalg.norm(x) # assignment, assignment_lhs_identifier:alpha, assignment_rhs_identifier:np, assignment_rhs_identifier:x, call_argument:x, method_call:norm
        v = x + np.sign(x[0]) * alpha * e1 # assignment, assignment_lhs_identifier:v, assignment_rhs_identifier:alpha, assignment_rhs_identifier:e1, assignment_rhs_identifier:np, assignment_rhs_identifier:x, binary_operator:Add, binary_operator:Mult, call_argument:, index, int_literal, literal:Num, method_call:sign
...
def QuadraticEquation(a: int, b: int, c: int) -> Tuple[str, str]: # function:QuadraticEquation (-> +10), function_returning_something:QuadraticEquation (-> +10), index
    ...
    snd = sqrt(-delta) # assignment, assignment_lhs_identifier:snd, assignment_rhs_identifier:delta, assignment_rhs_identifier:sqrt, call_argument:, function_call:sqrt, unary_operator:USub
    ...
    b = -abs(b) # assignment, assignment_lhs_identifier:b, assignment_rhs_identifier:abs, assignment_rhs_identifier:b, call_argument:b, function_call:abs, unary_operator:USub
...
class FFT:
    def __init__(self, polyA=[0], polyB=[0]): # function:__init__ (-> +17), function_returning_nothing:__init__ (-> +17), function_with_default_positional_arguments:__init__ (-> +17), int_literal, literal:List, literal:Num
        self.polyA = list(polyA)[:] # assignment, assignment_lhs_identifier:self, assignment_rhs_identifier:list, assignment_rhs_identifier:polyA, call_argument:polyA, function_call:list, slice
        self.polyB = list(polyB)[:] # assignment, assignment_lhs_identifier:self, assignment_rhs_identifier:list, assignment_rhs_identifier:polyB, call_argument:polyB, function_call:list, slice
        ...
        self.len_A = len(self.polyA) # assignment, assignment_lhs_identifier:self, assignment_rhs_identifier:len, assignment_rhs_identifier:self, call_argument:, function_call:len
        ...
        self.len_B = len(self.polyB) # assignment, assignment_lhs_identifier:self, assignment_rhs_identifier:len, assignment_rhs_identifier:self, call_argument:, function_call:len
        self.C_max_length = int( # assignment, assignment_lhs_identifier:self, assignment_rhs_identifier:int, composition, function_call:int
            2 ** np.ceil(np.log2(len(self.polyA) + len(self.polyB) - 1)) # assignment_rhs_identifier:len, assignment_rhs_identifier:np, assignment_rhs_identifier:self, binary_operator:Add, binary_operator:Pow, binary_operator:Sub, call_argument:, composition, function_call:len, int_literal, literal:Num, method_call:ceil, method_call:log2
        )
        ...
        self.root = complex(mpmath.root(x=1, n=self.C_max_length, k=1)) # assignment, assignment_lhs_identifier:self, assignment_rhs_identifier:complex, assignment_rhs_identifier:mpmath, assignment_rhs_identifier:self, call_argument:, composition, function_call:complex, int_literal, literal:Num, method_call:root
    ...
    def __DFT(self, which): # function:__DFT (-> +23), function_returning_something:__DFT (-> +23)
        ...
        while next_ncol > 0: # comparison_operator:Gt, evolve_state (-> +14), int_literal, literal:Num, while (-> +14)
            new_dft = [[] for i in range(next_ncol)] # assignment, assignment_lhs_identifier:new_dft, assignment_rhs_identifier:i, assignment_rhs_identifier:next_ncol, assignment_rhs_identifier:range, call_argument:next_ncol, comprehension:List, comprehension_for_count:1, function_call:range, literal:List
            ...
            for j in range(self.C_max_length // (next_ncol * 2)): # binary_operator:FloorDiv, binary_operator:Mult, call_argument:, for:j (-> +3), for_range_stop (-> +3), function_call:range, int_literal, literal:Num
                for i in range(next_ncol): # call_argument:next_ncol, for:i (-> +1), for_range_stop (-> +1), function_call:range, nested_for:1 (-> +1)
    ...
    def __multiply(self): # function:__multiply (-> +35), function_returning_something:__multiply (-> +35)
        dftA = self.__DFT("A") # assignment, assignment_lhs_identifier:dftA, assignment_rhs_identifier:self, call_argument:, literal:Str, method_call:__DFT
        dftB = self.__DFT("B") # assignment, assignment_lhs_identifier:dftB, assignment_rhs_identifier:self, call_argument:, literal:Str, method_call:__DFT
        inverseC = [[dftA[i] * dftB[i] for i in range(self.C_max_length)]] # assignment, assignment_lhs_identifier:inverseC, assignment_rhs_identifier:dftA, assignment_rhs_identifier:dftB, assignment_rhs_identifier:i, assignment_rhs_identifier:range, assignment_rhs_identifier:self, binary_operator:Mult, call_argument:, comprehension:List, comprehension_for_count:1, function_call:range, index
        del dftA
        del dftB
        ...
        while next_ncol <= self.C_max_length: # comparison_operator:LtE, evolve_state (-> +22), while (-> +22)
            new_inverseC = [[] for i in range(next_ncol)] # assignment, assignment_lhs_identifier:new_inverseC, assignment_rhs_identifier:i, assignment_rhs_identifier:next_ncol, assignment_rhs_identifier:range, call_argument:next_ncol, comprehension:List, comprehension_for_count:1, function_call:range, literal:List
            ...
            current_root = 1 # assignment, assignment_lhs_identifier:current_root, int_literal, literal:Num
            for j in range(self.C_max_length // next_ncol): # binary_operator:FloorDiv, call_argument:, for:j (-> +16), for_range_stop (-> +16), function_call:range
                for i in range(next_ncol // 2): # binary_operator:FloorDiv, call_argument:, for:i (-> +13), for_range_stop (-> +13), function_call:range, int_literal, literal:Num, nested_for:1 (-> +13)
...
def runge_kutta(f, y0, x0, h, x_end): # function:runge_kutta (-> +12), function_returning_something:runge_kutta (-> +12)
    ...
    y[0] = y0 # assignment, assignment_lhs_identifier:y, assignment_rhs_identifier:y0, index, int_literal, literal:Num
    x = x0 # assignment, assignment_lhs_identifier:x, assignment_rhs_identifier:x0
    for k in range(N): # accumulate_elements:1 (-> +6), call_argument:N, for:k (-> +6), for_range_stop (-> +6), function_call:range
        k1 = f(x, y[k]) # assignment, assignment_lhs_identifier:k1, assignment_rhs_identifier:f, assignment_rhs_identifier:k, assignment_rhs_identifier:x, assignment_rhs_identifier:y, call_argument:, call_argument:x, function_call:f, index
...
def sieve(n): # function:sieve (-> +33), function_returning_something:sieve (-> +33)
    ...
    end = int(math.sqrt(n)) # assignment, assignment_lhs_identifier:end, assignment_rhs_identifier:int, assignment_rhs_identifier:math, assignment_rhs_identifier:n, call_argument:, call_argument:n, composition, function_call:int, method_call:sqrt
    ...
    while start <= end: # comparison_operator:LtE, while (-> +6)
        if temp[start] is True: # comparison_operator:Is, if (-> +4), if_test_id:start, if_test_id:temp, index, literal:True
            ...
            for i in range(start * start, end + 1, start): # binary_operator:Add, binary_operator:Mult, call_argument:, call_argument:start, for:i (-> +2), for_range_step (-> +2), function_call:range, int_literal, literal:Num
    ...
    while low <= n: # comparison_operator:LtE, while (-> +14)
        ...
        for each in in_prime: # accumulate_elements:1 (-> +5), for:each (-> +5), for_each (-> +5)
            t = math.floor(low / each) * each # assignment, assignment_lhs_identifier:t, assignment_rhs_identifier:each, assignment_rhs_identifier:low, assignment_rhs_identifier:math, binary_operator:Div, binary_operator:Mult, call_argument:, method_call:floor
            ...
            for j in range(t, high + 1, each): # binary_operator:Add, call_argument:, call_argument:each, call_argument:t, for:j (-> +1), for_range_step (-> +1), function_call:range, int_literal, literal:Num, nested_for:1 (-> +1)
                temp[j - low] = False # assignment, assignment_lhs_identifier:temp, binary_operator:Sub, index, index_arithmetic, literal:False
        for j in range(len(temp)): # call_argument:, call_argument:temp, composition, for:j (-> +2), for_indexes (-> +2), for_range_stop (-> +2), function_call:len, function_call:range
...
def sieve(n): # function:sieve (-> +15), function_returning_something:sieve (-> +15)
    ...
    while start <= end: # comparison_operator:LtE, while (-> +6)
        if l[start] is True: # comparison_operator:Is, if (-> +4), if_test_id:l, if_test_id:start, index, literal:True
            ...
            for i in range(start * start, n + 1, start): # binary_operator:Add, binary_operator:Mult, call_argument:, call_argument:start, for:i (-> +2), for_range_step (-> +2), function_call:range, int_literal, literal:Num
        ...
        start += 1 # assignment_lhs_identifier:start, augmented_assignment, int_literal, literal:Num
    for j in range(end + 1, n + 1): # accumulate_elements:1 (-> +2), binary_operator:Add, call_argument:, for:j (-> +2), for_range_start (-> +2), function_call:range, int_literal, literal:Num
...
def method_2(boundary, steps): # function:method_2 (-> +12), function_returning_something:method_2 (-> +12)
    ...
    x_i = make_points(a, b, h) # assignment, assignment_lhs_identifier:x_i, assignment_rhs_identifier:a, assignment_rhs_identifier:b, assignment_rhs_identifier:h, assignment_rhs_identifier:make_points, call_argument:a, call_argument:b, call_argument:h, function_call:make_points
    y = 0.0 # assignment, assignment_lhs_identifier:y, float_literal, literal:Num, suggest_constant_definition
    y += (h / 3.0) * f(a) # assignment_lhs_identifier:y, assignment_rhs_identifier:a, assignment_rhs_identifier:f, assignment_rhs_identifier:h, augmented_assignment, binary_operator:Div, binary_operator:Mult, call_argument:a, float_literal, function_call:f, literal:Num, suggest_constant_definition
    ...
    y += (h / 3.0) * f(b) # assignment_lhs_identifier:y, assignment_rhs_identifier:b, assignment_rhs_identifier:f, assignment_rhs_identifier:h, augmented_assignment, binary_operator:Div, binary_operator:Mult, call_argument:b, float_literal, function_call:f, literal:Num, suggest_constant_definition
    return y # return:y
...
def main(): # function:main (-> +6), function_returning_nothing:main (-> +6)
    ...
    y = method_2(boundary, steps) # assignment, assignment_lhs_identifier:y, assignment_rhs_identifier:boundary, assignment_rhs_identifier:method_2, assignment_rhs_identifier:steps, call_argument:boundary, call_argument:steps, function_call:method_2
...
def softmax(vector): # function:softmax (-> +4), function_returning_something:softmax (-> +4)
    exponentVector = np.exp(vector) # assignment, assignment_lhs_identifier:exponentVector, assignment_rhs_identifier:np, assignment_rhs_identifier:vector, call_argument:vector, method_call:exp
    sumOfExponents = np.sum(exponentVector) # assignment, assignment_lhs_identifier:sumOfExponents, assignment_rhs_identifier:exponentVector, assignment_rhs_identifier:np, call_argument:exponentVector, method_call:sum
...
def method_1(boundary, steps): # function:method_1 (-> +10), function_returning_something:method_1 (-> +10)
    ...
    x_i = make_points(a, b, h) # assignment, assignment_lhs_identifier:x_i, assignment_rhs_identifier:a, assignment_rhs_identifier:b, assignment_rhs_identifier:h, assignment_rhs_identifier:make_points, call_argument:a, call_argument:b, call_argument:h, function_call:make_points
    y = 0.0 # assignment, assignment_lhs_identifier:y, float_literal, literal:Num, suggest_constant_definition
    y += (h / 2.0) * f(a) # assignment_lhs_identifier:y, assignment_rhs_identifier:a, assignment_rhs_identifier:f, assignment_rhs_identifier:h, augmented_assignment, binary_operator:Div, binary_operator:Mult, call_argument:a, float_literal, function_call:f, literal:Num, suggest_constant_definition
    for i in x_i: # accumulate_elements:1 (-> +1), for:i (-> +1), for_each (-> +1)
        y += h * f(i) # assignment_lhs_identifier:y, assignment_rhs_identifier:f, assignment_rhs_identifier:h, assignment_rhs_identifier:i, augmented_assignment, binary_operator:Mult, call_argument:i, function_call:f
    y += (h / 2.0) * f(b) # assignment_lhs_identifier:y, assignment_rhs_identifier:b, assignment_rhs_identifier:f, assignment_rhs_identifier:h, augmented_assignment, binary_operator:Div, binary_operator:Mult, call_argument:b, float_literal, function_call:f, literal:Num, suggest_constant_definition
    return y # return:y
def make_points(a, b, h): # function:make_points (-> +4), generator:make_points (-> +4)
    x = a + h # assignment, assignment_lhs_identifier:x, assignment_rhs_identifier:a, assignment_rhs_identifier:h, binary_operator:Add
    while x < (b - h): # binary_operator:Sub, comparison_operator:Lt, while (-> +2)
...
def main(): # function:main (-> +6), function_returning_nothing:main (-> +6)
    ...
    steps = 10.0 # assignment, assignment_lhs_identifier:steps, float_literal, literal:Num, suggest_constant_definition
    boundary = [a, b] # assignment, assignment_lhs_identifier:boundary, assignment_rhs_identifier:a, assignment_rhs_identifier:b
    y = method_1(boundary, steps) # assignment, assignment_lhs_identifier:y, assignment_rhs_identifier:boundary, assignment_rhs_identifier:method_1, assignment_rhs_identifier:steps, call_argument:boundary, call_argument:steps, function_call:method_1
    print("y = {0}".format(y)) # call_argument:, call_argument:y, composition, function_call:print, literal:Str, method_call:format

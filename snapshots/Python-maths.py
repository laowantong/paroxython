...
def n31(a: int) -> Tuple[List[int], int]: # function_definition, index
    if not isinstance(a, int): # builtin_function_call-isinstance, if, unary_operator-Not
        raise TypeError("Must be int, not {0}".format(type(a).__name__)) # builtin_function_call-type, composition, literal-Str
    if a < 1: # comparison_operator-Lt, if, literal-Num
        raise ValueError("Given integer must be greater than 1, not {0}".format(a)) # composition, literal-Str
    path = [a] # assignment
    ...
    return path, len(path) # builtin_function_call-len
def test_n31(): # function_definition
    assert n31(4) == ([4, 2, 1], 3) # comparison_operator-Eq, literal-List, literal-Num, literal-Tuple, suggest_constant_definition
    assert n31(11) == ([11, 34, 17, 52, 26, 13, 40, 20, 10, 5, 16, 8, 4, 2, 1], 15) # comparison_operator-Eq, literal-List, literal-Num, literal-Tuple, suggest_constant_definition
    assert n31(31) == ( # literal-Num, suggest_constant_definition
...
def test_abs_val(): # function_definition
    assert 0 == abs_val(0) # comparison_operator-Eq, literal-Num
    assert 34 == abs_val(34) # comparison_operator-Eq, literal-Num, suggest_constant_definition
    assert 100000000000 == abs_val(-100000000000) # comparison_operator-Eq, literal-Num, suggest_constant_definition, unary_operator-USub
...
def abs_max(x: List[int]) -> int: # function_definition, index
    ...
    for i in x: # find_best_element (-> +2), for_each
        if abs(i) > abs(j): # builtin_function_call-abs, comparison_operator-Gt, if
            j = i # assignment
    return j
def abs_max_sort(x): # function_definition
    return sorted(x, key=abs)[-1] # builtin_function_call-sorted, index, literal-Num, unary_operator-USub
def main(): # function_definition
    a = [1, 2, -11] # assignment, literal-List, literal-Num, suggest_constant_definition, unary_operator-USub
    assert abs_max(a) == -11 # comparison_operator-Eq, literal-Num, suggest_constant_definition, unary_operator-USub
    assert abs_max_sort(a) == -11 # comparison_operator-Eq, literal-Num, suggest_constant_definition, unary_operator-USub
...
def absMin(x): # function_definition
    j = x[0] # assignment, index, literal-Num
    for i in x: # find_best_element (-> +2), for_each
        if abs_val(i) < abs_val(j): # comparison_operator-Lt, if
            j = i # assignment
    return j
def main(): # function_definition
    a = [-3, -1, 2, -11] # assignment, literal-List, literal-Num, suggest_constant_definition, unary_operator-USub
    print(absMin(a))  # = -1 # builtin_function_call-print, composition

# ----------------------------------------------------------------------------------------
# ../Python/maths/average_mean.py
# ----------------------------------------------------------------------------------------
def average(nums): # function_definition
    return sum(nums) / len(nums) # binary_operator-Div, builtin_function_call-len, builtin_function_call-sum
def test_average(): # function_definition
    assert 12.0 == average([3, 6, 9, 12, 15, 18, 21]) # comparison_operator-Eq, literal-List, literal-Num, suggest_constant_definition
    assert 20 == average([5, 10, 15, 20, 25, 30, 35]) # comparison_operator-Eq, literal-List, literal-Num, suggest_constant_definition
    assert 4.5 == average([1, 2, 3, 4, 5, 6, 7, 8]) # comparison_operator-Eq, literal-List, literal-Num, suggest_constant_definition
...
def median(nums): # function_definition
    ...
    if len(sorted_list) % 2 == 0: # binary_operator-Mod, builtin_function_call-len, comparison_operator-Eq, divisibility_test-2, if, if_else, literal-Num
        ...
        mid_index = (len(sorted_list) - 1) // 2 # assignment, binary_operator-FloorDiv, binary_operator-Sub, builtin_function_call-len, literal-Num
        med = sorted_list[mid_index] # assignment, index
    return med
def main(): # function_definition
    print("Odd number of numbers:") # builtin_function_call-print, literal-Str
    print(median([2, 4, 6, 8, 20, 50, 70])) # builtin_function_call-print, composition, literal-List, literal-Num, suggest_constant_definition
    print("Even number of numbers:") # builtin_function_call-print, literal-Str
    print(median([2, 4, 6, 8, 20, 50])) # builtin_function_call-print, composition, literal-List, literal-Num, suggest_constant_definition
...
def mode(input_list):  # Defining function "mode." # function_definition
    ...
    for x in input_list: # accumulate_elements-Attribute (-> +2), for_each
        result.append(input_list.count(x)) # composition, mutable_sequence_method_call-append, sequence_method_call-count, string_method_call-count
...
def prime_factors(n: int) -> list: # function_definition
    ...
    for i in range(3, int(math.sqrt(n)) + 1, 2): # accumulate_elements-Assign (-> +3), binary_operator-Add, builtin_function_call-int, builtin_function_call-range, composition, for_range_step, literal-Num, suggest_constant_definition
...
def number_of_divisors(n: int) -> int: # function_definition
    ...
    for i in range(3, int(math.sqrt(n)) + 1, 2): # accumulate_elements-Assign (-> +4), binary_operator-Add, builtin_function_call-int, builtin_function_call-range, composition, for_range_step, literal-Num, suggest_constant_definition
        temp = 1 # assignment, literal-Num
...
def euler_phi(n: int) -> int: # function_definition
    s = n # assignment
    for x in set(prime_factors(n)): # accumulate_elements-AugAssign (-> +1), builtin_function_call-set, composition
...
def binomial_coefficient(n, r): # function_definition
    C = [0 for i in range(r + 1)] # assignment, binary_operator-Add, builtin_function_call-range, literal-Num
    C[0] = 1 # assignment, index, literal-Num
    for i in range(1, n + 1): # binary_operator-Add, builtin_function_call-range, for_range_start, literal-Num
        j = min(i, r) # assignment, builtin_function_call-min
    ...
    return C[r] # index
print(binomial_coefficient(n=10, r=5)) # builtin_function_call-print, composition, literal-Num
...
def main(): # function_definition
    ...
    sequence = collatz_sequence(n) # assignment
    print(sequence) # builtin_function_call-print
    print("collatz sequence from %d took %d steps." % (n, len(sequence))) # binary_operator-Mod, builtin_function_call-len, builtin_function_call-print, composition, literal-Str
...
def explicit_euler(ode_func, y0, x0, stepsize, x_end): # function_definition
    N = int(np.ceil((x_end - x0) / stepsize)) # assignment, binary_operator-Div, binary_operator-Sub, builtin_function_call-int, composition
    y = np.zeros((N + 1,)) # assignment, binary_operator-Add, literal-Num
    y[0] = y0 # assignment, index, literal-Num
    x = x0 # assignment
    for k in range(N): # accumulate_elements-Assign (-> +1), builtin_function_call-range, for_range_stop
        y[k + 1] = y[k] + stepsize * ode_func(x, y[k]) # assignment, binary_operator-Add, binary_operator-Mult, index, literal-Num
...
def main(): # function_definition
    ...
    print(extended_euclidean_algorithm(m, n)) # builtin_function_call-print, composition
...
def factorial(input_number: int) -> int: # function_definition
    if input_number < 0: # comparison_operator-Lt, if, literal-Num
        raise ValueError("factorial() not defined for negative values") # literal-Str
    if not isinstance(input_number, int): # builtin_function_call-isinstance, if, unary_operator-Not
        raise ValueError("factorial() only accepts integral values") # literal-Str
    result = 1 # assignment, literal-Num
    for i in range(1, input_number): # accumulate_elements-Assign (-> +1), builtin_function_call-range, for_range_start, literal-Num
...
def factorial(n: int) -> int: # function_definition, recursive_function_definition (-> +5)
    if n < 0: # comparison_operator-Lt, if, literal-Num
        raise ValueError("factorial() not defined for negative values") # literal-Str
    if not isinstance(n, int): # builtin_function_call-isinstance, if, unary_operator-Not
        raise ValueError("factorial() only accepts integral values") # literal-Str
    return 1 if n == 0 or n == 1 else n * factorial(n - 1) # binary_operator-Mult, binary_operator-Sub, boolean_operator-Or, comparison_operator-Eq, literal-Num

# ----------------------------------------------------------------------------------------
# ../Python/maths/factors.py
# ----------------------------------------------------------------------------------------
def factors_of_a_number(num: int) -> list: # function_definition
    return [i for i in range(1, num + 1) if num % i == 0] # binary_operator-Add, binary_operator-Mod, builtin_function_call-range, comparison_operator-Eq, divisibility_test, literal-Num
...
print((a / b) % p == (a * binary_exponentiation(b, p - 2, p)) % p) # binary_operator-Div, binary_operator-Mod, binary_operator-Mult, binary_operator-Sub, builtin_function_call-print, comparison_operator-Eq, composition, divisibility_test, literal-Num
...
def main(): # function_definition
    limit = int(input("How many terms to include in fibonacci series: ")) # assignment, builtin_function_call-input, builtin_function_call-int, composition, literal-Str
    if limit > 0: # comparison_operator-Gt, if, if_else, literal-Num
        ...
        print([recur_fibo(n) for n in range(limit)]) # builtin_function_call-print, builtin_function_call-range, composition
    else:
        print("Please enter a positive integer: ") # builtin_function_call-print, literal-Str
...
def find_max(nums): # function_definition
    ...
    return max_num
def main(): # function_definition
    print(find_max([2, 4, 9, 7, 19, 94, 5]))  # 94 # builtin_function_call-print, composition, literal-List, literal-Num, suggest_constant_definition
...
def find_max(nums, left, right): # function_definition, recursive_function_definition (-> +5)
    ...
    left_max = find_max(nums, left, mid)  # find max in range[left, mid] # assignment
    right_max = find_max(nums, mid + 1, right)  # find max in range[mid + 1, right] # assignment, binary_operator-Add, literal-Num
...
def find_min(nums): # function_definition
    ...
    return min_num
def main(): # function_definition
    assert find_min([0, 1, 2, 3, 4, 5, -3, 24, -56]) == -56 # comparison_operator-Eq, literal-List, literal-Num, suggest_constant_definition, unary_operator-USub
...
def find_min(nums, left, right): # function_definition, recursive_function_definition (-> +5)
    if left == right: # comparison_operator-Eq, if
        return nums[left] # index
    mid = (left + right) >> 1  # the middle # assignment, binary_operator-Add, binary_operator-RShift, literal-Num
    left_min = find_min(nums, left, mid)  # find min in range[left, mid] # assignment
    right_min = find_min(nums, mid + 1, right)  # find min in range[mid + 1, right] # assignment, binary_operator-Add, literal-Num
...
def gaussian(x, mu: float = 0.0, sigma: float = 1.0) -> int: # function_definition, literal-Num
    return 1 / sqrt(2 * pi * sigma ** 2) * exp(-((x - mu) ** 2) / 2 * sigma ** 2) # binary_operator-Div, binary_operator-Mult, binary_operator-Pow, binary_operator-Sub, literal-Num, unary_operator-USub
...
def greatest_common_divisor(a, b): # function_definition, recursive_function_definition (-> +1)
    return b if a == 0 else greatest_common_divisor(b % a, a) # binary_operator-Mod, comparison_operator-Eq, literal-Num
...
def main(): # function_definition
    try:
        ...
        print( # builtin_function_call-print, composition
            f"greatest_common_divisor({num_1}, {num_2}) = {greatest_common_divisor(num_1, num_2)}" # literal-Str
        )
        print(f"By iterative gcd({num_1}, {num_2}) = {gcd_by_iterative(num_1, num_2)}") # builtin_function_call-print, composition, literal-Str
    except (IndexError, UnboundLocalError, ValueError):
        print("Wrong input") # builtin_function_call-print, literal-Str
...
def exactPrimeFactorCount(n): # function_definition
    ...
    while i <= int(math.sqrt(n)): # builtin_function_call-int, comparison_operator-LtE, composition, evolve_state (-> +4)
...
def is_square_free(factors: List[int]) -> bool: # function_definition, index
    return len(set(factors)) == len(factors) # builtin_function_call-len, builtin_function_call-set, comparison_operator-Eq, composition
...
def jaccard_similariy(setA, setB, alternativeUnion=False): # function_definition, literal-False
    if isinstance(setA, set) and isinstance(setB, set): # boolean_operator-And, builtin_function_call-isinstance, if
        intersection = len(setA.intersection(setB)) # assignment, builtin_function_call-len, composition, set_method_call-intersection
        if alternativeUnion: # if, if_else, suggest_conditional_expression (-> +3)
            ...
            union = len(setA.union(setB)) # assignment, builtin_function_call-len, composition, set_method_call-union
    ...
    if isinstance(setA, (list, tuple)) and isinstance(setB, (list, tuple)): # boolean_operator-And, builtin_function_call-isinstance, if
        ...
        return len(intersection) / len(union) # binary_operator-Div, builtin_function_call-len

# ----------------------------------------------------------------------------------------
# ../Python/maths/karatsuba.py
# ----------------------------------------------------------------------------------------
def karatsuba(a, b): # function_definition, recursive_function_definition (-> +10)
    if len(str(a)) == 1 or len(str(b)) == 1: # boolean_operator-Or, builtin_function_call-len, builtin_function_call-str, comparison_operator-Eq, composition, if, if_else, literal-Num
        return a * b # binary_operator-Mult
    else:
        m1 = max(len(str(a)), len(str(b))) # assignment, builtin_function_call-len, builtin_function_call-max, builtin_function_call-str, composition
        ...
        x = karatsuba(a2, b2) # assignment
        y = karatsuba((a1 + a2), (b1 + b2)) # assignment, binary_operator-Add
        z = karatsuba(a1, b1) # assignment
...
def main(): # function_definition
    print(karatsuba(15463, 23489)) # builtin_function_call-print, composition, literal-Num, suggest_constant_definition
...
def kthPermutation(k, n): # function_definition
    ...
    elements = list(range(n)) # assignment, builtin_function_call-list, builtin_function_call-range, composition
    while factorials:
...
def res(x, y): # function_definition
    if 0 not in (x, y): # comparison_operator-NotIn, if, if_elif, literal-Num
        return y * math.log10(x) # binary_operator-Mult
    else:
...
class TestLeastCommonMultiple(unittest.TestCase):
    ...
    def test_lcm_function(self): # function_definition
        for i, (first_num, second_num) in enumerate(self.test_inputs): # builtin_function_call-enumerate, for_indexes_values
...
def recur_luc(n): # function_definition, recursive_function_definition (-> +5)
    ...
    return recur_luc(n - 1) + recur_luc(n - 2) # binary_operator-Add, binary_operator-Sub, literal-Num
...
class Matrix(object):
    def __init__(self, arg): # function_definition
        if isinstance(arg, list):  # Initialzes a matrix identical to the one provided. # builtin_function_call-isinstance, if, if_else
            self.t = arg # assignment
            self.n = len(arg) # assignment, builtin_function_call-len
...
def modular_exponentiation(a, b): # function_definition
    matrix = Matrix([[1, 0], [0, 1]]) # assignment, literal-List, literal-Num
...
def fibonacci_with_matrix_exponentiation(n, f1, f2): # function_definition
    if n == 1: # comparison_operator-Eq, if, if_elif, literal-Num
        ...
        return f2
    matrix = Matrix([[1, 1], [1, 0]]) # assignment, literal-List, literal-Num
    matrix = modular_exponentiation(matrix, n - 2) # assignment, binary_operator-Sub, literal-Num
...
""" # literal-Str
    ...
    exec_time = timeit.timeit(setup=setup, stmt=code, number=100) # assignment, literal-Num, suggest_constant_definition
    print( # builtin_function_call-print
...
def mobius(n: int) -> int: # function_definition
    factors = prime_factors(n) # assignment
    if is_square_free(factors): # if
        return -1 if len(factors) % 2 else 1 # binary_operator-Mod, builtin_function_call-len, literal-Num, unary_operator-USub
    return 0 # literal-Num
...
def modular_exponential(base, power, mod): # function_definition
    ...
    return result
def main(): # function_definition
    print(modular_exponential(3, 200, 13)) # builtin_function_call-print, composition, literal-Num, suggest_constant_definition
...
def calc_derivative(f, a, h=0.001): # function_definition, literal-Num
    return (f(a + h) - f(a - h)) / (2 * h) # binary_operator-Add, binary_operator-Div, binary_operator-Mult, binary_operator-Sub, literal-Num
def newton_raphson(f, x0=0, maxiter=100, step=0.0001, maxerror=1e-6, logsteps=False): # function_definition, literal-False, literal-Num
    ...
    steps = [a] # assignment
    error = abs(f(a)) # assignment, builtin_function_call-abs, composition
    f1 = lambda x: calc_derivative(f, x, h=step)  # Derivative of f(x) # assignment
    for _ in range(maxiter): # builtin_function_call-range, for_range_stop
        if f1(a) == 0: # comparison_operator-Eq, if, literal-Num
        ...
        raise ValueError("Iteration limit reached, no converging solution found") # literal-Str
    if logsteps: # if
...
def perfect_square(num: int) -> bool: # function_definition
    return math.sqrt(num) * math.sqrt(num) == num # binary_operator-Mult, comparison_operator-Eq
...
def evaluate_poly(poly: Sequence[float], x: float) -> float: # function_definition, index
    return sum(c * (x ** i) for i, c in enumerate(poly)) # binary_operator-Mult, binary_operator-Pow, builtin_function_call-enumerate, builtin_function_call-sum, composition
def horner(poly: Sequence[float], x: float) -> float: # function_definition, index
    ...
    for coeff in reversed(poly): # accumulate_elements-Assign (-> +1), builtin_function_call-reversed
...
def prime_check(number): # function_definition
    ...
    if number % 2 == 0: # binary_operator-Mod, comparison_operator-Eq, divisibility_test-2, if, literal-Num
        return False # literal-False
    odd_numbers = range(3, int(math.sqrt(number)) + 1, 2) # assignment, binary_operator-Add, builtin_function_call-int, builtin_function_call-range, composition, literal-Num, suggest_constant_definition
...
class Test(unittest.TestCase):
    def test_primes(self): # function_definition
        self.assertTrue(prime_check(2)) # composition, literal-Num
        self.assertTrue(prime_check(3)) # composition, literal-Num, suggest_constant_definition
        self.assertTrue(prime_check(5)) # composition, literal-Num, suggest_constant_definition
        self.assertTrue(prime_check(7)) # composition, literal-Num, suggest_constant_definition
        self.assertTrue(prime_check(11)) # composition, literal-Num, suggest_constant_definition
        self.assertTrue(prime_check(13)) # composition, literal-Num, suggest_constant_definition
        self.assertTrue(prime_check(17)) # composition, literal-Num, suggest_constant_definition
        self.assertTrue(prime_check(19)) # composition, literal-Num, suggest_constant_definition
        self.assertTrue(prime_check(23)) # composition, literal-Num, suggest_constant_definition
        self.assertTrue(prime_check(29)) # composition, literal-Num, suggest_constant_definition
    def test_not_primes(self): # function_definition
        self.assertFalse(prime_check(-19), "Negative numbers are not prime.") # composition, literal-Num, literal-Str, suggest_constant_definition, unary_operator-USub
        self.assertFalse( # composition
            prime_check(0), "Zero doesn't have any divider, primes must have two" # literal-Num, literal-Str
        )
        self.assertFalse( # composition
            prime_check(1), "One just have 1 divider, primes must have two." # literal-Num, literal-Str
        )
        self.assertFalse(prime_check(2 * 2)) # binary_operator-Mult, composition, literal-Num
        self.assertFalse(prime_check(2 * 3)) # binary_operator-Mult, composition, literal-Num, suggest_constant_definition
        self.assertFalse(prime_check(3 * 3)) # binary_operator-Mult, composition, literal-Num, suggest_constant_definition
        self.assertFalse(prime_check(3 * 5)) # binary_operator-Mult, composition, literal-Num, suggest_constant_definition
        self.assertFalse(prime_check(3 * 5 * 7)) # binary_operator-Mult, composition, literal-Num, suggest_constant_definition
...
def primes(max: int) -> List[int]: # function_definition, index
    ...
    for i in range(2, max): # accumulate_elements-Attribute (-> +4), builtin_function_call-range, for_range_start, literal-Num
...
def prime_sieve_eratosthenes(num): # function_definition
    primes = [True for i in range(num + 1)] # assignment, binary_operator-Add, builtin_function_call-range, literal-Num, literal-True
    ...
    while p * p <= num: # binary_operator-Mult, comparison_operator-LtE
        if primes[p] == True: # comparison_operator-Eq, if, index, literal-True
            for i in range(p * p, num + 1, p): # binary_operator-Add, binary_operator-Mult, builtin_function_call-range, for_range_step, literal-Num
                primes[i] = False # assignment, index, literal-False
        p += 1 # augmented_assignment, literal-Num
    for prime in range(2, num + 1): # binary_operator-Add, builtin_function_call-range, for_range_start, literal-Num
        if primes[prime]: # if, index
            print(prime, end=" ") # builtin_function_call-print, literal-Str
...
def qr_householder(A): # function_definition
    ...
    for k in range(t - 1): # accumulate_elements-Assign (-> +8), binary_operator-Sub, builtin_function_call-range, for_range_stop, literal-Num
        ...
        alpha = np.linalg.norm(x) # assignment
        v = x + np.sign(x[0]) * alpha * e1 # assignment, binary_operator-Add, binary_operator-Mult, index, literal-Num
        v /= np.linalg.norm(v) # augmented_assignment
        Q_k = np.eye(m - k) - 2.0 * v @ v.T # assignment, binary_operator-MatMult, binary_operator-Mult, binary_operator-Sub, literal-Num, suggest_constant_definition
        Q_k = np.block([[np.eye(k), np.zeros((k, m - k))], [np.zeros((m - k, k)), Q_k]]) # assignment, binary_operator-Sub, composition
...
def QuadraticEquation(a: int, b: int, c: int) -> Tuple[str, str]: # function_definition, index
    ...
    if delta >= 0: # comparison_operator-GtE, if, literal-Num
        return str((-b + sqrt(delta)) / (2 * a)), str((-b - sqrt(delta)) / (2 * a)) # binary_operator-Add, binary_operator-Div, binary_operator-Mult, binary_operator-Sub, builtin_function_call-str, composition, literal-Num, unary_operator-USub
...
def main(): # function_definition
    ...
    print("The equation solutions are: {} and {}".format(*solutions)) # builtin_function_call-print, composition, literal-Str
...
class FFT:
    def __init__(self, polyA=[0], polyB=[0]): # function_definition, literal-List, literal-Num
        ...
        self.C_max_length = int( # assignment, builtin_function_call-int, composition
            2 ** np.ceil(np.log2(len(self.polyA) + len(self.polyB) - 1)) # binary_operator-Add, binary_operator-Pow, binary_operator-Sub, builtin_function_call-len, composition, literal-Num
        )
        ...
        self.root = complex(mpmath.root(x=1, n=self.C_max_length, k=1)) # assignment, builtin_function_call-complex, composition, literal-Num
    ...
    def __DFT(self, which): # function_definition
        ...
        if len(dft) <= 1: # builtin_function_call-len, comparison_operator-LtE, if, literal-Num
    ...
    def __multiply(self): # function_definition
        dftA = self.__DFT("A") # assignment, literal-Str
        dftB = self.__DFT("B") # assignment, literal-Str
        inverseC = [[dftA[i] * dftB[i] for i in range(self.C_max_length)]] # assignment, binary_operator-Mult, builtin_function_call-range, index
        del dftA
        del dftB
        if len(inverseC[0]) <= 1: # builtin_function_call-len, comparison_operator-LtE, if, index, literal-Num
    ...
    def __str__(self): # function_definition
        A = "A = " + " + ".join( # assignment, binary_operator-Add, composition, literal-Str, string_method_call-join
            f"{coef}*x^{i}" for coef, i in enumerate(self.polyA[: self.len_A]) # builtin_function_call-enumerate, literal-Str, slice
        )
        B = "B = " + " + ".join( # assignment, binary_operator-Add, composition, literal-Str, string_method_call-join
            f"{coef}*x^{i}" for coef, i in enumerate(self.polyB[: self.len_B]) # builtin_function_call-enumerate, literal-Str, slice
        )
        C = "A*B = " + " + ".join( # assignment, binary_operator-Add, composition, literal-Str, string_method_call-join
...
def runge_kutta(f, y0, x0, h, x_end): # function_definition
    N = int(np.ceil((x_end - x0) / h)) # assignment, binary_operator-Div, binary_operator-Sub, builtin_function_call-int, composition
    y = np.zeros((N + 1,)) # assignment, binary_operator-Add, literal-Num
    y[0] = y0 # assignment, index, literal-Num
    x = x0 # assignment
    for k in range(N): # accumulate_elements-Assign (-> +5), builtin_function_call-range, for_range_stop
        k1 = f(x, y[k]) # assignment, index
        k2 = f(x + 0.5 * h, y[k] + 0.5 * h * k1) # assignment, binary_operator-Add, binary_operator-Mult, index, literal-Num, suggest_constant_definition
        k3 = f(x + 0.5 * h, y[k] + 0.5 * h * k2) # assignment, binary_operator-Add, binary_operator-Mult, index, literal-Num, suggest_constant_definition
        k4 = f(x + h, y[k] + h * k3) # assignment, binary_operator-Add, binary_operator-Mult, index
...
def sieve(n): # function_definition
    ...
    end = int(math.sqrt(n))  # Size of every segment # assignment, builtin_function_call-int, composition
    ...
    while low <= n: # comparison_operator-LtE
        ...
        for each in in_prime: # accumulate_elements-AugAssign (-> +3), for_each
            ...
            for j in range(t, high + 1, each): # binary_operator-Add, builtin_function_call-range, for_range_step, literal-Num
                temp[j - low] = False # assignment, binary_operator-Sub, index, literal-False
        for j in range(len(temp)): # builtin_function_call-len, builtin_function_call-range, composition, for_indexes, for_range_stop
        ...
        if high > n: # comparison_operator-Gt, if
            high = n # assignment
    return prime
print(sieve(10 ** 6)) # binary_operator-Pow, builtin_function_call-print, composition, literal-Num
...
def sieve(n): # function_definition
    ...
    prime = [] # assignment, literal-List
    start = 2 # assignment, literal-Num
    end = int(math.sqrt(n)) # assignment, builtin_function_call-int, composition
    while start <= end: # comparison_operator-LtE
...
def method_2(boundary, steps): # function_definition
    ...
    x_i = make_points(a, b, h) # assignment
    y = 0.0 # assignment, literal-Num, suggest_constant_definition
    y += (h / 3.0) * f(a) # augmented_assignment, binary_operator-Div, binary_operator-Mult, literal-Num, suggest_constant_definition
    cnt = 2 # assignment, literal-Num
    for i in x_i: # accumulate_elements-AugAssign (-> +1), for_each
        y += (h / 3) * (4 - 2 * (cnt % 2)) * f(i) # augmented_assignment, binary_operator-Div, binary_operator-Mod, binary_operator-Mult, binary_operator-Sub, literal-Num, suggest_constant_definition
        cnt += 1 # augmented_assignment, literal-Num
    y += (h / 3.0) * f(b) # augmented_assignment, binary_operator-Div, binary_operator-Mult, literal-Num, suggest_constant_definition
    return y
...
def main(): # function_definition
    ...
    y = method_2(boundary, steps) # assignment
    print("y = {0}".format(y)) # builtin_function_call-print, composition, literal-Str
...
def softmax(vector): # function_definition
    exponentVector = np.exp(vector) # assignment
    sumOfExponents = np.sum(exponentVector) # assignment
...
def sum_of_series(first_term, common_diff, num_of_terms): # function_definition
    ...
    return sum
def main(): # function_definition
    print(sum_of_series(1, 1, 10)) # builtin_function_call-print, composition, literal-Num, suggest_constant_definition
...
def method_1(boundary, steps): # function_definition
    ...
    x_i = make_points(a, b, h) # assignment
    y = 0.0 # assignment, literal-Num, suggest_constant_definition
    y += (h / 2.0) * f(a) # augmented_assignment, binary_operator-Div, binary_operator-Mult, literal-Num, suggest_constant_definition
    for i in x_i: # accumulate_elements-AugAssign (-> +1), for_each
        y += h * f(i) # augmented_assignment, binary_operator-Mult
    y += (h / 2.0) * f(b) # augmented_assignment, binary_operator-Div, binary_operator-Mult, literal-Num, suggest_constant_definition
    return y
def make_points(a, b, h): # function_definition
    x = a + h # assignment, binary_operator-Add
    while x < (b - h): # binary_operator-Sub, comparison_operator-Lt
...
def main(): # function_definition
    ...
    steps = 10.0  # define number of steps or resolution # assignment, literal-Num, suggest_constant_definition
    boundary = [a, b]  # define boundary of integration # assignment
    y = method_1(boundary, steps) # assignment
    print("y = {0}".format(y)) # builtin_function_call-print, composition, literal-Str
...
def vol_cube(side_length): # function_definition
    return float(side_length ** 3) # binary_operator-Pow, builtin_function_call-float, literal-Num, suggest_constant_definition
def vol_cuboid(width, height, length): # function_definition
    return float(width * height * length) # binary_operator-Mult, builtin_function_call-float
def vol_cone(area_of_base, height): # function_definition
    return (float(1) / 3) * area_of_base * height # binary_operator-Div, binary_operator-Mult, builtin_function_call-float, literal-Num, suggest_constant_definition
def vol_right_circ_cone(radius, height): # function_definition
    return (float(1) / 3) * pi * (radius ** 2) * height # binary_operator-Div, binary_operator-Mult, binary_operator-Pow, builtin_function_call-float, literal-Num, suggest_constant_definition
def vol_prism(area_of_base, height): # function_definition
    return float(area_of_base * height) # binary_operator-Mult, builtin_function_call-float
def vol_pyramid(area_of_base, height): # function_definition
    return (float(1) / 3) * area_of_base * height # binary_operator-Div, binary_operator-Mult, builtin_function_call-float, literal-Num, suggest_constant_definition
def vol_sphere(radius): # function_definition
    return (float(4) / 3) * pi * radius ** 3 # binary_operator-Div, binary_operator-Mult, binary_operator-Pow, builtin_function_call-float, literal-Num, suggest_constant_definition
...
def main(): # function_definition
    ...
    print("Cube: " + str(vol_cube(2)))  # = 8 # binary_operator-Add, builtin_function_call-print, builtin_function_call-str, composition, literal-Num, literal-Str
    print("Cuboid: " + str(vol_cuboid(2, 2, 2)))  # = 8 # binary_operator-Add, builtin_function_call-print, builtin_function_call-str, composition, literal-Num, literal-Str
    print("Cone: " + str(vol_cone(2, 2)))  # ~= 1.33 # binary_operator-Add, builtin_function_call-print, builtin_function_call-str, composition, literal-Num, literal-Str
    print("Right Circular Cone: " + str(vol_right_circ_cone(2, 2)))  # ~= 8.38 # binary_operator-Add, builtin_function_call-print, builtin_function_call-str, composition, literal-Num, literal-Str
    print("Prism: " + str(vol_prism(2, 2)))  # = 4 # binary_operator-Add, builtin_function_call-print, builtin_function_call-str, composition, literal-Num, literal-Str
    print("Pyramid: " + str(vol_pyramid(2, 2)))  # ~= 1.33 # binary_operator-Add, builtin_function_call-print, builtin_function_call-str, composition, literal-Num, literal-Str
    print("Sphere: " + str(vol_sphere(2)))  # ~= 33.5 # binary_operator-Add, builtin_function_call-print, builtin_function_call-str, composition, literal-Num, literal-Str
    print("Circular Cylinder: " + str(vol_circular_cylinder(2, 2)))  # ~= 25.1 # binary_operator-Add, builtin_function_call-print, builtin_function_call-str, composition, literal-Num, literal-Str
...
def zeller(date_input: str) -> str: # function_definition
    ...
    dt_ck = datetime.date(int(y), int(m), int(d)) # assignment, builtin_function_call-int, composition
    ...
    c: int = int(str(y)[:2]) # builtin_function_call-int, builtin_function_call-str, composition, literal-Num, slice
    k: int = int(str(y)[2:]) # builtin_function_call-int, builtin_function_call-str, composition, literal-Num, slice

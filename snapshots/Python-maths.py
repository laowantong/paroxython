...
def n31(a: int) -> Tuple[List[int], int]: # function:n31 (-> +12), function_returning_something:n31 (-> +12), index
    ...
    while a != 1: # comparison_operator:NotEq, evolve_state (-> +5), int_literal, literal:Num, while (-> +5)
        if a % 2 == 0: # binary_operator:Mod, comparison_operator:Eq, divisibility_test:2, if (-> +3), if_test_id:a, int_literal, literal:Num, suggest_conditional_expression (-> +3)
            a = a // 2 # assignment, assignment_lhs_identifier:a, assignment_rhs_atom:2, assignment_rhs_atom:a, binary_operator:FloorDiv, if_then_branch, int_literal, literal:Num, suggest_augmented_assignment
        else:
            a = 3 * a + 1 # assignment, assignment_lhs_identifier:a, assignment_rhs_atom:1, assignment_rhs_atom:3, assignment_rhs_atom:a, binary_operator:Add, binary_operator:Mult, if_else_branch, int_literal, literal:Num, suggest_constant_definition
        path += [a] # assignment_lhs_identifier:path, assignment_rhs_atom:a, augmented_assignment
...
def median(nums): # function:median (-> +10), function_returning_something:median (-> +10)
    sorted_list = sorted(nums) # assignment, assignment_lhs_identifier:sorted_list, assignment_rhs_atom:nums, assignment_rhs_atom:sorted, call_argument:nums, function_call:sorted
    ...
    if len(sorted_list) % 2 == 0: # binary_operator:Mod, call_argument:sorted_list, comparison_operator:Eq, divisibility_test:2, function_call:len, if (-> +6), if_test_id:len, if_test_id:sorted_list, int_literal, literal:Num
        mid_index_1 = len(sorted_list) // 2 # assignment, assignment_lhs_identifier:mid_index_1, assignment_rhs_atom:2, assignment_rhs_atom:len, assignment_rhs_atom:sorted_list, binary_operator:FloorDiv, call_argument:sorted_list, function_call:len, if_then_branch (-> +2), int_literal, literal:Num
        mid_index_2 = (len(sorted_list) // 2) - 1 # assignment, assignment_lhs_identifier:mid_index_2, assignment_rhs_atom:1, assignment_rhs_atom:2, assignment_rhs_atom:len, assignment_rhs_atom:sorted_list, binary_operator:FloorDiv, binary_operator:Sub, call_argument:sorted_list, function_call:len, int_literal, literal:Num
        med = (sorted_list[mid_index_1] + sorted_list[mid_index_2]) / float(2) # assignment, assignment_lhs_identifier:med, assignment_rhs_atom:2, assignment_rhs_atom:float, assignment_rhs_atom:mid_index_1, assignment_rhs_atom:mid_index_2, assignment_rhs_atom:sorted_list, binary_operator:Add, binary_operator:Div, call_argument:2, function_call:float, index, int_literal, literal:Num
    else:
        mid_index = (len(sorted_list) - 1) // 2 # assignment, assignment_lhs_identifier:mid_index, assignment_rhs_atom:1, assignment_rhs_atom:2, assignment_rhs_atom:len, assignment_rhs_atom:sorted_list, binary_operator:FloorDiv, binary_operator:Sub, call_argument:sorted_list, function_call:len, if_else_branch (-> +1), int_literal, literal:Num
...
def mode(input_list): # function:mode (-> +7), function_returning_something:mode (-> +7)
    check_list = input_list.copy() # assignment, assignment_lhs_identifier:check_list, assignment_rhs_atom:input_list, method_call:copy
    result = list() # assignment, assignment_lhs_identifier:result, assignment_rhs_atom:list, function_call:list
    for x in input_list: # accumulate_elements:1 (-> +4), for:x (-> +4), for_each (-> +4)
        result.append(input_list.count(x)) # call_argument:, call_argument:x, composition, method_call:append, method_call:count, method_call_object:result
        input_list.remove(x) # call_argument:x, method_call:remove, method_call_object:input_list
        y = max(result) # assignment, assignment_lhs_identifier:y, assignment_rhs_atom:max, assignment_rhs_atom:result, call_argument:result, function_call:max
        return check_list[result.index(y)] # call_argument:y, index, method_call:index, method_call_object:result, return
...
def prime_factors(n: int) -> list: # function:prime_factors (-> +11), function_returning_something:prime_factors (-> +11)
    ...
    while n % 2 == 0: # binary_operator:Mod, comparison_operator:Eq, divisibility_test:2, evolve_state (-> +2), int_literal, literal:Num, while (-> +2)
        pf.append(2) # call_argument:2, int_literal, literal:Num, method_call:append, method_call_object:pf
        n = int(n / 2) # assignment, assignment_lhs_identifier:n, assignment_rhs_atom:2, assignment_rhs_atom:int, assignment_rhs_atom:n, binary_operator:Div, call_argument:, function_call:int, int_literal, literal:Num
    for i in range(3, int(math.sqrt(n)) + 1, 2): # accumulate_elements:2 (-> +3), binary_operator:Add, call_argument:, call_argument:2, call_argument:3, call_argument:n, composition, for:i (-> +3), for_range:3:_:2 (-> +3), function_call:int, function_call:range, int_literal, literal:Num, method_call:sqrt, range:3:_:2, suggest_constant_definition
        while n % i == 0: # binary_operator:Mod, comparison_operator:Eq, divisibility_test, evolve_state (-> +2), int_literal, literal:Num, while (-> +2)
            pf.append(i) # call_argument:i, method_call:append, method_call_object:pf
            n = int(n / i) # assignment, assignment_lhs_identifier:n, assignment_rhs_atom:i, assignment_rhs_atom:int, assignment_rhs_atom:n, binary_operator:Div, call_argument:, function_call:int
    if n > 2: # comparison_operator:Gt, if (-> +1), if_test_id:n, int_literal, literal:Num
        pf.append(n) # call_argument:n, if_then_branch, method_call:append, method_call_object:pf
    return pf # return:pf
def number_of_divisors(n: int) -> int: # function:number_of_divisors (-> +13), function_returning_something:number_of_divisors (-> +13)
    ...
    while n % 2 == 0: # binary_operator:Mod, comparison_operator:Eq, divisibility_test:2, evolve_state (-> +2), int_literal, literal:Num, while (-> +2)
        temp += 1 # assignment_lhs_identifier:temp, assignment_rhs_atom:1, augmented_assignment, int_literal, literal:Num
        n = int(n / 2) # assignment, assignment_lhs_identifier:n, assignment_rhs_atom:2, assignment_rhs_atom:int, assignment_rhs_atom:n, binary_operator:Div, call_argument:, function_call:int, int_literal, literal:Num
    div *= temp # assignment_lhs_identifier:div, assignment_rhs_atom:temp, augmented_assignment
    for i in range(3, int(math.sqrt(n)) + 1, 2): # accumulate_elements:1 (-> +5), binary_operator:Add, call_argument:, call_argument:2, call_argument:3, call_argument:n, composition, for:i (-> +5), for_range:3:_:2 (-> +5), function_call:int, function_call:range, int_literal, literal:Num, method_call:sqrt, range:3:_:2, suggest_constant_definition
        temp = 1 # assignment, assignment_lhs_identifier:temp, assignment_rhs_atom:1, int_literal, literal:Num
...
def sum_of_divisors(n: int) -> int: # function:sum_of_divisors (-> +15), function_returning_something:sum_of_divisors (-> +15)
    ...
    if temp > 1: # comparison_operator:Gt, if (-> +1), if_test_id:temp, int_literal, literal:Num
        s *= (2 ** temp - 1) / (2 - 1) # assignment_lhs_identifier:s, assignment_rhs_atom:1, assignment_rhs_atom:2, assignment_rhs_atom:temp, augmented_assignment, binary_operator:Div, binary_operator:Pow, binary_operator:Sub, if_then_branch, int_literal, literal:Num
    for i in range(3, int(math.sqrt(n)) + 1, 2): # accumulate_elements:2 (-> +6), binary_operator:Add, call_argument:, call_argument:2, call_argument:3, call_argument:n, composition, for:i (-> +6), for_range:3:_:2 (-> +6), function_call:int, function_call:range, int_literal, literal:Num, method_call:sqrt, range:3:_:2, suggest_constant_definition
        temp = 1 # assignment, assignment_lhs_identifier:temp, assignment_rhs_atom:1, int_literal, literal:Num
        while n % i == 0: # binary_operator:Mod, comparison_operator:Eq, divisibility_test, int_literal, literal:Num, while (-> +2)
            temp += 1 # assignment_lhs_identifier:temp, assignment_rhs_atom:1, augmented_assignment, int_literal, literal:Num
            n = int(n / i) # assignment, assignment_lhs_identifier:n, assignment_rhs_atom:i, assignment_rhs_atom:int, assignment_rhs_atom:n, binary_operator:Div, call_argument:, function_call:int
        if temp > 1: # comparison_operator:Gt, if (-> +1), if_test_id:temp, int_literal, literal:Num
            s *= (i ** temp - 1) / (i - 1) # assignment_lhs_identifier:s, assignment_rhs_atom:1, assignment_rhs_atom:i, assignment_rhs_atom:temp, augmented_assignment, binary_operator:Div, binary_operator:Pow, binary_operator:Sub, if_then_branch, int_literal, literal:Num
...
def euler_phi(n: int) -> int: # function:euler_phi (-> +4), function_returning_something:euler_phi (-> +4)
    ...
    for x in set(prime_factors(n)): # accumulate_elements:1 (-> +1), call_argument:, call_argument:n, composition, for:x (-> +1), function_call:prime_factors, function_call:set
        s *= (x - 1) / x # assignment_lhs_identifier:s, assignment_rhs_atom:1, assignment_rhs_atom:x, augmented_assignment, binary_operator:Div, binary_operator:Sub, int_literal, literal:Num
    return int(s) # call_argument:s, function_call:int, function_tail_call:int, return
...
def binary_exponentiation(a, n): # body_recursive_function:binary_exponentiation (-> +7), function:binary_exponentiation (-> +7), function_returning_something:binary_exponentiation (-> +7), recursive_function:binary_exponentiation (-> +7)
    if n == 0: # comparison_operator:Eq, if (-> +6), if_test_id:n, int_literal, literal:Num
        ...
        b = binary_exponentiation(a, n / 2) # assignment, assignment_lhs_identifier:b, assignment_rhs_atom:2, assignment_rhs_atom:a, assignment_rhs_atom:binary_exponentiation, assignment_rhs_atom:n, binary_operator:Div, call_argument:, call_argument:a, function_call:binary_exponentiation, if_else_branch (-> +1), int_literal, literal:Num
...
def binomial_coefficient(n, r): # function:binomial_coefficient (-> +8), function_returning_something:binomial_coefficient (-> +8)
    C = [0 for i in range(r + 1)] # assignment, assignment_lhs_identifier:C, assignment_rhs_atom:0, assignment_rhs_atom:1, assignment_rhs_atom:i, assignment_rhs_atom:r, assignment_rhs_atom:range, binary_operator:Add, call_argument:, comprehension:List, comprehension_for_count:1, function_call:range, int_literal, literal:Num, range:_
    ...
    for i in range(1, n + 1): # binary_operator:Add, call_argument:, call_argument:1, for:i (-> +4), for_range:1:_ (-> +4), function_call:range, int_literal, literal:Num, range:1:_
        j = min(i, r) # assignment, assignment_lhs_identifier:j, assignment_rhs_atom:i, assignment_rhs_atom:min, assignment_rhs_atom:r, call_argument:i, call_argument:r, function_call:min
        while j > 0: # comparison_operator:Gt, evolve_state (-> +2), int_literal, literal:Num, while (-> +2)
            C[j] += C[j - 1] # assignment_lhs_identifier:C, assignment_rhs_atom:1, assignment_rhs_atom:C, assignment_rhs_atom:j, augmented_assignment, binary_operator:Sub, index, index_arithmetic, int_literal, literal:Num
            j -= 1 # assignment_lhs_identifier:j, assignment_rhs_atom:1, augmented_assignment, int_literal, literal:Num
...
def collatz_sequence(n): # function:collatz_sequence (-> +8), function_returning_something:collatz_sequence (-> +8)
    ...
    while n != 1: # comparison_operator:NotEq, evolve_state (-> +5), int_literal, literal:Num, while (-> +5)
        if n % 2 == 0: # binary_operator:Mod, comparison_operator:Eq, divisibility_test:2, if (-> +3), if_test_id:n, int_literal, literal:Num
            n //= 2 # assignment_lhs_identifier:n, assignment_rhs_atom:2, augmented_assignment, if_then_branch, int_literal, literal:Num
        else:
            n = 3 * n + 1 # assignment, assignment_lhs_identifier:n, assignment_rhs_atom:1, assignment_rhs_atom:3, assignment_rhs_atom:n, binary_operator:Add, binary_operator:Mult, if_else_branch, int_literal, literal:Num, suggest_constant_definition
...
def main(): # function:main (-> +4), function_returning_nothing:main (-> +4)
    ...
    sequence = collatz_sequence(n) # assignment, assignment_lhs_identifier:sequence, assignment_rhs_atom:collatz_sequence, assignment_rhs_atom:n, call_argument:n, function_call:collatz_sequence
...
def explicit_euler(ode_func, y0, x0, stepsize, x_end): # function:explicit_euler (-> +8), function_returning_something:explicit_euler (-> +8)
    N = int(np.ceil((x_end - x0) / stepsize)) # assignment, assignment_lhs_identifier:N, assignment_rhs_atom:int, assignment_rhs_atom:np, assignment_rhs_atom:stepsize, assignment_rhs_atom:x0, assignment_rhs_atom:x_end, binary_operator:Div, binary_operator:Sub, call_argument:, composition, function_call:int, method_call:ceil
    y = np.zeros((N + 1,)) # assignment, assignment_lhs_identifier:y, assignment_rhs_atom:1, assignment_rhs_atom:N, assignment_rhs_atom:np, binary_operator:Add, call_argument:, int_literal, literal:Num, method_call:zeros
    ...
    for k in range(N): # accumulate_elements:1 (-> +2), call_argument:N, for:k (-> +2), for_range:N (-> +2), function_call:range, range:N
        y[k + 1] = y[k] + stepsize * ode_func(x, y[k]) # assignment, assignment_lhs_identifier:y, assignment_rhs_atom:k, assignment_rhs_atom:ode_func, assignment_rhs_atom:stepsize, assignment_rhs_atom:x, assignment_rhs_atom:y, binary_operator:Add, binary_operator:Mult, call_argument:, call_argument:x, function_call:ode_func, index, index_arithmetic, int_literal, literal:Num
        x += stepsize # assignment_lhs_identifier:x, assignment_rhs_atom:stepsize, augmented_assignment
    return y # return:y
...
def extended_euclidean_algorithm(m, n): # function:extended_euclidean_algorithm (-> +31), function_returning_something:extended_euclidean_algorithm (-> +31)
    ...
    while True: # literal:True, while (-> +12)
        q = int(c / d) # assignment, assignment_lhs_identifier:q, assignment_rhs_atom:c, assignment_rhs_atom:d, assignment_rhs_atom:int, binary_operator:Div, call_argument:, function_call:int
        ...
        a = t - q * a # assignment, assignment_lhs_identifier:a, assignment_rhs_atom:a, assignment_rhs_atom:q, assignment_rhs_atom:t, binary_operator:Mult, binary_operator:Sub
        ...
        b = t - q * b # assignment, assignment_lhs_identifier:b, assignment_rhs_atom:b, assignment_rhs_atom:q, assignment_rhs_atom:t, binary_operator:Mult, binary_operator:Sub
...
def main(): # function:main (-> +6), function_returning_nothing:main (-> +6)
    ...
    m = int(sys.argv[1]) # assignment, assignment_lhs_identifier:m, assignment_rhs_atom:1, assignment_rhs_atom:int, assignment_rhs_atom:sys, call_argument:, function_call:int, index, int_literal, literal:Num
    n = int(sys.argv[2]) # assignment, assignment_lhs_identifier:n, assignment_rhs_atom:2, assignment_rhs_atom:int, assignment_rhs_atom:sys, call_argument:, function_call:int, index, int_literal, literal:Num
...
def factorial(input_number: int) -> int: # function:factorial (-> +8), function_returning_something:factorial (-> +8)
    ...
    for i in range(1, input_number): # accumulate_elements:1 (-> +1), call_argument:1, call_argument:input_number, for:i (-> +1), for_range:1:input_number (-> +1), function_call:range, int_literal, literal:Num, range:1:input_number
        result = result * (i + 1) # assignment, assignment_lhs_identifier:result, assignment_rhs_atom:1, assignment_rhs_atom:i, assignment_rhs_atom:result, binary_operator:Add, binary_operator:Mult, int_literal, literal:Num, suggest_augmented_assignment
...
def timer_decorator(func): # closure:timer_decorator (-> +10), function:timer_decorator (-> +10), function_returning_something:timer_decorator (-> +10), nested_function:timer_decorator (-> +10)
    def timer_wrapper(*args, **kwargs): # function:timer_wrapper (-> +8), function_returning_something:timer_wrapper (-> +8)
        start = time.time() # assignment, assignment_lhs_identifier:start, assignment_rhs_atom:time, method_call:time
        func(*args, **kwargs) # call_argument:, function_call:func
        end = time.time() # assignment, assignment_lhs_identifier:end, assignment_rhs_atom:time, method_call:time
...
def fib_iterative(n): # function:fib_iterative (-> +8), function_returning_something:fib_iterative (-> +8)
    n = int(n) # assignment, assignment_lhs_identifier:n, assignment_rhs_atom:int, assignment_rhs_atom:n, call_argument:n, function_call:int
    if _check_number_input(n, 2): # call_argument:2, call_argument:n, function_call:_check_number_input, if (-> +6), if_test_id:_check_number_input, if_test_id:n, int_literal, literal:Num
        ...
        for _ in range(n - len(seq_out)): # binary_operator:Sub, call_argument:, call_argument:seq_out, composition, for:_ (-> +2), for_range:_ (-> +2), function_call:len, function_call:range, range:_
            a, b = b, a + b # assignment, assignment_lhs_identifier:a, assignment_lhs_identifier:b, assignment_rhs_atom:a, assignment_rhs_atom:b, binary_operator:Add
            seq_out.append(b) # call_argument:b, method_call:append, method_call_object:seq_out
...
def fib_formula(n): # function:fib_formula (-> +12), function_returning_something:fib_formula (-> +12)
    ...
    if _check_number_input(n, 2, 1000000): # call_argument:1000000, call_argument:2, call_argument:n, function_call:_check_number_input, if (-> +9), if_test_id:_check_number_input, if_test_id:n, int_literal, literal:Num, suggest_constant_definition
        sqrt = Decimal(math.sqrt(5)) # assignment, assignment_lhs_identifier:sqrt, assignment_rhs_atom:5, assignment_rhs_atom:Decimal, assignment_rhs_atom:math, call_argument:, call_argument:5, composition, function_call:Decimal, if_then_branch (-> +8), int_literal, literal:Num, method_call:sqrt, suggest_constant_definition
        phi_1 = Decimal(1 + sqrt) / Decimal(2) # assignment, assignment_lhs_identifier:phi_1, assignment_rhs_atom:1, assignment_rhs_atom:2, assignment_rhs_atom:Decimal, assignment_rhs_atom:sqrt, binary_operator:Add, binary_operator:Div, call_argument:, call_argument:2, function_call:Decimal, int_literal, literal:Num
        phi_2 = Decimal(1 - sqrt) / Decimal(2) # assignment, assignment_lhs_identifier:phi_2, assignment_rhs_atom:1, assignment_rhs_atom:2, assignment_rhs_atom:Decimal, assignment_rhs_atom:sqrt, binary_operator:Div, binary_operator:Sub, call_argument:, call_argument:2, function_call:Decimal, int_literal, literal:Num
        for i in range(2, n): # call_argument:2, call_argument:n, for:i (-> +4), for_range:2:n (-> +4), function_call:range, int_literal, literal:Num, range:2:n
            temp_out = ((phi_1 ** Decimal(i)) - (phi_2 ** Decimal(i))) * ( # assignment, assignment_lhs_identifier:temp_out, assignment_rhs_atom:Decimal, assignment_rhs_atom:i, assignment_rhs_atom:phi_1, assignment_rhs_atom:phi_2, binary_operator:Mult, binary_operator:Pow, binary_operator:Sub, call_argument:i, function_call:Decimal
                Decimal(sqrt) ** Decimal(-1) # assignment_rhs_atom:-1, assignment_rhs_atom:Decimal, assignment_rhs_atom:sqrt, binary_operator:Pow, call_argument:-1, call_argument:sqrt, function_call:Decimal, int_literal, literal:Num
            )
            seq_out.append(int(temp_out)) # call_argument:, call_argument:temp_out, composition, function_call:int, method_call:append, method_call_object:seq_out
        return seq_out # return:seq_out
...
def main(): # function:main (-> +6), function_returning_nothing:main (-> +6)
    limit = int(input("How many terms to include in fibonacci series: ")) # assignment, assignment_lhs_identifier:limit, assignment_rhs_atom:input, assignment_rhs_atom:int, call_argument:, composition, function_call:input, function_call:int, literal:Str
...
def find_max(nums, left, right): # body_recursive_function:find_max (-> +6), function:find_max (-> +6), function_returning_something:find_max (-> +6), recursive_function:find_max (-> +6)
    ...
    left_max = find_max(nums, left, mid) # assignment, assignment_lhs_identifier:left_max, assignment_rhs_atom:find_max, assignment_rhs_atom:left, assignment_rhs_atom:mid, assignment_rhs_atom:nums, call_argument:left, call_argument:mid, call_argument:nums, function_call:find_max
    right_max = find_max(nums, mid + 1, right) # assignment, assignment_lhs_identifier:right_max, assignment_rhs_atom:1, assignment_rhs_atom:find_max, assignment_rhs_atom:mid, assignment_rhs_atom:nums, assignment_rhs_atom:right, binary_operator:Add, call_argument:, call_argument:nums, call_argument:right, function_call:find_max, int_literal, literal:Num
...
def find_min(nums, left, right): # body_recursive_function:find_min (-> +6), function:find_min (-> +6), function_returning_something:find_min (-> +6), recursive_function:find_min (-> +6)
    if left == right: # comparison_operator:Eq, if (-> +1), if_test_id:left, if_test_id:right
        return nums[left] # if_then_branch, index, return
    mid = (left + right) >> 1 # assignment, assignment_lhs_identifier:mid, assignment_rhs_atom:1, assignment_rhs_atom:left, assignment_rhs_atom:right, binary_operator:Add, binary_operator:RShift, int_literal, literal:Num
    left_min = find_min(nums, left, mid) # assignment, assignment_lhs_identifier:left_min, assignment_rhs_atom:find_min, assignment_rhs_atom:left, assignment_rhs_atom:mid, assignment_rhs_atom:nums, call_argument:left, call_argument:mid, call_argument:nums, function_call:find_min
    right_min = find_min(nums, mid + 1, right) # assignment, assignment_lhs_identifier:right_min, assignment_rhs_atom:1, assignment_rhs_atom:find_min, assignment_rhs_atom:mid, assignment_rhs_atom:nums, assignment_rhs_atom:right, binary_operator:Add, call_argument:, call_argument:nums, call_argument:right, function_call:find_min, int_literal, literal:Num
...
def gcd_by_iterative(x, y): # function:gcd_by_iterative (-> +3), function_returning_something:gcd_by_iterative (-> +3)
    while y: # while (-> +1)
        x, y = y, x % y # assignment, assignment_lhs_identifier:x, assignment_lhs_identifier:y, assignment_rhs_atom:x, assignment_rhs_atom:y, binary_operator:Mod
    return x # return:x
def main(): # function:main (-> +10), function_returning_nothing:main (-> +10)
    try: # try (-> +9), try_except:IndexError (-> +9), try_except:UnboundLocalError (-> +9), try_except:ValueError (-> +9)
        nums = input("Enter two integers separated by comma (,): ").split(",") # assignment, assignment_lhs_identifier:nums, assignment_rhs_atom:input, call_argument:, function_call:input, literal:Str, method_call:split
        num_1 = int(nums[0]) # assignment, assignment_lhs_identifier:num_1, assignment_rhs_atom:0, assignment_rhs_atom:int, assignment_rhs_atom:nums, call_argument:, function_call:int, index, int_literal, literal:Num
        num_2 = int(nums[1]) # assignment, assignment_lhs_identifier:num_2, assignment_rhs_atom:1, assignment_rhs_atom:int, assignment_rhs_atom:nums, call_argument:, function_call:int, index, int_literal, literal:Num
...
def exactPrimeFactorCount(n): # function:exactPrimeFactorCount (-> +15), function_returning_something:exactPrimeFactorCount (-> +15)
    ...
    if n % 2 == 0: # binary_operator:Mod, comparison_operator:Eq, divisibility_test:2, if (-> +3), if_test_id:n, int_literal, literal:Num
        count += 1 # assignment_lhs_identifier:count, assignment_rhs_atom:1, augmented_assignment, if_then_branch (-> +2), int_literal, literal:Num
    ...
    while i <= int(math.sqrt(n)): # call_argument:, call_argument:n, comparison_operator:LtE, composition, evolve_state (-> +5), function_call:int, method_call:sqrt, while (-> +5)
        if n % i == 0: # binary_operator:Mod, comparison_operator:Eq, divisibility_test, if (-> +3), if_test_id:i, if_test_id:n, int_literal, literal:Num
            ...
            while n % i == 0: # binary_operator:Mod, comparison_operator:Eq, divisibility_test, int_literal, literal:Num, while (-> +1)
                n = int(n / i) # assignment, assignment_lhs_identifier:n, assignment_rhs_atom:i, assignment_rhs_atom:int, assignment_rhs_atom:n, binary_operator:Div, call_argument:, function_call:int
        i = i + 2 # assignment, assignment_lhs_identifier:i, assignment_rhs_atom:2, assignment_rhs_atom:i, binary_operator:Add, int_literal, literal:Num, suggest_augmented_assignment
    if n > 2: # comparison_operator:Gt, if (-> +1), if_test_id:n, int_literal, literal:Num
        count += 1 # assignment_lhs_identifier:count, assignment_rhs_atom:1, augmented_assignment, if_then_branch, int_literal, literal:Num
...
def jaccard_similariy(setA, setB, alternativeUnion=False): # function:jaccard_similariy (-> +14), function_returning_something:jaccard_similariy (-> +14), function_with_default_positional_arguments:jaccard_similariy (-> +14), literal:False
    if isinstance(setA, set) and isinstance(setB, set): # boolean_operator:And, call_argument:set, call_argument:setA, call_argument:setB, function_call:isinstance, if (-> +6), if_test_id:isinstance, if_test_id:set, if_test_id:setA, if_test_id:setB
        intersection = len(setA.intersection(setB)) # assignment, assignment_lhs_identifier:intersection, assignment_rhs_atom:len, assignment_rhs_atom:setA, assignment_rhs_atom:setB, call_argument:, call_argument:setB, composition, function_call:len, if_then_branch (-> +5), method_call:intersection
        if alternativeUnion: # if (-> +3), nested_if:1 (-> +3), suggest_conditional_expression (-> +3)
            union = len(setA) + len(setB) # assignment, assignment_lhs_identifier:union, assignment_rhs_atom:len, assignment_rhs_atom:setA, assignment_rhs_atom:setB, binary_operator:Add, call_argument:setA, call_argument:setB, function_call:len, if_then_branch
        else:
            union = len(setA.union(setB)) # assignment, assignment_lhs_identifier:union, assignment_rhs_atom:len, assignment_rhs_atom:setA, assignment_rhs_atom:setB, call_argument:, call_argument:setB, composition, function_call:len, if_else_branch, method_call:union
...
def karatsuba(a, b): # body_recursive_function:karatsuba (-> +11), function:karatsuba (-> +11), function_returning_something:karatsuba (-> +11), recursive_function:karatsuba (-> +11)
    if len(str(a)) == 1 or len(str(b)) == 1: # boolean_operator:Or, call_argument:, call_argument:a, call_argument:b, comparison_operator:Eq, composition, function_call:len, function_call:str, if (-> +10), if_test_id:a, if_test_id:b, if_test_id:len, if_test_id:str, int_literal, literal:Num
        ...
        m1 = max(len(str(a)), len(str(b))) # assignment, assignment_lhs_identifier:m1, assignment_rhs_atom:a, assignment_rhs_atom:b, assignment_rhs_atom:len, assignment_rhs_atom:max, assignment_rhs_atom:str, call_argument:, call_argument:a, call_argument:b, composition, function_call:len, function_call:max, function_call:str, if_else_branch (-> +7)
        ...
        a1, a2 = divmod(a, 10 ** m2) # assignment, assignment_lhs_identifier:a1, assignment_lhs_identifier:a2, assignment_rhs_atom:10, assignment_rhs_atom:a, assignment_rhs_atom:divmod, assignment_rhs_atom:m2, binary_operator:Pow, call_argument:, call_argument:a, function_call:divmod, int_literal, literal:Num, suggest_constant_definition
        b1, b2 = divmod(b, 10 ** m2) # assignment, assignment_lhs_identifier:b1, assignment_lhs_identifier:b2, assignment_rhs_atom:10, assignment_rhs_atom:b, assignment_rhs_atom:divmod, assignment_rhs_atom:m2, binary_operator:Pow, call_argument:, call_argument:b, function_call:divmod, int_literal, literal:Num, suggest_constant_definition
        x = karatsuba(a2, b2) # assignment, assignment_lhs_identifier:x, assignment_rhs_atom:a2, assignment_rhs_atom:b2, assignment_rhs_atom:karatsuba, call_argument:a2, call_argument:b2, function_call:karatsuba
        y = karatsuba((a1 + a2), (b1 + b2)) # assignment, assignment_lhs_identifier:y, assignment_rhs_atom:a1, assignment_rhs_atom:a2, assignment_rhs_atom:b1, assignment_rhs_atom:b2, assignment_rhs_atom:karatsuba, binary_operator:Add, call_argument:, function_call:karatsuba
        z = karatsuba(a1, b1) # assignment, assignment_lhs_identifier:z, assignment_rhs_atom:a1, assignment_rhs_atom:b1, assignment_rhs_atom:karatsuba, call_argument:a1, call_argument:b1, function_call:karatsuba
...
def kthPermutation(k, n): # function:kthPermutation (-> +13), function_returning_something:kthPermutation (-> +13)
    ...
    for i in range(2, n): # call_argument:2, call_argument:n, for:i (-> +1), for_range:2:n (-> +1), function_call:range, int_literal, literal:Num, range:2:n
        factorials.append(factorials[-1] * i) # binary_operator:Mult, call_argument:, index, int_literal, literal:Num, method_call:append, method_call_object:factorials, negative_index:-1
    ...
    elements = list(range(n)) # assignment, assignment_lhs_identifier:elements, assignment_rhs_atom:list, assignment_rhs_atom:n, assignment_rhs_atom:range, call_argument:, call_argument:n, composition, function_call:list, function_call:range, range:n
    while factorials: # while (-> +4)
        factorial = factorials.pop() # assignment, assignment_lhs_identifier:factorial, assignment_rhs_atom:factorials, method_call:pop
        number, k = divmod(k, factorial) # assignment, assignment_lhs_identifier:k, assignment_lhs_identifier:number, assignment_rhs_atom:divmod, assignment_rhs_atom:factorial, assignment_rhs_atom:k, call_argument:factorial, call_argument:k, function_call:divmod
        permutation.append(elements[number]) # call_argument:, index, method_call:append, method_call_object:permutation
        elements.remove(elements[number]) # call_argument:, index, method_call:remove, method_call_object:elements
    permutation.append(elements[0]) # call_argument:, index, int_literal, literal:Num, method_call:append, method_call_object:permutation
...
def find_lcm(first_num: int, second_num: int) -> int: # function:find_lcm (-> +5), function_returning_something:find_lcm (-> +5)
    ...
    while (common_mult % first_num > 0) or (common_mult % second_num > 0): # binary_operator:Mod, boolean_operator:Or, comparison_operator:Gt, int_literal, literal:Num, while (-> +1)
        common_mult += max_num # assignment_lhs_identifier:common_mult, assignment_rhs_atom:max_num, augmented_assignment
...
class TestLeastCommonMultiple(unittest.TestCase):
    ...
    def test_lcm_function(self): # function:test_lcm_function (-> +4), function_returning_nothing:test_lcm_function (-> +4)
        for i, (first_num, second_num) in enumerate(self.test_inputs): # call_argument:, for:i:first_num:second_num (-> +3), for_indexes_elements (-> +3), function_call:enumerate
            actual_result = find_lcm(first_num, second_num) # assignment, assignment_lhs_identifier:actual_result, assignment_rhs_atom:find_lcm, assignment_rhs_atom:first_num, assignment_rhs_atom:second_num, call_argument:first_num, call_argument:second_num, function_call:find_lcm
            with self.subTest(i=i): # method_call:subTest
                self.assertEqual(actual_result, self.expected_results[i]) # call_argument:, call_argument:actual_result, index, method_call:assertEqual, method_call_object:self
...
class Matrix(object):
    def __init__(self, arg): # function:__init__ (-> +6), function_returning_nothing:__init__ (-> +6)
        if isinstance(arg, list): # call_argument:arg, call_argument:list, function_call:isinstance, if (-> +5), if_test_id:arg, if_test_id:isinstance, if_test_id:list
            ...
            self.n = len(arg) # assignment, assignment_lhs_identifier:self, assignment_rhs_atom:arg, assignment_rhs_atom:len, call_argument:arg, function_call:len
        else:
            self.n = arg # assignment, assignment_lhs_identifier:self, assignment_rhs_atom:arg, if_else_branch (-> +1)
            self.t = [[0 for _ in range(self.n)] for _ in range(self.n)] # assignment, assignment_lhs_identifier:self, assignment_rhs_atom:0, assignment_rhs_atom:_, assignment_rhs_atom:range, assignment_rhs_atom:self, call_argument:, comprehension:List, comprehension_for_count:1, function_call:range, int_literal, literal:Num, range:_
    def __mul__(self, b): # function:__mul__ (-> +6), function_returning_something:__mul__ (-> +6)
        matrix = Matrix(self.n) # assignment, assignment_lhs_identifier:matrix, assignment_rhs_atom:Matrix, assignment_rhs_atom:self, call_argument:, function_call:Matrix
...
def modular_exponentiation(a, b): # function:modular_exponentiation (-> +7), function_returning_something:modular_exponentiation (-> +7)
    matrix = Matrix([[1, 0], [0, 1]]) # assignment, assignment_lhs_identifier:matrix, assignment_rhs_atom:0, assignment_rhs_atom:1, assignment_rhs_atom:Matrix, call_argument:, function_call:Matrix, int_literal, literal:List, literal:Num
    while b > 0: # comparison_operator:Gt, evolve_state (-> +4), int_literal, literal:Num, while (-> +4)
        if b & 1: # binary_operator:BitAnd, if (-> +1), if_test_id:b, int_literal, literal:Num
            matrix *= a # assignment_lhs_identifier:matrix, assignment_rhs_atom:a, augmented_assignment, if_then_branch
        a *= a # assignment_lhs_identifier:a, assignment_rhs_atom:a, augmented_assignment
        b >>= 1 # assignment_lhs_identifier:b, assignment_rhs_atom:1, augmented_assignment, int_literal, literal:Num
    return matrix # return:matrix
def fibonacci_with_matrix_exponentiation(n, f1, f2): # function:fibonacci_with_matrix_exponentiation (-> +7), function_returning_something:fibonacci_with_matrix_exponentiation (-> +7)
    ...
    matrix = Matrix([[1, 1], [1, 0]]) # assignment, assignment_lhs_identifier:matrix, assignment_rhs_atom:0, assignment_rhs_atom:1, assignment_rhs_atom:Matrix, call_argument:, function_call:Matrix, int_literal, literal:List, literal:Num
    matrix = modular_exponentiation(matrix, n - 2) # assignment, assignment_lhs_identifier:matrix, assignment_rhs_atom:2, assignment_rhs_atom:matrix, assignment_rhs_atom:modular_exponentiation, assignment_rhs_atom:n, binary_operator:Sub, call_argument:, call_argument:matrix, function_call:modular_exponentiation, int_literal, literal:Num
...
def simple_fibonacci(n, f1, f2): # function:simple_fibonacci (-> +11), function_returning_something:simple_fibonacci (-> +11)
    ...
    n -= 2 # assignment_lhs_identifier:n, assignment_rhs_atom:2, augmented_assignment, int_literal, literal:Num
    while n > 0: # comparison_operator:Gt, evolve_state (-> +2), int_literal, literal:Num, while (-> +2)
        fn_1, fn_2 = fn_1 + fn_2, fn_1 # assignment, assignment_lhs_identifier:fn_1, assignment_lhs_identifier:fn_2, assignment_rhs_atom:fn_1, assignment_rhs_atom:fn_2, binary_operator:Add
        n -= 1 # assignment_lhs_identifier:n, assignment_rhs_atom:1, augmented_assignment, int_literal, literal:Num
...
""" # literal:Str
    ...
    exec_time = timeit.timeit(setup=setup, stmt=code, number=100) # assignment, assignment_lhs_identifier:exec_time, assignment_rhs_atom:100, assignment_rhs_atom:code, assignment_rhs_atom:setup, assignment_rhs_atom:timeit, int_literal, literal:Num, method_call:timeit, suggest_constant_definition
...
def mobius(n: int) -> int: # function:mobius (-> +4), function_returning_something:mobius (-> +4)
    factors = prime_factors(n) # assignment, assignment_lhs_identifier:factors, assignment_rhs_atom:n, assignment_rhs_atom:prime_factors, call_argument:n, function_call:prime_factors
...
def modular_exponential(base, power, mod): # function:modular_exponential (-> +10), function_returning_something:modular_exponential (-> +10)
    if power < 0: # comparison_operator:Lt, if (-> +1), if_test_id:power, int_literal, literal:Num
        return -1 # if_then_branch, int_literal, literal:Num, return:-1
    base %= mod # assignment_lhs_identifier:base, assignment_rhs_atom:mod, augmented_assignment
    result = 1 # assignment, assignment_lhs_identifier:result, assignment_rhs_atom:1, int_literal, literal:Num
    while power > 0: # comparison_operator:Gt, evolve_state (-> +4), int_literal, literal:Num, while (-> +4)
        if power & 1: # binary_operator:BitAnd, if (-> +1), if_test_id:power, int_literal, literal:Num
            result = (result * base) % mod # assignment, assignment_lhs_identifier:result, assignment_rhs_atom:base, assignment_rhs_atom:mod, assignment_rhs_atom:result, binary_operator:Mod, binary_operator:Mult, if_then_branch
        power = power >> 1 # assignment, assignment_lhs_identifier:power, assignment_rhs_atom:1, assignment_rhs_atom:power, binary_operator:RShift, int_literal, literal:Num, suggest_augmented_assignment
        base = (base * base) % mod # assignment, assignment_lhs_identifier:base, assignment_rhs_atom:base, assignment_rhs_atom:mod, binary_operator:Mod, binary_operator:Mult
    return result # return:result
def main(): # function:main (-> +1), function_returning_nothing:main (-> +1)
...
def newton_raphson(f, x0=0, maxiter=100, step=0.0001, maxerror=1e-6, logsteps=False): # float_literal, function:newton_raphson (-> +17), function_returning_something:newton_raphson (-> +17), function_with_default_positional_arguments:newton_raphson (-> +17), int_literal, literal:False, literal:Num
    ...
    error = abs(f(a)) # assignment, assignment_lhs_identifier:error, assignment_rhs_atom:a, assignment_rhs_atom:abs, assignment_rhs_atom:f, call_argument:, call_argument:a, composition, function_call:abs, function_call:f
    f1 = lambda x: calc_derivative(f, x, h=step) # assignment, assignment_lhs_identifier:f1, assignment_rhs_atom:calc_derivative, assignment_rhs_atom:f, assignment_rhs_atom:step, assignment_rhs_atom:x, call_argument:f, call_argument:x, function_call:calc_derivative, lambda_function
    for _ in range(maxiter): # call_argument:maxiter, for:_ (-> +9), for_range:maxiter (-> +9), function_call:range, range:maxiter
        ...
        a = a - f(a) / f1(a) # assignment, assignment_lhs_identifier:a, assignment_rhs_atom:a, assignment_rhs_atom:f, assignment_rhs_atom:f1, binary_operator:Div, binary_operator:Sub, call_argument:a, function_call:f, function_call:f1, suggest_augmented_assignment
        if logsteps: # if (-> +1)
            steps.append(a) # call_argument:a, if_then_branch, method_call:append, method_call_object:steps
...
def horner(poly: Sequence[float], x: float) -> float: # function:horner (-> +4), function_returning_something:horner (-> +4), index
    ...
    for coeff in reversed(poly): # accumulate_elements:1 (-> +1), call_argument:poly, for:coeff (-> +1), function_call:reversed
        result = result * x + coeff # assignment, assignment_lhs_identifier:result, assignment_rhs_atom:coeff, assignment_rhs_atom:result, assignment_rhs_atom:x, binary_operator:Add, binary_operator:Mult
    return result # return:result
...
def prime_check(number): # function:prime_check (-> +8), function_returning_something:prime_check (-> +8)
    ...
    if number % 2 == 0: # binary_operator:Mod, comparison_operator:Eq, divisibility_test:2, if (-> +1), if_test_id:number, int_literal, literal:Num
        return False # if_then_branch, literal:False, return:False
    odd_numbers = range(3, int(math.sqrt(number)) + 1, 2) # assignment, assignment_lhs_identifier:odd_numbers, assignment_rhs_atom:1, assignment_rhs_atom:2, assignment_rhs_atom:3, assignment_rhs_atom:int, assignment_rhs_atom:math, assignment_rhs_atom:number, assignment_rhs_atom:range, binary_operator:Add, call_argument:, call_argument:2, call_argument:3, call_argument:number, composition, function_call:int, function_call:range, int_literal, literal:Num, method_call:sqrt, range:3:_:2, suggest_constant_definition
...
def prime_factors(n: int) -> List[int]: # function:prime_factors (-> +11), function_returning_something:prime_factors (-> +11), index
    ...
    while i * i <= n: # binary_operator:Mult, comparison_operator:LtE, evolve_state (-> +5), while (-> +5)
        if n % i: # binary_operator:Mod, if (-> +4), if_test_id:i, if_test_id:n
            i += 1 # assignment_lhs_identifier:i, assignment_rhs_atom:1, augmented_assignment, if_then_branch, int_literal, literal:Num
        else:
            n //= i # assignment_lhs_identifier:n, assignment_rhs_atom:i, augmented_assignment, if_else_branch (-> +1)
            factors.append(i) # call_argument:i, method_call:append, method_call_object:factors
    if n > 1: # comparison_operator:Gt, if (-> +1), if_test_id:n, int_literal, literal:Num
        factors.append(n) # call_argument:n, if_then_branch, method_call:append, method_call_object:factors
...
def primes(max: int) -> List[int]: # function:primes (-> +9), function_returning_something:primes (-> +9), index
    max += 1 # assignment_lhs_identifier:max, assignment_rhs_atom:1, augmented_assignment, int_literal, literal:Num
    ...
    for i in range(2, max): # accumulate_elements:1 (-> +4), call_argument:2, call_argument:max, for:i (-> +4), for_range:2:max (-> +4), for_range:i:max:i (-> +4), function_call:range, int_literal, literal:Num, range:2:max
        if not numbers[i]: # if (-> +3), if_test_id:i, if_test_id:numbers, index, unary_operator:Not
            ...
            ret.append(i) # call_argument:i, method_call:append, method_call_object:ret
...
def prime_sieve_eratosthenes(num): # function:prime_sieve_eratosthenes (-> +10), function_returning_nothing:prime_sieve_eratosthenes (-> +10)
    ...
    while p * p <= num: # binary_operator:Mult, comparison_operator:LtE, while (-> +4)
        ...
        p += 1 # assignment_lhs_identifier:p, assignment_rhs_atom:1, augmented_assignment, int_literal, literal:Num
...
def qr_householder(A): # function:qr_householder (-> +16), function_returning_something:qr_householder (-> +16)
    ...
    t = min(m, n) # assignment, assignment_lhs_identifier:t, assignment_rhs_atom:m, assignment_rhs_atom:min, assignment_rhs_atom:n, call_argument:m, call_argument:n, function_call:min
    Q = np.eye(m) # assignment, assignment_lhs_identifier:Q, assignment_rhs_atom:m, assignment_rhs_atom:np, call_argument:m, method_call:eye
    R = A.copy() # assignment, assignment_lhs_identifier:R, assignment_rhs_atom:A, method_call:copy
    for k in range(t - 1): # accumulate_elements:1 (-> +10), binary_operator:Sub, call_argument:, for:k (-> +10), for_range:_ (-> +10), function_call:range, int_literal, literal:Num, range:_
        ...
        e1 = np.zeros_like(x) # assignment, assignment_lhs_identifier:e1, assignment_rhs_atom:np, assignment_rhs_atom:x, call_argument:x, method_call:zeros_like
        ...
        alpha = np.linalg.norm(x) # assignment, assignment_lhs_identifier:alpha, assignment_rhs_atom:np, assignment_rhs_atom:x, call_argument:x, method_call:norm
        v = x + np.sign(x[0]) * alpha * e1 # assignment, assignment_lhs_identifier:v, assignment_rhs_atom:0, assignment_rhs_atom:alpha, assignment_rhs_atom:e1, assignment_rhs_atom:np, assignment_rhs_atom:x, binary_operator:Add, binary_operator:Mult, call_argument:, index, int_literal, literal:Num, method_call:sign
        v /= np.linalg.norm(v) # assignment_lhs_identifier:v, assignment_rhs_atom:np, assignment_rhs_atom:v, augmented_assignment, call_argument:v, method_call:norm
        ...
        Q = Q @ Q_k.T # assignment, assignment_lhs_identifier:Q, assignment_rhs_atom:Q, assignment_rhs_atom:Q_k, binary_operator:MatMult, suggest_augmented_assignment
        R = Q_k @ R # assignment, assignment_lhs_identifier:R, assignment_rhs_atom:Q_k, assignment_rhs_atom:R, binary_operator:MatMult
    return Q, R # return
...
def QuadraticEquation(a: int, b: int, c: int) -> Tuple[str, str]: # function:QuadraticEquation (-> +10), function_returning_something:QuadraticEquation (-> +10), index
    ...
    snd = sqrt(-delta) # assignment, assignment_lhs_identifier:snd, assignment_rhs_atom:delta, assignment_rhs_atom:sqrt, call_argument:, function_call:sqrt, unary_operator:USub
    ...
    b = -abs(b) # assignment, assignment_lhs_identifier:b, assignment_rhs_atom:abs, assignment_rhs_atom:b, call_argument:b, function_call:abs, unary_operator:USub
    return f"({b}+{snd} * i) / 2", f"({b}+{snd} * i) / {2 * a}" # binary_operator:Mult, int_literal, literal:Num, literal:Str, return
def main(): # function:main (-> +2), function_returning_nothing:main (-> +2)
    solutions = QuadraticEquation(a=5, b=6, c=1) # assignment, assignment_lhs_identifier:solutions, assignment_rhs_atom:1, assignment_rhs_atom:5, assignment_rhs_atom:6, assignment_rhs_atom:QuadraticEquation, function_call:QuadraticEquation, int_literal, literal:Num, suggest_constant_definition
...
class FFT:
    def __init__(self, polyA=[0], polyB=[0]): # function:__init__ (-> +17), function_returning_nothing:__init__ (-> +17), function_with_default_positional_arguments:__init__ (-> +17), int_literal, literal:List, literal:Num
        self.polyA = list(polyA)[:] # assignment, assignment_lhs_identifier:self, assignment_rhs_atom:list, assignment_rhs_atom:polyA, call_argument:polyA, function_call:list, slice
        self.polyB = list(polyB)[:] # assignment, assignment_lhs_identifier:self, assignment_rhs_atom:list, assignment_rhs_atom:polyB, call_argument:polyB, function_call:list, slice
        ...
        self.len_A = len(self.polyA) # assignment, assignment_lhs_identifier:self, assignment_rhs_atom:len, assignment_rhs_atom:self, call_argument:, function_call:len
        ...
        self.len_B = len(self.polyB) # assignment, assignment_lhs_identifier:self, assignment_rhs_atom:len, assignment_rhs_atom:self, call_argument:, function_call:len
        self.C_max_length = int( # assignment, assignment_lhs_identifier:self, assignment_rhs_atom:int, composition, function_call:int
            2 ** np.ceil(np.log2(len(self.polyA) + len(self.polyB) - 1)) # assignment_rhs_atom:1, assignment_rhs_atom:2, assignment_rhs_atom:len, assignment_rhs_atom:np, assignment_rhs_atom:self, binary_operator:Add, binary_operator:Pow, binary_operator:Sub, call_argument:, composition, function_call:len, int_literal, literal:Num, method_call:ceil, method_call:log2
        )
        while len(self.polyA) < self.C_max_length: # call_argument:, comparison_operator:Lt, evolve_state (-> +1), function_call:len, while (-> +1)
            self.polyA.append(0) # call_argument:0, int_literal, literal:Num, method_call:append
        while len(self.polyB) < self.C_max_length: # call_argument:, comparison_operator:Lt, evolve_state (-> +1), function_call:len, while (-> +1)
            self.polyB.append(0) # call_argument:0, int_literal, literal:Num, method_call:append
        self.root = complex(mpmath.root(x=1, n=self.C_max_length, k=1)) # assignment, assignment_lhs_identifier:self, assignment_rhs_atom:1, assignment_rhs_atom:complex, assignment_rhs_atom:mpmath, assignment_rhs_atom:self, call_argument:, composition, function_call:complex, int_literal, literal:Num, method_call:root
    ...
    def __DFT(self, which): # function:__DFT (-> +23), function_returning_something:__DFT (-> +23)
        ...
        while next_ncol > 0: # comparison_operator:Gt, evolve_state (-> +14), int_literal, literal:Num, while (-> +14)
            new_dft = [[] for i in range(next_ncol)] # assignment, assignment_lhs_identifier:new_dft, assignment_rhs_atom:i, assignment_rhs_atom:next_ncol, assignment_rhs_atom:range, call_argument:next_ncol, comprehension:List, comprehension_for_count:1, function_call:range, literal:List, range:next_ncol
            ...
            for j in range(self.C_max_length // (next_ncol * 2)): # binary_operator:FloorDiv, binary_operator:Mult, call_argument:, for:j (-> +3), for_range:_ (-> +3), for_range:next_ncol (-> +3), function_call:range, int_literal, literal:Num, range:_
                for i in range(next_ncol): # call_argument:next_ncol, for:i (-> +1), for_range:next_ncol (-> +1), function_call:range, nested_for:1 (-> +1), range:next_ncol
                    new_dft[i].append(dft[i][j] + current_root * dft[i + next_ncol][j]) # binary_operator:Add, binary_operator:Mult, call_argument:, index, index_arithmetic, method_call:append
                current_root *= root # assignment_lhs_identifier:current_root, assignment_rhs_atom:root, augmented_assignment
            current_root = 1 # assignment, assignment_lhs_identifier:current_root, assignment_rhs_atom:1, int_literal, literal:Num
            for j in range(self.C_max_length // (next_ncol * 2)): # binary_operator:FloorDiv, binary_operator:Mult, call_argument:, for:j (-> +3), for_range:_ (-> +3), for_range:next_ncol (-> +3), function_call:range, int_literal, literal:Num, range:_
                for i in range(next_ncol): # call_argument:next_ncol, for:i (-> +1), for_range:next_ncol (-> +1), function_call:range, nested_for:1 (-> +1), range:next_ncol
                    new_dft[i].append(dft[i][j] - current_root * dft[i + next_ncol][j]) # binary_operator:Add, binary_operator:Mult, binary_operator:Sub, call_argument:, index, index_arithmetic, method_call:append
                current_root *= root # assignment_lhs_identifier:current_root, assignment_rhs_atom:root, augmented_assignment
            dft = new_dft # assignment, assignment_lhs_identifier:dft, assignment_rhs_atom:new_dft
            next_ncol = next_ncol // 2 # assignment, assignment_lhs_identifier:next_ncol, assignment_rhs_atom:2, assignment_rhs_atom:next_ncol, binary_operator:FloorDiv, int_literal, literal:Num, suggest_augmented_assignment
    ...
    def __multiply(self): # function:__multiply (-> +35), function_returning_something:__multiply (-> +35)
        dftA = self.__DFT("A") # assignment, assignment_lhs_identifier:dftA, assignment_rhs_atom:self, call_argument:, literal:Str, method_call:__DFT
        dftB = self.__DFT("B") # assignment, assignment_lhs_identifier:dftB, assignment_rhs_atom:self, call_argument:, literal:Str, method_call:__DFT
        inverseC = [[dftA[i] * dftB[i] for i in range(self.C_max_length)]] # assignment, assignment_lhs_identifier:inverseC, assignment_rhs_atom:dftA, assignment_rhs_atom:dftB, assignment_rhs_atom:i, assignment_rhs_atom:range, assignment_rhs_atom:self, binary_operator:Mult, call_argument:, comprehension:List, comprehension_for_count:1, function_call:range, index, range:_
        del dftA
        del dftB
        ...
        while next_ncol <= self.C_max_length: # comparison_operator:LtE, evolve_state (-> +22), while (-> +22)
            new_inverseC = [[] for i in range(next_ncol)] # assignment, assignment_lhs_identifier:new_inverseC, assignment_rhs_atom:i, assignment_rhs_atom:next_ncol, assignment_rhs_atom:range, call_argument:next_ncol, comprehension:List, comprehension_for_count:1, function_call:range, literal:List, range:next_ncol
            ...
            next_ncol *= 2 # assignment_lhs_identifier:next_ncol, assignment_rhs_atom:2, augmented_assignment, int_literal, literal:Num
        inverseC = [round(x[0].real, 8) + round(x[0].imag, 8) * 1j for x in inverseC] # assignment, assignment_lhs_identifier:inverseC, assignment_rhs_atom:0, assignment_rhs_atom:1j, assignment_rhs_atom:8, assignment_rhs_atom:inverseC, assignment_rhs_atom:round, assignment_rhs_atom:x, binary_operator:Add, binary_operator:Mult, call_argument:, call_argument:8, comprehension:List, comprehension_for_count:1, function_call:round, imaginary_literal, index, int_literal, literal:Num, suggest_constant_definition
    ...
    def __str__(self): # function:__str__ (-> +10), function_returning_something:__str__ (-> +10)
        A = "A = " + " + ".join( # assignment, assignment_lhs_identifier:A, binary_operator:Add, composition, literal:Str, method_call:join
            f"{coef}*x^{i}" for coef, i in enumerate(self.polyA[: self.len_A]) # assignment_rhs_atom:coef, assignment_rhs_atom:enumerate, assignment_rhs_atom:i, assignment_rhs_atom:self, call_argument:, comprehension:Generator, comprehension_for_count:1, function_call:enumerate, literal:Str, slice
        )
        B = "B = " + " + ".join( # assignment, assignment_lhs_identifier:B, binary_operator:Add, composition, literal:Str, method_call:join
            f"{coef}*x^{i}" for coef, i in enumerate(self.polyB[: self.len_B]) # assignment_rhs_atom:coef, assignment_rhs_atom:enumerate, assignment_rhs_atom:i, assignment_rhs_atom:self, call_argument:, comprehension:Generator, comprehension_for_count:1, function_call:enumerate, literal:Str, slice
        )
        C = "A*B = " + " + ".join( # assignment, assignment_lhs_identifier:C, binary_operator:Add, composition, literal:Str, method_call:join
            f"{coef}*x^{i}" for coef, i in enumerate(self.product) # assignment_rhs_atom:coef, assignment_rhs_atom:enumerate, assignment_rhs_atom:i, assignment_rhs_atom:self, call_argument:, comprehension:Generator, comprehension_for_count:1, function_call:enumerate, literal:Str
        )
...
def runge_kutta(f, y0, x0, h, x_end): # function:runge_kutta (-> +12), function_returning_something:runge_kutta (-> +12)
    N = int(np.ceil((x_end - x0) / h)) # assignment, assignment_lhs_identifier:N, assignment_rhs_atom:h, assignment_rhs_atom:int, assignment_rhs_atom:np, assignment_rhs_atom:x0, assignment_rhs_atom:x_end, binary_operator:Div, binary_operator:Sub, call_argument:, composition, function_call:int, method_call:ceil
    y = np.zeros((N + 1,)) # assignment, assignment_lhs_identifier:y, assignment_rhs_atom:1, assignment_rhs_atom:N, assignment_rhs_atom:np, binary_operator:Add, call_argument:, int_literal, literal:Num, method_call:zeros
    y[0] = y0 # assignment, assignment_lhs_identifier:y, assignment_rhs_atom:y0, index, int_literal, literal:Num
    x = x0 # assignment, assignment_lhs_identifier:x, assignment_rhs_atom:x0
    for k in range(N): # accumulate_elements:1 (-> +6), call_argument:N, for:k (-> +6), for_range:N (-> +6), function_call:range, range:N
        k1 = f(x, y[k]) # assignment, assignment_lhs_identifier:k1, assignment_rhs_atom:f, assignment_rhs_atom:k, assignment_rhs_atom:x, assignment_rhs_atom:y, call_argument:, call_argument:x, function_call:f, index
        k2 = f(x + 0.5 * h, y[k] + 0.5 * h * k1) # assignment, assignment_lhs_identifier:k2, assignment_rhs_atom:0.5, assignment_rhs_atom:f, assignment_rhs_atom:h, assignment_rhs_atom:k, assignment_rhs_atom:k1, assignment_rhs_atom:x, assignment_rhs_atom:y, binary_operator:Add, binary_operator:Mult, call_argument:, float_literal, function_call:f, index, literal:Num, suggest_constant_definition
        k3 = f(x + 0.5 * h, y[k] + 0.5 * h * k2) # assignment, assignment_lhs_identifier:k3, assignment_rhs_atom:0.5, assignment_rhs_atom:f, assignment_rhs_atom:h, assignment_rhs_atom:k, assignment_rhs_atom:k2, assignment_rhs_atom:x, assignment_rhs_atom:y, binary_operator:Add, binary_operator:Mult, call_argument:, float_literal, function_call:f, index, literal:Num, suggest_constant_definition
        k4 = f(x + h, y[k] + h * k3) # assignment, assignment_lhs_identifier:k4, assignment_rhs_atom:f, assignment_rhs_atom:h, assignment_rhs_atom:k, assignment_rhs_atom:k3, assignment_rhs_atom:x, assignment_rhs_atom:y, binary_operator:Add, binary_operator:Mult, call_argument:, function_call:f, index
        ...
        x += h # assignment_lhs_identifier:x, assignment_rhs_atom:h, augmented_assignment
    return y # return:y
...
def sieve(n): # function:sieve (-> +33), function_returning_something:sieve (-> +33)
    ...
    end = int(math.sqrt(n)) # assignment, assignment_lhs_identifier:end, assignment_rhs_atom:int, assignment_rhs_atom:math, assignment_rhs_atom:n, call_argument:, call_argument:n, composition, function_call:int, method_call:sqrt
    ...
    while start <= end: # comparison_operator:LtE, while (-> +6)
        if temp[start] is True: # comparison_operator:Is, if (-> +4), if_test_id:start, if_test_id:temp, index, literal:True
            in_prime.append(start) # call_argument:start, if_then_branch (-> +3), method_call:append, method_call_object:in_prime
        ...
        start += 1 # assignment_lhs_identifier:start, assignment_rhs_atom:1, augmented_assignment, int_literal, literal:Num
    prime += in_prime # assignment_lhs_identifier:prime, assignment_rhs_atom:in_prime, augmented_assignment
    ...
    while low <= n: # comparison_operator:LtE, while (-> +14)
        ...
        for each in in_prime: # accumulate_elements:1 (-> +5), for:each (-> +5), for_each (-> +5), for_range:t:_:each (-> +5)
            t = math.floor(low / each) * each # assignment, assignment_lhs_identifier:t, assignment_rhs_atom:each, assignment_rhs_atom:low, assignment_rhs_atom:math, binary_operator:Div, binary_operator:Mult, call_argument:, method_call:floor
            if t < low: # comparison_operator:Lt, if (-> +1), if_test_id:low, if_test_id:t
                t += each # assignment_lhs_identifier:t, assignment_rhs_atom:each, augmented_assignment, if_then_branch
...
def sieve(n): # function:sieve (-> +15), function_returning_something:sieve (-> +15)
    ...
    while start <= end: # comparison_operator:LtE, while (-> +6)
        if l[start] is True: # comparison_operator:Is, if (-> +4), if_test_id:l, if_test_id:start, index, literal:True
            prime.append(start) # call_argument:start, if_then_branch (-> +3), method_call:append, method_call_object:prime
    ...
    for j in range(end + 1, n + 1): # accumulate_elements:1 (-> +2), binary_operator:Add, call_argument:, for:j (-> +2), for_range:_:_ (-> +2), function_call:range, int_literal, literal:Num, range:_:_
        if l[j] is True: # comparison_operator:Is, if (-> +1), if_test_id:j, if_test_id:l, index, literal:True
            prime.append(j) # call_argument:j, if_then_branch, method_call:append, method_call_object:prime
    return prime # return:prime
...
def method_2(boundary, steps): # function:method_2 (-> +12), function_returning_something:method_2 (-> +12)
    ...
    x_i = make_points(a, b, h) # assignment, assignment_lhs_identifier:x_i, assignment_rhs_atom:a, assignment_rhs_atom:b, assignment_rhs_atom:h, assignment_rhs_atom:make_points, call_argument:a, call_argument:b, call_argument:h, function_call:make_points
    ...
    y += (h / 3.0) * f(a) # assignment_lhs_identifier:y, assignment_rhs_atom:3.0, assignment_rhs_atom:a, assignment_rhs_atom:f, assignment_rhs_atom:h, augmented_assignment, binary_operator:Div, binary_operator:Mult, call_argument:a, float_literal, function_call:f, literal:Num, suggest_constant_definition
    ...
    for i in x_i: # accumulate_elements:1 (-> +2), for:i (-> +2), for_each (-> +2)
        y += (h / 3) * (4 - 2 * (cnt % 2)) * f(i) # assignment_lhs_identifier:y, assignment_rhs_atom:2, assignment_rhs_atom:3, assignment_rhs_atom:4, assignment_rhs_atom:cnt, assignment_rhs_atom:f, assignment_rhs_atom:h, assignment_rhs_atom:i, augmented_assignment, binary_operator:Div, binary_operator:Mod, binary_operator:Mult, binary_operator:Sub, call_argument:i, function_call:f, int_literal, literal:Num, suggest_constant_definition
        cnt += 1 # assignment_lhs_identifier:cnt, assignment_rhs_atom:1, augmented_assignment, int_literal, literal:Num
    y += (h / 3.0) * f(b) # assignment_lhs_identifier:y, assignment_rhs_atom:3.0, assignment_rhs_atom:b, assignment_rhs_atom:f, assignment_rhs_atom:h, augmented_assignment, binary_operator:Div, binary_operator:Mult, call_argument:b, float_literal, function_call:f, literal:Num, suggest_constant_definition
    return y # return:y
def make_points(a, b, h): # function:make_points (-> +4), generator:make_points (-> +4)
    ...
    while x < (b - h): # binary_operator:Sub, comparison_operator:Lt, while (-> +2)
        yield x # yield:x
        x = x + h # assignment, assignment_lhs_identifier:x, assignment_rhs_atom:h, assignment_rhs_atom:x, binary_operator:Add, suggest_augmented_assignment
...
def main(): # function:main (-> +6), function_returning_nothing:main (-> +6)
    ...
    y = method_2(boundary, steps) # assignment, assignment_lhs_identifier:y, assignment_rhs_atom:boundary, assignment_rhs_atom:method_2, assignment_rhs_atom:steps, call_argument:boundary, call_argument:steps, function_call:method_2
...
def softmax(vector): # function:softmax (-> +4), function_returning_something:softmax (-> +4)
    exponentVector = np.exp(vector) # assignment, assignment_lhs_identifier:exponentVector, assignment_rhs_atom:np, assignment_rhs_atom:vector, call_argument:vector, method_call:exp
    sumOfExponents = np.sum(exponentVector) # assignment, assignment_lhs_identifier:sumOfExponents, assignment_rhs_atom:exponentVector, assignment_rhs_atom:np, call_argument:exponentVector, method_call:sum
...
def method_1(boundary, steps): # function:method_1 (-> +10), function_returning_something:method_1 (-> +10)
    ...
    x_i = make_points(a, b, h) # assignment, assignment_lhs_identifier:x_i, assignment_rhs_atom:a, assignment_rhs_atom:b, assignment_rhs_atom:h, assignment_rhs_atom:make_points, call_argument:a, call_argument:b, call_argument:h, function_call:make_points
    y = 0.0 # assignment, assignment_lhs_identifier:y, assignment_rhs_atom:0.0, float_literal, literal:Num, suggest_constant_definition
    y += (h / 2.0) * f(a) # assignment_lhs_identifier:y, assignment_rhs_atom:2.0, assignment_rhs_atom:a, assignment_rhs_atom:f, assignment_rhs_atom:h, augmented_assignment, binary_operator:Div, binary_operator:Mult, call_argument:a, float_literal, function_call:f, literal:Num, suggest_constant_definition
    for i in x_i: # accumulate_elements:1 (-> +1), for:i (-> +1), for_each (-> +1)
        y += h * f(i) # assignment_lhs_identifier:y, assignment_rhs_atom:f, assignment_rhs_atom:h, assignment_rhs_atom:i, augmented_assignment, binary_operator:Mult, call_argument:i, function_call:f
    y += (h / 2.0) * f(b) # assignment_lhs_identifier:y, assignment_rhs_atom:2.0, assignment_rhs_atom:b, assignment_rhs_atom:f, assignment_rhs_atom:h, augmented_assignment, binary_operator:Div, binary_operator:Mult, call_argument:b, float_literal, function_call:f, literal:Num, suggest_constant_definition
    return y # return:y
def make_points(a, b, h): # function:make_points (-> +4), generator:make_points (-> +4)
    x = a + h # assignment, assignment_lhs_identifier:x, assignment_rhs_atom:a, assignment_rhs_atom:h, binary_operator:Add
    while x < (b - h): # binary_operator:Sub, comparison_operator:Lt, while (-> +2)
...
def main(): # function:main (-> +6), function_returning_nothing:main (-> +6)
    ...
    steps = 10.0 # assignment, assignment_lhs_identifier:steps, assignment_rhs_atom:10.0, float_literal, literal:Num, suggest_constant_definition
    boundary = [a, b] # assignment, assignment_lhs_identifier:boundary, assignment_rhs_atom:a, assignment_rhs_atom:b
    y = method_1(boundary, steps) # assignment, assignment_lhs_identifier:y, assignment_rhs_atom:boundary, assignment_rhs_atom:method_1, assignment_rhs_atom:steps, call_argument:boundary, call_argument:steps, function_call:method_1
    print("y = {0}".format(y)) # call_argument:, call_argument:y, composition, function_call:print, literal:Str, method_call:format
...
def zeller(date_input: str) -> str: # function:zeller (-> +46), function_returning_something:zeller (-> +46)
    ...
    dt_ck = datetime.date(int(y), int(m), int(d)) # assignment, assignment_lhs_identifier:dt_ck, assignment_rhs_atom:d, assignment_rhs_atom:datetime, assignment_rhs_atom:int, assignment_rhs_atom:m, assignment_rhs_atom:y, call_argument:, call_argument:d, call_argument:m, call_argument:y, composition, function_call:int, method_call:date
    if m <= 2: # comparison_operator:LtE, if (-> +2), if_test_id:m, int_literal, literal:Num
        y = y - 1 # assignment, assignment_lhs_identifier:y, assignment_rhs_atom:1, assignment_rhs_atom:y, binary_operator:Sub, if_then_branch (-> +1), int_literal, literal:Num, suggest_augmented_assignment
        m = m + 12 # assignment, assignment_lhs_identifier:m, assignment_rhs_atom:12, assignment_rhs_atom:m, binary_operator:Add, int_literal, literal:Num, suggest_augmented_assignment, suggest_constant_definition

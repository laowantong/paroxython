...
def test_init():
    ...
    assert set(dbf.recommended_programs.keys()) == {
        "assignment.py",
        "collatz_print.py",
        "fizzbuzz.py",
        "is_even.py",
...
def test_programs_of_taxons():
    ...
    # This corresponds to follow_import=False. It is indirectly featured by fizzbuzz.py (which
    # imports collatz_print.py) and by is_even.py (which imports fizzbuzz.py). Their addition
    # corresponds to follow_import=True.

    programs = dbf.programs_of_taxons(taxons, follow_import=False)
    print(set(programs))
    assert set(programs) == {"assignment.py", "collatz_print.py"}

    programs = dbf.programs_of_taxons(taxons, follow_import=True)
    print(set(programs))
    assert set(programs) == {
        "assignment.py",
        "collatz_print.py",
...
def test_exclude_taxons():
    ...
    assert set(dbf.recommended_programs.keys()) == set()
    ...
    print(set(dbf.recommended_programs.keys()))
    assert set(dbf.recommended_programs.keys()) == {"assignment.py"}
    ...
    print(set(dbf.recommended_programs.keys()))
    assert set(dbf.recommended_programs.keys()) == {"assignment.py", "collatz_print.py"}
...
def test_include_taxons():
    ...
    print(set(dbf.recommended_programs.keys()))
    assert set(dbf.recommended_programs.keys()) == {"collatz_print.py"}
    ...
    print(set(dbf.recommended_programs.keys()))
    assert set(dbf.recommended_programs.keys()) == {"collatz_print.py", "fizzbuzz.py"}
...
def test_exclude_programs():
    ...
    # The program is_even.py is not imported. Excluding it has no other effect on recommendations.
    dbf = ProgramFilter(db)
...
def test_include_programs():
    ...
    # The program collatz_print.py is self-contained. Including it has no other effect on
    # the recommendations.
    dbf = ProgramFilter(db)
    ...
    print(set(dbf.recommended_programs.keys()))
    assert set(dbf.recommended_programs.keys()) == {"fizzbuzz.py", "collatz_print.py"}

    # The program is_even.py import collatz_print.py, which imports collatz_print.py. Including
    # the former includes the three of them.
    dbf = ProgramFilter(db)
...
def test_impart_programs():
    ...
    # The program collatz_print.py is self-contained. Imparting it excludes it from the
    # recommendations, but has no other effect.
    dbf = ProgramFilter(db)
    ...
    # latter has already been studied, and must be excluded from the recommendations.
    dbf = ProgramFilter(db)

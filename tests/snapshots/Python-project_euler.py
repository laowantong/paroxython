...
# problem_01/sol1.py
# ----------------------------------------------------------------------------------------
def solution(n): # function:solution (-> +1), function_argument:n, function_argument_flavor:arg, function_returning_something:solution (-> +1), whole_span:2 (-> +1)
    return sum([e for e in range(3, n) if e % 3 == 0 or e % 5 == 0]) # binary_operator:Mod, boolean_operator:Or, call_argument:, call_argument:3, call_argument:n, comparison_operator:Eq, composition, comprehension:List, comprehension_for_count:1, divisibility_test:3, divisibility_test:5, external_free_call:range, external_free_call:sum, filtered_comprehension, free_call:range, free_call:sum, free_tail_call:sum, literal:0, literal:3, literal:5, modulo_operator, range:3:n, return, suggest_constant_definition

# ----------------------------------------------------------------------------------------
# problem_01/sol2.py
# ----------------------------------------------------------------------------------------
def solution(n): # function:solution (-> +8), function_argument:n, function_argument_flavor:arg, function_returning_something:solution (-> +8), whole_span:9 (-> +8)
...
# problem_01/sol3.py
# ----------------------------------------------------------------------------------------
def solution(n): # function:solution (-> +32), function_argument:n, function_argument_flavor:arg, function_returning_something:solution (-> +32), whole_span:33 (-> +32)
    sum = 0 # assignment:0, assignment_lhs_identifier:sum, assignment_rhs_atom:0, literal:0, single_assignment:sum
    num = 0 # assignment:0, assignment_lhs_identifier:num, assignment_rhs_atom:0, literal:0, single_assignment:num
    while 1: # infinite_while (-> +28), literal:1, loop:while (-> +28), loop_with_break:while (-> +28), loop_with_early_exit:while:break (-> +28), while (-> +28)
...
# problem_01/sol4.py
# ----------------------------------------------------------------------------------------
def solution(n): # function:solution (-> +22), function_argument:n, function_argument_flavor:arg, function_returning_something:solution (-> +22), whole_span:23 (-> +22)
    ...
    while True: # infinite_while (-> +7), literal:True, loop:while (-> +7), loop_with_break:while (-> +7), loop_with_early_exit:while:break (-> +7), while (-> +7)
        ...
        if result < n: # comparison_operator:Lt, if (-> +5), if_test_atom:n, if_test_atom:result
            zmulti.append(result) # call_argument:result, if_then_branch (-> +1), member_call:append, member_call_object:zmulti, member_call_without_result:append, update:zmulti:result, update_by_member_call:zmulti:result, update_by_member_call_with:append, update_with:append
            ...
            break # break
    while True: # count_states:temp (-> +6), infinite_while (-> +6), literal:True, loop:while (-> +6), loop_with_break:while (-> +6), loop_with_early_exit:while:break (-> +6), while (-> +6)
        ...
        if result < n: # comparison_operator:Lt, if (-> +4), if_test_atom:n, if_test_atom:result
            xmulti.append(result) # call_argument:result, if_then_branch (-> +1), member_call:append, member_call_object:xmulti, member_call_without_result:append, update:xmulti:result, update_by_member_call:xmulti:result, update_by_member_call_with:append, update_with:append
            temp += 1 # assignment_lhs_identifier:temp, assignment_rhs_atom:1, augmented_assignment:Add, increment:temp, literal:1, update:temp:1, update_by_augmented_assignment:temp:1, update_by_augmented_assignment_with:Add, update_with:Add
        else:
            break # break, if_else_branch
    collection = list(set(xmulti + zmulti)) # addition_operator, assignment:list, assignment_lhs_identifier:collection, assignment_rhs_atom:xmulti, assignment_rhs_atom:zmulti, binary_operator:Add, call_argument:, composition, external_free_call:list, external_free_call:set, free_call:list, free_call:set, single_assignment:collection
    return sum(collection) # call_argument:collection, external_free_call:sum, free_call:sum, free_tail_call:sum, return

# ----------------------------------------------------------------------------------------
# problem_01/sol5.py
# ----------------------------------------------------------------------------------------
def solution(n): # function:solution (-> +1), function_argument:n, function_argument_flavor:arg, function_returning_something:solution (-> +1), whole_span:2 (-> +1)
    return sum([i for i in range(n) if i % 3 == 0 or i % 5 == 0]) # binary_operator:Mod, boolean_operator:Or, call_argument:, call_argument:n, comparison_operator:Eq, composition, comprehension:List, comprehension_for_count:1, divisibility_test:3, divisibility_test:5, external_free_call:range, external_free_call:sum, filtered_comprehension, free_call:range, free_call:sum, free_tail_call:sum, literal:0, literal:3, literal:5, modulo_operator, range:n, return, suggest_constant_definition

# ----------------------------------------------------------------------------------------
# problem_01/sol6.py
# ----------------------------------------------------------------------------------------
def solution(n): # function:solution (-> +9), function_argument:n, function_argument_flavor:arg, function_returning_something:solution (-> +9), whole_span:10 (-> +9)
    ...
    while a < n: # comparison_operator:Lt, count_states:a (-> +5), loop:while (-> +5), loop_with_late_exit:while (-> +5), while (-> +5)
...
# problem_01/sol7.py
# ----------------------------------------------------------------------------------------
def solution(n): # function:solution (-> +7), function_argument:n, function_argument_flavor:arg, function_returning_something:solution (-> +7), whole_span:8 (-> +7)
    result = 0 # assignment:0, assignment_lhs_identifier:result, assignment_rhs_atom:0, literal:0, single_assignment:result
    for i in range(n): # accumulate_elements:Add (-> +4), accumulate_some_elements:Add (-> +4), call_argument:n, external_free_call:range, for:i (-> +4), for_range:n (-> +4), free_call:range, loop:for (-> +4), loop_with_late_exit:for (-> +4), range:n
...
# problem_02/sol4.py
# ----------------------------------------------------------------------------------------
import math # import:math, import_module:math, whole_span:15 (-> +14)
...
def solution(n): # function:solution (-> +12), function_argument:n, function_argument_flavor:arg, function_returning_something:solution (-> +12)
    ...
    getcontext().prec = 100 # assignment:100, assignment_rhs_atom:100, external_free_call:getcontext, free_call:getcontext, free_call_without_arguments:getcontext, literal:100, suggest_constant_definition
    phi = (Decimal(5) ** Decimal(0.5) + 1) / Decimal(2) # addition_operator, assignment:Div, assignment_lhs_identifier:phi, assignment_rhs_atom:0.5, assignment_rhs_atom:1, assignment_rhs_atom:2, assignment_rhs_atom:5, binary_operator:Add, binary_operator:Div, binary_operator:Pow, call_argument:0.5, call_argument:2, call_argument:5, external_free_call:Decimal, free_call:Decimal, literal:0.5, literal:1, literal:2, literal:5, single_assignment:phi, suggest_constant_definition
    index = (math.floor(math.log(n * (phi + 2), phi) - 1) // 3) * 3 + 2 # addition_operator, assignment:Add, assignment_lhs_identifier:index, assignment_rhs_atom:1, assignment_rhs_atom:2, assignment_rhs_atom:3, assignment_rhs_atom:math, assignment_rhs_atom:n, assignment_rhs_atom:phi, binary_operator:Add, binary_operator:FloorDiv, binary_operator:Mult, binary_operator:Sub, call_argument:, call_argument:phi, composition, literal:1, literal:2, literal:3, member_call:floor, member_call:log, multiplication_operator, single_assignment:index, suggest_constant_definition
    num = Decimal(round(phi ** Decimal(index + 1))) / (phi + 2) # addition_operator, assignment:Div, assignment_lhs_identifier:num, assignment_rhs_atom:1, assignment_rhs_atom:2, assignment_rhs_atom:index, assignment_rhs_atom:phi, binary_operator:Add, binary_operator:Div, binary_operator:Pow, call_argument:, composition, external_free_call:Decimal, external_free_call:round, free_call:Decimal, free_call:round, literal:1, literal:2, single_assignment:num
    ...
    return int(sum) # call_argument:sum, external_free_call:int, free_call:int, free_tail_call:int, return

# ----------------------------------------------------------------------------------------
# problem_02/sol5.py
# ----------------------------------------------------------------------------------------
def solution(n): # function:solution (-> +12), function_argument:n, function_argument_flavor:arg, function_returning_something:solution (-> +12), whole_span:13 (-> +12)
    ...
    while a[i] <= n: # comparison_operator:LtE, count_states:i (-> +4), index:i, loop:while (-> +4), loop_with_break:while (-> +4), loop_with_early_exit:while:break (-> +4), while (-> +4)
        a.append(a[i] + a[i + 1]) # addition_operator, binary_operator:Add, call_argument:, index:_, index:i, index_arithmetic, literal:1, member_call:append, member_call_object:a, member_call_without_result:append
    ...
    sum = 0 # assignment:0, assignment_lhs_identifier:sum, assignment_rhs_atom:0, literal:0, single_assignment:sum
    for j in range(len(a) - 1): # accumulate_elements:Add (-> +2), accumulate_some_elements:Add (-> +2), binary_operator:Sub, call_argument:, call_argument:a, composition, external_free_call:len, external_free_call:range, for:j (-> +2), for_range:_ (-> +2), free_call:len, free_call:range, literal:1, loop:for (-> +2), loop_with_late_exit:for (-> +2), range:_
...
def isprime(no): # function:isprime (-> +9), function_argument:no, function_argument_flavor:arg, function_returning_something:isprime (-> +9)
    ...
    sq = int(math.sqrt(no)) + 1 # addition_operator, assignment:Add, assignment_lhs_identifier:sq, assignment_rhs_atom:1, assignment_rhs_atom:math, assignment_rhs_atom:no, binary_operator:Add, call_argument:, call_argument:no, composition, external_free_call:int, free_call:int, literal:1, member_call:sqrt, single_assignment:sq
    for i in range(3, sq, 2): # call_argument:2, call_argument:3, call_argument:sq, external_free_call:range, for:i (-> +2), for_range:3:sq:2 (-> +2), free_call:range, literal:2, literal:3, loop:for (-> +2), loop_with_early_exit:for:return (-> +2), loop_with_return:for (-> +2), range:3:sq:2, suggest_constant_definition, universal_quantification:i (-> +2)
...
def solution(n): # function:solution (-> +24), function_argument:n, function_argument_flavor:arg, function_returning_something:solution (-> +24)
    ...
    if isprime(n): # call_argument:n, free_call:isprime, if (-> +16), if_test_atom:n, internal_free_call:isprime
        return n # if_then_branch, return:n
    else:
        while n % 2 == 0: # binary_operator:Mod, comparison_operator:Eq, divisibility_test:2, if_else_branch (-> +13), literal:0, literal:2, loop:while (-> +1), loop_with_late_exit:while (-> +1), modulo_operator, while (-> +1)
        ...
        if isprime(n): # call_argument:n, free_call:isprime, if (-> +11), if_test_atom:n, internal_free_call:isprime, nested_if:1 (-> +11)
            return int(n) # call_argument:n, external_free_call:int, free_call:int, free_tail_call:int, if_then_branch, return
        else:
            n1 = int(math.sqrt(n)) + 1 # addition_operator, assignment:Add, assignment_lhs_identifier:n1, assignment_rhs_atom:1, assignment_rhs_atom:math, assignment_rhs_atom:n, binary_operator:Add, call_argument:, call_argument:n, composition, external_free_call:int, free_call:int, if_else_branch (-> +8), literal:1, member_call:sqrt, single_assignment:n1
            for i in range(3, n1, 2): # call_argument:2, call_argument:3, call_argument:n1, external_free_call:range, for:i (-> +6), for_range:3:n1:2 (-> +6), free_call:range, literal:2, literal:3, loop:for (-> +6), loop_with_break:for (-> +6), loop_with_early_exit:for:break (-> +6), range:3:n1:2, suggest_constant_definition
                if n % i == 0: # binary_operator:Mod, comparison_operator:Eq, divisibility_test, if (-> +5), if_test_atom:0, if_test_atom:i, if_test_atom:n, if_without_else (-> +5), literal:0, modulo_operator, nested_if:2 (-> +5)
                    if isprime(n / i): # binary_operator:Div, call_argument:, free_call:isprime, if (-> +4), if_test_atom:i, if_test_atom:n, if_then_branch (-> +4), internal_free_call:isprime, nested_if:3 (-> +4)
                        ...
                        break # break
                    elif isprime(i): # call_argument:i, free_call:isprime, if (-> +1), if_test_atom:i, internal_free_call:isprime, nested_if:3 (-> +1)
...
def solution(n): # function:solution (-> +16), function_argument:n, function_argument_flavor:arg, function_returning_something:solution (-> +16), whole_span:17 (-> +16)
    ...
    while i * i <= n: # binary_operator:Mult, comparison_operator:LtE, count_states:i (-> +4), loop:while (-> +4), loop_with_late_exit:while (-> +4), multiplication_operator, while (-> +4)
    ...
    return int(prime) # call_argument:prime, external_free_call:int, free_call:int, free_tail_call:int, return
...
def solution(n: int) -> int: # function:solution (-> +18), function_argument:n, function_argument_flavor:arg, function_returning_something:solution (-> +18), whole_span:19 (-> +18)
    ...
    while n > 2: # comparison_operator:Gt, literal:2, loop:while (-> +6), loop_with_late_exit:while (-> +6), while (-> +6)
        while n % i != 0: # binary_operator:Mod, comparison_operator:NotEq, count_states:i (-> +1), divisibility_test, literal:0, loop:while (-> +1), loop_with_late_exit:while (-> +1), modulo_operator, while (-> +1)
            i += 1 # assignment_lhs_identifier:i, assignment_rhs_atom:1, augmented_assignment:Add, increment:i, literal:1, update:i:1, update_by_augmented_assignment:i:1, update_by_augmented_assignment_with:Add, update_with:Add
        ...
        i += 1 # assignment_lhs_identifier:i, assignment_rhs_atom:1, augmented_assignment:Add, increment:i, literal:1, update:i:1, update_by_augmented_assignment:i:1, update_by_augmented_assignment_with:Add, update_with:Add
    return int(ans) # call_argument:ans, external_free_call:int, free_call:int, free_tail_call:int, return

# ----------------------------------------------------------------------------------------
# problem_04/sol1.py
# ----------------------------------------------------------------------------------------
def solution(n): # function:solution (-> +8), function_argument:n, function_argument_flavor:arg, function_returning_something:solution (-> +8), whole_span:9 (-> +8)
    for number in range(n - 1, 10000, -1): # binary_operator:Sub, call_argument:, call_argument:-1, call_argument:10000, external_free_call:range, find_first_element:number (-> +7), for:number (-> +7), for_range:_:10000:-1 (-> +7), free_call:range, literal:-1, literal:1, literal:10000, loop:for (-> +7), loop_with_early_exit:for:return (-> +7), loop_with_return:for (-> +7), range:_:10000:-1, suggest_constant_definition
        strNumber = str(number) # assignment:str, assignment_lhs_identifier:strNumber, assignment_rhs_atom:number, call_argument:number, external_free_call:str, free_call:str, single_assignment:strNumber
        if strNumber == strNumber[::-1]: # comparison_operator:Eq, if (-> +5), if_test_atom:-1, if_test_atom:strNumber, if_without_else (-> +5), literal:-1, slice:::-1, slice_lower:, slice_step:-1, slice_upper:
            ...
            while divisor != 99: # comparison_operator:NotEq, literal:99, loop:while (-> +3), loop_with_early_exit:while:return (-> +3), loop_with_return:while (-> +3), suggest_constant_definition, while (-> +3)
                if (number % divisor == 0) and (len(str(int(number / divisor))) == 3): # binary_operator:Div, binary_operator:Mod, boolean_operator:And, call_argument:, comparison_operator:Eq, composition, divisibility_test, external_free_call:int, external_free_call:len, external_free_call:str, free_call:int, free_call:len, free_call:str, if (-> +1), if_test_atom:0, if_test_atom:3, if_test_atom:divisor, if_test_atom:number, if_without_else (-> +1), literal:0, literal:3, modulo_operator, nested_if:1 (-> +1), suggest_constant_definition
...
def solution(n): # function:solution (-> +7), function_argument:n, function_argument_flavor:arg, function_returning_something:solution (-> +7), whole_span:8 (-> +7)
    ...
    for i in range(999, 99, -1): # accumulate_elements:max (-> +4), accumulate_some_elements:max (-> +4), call_argument:-1, call_argument:99, call_argument:999, external_free_call:range, for:i (-> +4), for_range:999:99:-1 (-> +4), free_call:range, literal:-1, literal:99, literal:999, loop:for (-> +4), loop_with_late_exit:for (-> +4), range:999:99:-1, square_nested_for (-> +4), suggest_constant_definition
        for j in range(999, 99, -1): # accumulate_all_elements:max (-> +3), accumulate_elements:max (-> +3), call_argument:-1, call_argument:99, call_argument:999, external_free_call:range, for:j (-> +3), for_range:999:99:-1 (-> +3), free_call:range, literal:-1, literal:99, literal:999, loop:for (-> +3), loop_with_late_exit:for (-> +3), nested_for:1 (-> +3), range:999:99:-1, suggest_constant_definition
            t = str(i * j) # assignment:str, assignment_lhs_identifier:t, assignment_rhs_atom:i, assignment_rhs_atom:j, binary_operator:Mult, call_argument:, external_free_call:str, free_call:str, multiplication_operator, single_assignment:t
            if t == t[::-1] and i * j < n: # binary_operator:Mult, boolean_operator:And, comparison_operator:Eq, comparison_operator:Lt, if (-> +1), if_test_atom:-1, if_test_atom:i, if_test_atom:j, if_test_atom:n, if_test_atom:t, if_without_else (-> +1), literal:-1, multiplication_operator, slice:::-1, slice_lower:, slice_step:-1, slice_upper:
                answer = max(answer, i * j) # assignment:max, assignment_lhs_identifier:answer, assignment_rhs_atom:answer, assignment_rhs_atom:i, assignment_rhs_atom:j, binary_operator:Mult, call_argument:, call_argument:answer, external_free_call:max, free_call:max, if_then_branch, multiplication_operator, single_assignment:answer, update:answer:i, update:answer:j, update_by_assignment:answer:i, update_by_assignment:answer:j, update_by_assignment_with:max, update_with:max
...
def solution(n): # function:solution (-> +18), function_argument:n, function_argument_flavor:arg, function_returning_something:solution (-> +18), whole_span:19 (-> +18)
    ...
    if n <= 0: # comparison_operator:LtE, if (-> +1), if_test_atom:0, if_test_atom:n, if_without_else (-> +1), literal:0
        raise ValueError("Parameter n must be greater or equal to one.") # call_argument:, external_free_call:ValueError, free_call:ValueError, if_then_branch, literal:Str, raise:ValueError
    i = 0 # assignment:0, assignment_lhs_identifier:i, assignment_rhs_atom:0, literal:0, single_assignment:i
    while 1: # infinite_while (-> +10), literal:1, loop:while (-> +10), loop_with_early_exit:while:return (-> +10), loop_with_return:while (-> +10), while (-> +10)
        ...
        for j in range(2, n): # call_argument:2, call_argument:n, external_free_call:range, for:j (-> +3), for_range:2:n (-> +3), free_call:range, literal:2, loop:for (-> +3), loop_with_break:for (-> +3), loop_with_early_exit:for:break (-> +3), range:2:n
...
# problem_05/sol2.py
# ----------------------------------------------------------------------------------------
def gcd(x, y): # function:gcd (-> +1), function_argument:x, function_argument:y, function_argument_flavor:arg, function_returning_something:gcd (-> +1), recursive_function:gcd (-> +1), tail_recursive_function:gcd (-> +1), whole_span:9 (-> +8)
    return x if y == 0 else gcd(y, x % y) # binary_operator:Mod, call_argument:, call_argument:y, comparison_operator:Eq, conditional_expression, free_call:gcd, free_tail_call:gcd, internal_free_call:gcd, literal:0, modulo_operator, return
def lcm(x, y): # function:lcm (-> +1), function_argument:x, function_argument:y, function_argument_flavor:arg, function_returning_something:lcm (-> +1)
    return (x * y) // gcd(x, y) # binary_operator:FloorDiv, binary_operator:Mult, call_argument:x, call_argument:y, free_call:gcd, internal_free_call:gcd, multiplication_operator, return
def solution(n): # function:solution (-> +4), function_argument:n, function_argument_flavor:arg, function_returning_something:solution (-> +4)
    ...
    for i in range(1, n + 1): # accumulate_all_elements:lcm (-> +1), accumulate_elements:lcm (-> +1), addition_operator, binary_operator:Add, call_argument:, call_argument:1, external_free_call:range, for:i (-> +1), for_range:1:_ (-> +1), free_call:range, literal:1, loop:for (-> +1), loop_with_late_exit:for (-> +1), range:1:_
        g = lcm(g, i) # assignment:lcm, assignment_lhs_identifier:g, assignment_rhs_atom:g, assignment_rhs_atom:i, call_argument:g, call_argument:i, free_call:lcm, internal_free_call:lcm, single_assignment:g, update:g:i, update_by_assignment:g:i, update_by_assignment_with:lcm, update_with:lcm
    return g # return:g
...
def solution(n): # function:solution (-> +7), function_argument:n, function_argument_flavor:arg, function_returning_something:solution (-> +7), whole_span:8 (-> +7)
    ...
    for i in range(1, n + 1): # accumulate_all_elements:Add (-> +2), accumulate_elements:Add (-> +2), addition_operator, binary_operator:Add, call_argument:, call_argument:1, external_free_call:range, for:i (-> +2), for_range:1:_ (-> +2), free_call:range, literal:1, loop:for (-> +2), loop_with_late_exit:for (-> +2), range:1:_
...
# problem_06/sol2.py
# ----------------------------------------------------------------------------------------
def solution(n): # function:solution (-> +4), function_argument:n, function_argument_flavor:arg, function_returning_something:solution (-> +4), whole_span:5 (-> +4)
    ...
    return int(suma - sumb) # binary_operator:Sub, call_argument:, external_free_call:int, free_call:int, free_tail_call:int, return

# ----------------------------------------------------------------------------------------
# problem_06/sol3.py
# ----------------------------------------------------------------------------------------
import math # import:math, import_module:math, whole_span:5 (-> +4)
def solution(n): # function:solution (-> +3), function_argument:n, function_argument_flavor:arg, function_returning_something:solution (-> +3)
    sum_of_squares = sum([i * i for i in range(1, n + 1)]) # addition_operator, assignment:sum, assignment_lhs_identifier:sum_of_squares, assignment_rhs_atom:1, assignment_rhs_atom:i, assignment_rhs_atom:n, binary_operator:Add, binary_operator:Mult, call_argument:, call_argument:1, composition, comprehension:List, comprehension_for_count:1, external_free_call:range, external_free_call:sum, free_call:range, free_call:sum, literal:1, multiplication_operator, range:1:_, single_assignment:sum_of_squares
    square_of_sum = int(math.pow(sum(range(1, n + 1)), 2)) # addition_operator, assignment:int, assignment_lhs_identifier:square_of_sum, assignment_rhs_atom:1, assignment_rhs_atom:2, assignment_rhs_atom:math, assignment_rhs_atom:n, binary_operator:Add, call_argument:, call_argument:1, call_argument:2, composition, external_free_call:int, external_free_call:range, external_free_call:sum, free_call:int, free_call:range, free_call:sum, literal:1, literal:2, member_call:pow, range:1:_, single_assignment:square_of_sum
    return square_of_sum - sum_of_squares # binary_operator:Sub, return

# ----------------------------------------------------------------------------------------
# problem_06/sol4.py
# ----------------------------------------------------------------------------------------
def solution(n): # function:solution (-> +3), function_argument:n, function_argument_flavor:arg, function_returning_something:solution (-> +3), whole_span:4 (-> +3)
    ...
    return int(square_of_sum - sum_of_squares) # binary_operator:Sub, call_argument:, external_free_call:int, free_call:int, free_tail_call:int, return

# ----------------------------------------------------------------------------------------
# problem_07/sol1.py
# ----------------------------------------------------------------------------------------
from math import sqrt # import:math:sqrt, import_module:math, import_name:sqrt, whole_span:24 (-> +23)
def isprime(n): # function:isprime (-> +10), function_argument:n, function_argument_flavor:arg, function_returning_something:isprime (-> +10)
    if n == 2: # comparison_operator:Eq, if (-> +8), if_test_atom:2, if_test_atom:n, literal:2
        ...
        sq = int(sqrt(n)) + 1 # addition_operator, assignment:Add, assignment_lhs_identifier:sq, assignment_rhs_atom:1, assignment_rhs_atom:n, binary_operator:Add, call_argument:, call_argument:n, composition, external_free_call:int, external_free_call:sqrt, free_call:int, free_call:sqrt, if_else_branch (-> +3), literal:1, single_assignment:sq
        for i in range(3, sq, 2): # call_argument:2, call_argument:3, call_argument:sq, external_free_call:range, for:i (-> +2), for_range:3:sq:2 (-> +2), free_call:range, literal:2, literal:3, loop:for (-> +2), loop_with_early_exit:for:return (-> +2), loop_with_return:for (-> +2), range:3:sq:2, suggest_constant_definition, universal_quantification:i (-> +2)
...
def solution(n): # function:solution (-> +11), function_argument:n, function_argument_flavor:arg, function_returning_something:solution (-> +11)
    ...
    while i != n and j < 3: # boolean_operator:And, comparison_operator:Lt, comparison_operator:NotEq, count_states:i (-> +3), count_states:j (-> +3), literal:3, loop:while (-> +3), loop_with_late_exit:while (-> +3), suggest_constant_definition, while (-> +3)
        ...
        if isprime(j): # call_argument:j, free_call:isprime, if (-> +1), if_test_atom:j, if_without_else (-> +1), internal_free_call:isprime
    ...
    while i != n: # comparison_operator:NotEq, count_states:i (-> +3), loop:while (-> +3), loop_with_late_exit:while (-> +3), while (-> +3)
...
# problem_07/sol2.py
# ----------------------------------------------------------------------------------------
def isprime(number): # function:isprime (-> +4), function_argument:number, function_argument_flavor:arg, function_returning_something:isprime (-> +4), whole_span:21 (-> +20)
    for i in range(2, int(number ** 0.5) + 1): # addition_operator, binary_operator:Add, binary_operator:Pow, call_argument:, call_argument:2, composition, external_free_call:int, external_free_call:range, for:i (-> +2), for_range:2:_ (-> +2), free_call:int, free_call:range, literal:0.5, literal:1, literal:2, loop:for (-> +2), loop_with_early_exit:for:return (-> +2), loop_with_return:for (-> +2), range:2:_, suggest_constant_definition, universal_quantification:i (-> +2)
...
def solution(n): # function:solution (-> +15), function_argument:n, function_argument_flavor:arg, function_returning_something:solution (-> +15)
    ...
    while len(primes) < n: # call_argument:primes, comparison_operator:Lt, external_free_call:len, free_call:len, loop:while (-> +5), loop_with_late_exit:while (-> +5), while (-> +5)
        if isprime(num): # call_argument:num, free_call:isprime, if (-> +4), if_test_atom:num, internal_free_call:isprime
            primes.append(num) # call_argument:num, if_then_branch (-> +1), member_call:append, member_call_object:primes, member_call_without_result:append, update:primes:num, update_by_member_call:primes:num, update_by_member_call_with:append, update_with:append
            num += 1 # assignment_lhs_identifier:num, assignment_rhs_atom:1, augmented_assignment:Add, increment:num, literal:1, update:num:1, update_by_augmented_assignment:num:1, update_by_augmented_assignment_with:Add, update_with:Add
        else:
    ...
    return primes[len(primes) - 1] # binary_operator:Sub, call_argument:primes, external_free_call:len, free_call:len, index:_, index_arithmetic, literal:1, return

# ----------------------------------------------------------------------------------------
# problem_07/sol3.py
# ----------------------------------------------------------------------------------------
import math # import:math, import_module:math, whole_span:14 (-> +13)
...
def primeCheck(number): # function:primeCheck (-> +3), function_argument:number, function_argument_flavor:arg, function_returning_something:primeCheck (-> +3)
    if number % 2 == 0 and number > 2: # binary_operator:Mod, boolean_operator:And, comparison_operator:Eq, comparison_operator:Gt, divisibility_test:2, if (-> +1), if_guard (-> +1), if_test_atom:0, if_test_atom:2, if_test_atom:number, if_without_else (-> +1), literal:0, literal:2, modulo_operator
        return False # if_then_branch, literal:False, return:False
    return all(number % i for i in range(3, int(math.sqrt(number)) + 1, 2)) # addition_operator, binary_operator:Add, binary_operator:Mod, call_argument:, call_argument:2, call_argument:3, call_argument:number, composition, comprehension:Generator, comprehension_for_count:1, external_free_call:all, external_free_call:int, external_free_call:range, free_call:all, free_call:int, free_call:range, free_tail_call:all, literal:1, literal:2, literal:3, member_call:sqrt, modulo_operator, range:3:_:2, return, suggest_constant_definition
def prime_generator(): # function:prime_generator (-> +5), function_without_arguments:prime_generator (-> +5), generator:prime_generator (-> +5)
    num = 2 # assignment:2, assignment_lhs_identifier:num, assignment_rhs_atom:2, literal:2, single_assignment:num
    while True: # count_states:num (-> +3), infinite_while (-> +3), literal:True, loop:while (-> +3), loop_with_late_exit:while (-> +3), while (-> +3)
        if primeCheck(num): # call_argument:num, free_call:primeCheck, if (-> +1), if_test_atom:num, if_without_else (-> +1), internal_free_call:primeCheck
...
def solution(n): # function:solution (-> +1), function_argument:n, function_argument_flavor:arg, function_returning_something:solution (-> +1)
    return next(itertools.islice(prime_generator(), n - 1, n)) # binary_operator:Sub, call_argument:, call_argument:n, composition, external_free_call:next, free_call:next, free_call:prime_generator, free_call_without_arguments:prime_generator, free_tail_call:next, internal_free_call:prime_generator, literal:1, member_call:islice, return

# ----------------------------------------------------------------------------------------
# problem_08/sol1.py
# ----------------------------------------------------------------------------------------
import sys # import:sys, import_module:sys, whole_span:30 (-> +29)
...
def solution(n): # function:solution (-> +8), function_argument:n, function_argument_flavor:arg, function_returning_something:solution (-> +8)
    ...
    for i in range(len(n) - 12): # accumulate_all_elements:Mult (-> +5), accumulate_elements:Mult (-> +5), binary_operator:Sub, call_argument:, call_argument:n, composition, external_free_call:len, external_free_call:range, for:i (-> +5), for_range:13 (-> +5), for_range:_ (-> +5), free_call:len, free_call:range, literal:12, loop:for (-> +5), loop_with_late_exit:for (-> +5), range:_, suggest_constant_definition
        product = 1 # assignment:1, assignment_lhs_identifier:product, assignment_rhs_atom:1, literal:1, single_assignment:product
        for j in range(13): # accumulate_all_elements:Mult (-> +1), accumulate_elements:Mult (-> +1), call_argument:13, external_free_call:range, for:j (-> +1), for_range:13 (-> +1), free_call:range, literal:13, loop:for (-> +1), loop_with_late_exit:for (-> +1), nested_for:1 (-> +1), range:13, suggest_constant_definition
            product *= int(n[i + j]) # addition_operator, assignment_lhs_identifier:product, assignment_rhs_atom:i, assignment_rhs_atom:j, assignment_rhs_atom:n, augmented_assignment:Mult, binary_operator:Add, call_argument:, external_free_call:int, free_call:int, index:_, index_arithmetic, update:product:i, update:product:j, update:product:n, update_by_augmented_assignment:product:i, update_by_augmented_assignment:product:j, update_by_augmented_assignment:product:n, update_by_augmented_assignment_with:Mult, update_with:Mult
...
# problem_08/sol2.py
# ----------------------------------------------------------------------------------------
from functools import reduce # import:functools:reduce, import_module:functools, import_name:reduce, whole_span:28 (-> +27)
...
def solution(n): # function:solution (-> +4), function_argument:n, function_argument_flavor:arg, function_returning_something:solution (-> +4)
    return max( # composition, external_free_call:max, free_call:max, free_tail_call:max, return
        [
            reduce(lambda x, y: int(x) * int(y), n[i : i + 13]) # addition_operator, binary_operator:Add, binary_operator:Mult, call_argument:, call_argument:x, call_argument:y, composition, comprehension:List, comprehension_for_count:1, external_free_call:int, external_free_call:reduce, free_call:int, free_call:reduce, function_argument:x, function_argument:y, function_argument_flavor:arg, lambda_function, literal:13, multiplication_operator, slice:i:_:, slice_lower:i, slice_step:, slice_upper:_, suggest_constant_definition
            for i in range(len(n) - 12) # binary_operator:Sub, call_argument:, call_argument:n, composition, external_free_call:len, external_free_call:range, free_call:len, free_call:range, literal:12, range:_, suggest_constant_definition
        ]
    )
...
# problem_08/sol3.py
# ----------------------------------------------------------------------------------------
import sys # import:sys, import_module:sys, whole_span:39 (-> +38)
N = """73167176531330624919225119674426574742355349194934\ # assignment, assignment_lhs_identifier:N, single_assignment:N
96983520312774506326239578318016984801869478851843\
85861560789112949495459501737958331952853208805511\
12540698747158523863050715693290963295227443043557\
...
def streval(s: str) -> int: # function:streval (-> +4), function_argument:s, function_argument_flavor:arg, function_returning_something:streval (-> +4)
    ...
    for it in s: # accumulate_all_elements:Mult (-> +1), accumulate_elements:Mult (-> +1), for:it (-> +1), for_each (-> +1), loop:for (-> +1), loop_with_late_exit:for (-> +1)
        ret *= int(it) # assignment_lhs_identifier:ret, assignment_rhs_atom:it, augmented_assignment:Mult, call_argument:it, external_free_call:int, free_call:int, update:ret:it, update_by_augmented_assignment:ret:it, update_by_augmented_assignment_with:Mult, update_with:Mult
...
def solution(n: str) -> int: # function:solution (-> +12), function_argument:n, function_argument_flavor:arg, function_returning_something:solution (-> +12)
    ...
    while cur_index < len(n) - 13: # binary_operator:Sub, call_argument:n, comparison_operator:Lt, external_free_call:len, free_call:len, literal:13, loop:while (-> +7), loop_with_late_exit:while (-> +7), suggest_constant_definition, while (-> +7)
        if int(n[cur_index]) >= int(substr[0]): # call_argument:, comparison_operator:GtE, external_free_call:int, free_call:int, if (-> +6), if_test_atom:0, if_test_atom:cur_index, if_test_atom:n, if_test_atom:substr, index:0, index:cur_index, literal:0
            ...
            LargestProduct = max(LargestProduct, streval(substr)) # assignment:max, assignment_lhs_identifier:LargestProduct, assignment_rhs_atom:LargestProduct, assignment_rhs_atom:substr, call_argument:, call_argument:LargestProduct, call_argument:substr, composition, external_free_call:max, free_call:max, free_call:streval, if_else_branch (-> +2), internal_free_call:streval, single_assignment:LargestProduct, update:LargestProduct:substr, update_by_assignment:LargestProduct:substr, update_by_assignment_with:max, update_with:max
...
# problem_09/sol1.py
# ----------------------------------------------------------------------------------------
def solution(): # function:solution (-> +7), function_returning_something:solution (-> +7), function_without_arguments:solution (-> +7), whole_span:8 (-> +7)
    for a in range(300): # call_argument:300, external_free_call:range, for:a (-> +6), for_range:300 (-> +6), for_range:400 (-> +6), for_range:500 (-> +6), free_call:range, literal:300, loop:for (-> +6), loop_with_early_exit:for:return (-> +6), loop_with_return:for (-> +6), range:300, suggest_constant_definition
        for b in range(400): # call_argument:400, external_free_call:range, for:b (-> +5), for_range:400 (-> +5), for_range:500 (-> +5), free_call:range, literal:400, loop:for (-> +5), loop_with_early_exit:for:return (-> +5), loop_with_return:for (-> +5), nested_for:1 (-> +5), range:400, suggest_constant_definition
            for c in range(500): # call_argument:500, external_free_call:range, for:c (-> +4), for_range:500 (-> +4), free_call:range, literal:500, loop:for (-> +4), loop_with_early_exit:for:return (-> +4), loop_with_return:for (-> +4), nested_for:2 (-> +4), range:500, suggest_constant_definition
...
# problem_09/sol2.py
# ----------------------------------------------------------------------------------------
def solution(n): # function:solution (-> +10), function_argument:n, function_argument_flavor:arg, function_returning_something:solution (-> +10), whole_span:11 (-> +10)
    product = -1 # assignment:-1, assignment_lhs_identifier:product, assignment_rhs_atom:-1, literal:-1, single_assignment:product
    d = 0 # assignment:0, assignment_lhs_identifier:d, assignment_rhs_atom:0, literal:0, single_assignment:d
    for a in range(1, n // 3): # binary_operator:FloorDiv, call_argument:, call_argument:1, external_free_call:range, for:a (-> +6), for_range:1:_ (-> +6), free_call:range, literal:1, literal:3, loop:for (-> +6), loop_with_late_exit:for (-> +6), range:1:_, suggest_constant_definition
...
def solution(): # function:solution (-> +7), function_returning_something:solution (-> +7), function_without_arguments:solution (-> +7), whole_span:8 (-> +7)
    return [ # return
        ...
        for a in range(1, 999) # call_argument:1, call_argument:999, external_free_call:range, free_call:range, literal:1, literal:999, range:1:999, suggest_constant_definition
        for b in range(a, 999) # call_argument:999, call_argument:a, external_free_call:range, free_call:range, literal:999, range:a:999, suggest_constant_definition
        for c in range(b, 999) # call_argument:999, call_argument:b, external_free_call:range, free_call:range, literal:999, range:b:999, suggest_constant_definition
...
# problem_10/sol1.py
# ----------------------------------------------------------------------------------------
from math import sqrt # import:math:sqrt, import_module:math, import_name:sqrt, whole_span:17 (-> +16)
def is_prime(n): # function:is_prime (-> +4), function_argument:n, function_argument_flavor:arg, function_returning_something:is_prime (-> +4)
    for i in range(2, int(sqrt(n)) + 1): # addition_operator, binary_operator:Add, call_argument:, call_argument:2, call_argument:n, composition, external_free_call:int, external_free_call:range, external_free_call:sqrt, for:i (-> +2), for_range:2:_ (-> +2), free_call:int, free_call:range, free_call:sqrt, literal:1, literal:2, loop:for (-> +2), loop_with_early_exit:for:return (-> +2), loop_with_return:for (-> +2), range:2:_, universal_quantification:i (-> +2)
...
def sum_of_primes(n): # function:sum_of_primes (-> +8), function_argument:n, function_argument_flavor:arg, function_returning_something:sum_of_primes (-> +8)
    ...
    for i in range(3, n, 2): # accumulate_elements:Add (-> +2), accumulate_some_elements:Add (-> +2), call_argument:2, call_argument:3, call_argument:n, external_free_call:range, for:i (-> +2), for_range:3:n:2 (-> +2), free_call:range, literal:2, literal:3, loop:for (-> +2), loop_with_late_exit:for (-> +2), range:3:n:2, suggest_constant_definition
        if is_prime(i): # call_argument:i, free_call:is_prime, if (-> +1), if_test_atom:i, if_without_else (-> +1), internal_free_call:is_prime
...
def solution(n): # function:solution (-> +1), function_argument:n, function_argument_flavor:arg, function_returning_something:solution (-> +1)
    return sum_of_primes(n) # call_argument:n, free_call:sum_of_primes, free_tail_call:sum_of_primes, internal_free_call:sum_of_primes, return
...
def prime_generator(): # function:prime_generator (-> +5), function_without_arguments:prime_generator (-> +5), generator:prime_generator (-> +5)
    ...
    while True: # count_states:num (-> +3), infinite_while (-> +3), literal:True, loop:while (-> +3), loop_with_late_exit:while (-> +3), while (-> +3)
        ...
        num += 1 # assignment_lhs_identifier:num, assignment_rhs_atom:1, augmented_assignment:Add, increment:num, literal:1, update:num:1, update_by_augmented_assignment:num:1, update_by_augmented_assignment_with:Add, update_with:Add
def solution(n): # function:solution (-> +1), function_argument:n, function_argument_flavor:arg, function_returning_something:solution (-> +1)
    return sum(takewhile(lambda x: x < n, prime_generator())) # call_argument:, comparison_operator:Lt, composition, external_free_call:sum, external_free_call:takewhile, free_call:prime_generator, free_call:sum, free_call:takewhile, free_call_without_arguments:prime_generator, free_tail_call:sum, function_argument:x, function_argument_flavor:arg, internal_free_call:prime_generator, lambda_function, return

# ----------------------------------------------------------------------------------------
# problem_10/sol3.py
# ----------------------------------------------------------------------------------------
def prime_sum(n: int) -> int: # function:prime_sum (-> +12), function_argument:n, function_argument_flavor:arg, function_returning_something:prime_sum (-> +12), whole_span:13 (-> +12)
    ...
    for i in range(2, int(n ** 0.5) + 1): # addition_operator, binary_operator:Add, binary_operator:Pow, call_argument:, call_argument:2, composition, external_free_call:int, external_free_call:range, for:i (-> +3), for_range:2:_ (-> +3), for_range:_:_:i (-> +3), free_call:int, free_call:range, literal:0.5, literal:1, literal:2, loop:for (-> +3), loop_with_late_exit:for (-> +3), range:2:_, suggest_constant_definition
        if list_[i] == 0: # comparison_operator:Eq, if (-> +2), if_test_atom:0, if_test_atom:i, if_test_atom:list_, if_without_else (-> +2), index:i, literal:0
            for j in range(i * i, n + 1, i): # addition_operator, binary_operator:Add, binary_operator:Mult, call_argument:, call_argument:i, external_free_call:range, for:j (-> +1), for_range:_:_:i (-> +1), free_call:range, if_then_branch (-> +1), literal:1, loop:for (-> +1), loop_with_late_exit:for (-> +1), multiplication_operator, nested_for:1 (-> +1), range:_:_:i
    ...
    for i in range(n): # accumulate_elements:Add (-> +2), accumulate_some_elements:Add (-> +2), call_argument:n, external_free_call:range, for:i (-> +2), for_range:n (-> +2), free_call:range, loop:for (-> +2), loop_with_late_exit:for (-> +2), range:n
...
# problem_11/sol1.py
# ----------------------------------------------------------------------------------------
import os # import:os, import_module:os, whole_span:36 (-> +35)
def largest_product(grid): # function:largest_product (-> +27), function_argument:grid, function_argument_flavor:arg, function_returning_something:largest_product (-> +27)
    nColumns = len(grid[0]) # assignment:len, assignment_lhs_identifier:nColumns, assignment_rhs_atom:0, assignment_rhs_atom:grid, call_argument:, external_free_call:len, free_call:len, index:0, literal:0, single_assignment:nColumns
    nRows = len(grid) # assignment:len, assignment_lhs_identifier:nRows, assignment_rhs_atom:grid, call_argument:grid, external_free_call:len, free_call:len, single_assignment:nRows
    ...
    for i in range(nColumns): # call_argument:nColumns, external_free_call:range, for:i (-> +20), for_range:_ (-> +20), for_range:nColumns (-> +20), free_call:range, loop:for (-> +20), loop_with_late_exit:for (-> +20), range:nColumns
        for j in range(nRows - 3): # binary_operator:Sub, call_argument:, external_free_call:range, for:j (-> +19), for_range:_ (-> +19), free_call:range, literal:3, loop:for (-> +19), loop_with_late_exit:for (-> +19), nested_for:1 (-> +19), range:_, suggest_constant_definition
            ...
            maxProduct = max(vertProduct, horzProduct, lrDiagProduct, rlDiagProduct) # assignment:max, assignment_lhs_identifier:maxProduct, assignment_rhs_atom:horzProduct, assignment_rhs_atom:lrDiagProduct, assignment_rhs_atom:rlDiagProduct, assignment_rhs_atom:vertProduct, call_argument:horzProduct, call_argument:lrDiagProduct, call_argument:rlDiagProduct, call_argument:vertProduct, external_free_call:max, free_call:max, single_assignment:maxProduct
...
def solution(): # function:solution (-> +6), function_returning_something:solution (-> +6), function_without_arguments:solution (-> +6)
    ...
    with open(os.path.dirname(__file__) + "/grid.txt") as file: # binary_operator:Add, call_argument:, call_argument:__file__, composition, concatenation_operator:Str, external_free_call:open, free_call:open, literal:Str, member_call:dirname
        for line in file: # for:line (-> +1), for_each (-> +1), loop:for (-> +1), loop_with_late_exit:for (-> +1)
            grid.append(line.strip("\n").split(" ")) # call_argument:, composition, literal:Str, member_call:append, member_call:split, member_call:strip, member_call_object:grid, member_call_object:line, member_call_without_result:append, method_chaining
    grid = [[int(i) for i in grid[j]] for j in range(len(grid))] # assignment, assignment_lhs_identifier:grid, assignment_rhs_atom:grid, assignment_rhs_atom:i, assignment_rhs_atom:j, call_argument:, call_argument:grid, call_argument:i, composition, comprehension:List, comprehension_for_count:1, external_free_call:int, external_free_call:len, external_free_call:range, free_call:int, free_call:len, free_call:range, index:j, range:_, single_assignment:grid, update:grid:i, update:grid:j, update_by_assignment:grid:i, update_by_assignment:grid:j, update_by_assignment_with, update_with
    return largest_product(grid) # call_argument:grid, free_call:largest_product, free_tail_call:largest_product, internal_free_call:largest_product, return

# ----------------------------------------------------------------------------------------
# problem_11/sol2.py
# ----------------------------------------------------------------------------------------
import os # import:os, import_module:os, whole_span:28 (-> +27)
def solution(): # function:solution (-> +26), function_returning_something:solution (-> +26), function_without_arguments:solution (-> +26)
    with open(os.path.dirname(__file__) + "/grid.txt") as f: # binary_operator:Add, call_argument:, call_argument:__file__, composition, concatenation_operator:Str, external_free_call:open, free_call:open, literal:Str, member_call:dirname
        l = [] # assignment, assignment_lhs_identifier:l, empty_literal:List, literal:List, single_assignment:l
        for i in range(20): # call_argument:20, external_free_call:range, for:i (-> +1), for_range:20 (-> +1), free_call:range, literal:20, loop:for (-> +1), loop_with_late_exit:for (-> +1), range:20, suggest_constant_definition
            l.append([int(x) for x in f.readline().split()]) # call_argument:, call_argument:x, composition, comprehension:List, comprehension_for_count:1, external_free_call:int, free_call:int, member_call:append, member_call:readline, member_call:split, member_call_object:f, member_call_object:l, member_call_without_result:append, method_chaining, update:f:x, update:l:x, update_by_member_call:f:x, update_by_member_call:l:x, update_by_member_call_with:append, update_with:append
        maximum = 0 # assignment:0, assignment_lhs_identifier:maximum, assignment_rhs_atom:0, literal:0, single_assignment:maximum
        for i in range(20): # call_argument:20, external_free_call:range, for:i (-> +4), for_range:17 (-> +4), for_range:20 (-> +4), free_call:range, literal:20, loop:for (-> +4), loop_with_late_exit:for (-> +4), range:20, suggest_constant_definition
            for j in range(17): # call_argument:17, external_free_call:range, for:j (-> +3), for_range:17 (-> +3), free_call:range, literal:17, loop:for (-> +3), loop_with_late_exit:for (-> +3), nested_for:1 (-> +3), range:17, suggest_constant_definition
        ...
        for i in range(17): # call_argument:17, external_free_call:range, for:i (-> +4), for_range:17 (-> +4), for_range:20 (-> +4), free_call:range, literal:17, loop:for (-> +4), loop_with_late_exit:for (-> +4), range:17, suggest_constant_definition
            for j in range(20): # call_argument:20, external_free_call:range, for:j (-> +3), for_range:20 (-> +3), free_call:range, literal:20, loop:for (-> +3), loop_with_late_exit:for (-> +3), nested_for:1 (-> +3), range:20, suggest_constant_definition
                temp = l[i][j] * l[i + 1][j] * l[i + 2][j] * l[i + 3][j] # addition_operator, assignment:Mult, assignment_lhs_identifier:temp, assignment_rhs_atom:1, assignment_rhs_atom:2, assignment_rhs_atom:3, assignment_rhs_atom:i, assignment_rhs_atom:j, assignment_rhs_atom:l, binary_operator:Add, binary_operator:Mult, index:_, index:i, index:j, index_arithmetic, literal:1, literal:2, literal:3, multiplication_operator, nested_index:2, single_assignment:temp, suggest_constant_definition
                if temp > maximum: # comparison_operator:Gt, if (-> +1), if_test_atom:maximum, if_test_atom:temp, if_without_else (-> +1)
                    maximum = temp # assignment, assignment_lhs_identifier:maximum, assignment_rhs_atom:temp, if_then_branch, single_assignment:maximum
        for i in range(17): # call_argument:17, external_free_call:range, for:i (-> +4), for_range:17 (-> +4), free_call:range, literal:17, loop:for (-> +4), loop_with_late_exit:for (-> +4), range:17, square_nested_for (-> +4), suggest_constant_definition
            for j in range(17): # call_argument:17, external_free_call:range, for:j (-> +3), for_range:17 (-> +3), free_call:range, literal:17, loop:for (-> +3), loop_with_late_exit:for (-> +3), nested_for:1 (-> +3), range:17, suggest_constant_definition
                temp = l[i][j] * l[i + 1][j + 1] * l[i + 2][j + 2] * l[i + 3][j + 3] # addition_operator, assignment:Mult, assignment_lhs_identifier:temp, assignment_rhs_atom:1, assignment_rhs_atom:2, assignment_rhs_atom:3, assignment_rhs_atom:i, assignment_rhs_atom:j, assignment_rhs_atom:l, binary_operator:Add, binary_operator:Mult, index:_, index:i, index:j, index_arithmetic, literal:1, literal:2, literal:3, multiplication_operator, nested_index:2, single_assignment:temp, suggest_constant_definition
                if temp > maximum: # comparison_operator:Gt, if (-> +1), if_test_atom:maximum, if_test_atom:temp, if_without_else (-> +1)
                    maximum = temp # assignment, assignment_lhs_identifier:maximum, assignment_rhs_atom:temp, if_then_branch, single_assignment:maximum
        for i in range(17): # call_argument:17, external_free_call:range, for:i (-> +4), for_range:17 (-> +4), for_range:3:20 (-> +4), free_call:range, literal:17, loop:for (-> +4), loop_with_late_exit:for (-> +4), range:17, suggest_constant_definition
            for j in range(3, 20): # call_argument:20, call_argument:3, external_free_call:range, for:j (-> +3), for_range:3:20 (-> +3), free_call:range, literal:20, literal:3, loop:for (-> +3), loop_with_late_exit:for (-> +3), nested_for:1 (-> +3), range:3:20, suggest_constant_definition
...
# problem_12/sol1.py
# ----------------------------------------------------------------------------------------
from math import sqrt # import:math:sqrt, import_module:math, import_name:sqrt, whole_span:18 (-> +17)
def count_divisors(n): # function:count_divisors (-> +7), function_argument:n, function_argument_flavor:arg, function_returning_something:count_divisors (-> +7)
    ...
    for i in range(1, int(sqrt(n)) + 1): # addition_operator, binary_operator:Add, call_argument:, call_argument:1, call_argument:n, composition, external_free_call:int, external_free_call:range, external_free_call:sqrt, for:i (-> +2), for_range:1:_ (-> +2), free_call:int, free_call:range, free_call:sqrt, literal:1, loop:for (-> +2), loop_with_late_exit:for (-> +2), range:1:_
        if n % i == 0: # binary_operator:Mod, comparison_operator:Eq, divisibility_test, if (-> +1), if_test_atom:0, if_test_atom:i, if_test_atom:n, if_without_else (-> +1), literal:0, modulo_operator
    ...
    if n ** 0.5 == int(n ** 0.5): # binary_operator:Pow, call_argument:, comparison_operator:Eq, external_free_call:int, free_call:int, if (-> +1), if_test_atom:0.5, if_test_atom:n, if_without_else (-> +1), literal:0.5, suggest_constant_definition
...
def solution(): # function:solution (-> +8), function_returning_something:solution (-> +8), function_without_arguments:solution (-> +8)
    ...
    i = 1 # assignment:1, assignment_lhs_identifier:i, assignment_rhs_atom:1, literal:1, single_assignment:i
    while True: # count_states:i (-> +4), infinite_while (-> +4), literal:True, loop:while (-> +4), loop_with_break:while (-> +4), loop_with_early_exit:while:break (-> +4), while (-> +4)
        i += 1 # assignment_lhs_identifier:i, assignment_rhs_atom:1, augmented_assignment:Add, increment:i, literal:1, update:i:1, update_by_augmented_assignment:i:1, update_by_augmented_assignment_with:Add, update_with:Add
        ...
        if count_divisors(tNum) > 500: # call_argument:tNum, comparison_operator:Gt, free_call:count_divisors, if (-> +1), if_test_atom:500, if_test_atom:tNum, if_without_else (-> +1), internal_free_call:count_divisors, literal:500, suggest_constant_definition
            break # break, if_then_branch
...
# problem_12/sol2.py
# ----------------------------------------------------------------------------------------
def triangle_number_generator(): # function:triangle_number_generator (-> +2), function_without_arguments:triangle_number_generator (-> +2), generator:triangle_number_generator (-> +2), whole_span:7 (-> +6)
    for n in range(1, 1000000): # call_argument:1, call_argument:1000000, external_free_call:range, for:n (-> +1), for_range:1:1000000 (-> +1), free_call:range, literal:1, literal:1000000, loop:for (-> +1), loop_with_late_exit:for (-> +1), range:1:1000000, suggest_constant_definition
...
def count_divisors(n): # function:count_divisors (-> +1), function_argument:n, function_argument_flavor:arg, function_returning_something:count_divisors (-> +1)
    return sum([2 for i in range(1, int(n ** 0.5) + 1) if n % i == 0 and i * i != n]) # addition_operator, binary_operator:Add, binary_operator:Mod, binary_operator:Mult, binary_operator:Pow, boolean_operator:And, call_argument:, call_argument:1, comparison_operator:Eq, comparison_operator:NotEq, composition, comprehension:List, comprehension_for_count:1, divisibility_test, external_free_call:int, external_free_call:range, external_free_call:sum, filtered_comprehension, free_call:int, free_call:range, free_call:sum, free_tail_call:sum, literal:0, literal:0.5, literal:1, literal:2, modulo_operator, multiplication_operator, range:1:_, return, suggest_constant_definition
def solution(): # function:solution (-> +1), function_returning_something:solution (-> +1), function_without_arguments:solution (-> +1)
    return next(i for i in triangle_number_generator() if count_divisors(i) > 500) # call_argument:, call_argument:i, comparison_operator:Gt, composition, comprehension:Generator, comprehension_for_count:1, external_free_call:next, filtered_comprehension, free_call:count_divisors, free_call:next, free_call:triangle_number_generator, free_call_without_arguments:triangle_number_generator, free_tail_call:next, internal_free_call:count_divisors, internal_free_call:triangle_number_generator, literal:500, return, suggest_constant_definition

# ----------------------------------------------------------------------------------------
# problem_13/sol1.py
# ----------------------------------------------------------------------------------------
def solution(array): # function:solution (-> +1), function_argument:array, function_argument_flavor:arg, function_returning_something:solution (-> +1), whole_span:2 (-> +1)
    return str(sum(array))[:10] # call_argument:, call_argument:array, composition, external_free_call:str, external_free_call:sum, free_call:str, free_call:sum, literal:10, return, slice::10:, slice_lower:, slice_step:, slice_upper:10, suggest_constant_definition
...
def solution(n): # function:solution (-> +16), function_argument:n, function_argument_flavor:arg, function_returning_something:solution (-> +16), whole_span:17 (-> +16)
    ...
    for input1 in range(n): # call_argument:n, external_free_call:range, for:input1 (-> +12), for_range:n (-> +12), free_call:range, loop:for (-> +12), loop_with_late_exit:for (-> +12), range:n
        ...
        while number > 1: # comparison_operator:Gt, literal:1, loop:while (-> +6), loop_with_late_exit:while (-> +6), while (-> +6)
...
# problem_15/sol1.py
# ----------------------------------------------------------------------------------------
from math import factorial # import:math:factorial, import_module:math, import_name:factorial, whole_span:5 (-> +4)
def lattice_paths(n): # function:lattice_paths (-> +3), function_argument:n, function_argument_flavor:arg, function_returning_something:lattice_paths (-> +3)
    ...
    return int(factorial(n) / (factorial(k) * factorial(n - k))) # binary_operator:Div, binary_operator:Mult, binary_operator:Sub, call_argument:, call_argument:k, call_argument:n, composition, external_free_call:factorial, external_free_call:int, free_call:factorial, free_call:int, free_tail_call:int, multiplication_operator, return

# ----------------------------------------------------------------------------------------
# problem_16/sol1.py
# ----------------------------------------------------------------------------------------
def solution(power): # function:solution (-> +7), function_argument:power, function_argument_flavor:arg, function_returning_something:solution (-> +7), whole_span:8 (-> +7)
    ...
    string_num = str(num) # assignment:str, assignment_lhs_identifier:string_num, assignment_rhs_atom:num, call_argument:num, external_free_call:str, free_call:str, single_assignment:string_num
    list_num = list(string_num) # assignment:list, assignment_lhs_identifier:list_num, assignment_rhs_atom:string_num, call_argument:string_num, external_free_call:list, free_call:list, single_assignment:list_num
    ...
    for i in list_num: # accumulate_all_elements:Add (-> +1), accumulate_elements:Add (-> +1), for:i (-> +1), for_each (-> +1), loop:for (-> +1), loop_with_late_exit:for (-> +1)
        sum_of_num += int(i) # assignment_lhs_identifier:sum_of_num, assignment_rhs_atom:i, augmented_assignment:Add, call_argument:i, external_free_call:int, free_call:int, update:sum_of_num:i, update_by_augmented_assignment:sum_of_num:i, update_by_augmented_assignment_with:Add, update_with:Add
    return sum_of_num # return:sum_of_num

# ----------------------------------------------------------------------------------------
# problem_16/sol2.py
# ----------------------------------------------------------------------------------------
def solution(power): # function:solution (-> +5), function_argument:power, function_argument_flavor:arg, function_returning_something:solution (-> +5), whole_span:6 (-> +5)
...
# problem_17/sol1.py
# ----------------------------------------------------------------------------------------
def solution(n): # function:solution (-> +17), function_argument:n, function_argument_flavor:arg, function_returning_something:solution (-> +17), whole_span:18 (-> +17)
    ...
    count = 0 # assignment:0, assignment_lhs_identifier:count, assignment_rhs_atom:0, literal:0, single_assignment:count
    for i in range(1, n + 1): # accumulate_elements:Add (-> +12), accumulate_some_elements:Add (-> +12), addition_operator, binary_operator:Add, call_argument:, call_argument:1, external_free_call:range, for:i (-> +12), for_range:1:_ (-> +12), free_call:range, literal:1, loop:for (-> +12), loop_with_late_exit:for (-> +12), range:1:_
...
# ----------------------------------------------------------------------------------------
import os # import:os, import_module:os, whole_span:19 (-> +18)
def solution(): # function:solution (-> +17), function_returning_something:solution (-> +17), function_without_arguments:solution (-> +17)
    script_dir = os.path.dirname(os.path.realpath(__file__)) # assignment:dirname, assignment_lhs_identifier:script_dir, assignment_rhs_atom:__file__, assignment_rhs_atom:os, call_argument:, call_argument:__file__, composition, member_call:dirname, member_call:realpath, single_assignment:script_dir
    triangle = os.path.join(script_dir, "triangle.txt") # assignment:join, assignment_lhs_identifier:triangle, assignment_rhs_atom:os, assignment_rhs_atom:script_dir, call_argument:, call_argument:script_dir, literal:Str, member_call:join, single_assignment:triangle
    with open(triangle, "r") as f: # call_argument:, call_argument:triangle, external_free_call:open, free_call:open, literal:Str
        triangle = f.readlines() # assignment:readlines, assignment_lhs_identifier:triangle, assignment_rhs_atom:f, member_call:readlines, single_assignment:triangle
    a = [[int(y) for y in x.rstrip("\r\n").split(" ")] for x in triangle] # assignment, assignment_lhs_identifier:a, assignment_rhs_atom:triangle, assignment_rhs_atom:x, assignment_rhs_atom:y, call_argument:, call_argument:y, comprehension:List, comprehension_for_count:1, external_free_call:int, free_call:int, literal:Str, member_call:rstrip, member_call:split, member_call_object:x, method_chaining, single_assignment:a
    for i in range(1, len(a)): # call_argument:, call_argument:1, call_argument:a, composition, external_free_call:len, external_free_call:range, for:i (-> +10), for_range:1:_ (-> +10), for_range:_ (-> +10), free_call:len, free_call:range, literal:1, loop:for (-> +10), loop_with_late_exit:for (-> +10), range:1:_
        for j in range(len(a[i])): # call_argument:, composition, external_free_call:len, external_free_call:range, for:j (-> +9), for_indexes (-> +9), for_range:_ (-> +9), free_call:len, free_call:range, index:i, loop:for (-> +9), loop_with_late_exit:for (-> +9), nested_for:1 (-> +9), range:_
            if j != len(a[i - 1]): # binary_operator:Sub, call_argument:, comparison_operator:NotEq, external_free_call:len, free_call:len, if (-> +3), if_test_atom:1, if_test_atom:a, if_test_atom:i, if_test_atom:j, index:_, index_arithmetic, literal:1, verbose_conditional_assignment (-> +3)
            ...
            a[i][j] += max(number1, number2) # assignment_rhs_atom:number1, assignment_rhs_atom:number2, augmented_assignment:Add, call_argument:number1, call_argument:number2, external_free_call:max, free_call:max, index:i, index:j, nested_index:2, subscript_augmented_assignment:Add
    return max(a[-1]) # call_argument:, external_free_call:max, free_call:max, free_tail_call:max, index:-1, literal:-1, negative_index:-1, return

# ----------------------------------------------------------------------------------------
# problem_19/sol1.py
# ----------------------------------------------------------------------------------------
def solution(): # function:solution (-> +24), function_returning_something:solution (-> +24), function_without_arguments:solution (-> +24), whole_span:25 (-> +24)
    ...
    while year < 2001: # comparison_operator:Lt, count_states:sundays (-> +17), count_states:year (-> +17), literal:2001, loop:while (-> +17), loop_with_late_exit:while (-> +17), suggest_constant_definition, while (-> +17)
...
# problem_20/sol1.py
# ----------------------------------------------------------------------------------------
def factorial(n): # function:factorial (-> +4), function_argument:n, function_argument_flavor:arg, function_returning_something:factorial (-> +4), whole_span:16 (-> +15)
    fact = 1 # assignment:1, assignment_lhs_identifier:fact, assignment_rhs_atom:1, literal:1, single_assignment:fact
    for i in range(1, n + 1): # accumulate_all_elements:Mult (-> +1), accumulate_elements:Mult (-> +1), addition_operator, binary_operator:Add, call_argument:, call_argument:1, external_free_call:range, for:i (-> +1), for_range:1:_ (-> +1), free_call:range, literal:1, loop:for (-> +1), loop_with_late_exit:for (-> +1), range:1:_
...
def split_and_add(number): # function:split_and_add (-> +6), function_argument:number, function_argument_flavor:arg, function_returning_something:split_and_add (-> +6)
    ...
    while number > 0: # comparison_operator:Gt, literal:0, loop:while (-> +3), loop_with_late_exit:while (-> +3), while (-> +3)
...
def solution(n): # function:solution (-> +3), function_argument:n, function_argument_flavor:arg, function_returning_something:solution (-> +3)
    f = factorial(n) # assignment:factorial, assignment_lhs_identifier:f, assignment_rhs_atom:n, call_argument:n, free_call:factorial, internal_free_call:factorial, single_assignment:f
    result = split_and_add(f) # assignment:split_and_add, assignment_lhs_identifier:result, assignment_rhs_atom:f, call_argument:f, free_call:split_and_add, internal_free_call:split_and_add, single_assignment:result
    return result # return:result

# ----------------------------------------------------------------------------------------
# problem_20/sol2.py
# ----------------------------------------------------------------------------------------
from math import factorial # import:math:factorial, import_module:math, import_name:factorial, whole_span:3 (-> +2)
def solution(n): # function:solution (-> +1), function_argument:n, function_argument_flavor:arg, function_returning_something:solution (-> +1)
    return sum([int(x) for x in str(factorial(n))]) # call_argument:, call_argument:n, call_argument:x, composition, comprehension:List, comprehension_for_count:1, external_free_call:factorial, external_free_call:int, external_free_call:str, external_free_call:sum, free_call:factorial, free_call:int, free_call:str, free_call:sum, free_tail_call:sum, return
...
from math import factorial # import:math:factorial, import_module:math, import_name:factorial, whole_span:3 (-> +2)
def solution(n): # function:solution (-> +1), function_argument:n, function_argument_flavor:arg, function_returning_something:solution (-> +1)
    return sum(map(int, str(factorial(n)))) # call_argument:, call_argument:int, call_argument:n, composition, external_free_call:factorial, external_free_call:map, external_free_call:str, external_free_call:sum, free_call:factorial, free_call:map, free_call:str, free_call:sum, free_tail_call:sum, return
...
def solution(n): # function:solution (-> +7), function_argument:n, function_argument_flavor:arg, function_returning_something:solution (-> +7), whole_span:8 (-> +7)
    ...
    for i in range(1, n + 1): # accumulate_all_elements:Mult (-> +1), accumulate_elements:Mult (-> +1), addition_operator, binary_operator:Add, call_argument:, call_argument:1, external_free_call:range, for:i (-> +1), for_range:1:_ (-> +1), free_call:range, literal:1, loop:for (-> +1), loop_with_late_exit:for (-> +1), range:1:_
        fact *= i # assignment_lhs_identifier:fact, assignment_rhs_atom:i, augmented_assignment:Mult, update:fact:i, update_by_augmented_assignment:fact:i, update_by_augmented_assignment_with:Mult, update_with:Mult
    for j in str(fact): # accumulate_all_elements:Add (-> +1), accumulate_elements:Add (-> +1), call_argument:fact, external_free_call:str, for:j (-> +1), free_call:str, loop:for (-> +1), loop_with_late_exit:for (-> +1)
        result += int(j) # assignment_lhs_identifier:result, assignment_rhs_atom:j, augmented_assignment:Add, call_argument:j, external_free_call:int, free_call:int, update:result:j, update_by_augmented_assignment:result:j, update_by_augmented_assignment_with:Add, update_with:Add
    return result # return:result
...
def sum_of_divisors(n): # function:sum_of_divisors (-> +7), function_argument:n, function_argument_flavor:arg, function_returning_something:sum_of_divisors (-> +7)
    ...
    for i in range(1, int(sqrt(n) + 1)): # accumulate_elements:Add (-> +4), accumulate_some_elements:Add (-> +4), addition_operator, binary_operator:Add, call_argument:, call_argument:1, call_argument:n, composition, external_free_call:int, external_free_call:range, external_free_call:sqrt, for:i (-> +4), for_range:1:_ (-> +4), free_call:int, free_call:range, free_call:sqrt, literal:1, loop:for (-> +4), loop_with_late_exit:for (-> +4), range:1:_
        if n % i == 0 and i != sqrt(n): # binary_operator:Mod, boolean_operator:And, call_argument:n, comparison_operator:Eq, comparison_operator:NotEq, divisibility_test, external_free_call:sqrt, free_call:sqrt, if (-> +3), if_test_atom:0, if_test_atom:i, if_test_atom:n, literal:0, modulo_operator
        ...
        elif i == sqrt(n): # call_argument:n, comparison_operator:Eq, external_free_call:sqrt, free_call:sqrt, if (-> +1), if_test_atom:i, if_test_atom:n
...
def solution(n): # function:solution (-> +8), function_argument:n, function_argument_flavor:arg, function_returning_something:solution (-> +8)
    total = sum( # assignment:sum, assignment_lhs_identifier:total, composition, external_free_call:sum, free_call:sum, single_assignment:total
        [
            i # assignment_rhs_atom:i, call_argument:, comprehension:List, comprehension_for_count:1
            for i in range(1, n) # assignment_rhs_atom:1, assignment_rhs_atom:i, assignment_rhs_atom:n, call_argument:1, call_argument:n, external_free_call:range, free_call:range, literal:1, range:1:n
            if sum_of_divisors(sum_of_divisors(i)) == i and sum_of_divisors(i) != i # assignment_rhs_atom:i, boolean_operator:And, call_argument:, call_argument:i, comparison_operator:Eq, comparison_operator:NotEq, composition, filtered_comprehension, free_call:sum_of_divisors, internal_free_call:sum_of_divisors
        ]
    )
...
# problem_22/sol1.py
# ----------------------------------------------------------------------------------------
import os # import:os, import_module:os, whole_span:14 (-> +13)
def solution(): # function:solution (-> +12), function_returning_something:solution (-> +12), function_without_arguments:solution (-> +12)
    with open(os.path.dirname(__file__) + "/p022_names.txt") as file: # binary_operator:Add, call_argument:, call_argument:__file__, composition, concatenation_operator:Str, external_free_call:open, free_call:open, literal:Str, member_call:dirname
        names = str(file.readlines()[0]) # assignment:str, assignment_lhs_identifier:names, assignment_rhs_atom:0, assignment_rhs_atom:file, call_argument:, composition, external_free_call:str, free_call:str, index:0, literal:0, member_call:readlines, member_call_object:file, single_assignment:names
        names = names.replace('"', "").split(",") # assignment:split, assignment_lhs_identifier:names, assignment_rhs_atom:names, call_argument:, empty_literal:Str, literal:Str, member_call:replace, member_call:split, member_call_object:names, method_chaining, single_assignment:names
    ...
    for i, name in enumerate(names): # accumulate_all_elements:Add (-> +4), accumulate_elements:Add (-> +4), call_argument:names, external_free_call:enumerate, for:i (-> +4), for:name (-> +4), for_indexes_elements (-> +4), free_call:enumerate, literal:Tuple, loop:for (-> +4), loop_with_late_exit:for (-> +4)
...
def solution(): # function:solution (-> +12), function_returning_something:solution (-> +12), function_without_arguments:solution (-> +12)
    ...
    with open(os.path.dirname(__file__) + "/p022_names.txt") as file: # binary_operator:Add, call_argument:, call_argument:__file__, composition, concatenation_operator:Str, external_free_call:open, free_call:open, literal:Str, member_call:dirname
        name = str(file.readlines()[0]) # assignment:str, assignment_lhs_identifier:name, assignment_rhs_atom:0, assignment_rhs_atom:file, call_argument:, composition, external_free_call:str, free_call:str, index:0, literal:0, member_call:readlines, member_call_object:file, single_assignment:name
        name = name.replace('"', "").split(",") # assignment:split, assignment_lhs_identifier:name, assignment_rhs_atom:name, call_argument:, empty_literal:Str, literal:Str, member_call:replace, member_call:split, member_call_object:name, method_chaining, single_assignment:name
    name.sort() # member_call:sort, member_call_object:name, member_call_without_result:sort
    for i in range(len(name)): # accumulate_all_elements:Add (-> +4), accumulate_elements:Add (-> +4), call_argument:, call_argument:name, composition, external_free_call:len, external_free_call:range, for:i (-> +4), for_indexes (-> +4), for_range:_ (-> +4), free_call:len, free_call:range, loop:for (-> +4), loop_with_late_exit:for (-> +4), range:_
        for j in name[i]: # accumulate_all_elements:Add (-> +1), accumulate_elements:Add (-> +1), for:j (-> +1), index:i, loop:for (-> +1), loop_with_late_exit:for (-> +1), nested_for:1 (-> +1)
            temp_sum += ord(j) - ord("A") + 1 # addition_operator, assignment_lhs_identifier:temp_sum, assignment_rhs_atom:1, assignment_rhs_atom:j, augmented_assignment:Add, binary_operator:Add, binary_operator:Sub, call_argument:, call_argument:j, external_free_call:ord, free_call:ord, literal:1, literal:Str, update:temp_sum:1, update:temp_sum:j, update_by_augmented_assignment:temp_sum:1, update_by_augmented_assignment:temp_sum:j, update_by_augmented_assignment_with:Add, update_with:Add
...
# problem_23/sol1.py
# ----------------------------------------------------------------------------------------
def solution(limit=28123): # function:solution (-> +13), function_argument:limit, function_argument_flavor:arg, function_returning_something:solution (-> +13), literal:28123, whole_span:14 (-> +13)
    ...
    for i in range(2, int(limit ** 0.5) + 1): # accumulate_all_elements:Add (-> +3), accumulate_elements:Add (-> +3), addition_operator, binary_operator:Add, binary_operator:Pow, call_argument:, call_argument:2, composition, external_free_call:int, external_free_call:range, for:i (-> +3), for_range:2:_ (-> +3), for_range:_:_ (-> +3), free_call:int, free_call:range, literal:0.5, literal:1, literal:2, loop:for (-> +3), loop_with_late_exit:for (-> +3), range:2:_, suggest_constant_definition
        ...
        for k in range(i + 1, limit // i + 1): # accumulate_all_elements:Add (-> +1), accumulate_elements:Add (-> +1), addition_operator, binary_operator:Add, binary_operator:FloorDiv, call_argument:, external_free_call:range, for:k (-> +1), for_range:_:_ (-> +1), free_call:range, literal:1, loop:for (-> +1), loop_with_late_exit:for (-> +1), nested_for:1 (-> +1), range:_:_
    ...
    abundants = set() # assignment:set, assignment_lhs_identifier:abundants, external_free_call:set, free_call:set, free_call_without_arguments:set, single_assignment:abundants
    res = 0 # assignment:0, assignment_lhs_identifier:res, assignment_rhs_atom:0, literal:0, single_assignment:res
    for n in range(1, limit + 1): # accumulate_elements:Add (-> +4), accumulate_elements:add (-> +4), accumulate_some_elements:Add (-> +4), accumulate_some_elements:add (-> +4), addition_operator, binary_operator:Add, call_argument:, call_argument:1, external_free_call:range, for:n (-> +4), for_range:1:_ (-> +4), free_call:range, literal:1, loop:for (-> +4), loop_with_late_exit:for (-> +4), range:1:_
        if sumDivs[n] > n: # comparison_operator:Gt, if (-> +1), if_test_atom:n, if_test_atom:sumDivs, if_without_else (-> +1), index:n
            abundants.add(n) # call_argument:n, if_then_branch, member_call:add, member_call_object:abundants, member_call_without_result:add, update:abundants:n, update_by_member_call:abundants:n, update_by_member_call_with:add, update_with:add
        if not any((n - a in abundants) for a in abundants): # binary_operator:Sub, call_argument:, comparison_operator:In, comprehension:Generator, comprehension_for_count:1, external_free_call:any, free_call:any, if (-> +1), if_test_atom:a, if_test_atom:abundants, if_test_atom:n, if_without_else (-> +1), unary_operator:Not
...
# problem_234/sol1.py
# ----------------------------------------------------------------------------------------
def fib(a, b, n): # function:fib (-> +13), function_argument:a, function_argument:b, function_argument:n, function_argument_flavor:arg, function_returning_something:fib (-> +13), whole_span:26 (-> +25)
    if n == 1: # comparison_operator:Eq, if (-> +5), if_test_atom:1, if_test_atom:n, literal:1
        ...
        return str(a) + str(b) # addition_operator, binary_operator:Add, call_argument:a, call_argument:b, external_free_call:str, free_call:str, if_elif_branch, return
    temp = 0 # assignment:0, assignment_lhs_identifier:temp, assignment_rhs_atom:0, literal:0, single_assignment:temp
    for x in range(2, n): # call_argument:2, call_argument:n, external_free_call:range, for:x (-> +4), for_range:2:n (-> +4), free_call:range, literal:2, loop:for (-> +4), loop_with_late_exit:for (-> +4), range:2:n
        c = str(a) + str(b) # addition_operator, assignment:Add, assignment_lhs_identifier:c, assignment_rhs_atom:a, assignment_rhs_atom:b, binary_operator:Add, call_argument:a, call_argument:b, external_free_call:str, free_call:str, single_assignment:c
...
def solution(n): # function:solution (-> +11), function_argument:n, function_argument_flavor:arg, function_returning_something:solution (-> +11)
    ...
    for x in range(n): # call_argument:n, external_free_call:range, for:x (-> +8), for_range:n (-> +8), free_call:range, loop:for (-> +8), loop_with_late_exit:for (-> +8), range:n
        l = [i for i in input().split()] # assignment, assignment_lhs_identifier:l, assignment_rhs_atom:i, comprehension:List, comprehension_for_count:1, external_free_call:input, free_call:input, free_call_without_arguments:input, member_call:split, single_assignment:l
        c2 = 1 # assignment:1, assignment_lhs_identifier:c2, assignment_rhs_atom:1, literal:1, single_assignment:c2
        while 1: # count_states:c2 (-> +4), infinite_while (-> +4), literal:1, loop:while (-> +4), loop_with_break:while (-> +4), loop_with_early_exit:while:break (-> +4), while (-> +4)
            if len(fib(l[0], l[1], c2)) < int(l[2]): # call_argument:, call_argument:c2, comparison_operator:Lt, composition, external_free_call:int, external_free_call:len, free_call:fib, free_call:int, free_call:len, if (-> +3), if_test_atom:0, if_test_atom:1, if_test_atom:2, if_test_atom:c2, if_test_atom:l, index:0, index:1, index:2, internal_free_call:fib, literal:0, literal:1, literal:2
                ...
                break # break, if_else_branch
        semidivisible.append(fib(l[0], l[1], c2 + 1)[int(l[2]) - 1]) # addition_operator, binary_operator:Add, binary_operator:Sub, call_argument:, composition, external_free_call:int, free_call:fib, free_call:int, index:0, index:1, index:2, index:_, index_arithmetic, internal_free_call:fib, literal:0, literal:1, literal:2, member_call:append, member_call_object:semidivisible, member_call_without_result:append, nested_index:4
    return semidivisible # return:semidivisible

# ----------------------------------------------------------------------------------------
# problem_24/sol1.py
# ----------------------------------------------------------------------------------------
from itertools import permutations # import:itertools:permutations, import_module:itertools, import_name:permutations, whole_span:4 (-> +3)
def solution(): # function:solution (-> +2), function_returning_something:solution (-> +2), function_without_arguments:solution (-> +2)
    result = list(map("".join, permutations("0123456789"))) # assignment:list, assignment_lhs_identifier:result, call_argument:, composition, empty_literal:Str, external_free_call:list, external_free_call:map, external_free_call:permutations, free_call:list, free_call:map, free_call:permutations, literal:Str, single_assignment:result
    return result[999999] # index:999999, literal:999999, return, suggest_constant_definition

# ----------------------------------------------------------------------------------------
# problem_25/sol1.py
# ----------------------------------------------------------------------------------------
def fibonacci(n): # function:fibonacci (-> +9), function_argument:n, function_argument_flavor:arg, function_returning_something:fibonacci (-> +9), whole_span:19 (-> +18)
    if n == 1 or type(n) is not int: # boolean_operator:Or, call_argument:n, comparison_operator:Eq, comparison_operator:IsNot, external_free_call:type, free_call:type, if (-> +8), if_test_atom:1, if_test_atom:int, if_test_atom:n, literal:1
        return 0 # if_then_branch, literal:0, return:0
        ...
        for i in range(2, n + 1): # addition_operator, binary_operator:Add, call_argument:, call_argument:2, external_free_call:range, for:i (-> +1), for_range:2:_ (-> +1), free_call:range, literal:1, literal:2, loop:for (-> +1), loop_with_late_exit:for (-> +1), range:2:_
            sequence.append(sequence[i - 1] + sequence[i - 2]) # addition_operator, binary_operator:Add, binary_operator:Sub, call_argument:, index:_, index_arithmetic, literal:1, literal:2, member_call:append, member_call_object:sequence, member_call_without_result:append
...
def fibonacci_digits_index(n): # function:fibonacci_digits_index (-> +6), function_argument:n, function_argument_flavor:arg, function_returning_something:fibonacci_digits_index (-> +6)
    ...
    while digits < n: # comparison_operator:Lt, count_states:index (-> +2), loop:while (-> +2), loop_with_late_exit:while (-> +2), while (-> +2)
        ...
        digits = len(str(fibonacci(index))) # assignment:len, assignment_lhs_identifier:digits, assignment_rhs_atom:index, call_argument:, call_argument:index, composition, external_free_call:len, external_free_call:str, free_call:fibonacci, free_call:len, free_call:str, internal_free_call:fibonacci, single_assignment:digits
    return index # return:index
def solution(n): # function:solution (-> +1), function_argument:n, function_argument_flavor:arg, function_returning_something:solution (-> +1)
    return fibonacci_digits_index(n) # call_argument:n, free_call:fibonacci_digits_index, free_tail_call:fibonacci_digits_index, internal_free_call:fibonacci_digits_index, return
...
def solution(n): # function:solution (-> +5), function_argument:n, function_argument_flavor:arg, function_returning_something:solution (-> +5)
    ...
    gen = fibonacci_generator() # assignment:fibonacci_generator, assignment_lhs_identifier:gen, free_call:fibonacci_generator, free_call_without_arguments:fibonacci_generator, internal_free_call:fibonacci_generator, single_assignment:gen
    while len(str(next(gen))) < n: # call_argument:, call_argument:gen, comparison_operator:Lt, composition, count_states:answer (-> +1), external_free_call:len, external_free_call:next, external_free_call:str, free_call:len, free_call:next, free_call:str, loop:while (-> +1), loop_with_late_exit:while (-> +1), while (-> +1)
...
def solution(n): # function:solution (-> +12), function_argument:n, function_argument_flavor:arg, function_returning_something:solution (-> +12), whole_span:13 (-> +12)
    ...
    index = 2 # assignment:2, assignment_lhs_identifier:index, assignment_rhs_atom:2, literal:2, single_assignment:index
    while True: # count_states:index (-> +8), infinite_while (-> +8), literal:True, loop:while (-> +8), loop_with_break:while (-> +8), loop_with_early_exit:while:break (-> +8), while (-> +8)
        i = 0 # assignment:0, assignment_lhs_identifier:i, assignment_rhs_atom:0, literal:0, single_assignment:i
...
# ----------------------------------------------------------------------------------------
import math # import:math, import_module:math, whole_span:25 (-> +24)
def is_prime(k: int) -> bool: # function:is_prime (-> +9), function_argument:k, function_argument_flavor:arg, function_returning_something:is_prime (-> +9)
    if k < 2 or k % 2 == 0: # binary_operator:Mod, boolean_operator:Or, comparison_operator:Eq, comparison_operator:Lt, divisibility_test:2, if (-> +7), if_test_atom:0, if_test_atom:2, if_test_atom:k, literal:0, literal:2, modulo_operator
        ...
        for x in range(3, int(math.sqrt(k) + 1), 2): # addition_operator, binary_operator:Add, call_argument:, call_argument:2, call_argument:3, call_argument:k, composition, external_free_call:int, external_free_call:range, for:x (-> +2), for_range:3:_:2 (-> +2), free_call:int, free_call:range, if_else_branch (-> +2), literal:1, literal:2, literal:3, loop:for (-> +2), loop_with_early_exit:for:return (-> +2), loop_with_return:for (-> +2), member_call:sqrt, range:3:_:2, suggest_constant_definition, universal_quantification:x (-> +2)
...
def solution(a_limit: int, b_limit: int) -> int: # function:solution (-> +13), function_argument:a_limit, function_argument:b_limit, function_argument_flavor:arg, function_returning_something:solution (-> +13)
    ...
    for a in range((a_limit * -1) + 1, a_limit): # addition_operator, binary_operator:Add, binary_operator:Mult, call_argument:, call_argument:a_limit, external_free_call:range, for:a (-> +9), for_range:2:b_limit (-> +9), for_range:_:a_limit (-> +9), free_call:range, literal:-1, literal:1, loop:for (-> +9), loop_with_late_exit:for (-> +9), multiplication_operator, range:_:a_limit
        for b in range(2, b_limit): # call_argument:2, call_argument:b_limit, external_free_call:range, for:b (-> +8), for_range:2:b_limit (-> +8), free_call:range, literal:2, loop:for (-> +8), loop_with_late_exit:for (-> +8), nested_for:1 (-> +8), range:2:b_limit
            if is_prime(b): # call_argument:b, free_call:is_prime, if (-> +7), if_test_atom:b, if_without_else (-> +7), internal_free_call:is_prime
                ...
                while is_prime((n ** 2) + (a * n) + b): # addition_operator, binary_operator:Add, binary_operator:Mult, binary_operator:Pow, call_argument:, count_states:count (-> +2), count_states:n (-> +2), free_call:is_prime, internal_free_call:is_prime, literal:2, loop:while (-> +2), loop_with_late_exit:while (-> +2), multiplication_operator, while (-> +2)
...
# problem_28/sol1.py
# ----------------------------------------------------------------------------------------
from math import ceil # import:math:ceil, import_module:math, import_name:ceil, whole_span:8 (-> +7)
def diagonal_sum(n): # function:diagonal_sum (-> +6), function_argument:n, function_argument_flavor:arg, function_returning_something:diagonal_sum (-> +6)
    ...
    for i in range(1, int(ceil(n / 2.0))): # binary_operator:Div, call_argument:, call_argument:1, composition, external_free_call:ceil, external_free_call:int, external_free_call:range, for:i (-> +3), for_range:1:_ (-> +3), free_call:ceil, free_call:int, free_call:range, literal:1, literal:2.0, loop:for (-> +3), loop_with_late_exit:for (-> +3), range:1:_, suggest_constant_definition
...
# ----------------------------------------------------------------------------------------
def solution(n): # function:solution (-> +8), function_argument:n, function_argument_flavor:arg, function_returning_something:solution (-> +8), whole_span:9 (-> +8)
    collectPowers = set() # assignment:set, assignment_lhs_identifier:collectPowers, external_free_call:set, free_call:set, free_call_without_arguments:set, single_assignment:collectPowers
    ...
    for a in range(2, N): # call_argument:2, call_argument:N, external_free_call:range, for:a (-> +3), for_range:2:N (-> +3), free_call:range, literal:2, loop:for (-> +3), loop_with_late_exit:for (-> +3), range:2:N, square_nested_for (-> +3)
        for b in range(2, N): # call_argument:2, call_argument:N, external_free_call:range, for:b (-> +2), for_range:2:N (-> +2), free_call:range, literal:2, loop:for (-> +2), loop_with_late_exit:for (-> +2), nested_for:1 (-> +2), range:2:N
            ...
            collectPowers.add(currentPow) # call_argument:currentPow, member_call:add, member_call_object:collectPowers, member_call_without_result:add, update:collectPowers:currentPow, update_by_member_call:collectPowers:currentPow, update_by_member_call_with:add, update_with:add
    return len(collectPowers) # call_argument:collectPowers, external_free_call:len, free_call:len, free_tail_call:len, return

# ----------------------------------------------------------------------------------------
# problem_31/sol1.py
# ----------------------------------------------------------------------------------------
def one_pence(): # function:one_pence (-> +1), function_returning_something:one_pence (-> +1), function_without_arguments:one_pence (-> +1), whole_span:18 (-> +17)
...
def two_pence(x): # body_recursive_function:two_pence (-> +1), function:two_pence (-> +1), function_argument:x, function_argument_flavor:arg, function_returning_something:two_pence (-> +1), recursive_function:two_pence (-> +1)
    return 0 if x < 0 else two_pence(x - 2) + one_pence() # addition_operator, binary_operator:Add, binary_operator:Sub, call_argument:, comparison_operator:Lt, conditional_expression, free_call:one_pence, free_call:two_pence, free_call_without_arguments:one_pence, internal_free_call:one_pence, internal_free_call:two_pence, literal:0, literal:2, return
def five_pence(x): # body_recursive_function:five_pence (-> +1), function:five_pence (-> +1), function_argument:x, function_argument_flavor:arg, function_returning_something:five_pence (-> +1), recursive_function:five_pence (-> +1)
    return 0 if x < 0 else five_pence(x - 5) + two_pence(x) # addition_operator, binary_operator:Add, binary_operator:Sub, call_argument:, call_argument:x, comparison_operator:Lt, conditional_expression, free_call:five_pence, free_call:two_pence, internal_free_call:five_pence, internal_free_call:two_pence, literal:0, literal:5, return, suggest_constant_definition
def ten_pence(x): # body_recursive_function:ten_pence (-> +1), function:ten_pence (-> +1), function_argument:x, function_argument_flavor:arg, function_returning_something:ten_pence (-> +1), recursive_function:ten_pence (-> +1)
    return 0 if x < 0 else ten_pence(x - 10) + five_pence(x) # addition_operator, binary_operator:Add, binary_operator:Sub, call_argument:, call_argument:x, comparison_operator:Lt, conditional_expression, free_call:five_pence, free_call:ten_pence, internal_free_call:five_pence, internal_free_call:ten_pence, literal:0, literal:10, return, suggest_constant_definition
def twenty_pence(x): # body_recursive_function:twenty_pence (-> +1), function:twenty_pence (-> +1), function_argument:x, function_argument_flavor:arg, function_returning_something:twenty_pence (-> +1), recursive_function:twenty_pence (-> +1)
    return 0 if x < 0 else twenty_pence(x - 20) + ten_pence(x) # addition_operator, binary_operator:Add, binary_operator:Sub, call_argument:, call_argument:x, comparison_operator:Lt, conditional_expression, free_call:ten_pence, free_call:twenty_pence, internal_free_call:ten_pence, internal_free_call:twenty_pence, literal:0, literal:20, return, suggest_constant_definition
def fifty_pence(x): # body_recursive_function:fifty_pence (-> +1), function:fifty_pence (-> +1), function_argument:x, function_argument_flavor:arg, function_returning_something:fifty_pence (-> +1), recursive_function:fifty_pence (-> +1)
    return 0 if x < 0 else fifty_pence(x - 50) + twenty_pence(x) # addition_operator, binary_operator:Add, binary_operator:Sub, call_argument:, call_argument:x, comparison_operator:Lt, conditional_expression, free_call:fifty_pence, free_call:twenty_pence, internal_free_call:fifty_pence, internal_free_call:twenty_pence, literal:0, literal:50, return, suggest_constant_definition
def one_pound(x): # body_recursive_function:one_pound (-> +1), function:one_pound (-> +1), function_argument:x, function_argument_flavor:arg, function_returning_something:one_pound (-> +1), recursive_function:one_pound (-> +1)
    return 0 if x < 0 else one_pound(x - 100) + fifty_pence(x) # addition_operator, binary_operator:Add, binary_operator:Sub, call_argument:, call_argument:x, comparison_operator:Lt, conditional_expression, free_call:fifty_pence, free_call:one_pound, internal_free_call:fifty_pence, internal_free_call:one_pound, literal:0, literal:100, return, suggest_constant_definition
def two_pound(x): # body_recursive_function:two_pound (-> +1), function:two_pound (-> +1), function_argument:x, function_argument_flavor:arg, function_returning_something:two_pound (-> +1), recursive_function:two_pound (-> +1)
    return 0 if x < 0 else two_pound(x - 200) + one_pound(x) # addition_operator, binary_operator:Add, binary_operator:Sub, call_argument:, call_argument:x, comparison_operator:Lt, conditional_expression, free_call:one_pound, free_call:two_pound, internal_free_call:one_pound, internal_free_call:two_pound, literal:0, literal:200, return, suggest_constant_definition
def solution(n): # function:solution (-> +1), function_argument:n, function_argument_flavor:arg, function_returning_something:solution (-> +1)
    return two_pound(n) # call_argument:n, free_call:two_pound, free_tail_call:two_pound, internal_free_call:two_pound, return

# ----------------------------------------------------------------------------------------
# problem_32/sol32.py
# ----------------------------------------------------------------------------------------
import itertools # import:itertools, import_module:itertools, whole_span:16 (-> +15)
def isCombinationValid(combination): # function:isCombinationValid (-> +6), function_argument:combination, function_argument_flavor:arg, function_returning_something:isCombinationValid (-> +6)
    return ( # boolean_operator:Or, return
        int("".join(combination[0:2])) * int("".join(combination[2:5])) # binary_operator:Mult, call_argument:, composition, empty_literal:Str, external_free_call:int, free_call:int, literal:0, literal:2, literal:5, literal:Str, member_call:join, multiplication_operator, slice:0:2:, slice:2:5:, slice_lower:0, slice_lower:2, slice_step:, slice_upper:2, slice_upper:5, suggest_constant_definition
        == int("".join(combination[5:9])) # call_argument:, comparison_operator:Eq, composition, empty_literal:Str, external_free_call:int, free_call:int, literal:5, literal:9, literal:Str, member_call:join, slice:5:9:, slice_lower:5, slice_step:, slice_upper:9, suggest_constant_definition
    ) or (
        int("".join(combination[0])) * int("".join(combination[1:5])) # binary_operator:Mult, call_argument:, composition, empty_literal:Str, external_free_call:int, free_call:int, index:0, literal:0, literal:1, literal:5, literal:Str, member_call:join, multiplication_operator, slice:1:5:, slice_lower:1, slice_step:, slice_upper:5, suggest_constant_definition
        == int("".join(combination[5:9])) # call_argument:, comparison_operator:Eq, composition, empty_literal:Str, external_free_call:int, free_call:int, literal:5, literal:9, literal:Str, member_call:join, slice:5:9:, slice_lower:5, slice_step:, slice_upper:9, suggest_constant_definition
    )
def solution(): # function:solution (-> +6), function_returning_something:solution (-> +6), function_without_arguments:solution (-> +6)
    return sum( # composition, external_free_call:sum, free_call:sum, free_tail_call:sum, return
        set( # call_argument:, composition, external_free_call:set, free_call:set
            [
                int("".join(pandigital[5:9])) # call_argument:, composition, comprehension:List, comprehension_for_count:1, empty_literal:Str, external_free_call:int, free_call:int, literal:5, literal:9, literal:Str, member_call:join, slice:5:9:, slice_lower:5, slice_step:, slice_upper:9, suggest_constant_definition
                for pandigital in itertools.permutations("123456789") # call_argument:, literal:Str, member_call:permutations
                if isCombinationValid(pandigital) # call_argument:pandigital, filtered_comprehension, free_call:isCombinationValid, internal_free_call:isCombinationValid
            ]
        )
    )
...
# problem_33/sol1.py
# ----------------------------------------------------------------------------------------
def isDigitCancelling(num, den): # function:isDigitCancelling (-> +4), function_argument:den, function_argument:num, function_argument_flavor:arg, function_returning_something:isDigitCancelling (-> +4), whole_span:19 (-> +18)
...
def solve(digit_len: int) -> str: # function:solve (-> +13), function_argument:digit_len, function_argument_flavor:arg, function_returning_something:solve (-> +13)
    ...
    last_digit = int("1" + "0" * digit_len) # assignment:int, assignment_lhs_identifier:last_digit, assignment_rhs_atom:digit_len, binary_operator:Add, binary_operator:Mult, call_argument:, concatenation_operator:Str, external_free_call:int, free_call:int, literal:Str, replication_operator:Str, single_assignment:last_digit
    for num in range(den, last_digit): # accumulate_elements:append (-> +7), accumulate_some_elements:append (-> +7), call_argument:den, call_argument:last_digit, count_elements:num (-> +7), external_free_call:range, for:num (-> +7), for_range:den:last_digit (-> +7), free_call:range, loop:for (-> +7), loop_with_late_exit:for (-> +7), range:den:last_digit
        while den <= 99: # comparison_operator:LtE, count_states:den (-> +4), literal:99, loop:while (-> +4), loop_with_late_exit:while (-> +4), suggest_constant_definition, while (-> +4)
            if (num != den) and (num % 10 == den // 10) and (den % 10 != 0): # binary_operator:FloorDiv, binary_operator:Mod, boolean_operator:And, comparison_operator:Eq, comparison_operator:NotEq, divisibility_test:10, if (-> +2), if_test_atom:0, if_test_atom:10, if_test_atom:den, if_test_atom:num, if_without_else (-> +2), literal:0, literal:10, modulo_operator, suggest_constant_definition
                if isDigitCancelling(num, den): # call_argument:den, call_argument:num, free_call:isDigitCancelling, if (-> +1), if_test_atom:den, if_test_atom:num, if_then_branch (-> +1), if_without_else (-> +1), internal_free_call:isDigitCancelling, nested_if:1 (-> +1)
                    solutions.append("{}/{}".format(num, den)) # call_argument:, call_argument:den, call_argument:num, composition, if_then_branch, literal:Str, member_call:append, member_call:format, member_call_object:solutions, member_call_without_result:append, update:solutions:den, update:solutions:num, update_by_member_call:solutions:den, update_by_member_call:solutions:num, update_by_member_call_with:append, update_with:append
    ...
    solutions = " , ".join(solutions) # assignment:join, assignment_lhs_identifier:solutions, assignment_rhs_atom:solutions, call_argument:solutions, literal:Str, member_call:join, single_assignment:solutions
    return solutions # return:solutions

# ----------------------------------------------------------------------------------------
# problem_36/sol1.py
# ----------------------------------------------------------------------------------------
def is_palindrome(n): # function:is_palindrome (-> +5), function_argument:n, function_argument_flavor:arg, function_returning_something:is_palindrome (-> +5), whole_span:12 (-> +11)
    n = str(n) # assignment:str, assignment_lhs_identifier:n, assignment_rhs_atom:n, call_argument:n, external_free_call:str, free_call:str, single_assignment:n
...
def solution(n): # function:solution (-> +5), function_argument:n, function_argument_flavor:arg, function_returning_something:solution (-> +5)
    total = 0 # assignment:0, assignment_lhs_identifier:total, assignment_rhs_atom:0, literal:0, single_assignment:total
    for i in range(1, n): # accumulate_elements:Add (-> +2), accumulate_some_elements:Add (-> +2), call_argument:1, call_argument:n, external_free_call:range, for:i (-> +2), for_range:1:n (-> +2), free_call:range, literal:1, loop:for (-> +2), loop_with_late_exit:for (-> +2), range:1:n
        if is_palindrome(i) and is_palindrome(bin(i).split("b")[1]): # boolean_operator:And, call_argument:, call_argument:i, composition, external_free_call:bin, free_call:bin, free_call:is_palindrome, if (-> +1), if_test_atom:1, if_test_atom:i, if_without_else (-> +1), index:1, internal_free_call:is_palindrome, literal:1, literal:Str, member_call:split
...
# problem_40/sol1.py
# ----------------------------------------------------------------------------------------
def solution(): # function:solution (-> +14), function_returning_something:solution (-> +14), function_without_arguments:solution (-> +14), whole_span:15 (-> +14)
    constant = [] # assignment, assignment_lhs_identifier:constant, empty_literal:List, literal:List, single_assignment:constant
    i = 1 # assignment:1, assignment_lhs_identifier:i, assignment_rhs_atom:1, literal:1, single_assignment:i
    while len(constant) < 1e6: # call_argument:constant, comparison_operator:Lt, count_states:i (-> +2), external_free_call:len, free_call:len, literal:1000000.0, loop:while (-> +2), loop_with_late_exit:while (-> +2), suggest_constant_definition, while (-> +2)
        constant.append(str(i)) # call_argument:, call_argument:i, composition, external_free_call:str, free_call:str, member_call:append, member_call_object:constant, member_call_without_result:append, update:constant:i, update_by_member_call:constant:i, update_by_member_call_with:append, update_with:append
        i += 1 # assignment_lhs_identifier:i, assignment_rhs_atom:1, augmented_assignment:Add, increment:i, literal:1, update:i:1, update_by_augmented_assignment:i:1, update_by_augmented_assignment_with:Add, update_with:Add
    constant = "".join(constant) # assignment:join, assignment_lhs_identifier:constant, assignment_rhs_atom:constant, call_argument:constant, empty_literal:Str, literal:Str, member_call:join, single_assignment:constant
    return ( # return
        int(constant[0]) # binary_operator:Mult, call_argument:, external_free_call:int, free_call:int, index:0, literal:0, multiplication_operator
        * int(constant[9]) # call_argument:, external_free_call:int, free_call:int, index:9, literal:9, suggest_constant_definition
        * int(constant[99]) # binary_operator:Mult, call_argument:, external_free_call:int, free_call:int, index:99, literal:99, multiplication_operator, suggest_constant_definition
        * int(constant[999]) # binary_operator:Mult, call_argument:, external_free_call:int, free_call:int, index:999, literal:999, multiplication_operator, suggest_constant_definition
        * int(constant[9999]) # binary_operator:Mult, call_argument:, external_free_call:int, free_call:int, index:9999, literal:9999, multiplication_operator, suggest_constant_definition
        * int(constant[99999]) # binary_operator:Mult, call_argument:, external_free_call:int, free_call:int, index:99999, literal:99999, multiplication_operator, suggest_constant_definition
        * int(constant[999999]) # binary_operator:Mult, call_argument:, external_free_call:int, free_call:int, index:999999, literal:999999, multiplication_operator, suggest_constant_definition
    )

# ----------------------------------------------------------------------------------------
# problem_42/solution42.py
# ----------------------------------------------------------------------------------------
import os # import:os, import_module:os, whole_span:16 (-> +15)
TRIANGULAR_NUMBERS = [int(0.5 * n * (n + 1)) for n in range(1, 101)] # addition_operator, assignment, assignment_lhs_identifier:TRIANGULAR_NUMBERS, assignment_rhs_atom:0.5, assignment_rhs_atom:1, assignment_rhs_atom:101, assignment_rhs_atom:n, binary_operator:Add, binary_operator:Mult, call_argument:, call_argument:1, call_argument:101, comprehension:List, comprehension_for_count:1, external_free_call:int, external_free_call:range, free_call:int, free_call:range, literal:0.5, literal:1, literal:101, multiplication_operator, range:1:101, single_assignment:TRIANGULAR_NUMBERS
def solution(): # function:solution (-> +13), function_returning_something:solution (-> +13), function_without_arguments:solution (-> +13)
    script_dir = os.path.dirname(os.path.realpath(__file__)) # assignment:dirname, assignment_lhs_identifier:script_dir, assignment_rhs_atom:__file__, assignment_rhs_atom:os, call_argument:, call_argument:__file__, composition, member_call:dirname, member_call:realpath, single_assignment:script_dir
    wordsFilePath = os.path.join(script_dir, "words.txt") # assignment:join, assignment_lhs_identifier:wordsFilePath, assignment_rhs_atom:os, assignment_rhs_atom:script_dir, call_argument:, call_argument:script_dir, literal:Str, member_call:join, single_assignment:wordsFilePath
    words = "" # assignment, assignment_lhs_identifier:words, empty_literal:Str, literal:Str, single_assignment:words
    with open(wordsFilePath, "r") as f: # call_argument:, call_argument:wordsFilePath, external_free_call:open, free_call:open, literal:Str
        words = f.readline() # assignment:readline, assignment_lhs_identifier:words, assignment_rhs_atom:f, member_call:readline, single_assignment:words
    words = list(map(lambda word: word.strip('"'), words.strip("\r\n").split(","))) # assignment:list, assignment_lhs_identifier:words, assignment_rhs_atom:word, assignment_rhs_atom:words, call_argument:, composition, external_free_call:list, external_free_call:map, free_call:list, free_call:map, function_argument:word, function_argument_flavor:arg, lambda_function, literal:Str, member_call:split, member_call:strip, member_call_object:words, method_chaining, single_assignment:words, update:words:word, update_by_assignment:words:word, update_by_assignment_with:list, update_with:list
    words = list( # assignment:list, assignment_lhs_identifier:words, composition, external_free_call:list, free_call:list, single_assignment:words, update:words:64, update:words:TRIANGULAR_NUMBERS, update:words:word, update:words:x, update_by_assignment:words:64, update_by_assignment:words:TRIANGULAR_NUMBERS, update_by_assignment:words:word, update_by_assignment:words:x, update_by_assignment_with:list, update_with:list
        filter( # call_argument:, composition, external_free_call:filter, free_call:filter
            ...
            map(lambda word: sum(map(lambda x: ord(x) - 64, word)), words), # assignment_rhs_atom:64, assignment_rhs_atom:word, assignment_rhs_atom:words, assignment_rhs_atom:x, binary_operator:Sub, call_argument:, call_argument:word, call_argument:words, call_argument:x, composition, external_free_call:map, external_free_call:ord, external_free_call:sum, free_call:map, free_call:ord, free_call:sum, function_argument:word, function_argument:x, function_argument_flavor:arg, lambda_function, literal:64, suggest_constant_definition
        )
    )
    return len(words) # call_argument:words, external_free_call:len, free_call:len, free_tail_call:len, return

# ----------------------------------------------------------------------------------------
# problem_48/sol1.py
# ----------------------------------------------------------------------------------------
def solution(): # function:solution (-> +4), function_returning_something:solution (-> +4), function_without_arguments:solution (-> +4), whole_span:5 (-> +4)
    total = 0 # assignment:0, assignment_lhs_identifier:total, assignment_rhs_atom:0, literal:0, single_assignment:total
    for i in range(1, 1001): # accumulate_all_elements:Add (-> +1), accumulate_elements:Add (-> +1), call_argument:1, call_argument:1001, external_free_call:range, for:i (-> +1), for_range:1:1001 (-> +1), free_call:range, literal:1, literal:1001, loop:for (-> +1), loop_with_late_exit:for (-> +1), range:1:1001, suggest_constant_definition
    ...
    return str(total)[-10:] # call_argument:total, external_free_call:str, free_call:str, literal:-10, return, slice:-10::, slice_lower:-10, slice_step:, slice_upper:, suggest_constant_definition

# ----------------------------------------------------------------------------------------
# problem_52/sol1.py
# ----------------------------------------------------------------------------------------
def solution(): # function:solution (-> +12), function_returning_something:solution (-> +12), function_without_arguments:solution (-> +12), whole_span:13 (-> +12)
    i = 1 # assignment:1, assignment_lhs_identifier:i, assignment_rhs_atom:1, literal:1, single_assignment:i
    while True: # count_states:i (-> +10), infinite_while (-> +10), literal:True, loop:while (-> +10), loop_with_early_exit:while:return (-> +10), loop_with_return:while (-> +10), while (-> +10)
        if ( # if (-> +8)
            sorted(list(str(i))) # call_argument:, call_argument:i, chained_comparison:5, chained_equalities:5, composition, external_free_call:list, external_free_call:sorted, external_free_call:str, free_call:list, free_call:sorted, free_call:str, if_test_atom:i
            == sorted(list(str(2 * i))) # binary_operator:Mult, call_argument:, comparison_operator:Eq, composition, external_free_call:list, external_free_call:sorted, external_free_call:str, free_call:list, free_call:sorted, free_call:str, if_test_atom:2, if_test_atom:i, literal:2, multiplication_operator
            == sorted(list(str(3 * i))) # binary_operator:Mult, call_argument:, comparison_operator:Eq, composition, external_free_call:list, external_free_call:sorted, external_free_call:str, free_call:list, free_call:sorted, free_call:str, if_test_atom:3, if_test_atom:i, literal:3, multiplication_operator, suggest_constant_definition
            == sorted(list(str(4 * i))) # binary_operator:Mult, call_argument:, comparison_operator:Eq, composition, external_free_call:list, external_free_call:sorted, external_free_call:str, free_call:list, free_call:sorted, free_call:str, if_test_atom:4, if_test_atom:i, literal:4, multiplication_operator, suggest_constant_definition
            == sorted(list(str(5 * i))) # binary_operator:Mult, call_argument:, comparison_operator:Eq, composition, external_free_call:list, external_free_call:sorted, external_free_call:str, free_call:list, free_call:sorted, free_call:str, if_test_atom:5, if_test_atom:i, literal:5, multiplication_operator, suggest_constant_definition
            == sorted(list(str(6 * i))) # binary_operator:Mult, call_argument:, comparison_operator:Eq, composition, external_free_call:list, external_free_call:sorted, external_free_call:str, free_call:list, free_call:sorted, free_call:str, if_test_atom:6, if_test_atom:i, literal:6, multiplication_operator, suggest_constant_definition
        ):
...
# problem_53/sol1.py
# ----------------------------------------------------------------------------------------
from math import factorial # import:math:factorial, import_module:math, import_name:factorial, whole_span:10 (-> +9)
def combinations(n, r): # function:combinations (-> +1), function_argument:n, function_argument:r, function_argument_flavor:arg, function_returning_something:combinations (-> +1)
    return factorial(n) / (factorial(r) * factorial(n - r)) # binary_operator:Div, binary_operator:Mult, binary_operator:Sub, call_argument:, call_argument:n, call_argument:r, external_free_call:factorial, free_call:factorial, multiplication_operator, return
def solution(): # function:solution (-> +6), function_returning_something:solution (-> +6), function_without_arguments:solution (-> +6)
    total = 0 # assignment:0, assignment_lhs_identifier:total, assignment_rhs_atom:0, literal:0, single_assignment:total
    for i in range(1, 101): # call_argument:1, call_argument:101, count_elements:total (-> +3), external_free_call:range, for:i (-> +3), for_range:1:101 (-> +3), for_range:1:_ (-> +3), free_call:range, literal:1, literal:101, loop:for (-> +3), loop_with_late_exit:for (-> +3), range:1:101, suggest_constant_definition
        for j in range(1, i + 1): # addition_operator, binary_operator:Add, call_argument:, call_argument:1, external_free_call:range, for:j (-> +2), for_range:1:_ (-> +2), free_call:range, literal:1, loop:for (-> +2), loop_with_late_exit:for (-> +2), nested_for:1 (-> +2), range:1:_
            if combinations(i, j) > 1e6: # call_argument:i, call_argument:j, comparison_operator:Gt, free_call:combinations, if (-> +1), if_test_atom:1000000.0, if_test_atom:i, if_test_atom:j, if_without_else (-> +1), internal_free_call:combinations, literal:1000000.0, suggest_constant_definition
...
def next_term(a_i, k, i, n): # body_recursive_function:next_term (-> +50), function:next_term (-> +50), function_argument:a_i, function_argument:i, function_argument:k, function_argument:n, function_argument_flavor:arg, function_returning_something:next_term (-> +50), recursive_function:next_term (-> +50)
    ...
    for j in range(k, len(a_i)): # accumulate_all_elements:Add (-> +1), accumulate_elements:Add (-> +1), call_argument:, call_argument:a_i, call_argument:k, composition, external_free_call:len, external_free_call:range, for:j (-> +1), for_range:k:_ (-> +1), free_call:len, free_call:range, loop:for (-> +1), loop_with_late_exit:for (-> +1), range:k:_
    ...
    for j in range(min(len(a_i), k)): # accumulate_all_elements:Add (-> +1), accumulate_elements:Add (-> +1), call_argument:, call_argument:a_i, call_argument:k, composition, external_free_call:len, external_free_call:min, external_free_call:range, for:j (-> +1), for_range:_ (-> +1), free_call:len, free_call:min, free_call:range, loop:for (-> +1), loop_with_late_exit:for (-> +1), range:_
    ...
    sub_memo = memo.get(ds_b) # assignment:get, assignment_lhs_identifier:sub_memo, assignment_rhs_atom:ds_b, assignment_rhs_atom:memo, call_argument:ds_b, member_call:get, single_assignment:sub_memo
    if sub_memo != None: # comparison_operator:NotEq, if (-> +19), if_test_atom:None, if_test_atom:sub_memo, literal:None
        jumps = sub_memo.get(c) # assignment:get, assignment_lhs_identifier:jumps, assignment_rhs_atom:c, assignment_rhs_atom:sub_memo, call_argument:c, if_then_branch (-> +15), member_call:get, single_assignment:jumps
        if jumps != None and len(jumps) > 0: # boolean_operator:And, call_argument:jumps, comparison_operator:Gt, comparison_operator:NotEq, external_free_call:len, free_call:len, if (-> +14), if_test_atom:0, if_test_atom:None, if_test_atom:jumps, literal:0, literal:None, nested_if:1 (-> +14)
            ...
            for _k in range(len(jumps) - 1, -1, -1): # binary_operator:Sub, call_argument:, call_argument:-1, call_argument:jumps, composition, external_free_call:len, external_free_call:range, for:_k (-> +3), for_range:_:-1:-1 (-> +3), free_call:len, free_call:range, literal:-1, literal:1, loop:for (-> +3), loop_with_break:for (-> +3), loop_with_early_exit:for:break (-> +3), range:_:-1:-1
    ...
    if k > ks[0]: # comparison_operator:Gt, if (-> +10), if_test_atom:0, if_test_atom:k, if_test_atom:ks, index:0, literal:0
        while True: # if_then_branch (-> +5), infinite_while (-> +5), literal:True, loop:while (-> +5), loop_with_break:while (-> +5), loop_with_early_exit:while:break (-> +5), while (-> +5)
    ...
    while j < len(jumps): # call_argument:jumps, comparison_operator:Lt, count_states:j (-> +3), external_free_call:len, free_call:len, loop:while (-> +3), loop_with_break:while (-> +3), loop_with_early_exit:while:break (-> +3), while (-> +3)
...
def compute(a_i, k, i, n): # function:compute (-> +25), function_argument:a_i, function_argument:i, function_argument:k, function_argument:n, function_argument_flavor:arg, function_returning_something:compute (-> +25)
    ...
    for j in range(len(a_i)): # accumulate_elements:Add (-> +4), accumulate_some_elements:Add (-> +4), call_argument:, call_argument:a_i, composition, external_free_call:len, external_free_call:range, for:j (-> +4), for_indexes (-> +4), for_range:_ (-> +4), free_call:len, free_call:range, loop:for (-> +4), loop_with_late_exit:for (-> +4), range:_
    ...
    while i < n: # comparison_operator:Lt, count_states:i (-> +10), loop:while (-> +10), loop_with_break:while (-> +10), loop_with_early_exit:while:break (-> +10), while (-> +10)
        i += 1 # assignment_lhs_identifier:i, assignment_rhs_atom:1, augmented_assignment:Add, increment:i, literal:1, update:i:1, update_by_augmented_assignment:i:1, update_by_augmented_assignment_with:Add, update_with:Add
...
def add(digits, k, addend): # function:add (-> +13), function_argument:addend, function_argument:digits, function_argument:k, function_argument_flavor:arg, function_returning_nothing:add (-> +13)
    for j in range(k, len(digits)): # call_argument:, call_argument:digits, call_argument:k, composition, external_free_call:len, external_free_call:range, for:j (-> +9), for_range:k:_ (-> +9), free_call:len, free_call:range, loop:for (-> +9), loop_with_break:for (-> +9), loop_with_early_exit:for:break (-> +9), range:k:_
...
def solution(n): # function:solution (-> +12), function_argument:n, function_argument_flavor:arg, function_returning_something:solution (-> +12)
    ...
    while True: # infinite_while (-> +4), literal:True, loop:while (-> +4), loop_with_break:while (-> +4), loop_with_early_exit:while:break (-> +4), while (-> +4)
    ...
    for j in range(len(digits)): # accumulate_all_elements:Add (-> +1), accumulate_elements:Add (-> +1), call_argument:, call_argument:digits, composition, external_free_call:len, external_free_call:range, for:j (-> +1), for_indexes (-> +1), for_range:_ (-> +1), free_call:len, free_call:range, loop:for (-> +1), loop_with_late_exit:for (-> +1), range:_
...
# problem_56/sol1.py
# ----------------------------------------------------------------------------------------
def maximum_digital_sum(a: int, b: int) -> int: # function:maximum_digital_sum (-> +5), function_argument:a, function_argument:b, function_argument_flavor:arg, function_returning_something:maximum_digital_sum (-> +5), whole_span:6 (-> +5)
    return max( # composition, external_free_call:max, free_call:max, free_tail_call:max, return
        [
            sum([int(x) for x in str(base ** power)]) # binary_operator:Pow, call_argument:, call_argument:x, composition, comprehension:List, comprehension_for_count:1, comprehension_for_count:2, external_free_call:int, external_free_call:str, external_free_call:sum, free_call:int, free_call:str, free_call:sum
            for base in range(a) # call_argument:a, external_free_call:range, free_call:range, range:a
            for power in range(b) # call_argument:b, external_free_call:range, free_call:range, range:b
        ]
    )
...
# problem_67/sol1.py
# ----------------------------------------------------------------------------------------
import os # import:os, import_module:os, whole_span:20 (-> +19)
def solution(): # function:solution (-> +18), function_returning_something:solution (-> +18), function_without_arguments:solution (-> +18)
    script_dir = os.path.dirname(os.path.realpath(__file__)) # assignment:dirname, assignment_lhs_identifier:script_dir, assignment_rhs_atom:__file__, assignment_rhs_atom:os, call_argument:, call_argument:__file__, composition, member_call:dirname, member_call:realpath, single_assignment:script_dir
    triangle = os.path.join(script_dir, "triangle.txt") # assignment:join, assignment_lhs_identifier:triangle, assignment_rhs_atom:os, assignment_rhs_atom:script_dir, call_argument:, call_argument:script_dir, literal:Str, member_call:join, single_assignment:triangle
    with open(triangle, "r") as f: # call_argument:, call_argument:triangle, external_free_call:open, free_call:open, literal:Str
        triangle = f.readlines() # assignment:readlines, assignment_lhs_identifier:triangle, assignment_rhs_atom:f, member_call:readlines, single_assignment:triangle
    a = map(lambda x: x.rstrip("\r\n").split(" "), triangle) # assignment:map, assignment_lhs_identifier:a, assignment_rhs_atom:triangle, assignment_rhs_atom:x, call_argument:, call_argument:triangle, composition, external_free_call:map, free_call:map, function_argument:x, function_argument_flavor:arg, lambda_function, literal:Str, member_call:rstrip, member_call:split, member_call_object:x, method_chaining, single_assignment:a
    a = list(map(lambda x: list(map(lambda y: int(y), x)), a)) # assignment:list, assignment_lhs_identifier:a, assignment_rhs_atom:a, assignment_rhs_atom:x, assignment_rhs_atom:y, call_argument:, call_argument:a, call_argument:x, call_argument:y, composition, external_free_call:int, external_free_call:list, external_free_call:map, free_call:int, free_call:list, free_call:map, function_argument:x, function_argument:y, function_argument_flavor:arg, lambda_function, single_assignment:a, update:a:x, update:a:y, update_by_assignment:a:x, update_by_assignment:a:y, update_by_assignment_with:list, update_with:list
    for i in range(1, len(a)): # call_argument:, call_argument:1, call_argument:a, composition, external_free_call:len, external_free_call:range, for:i (-> +10), for_range:1:_ (-> +10), for_range:_ (-> +10), free_call:len, free_call:range, literal:1, loop:for (-> +10), loop_with_late_exit:for (-> +10), range:1:_
        for j in range(len(a[i])): # call_argument:, composition, external_free_call:len, external_free_call:range, for:j (-> +9), for_indexes (-> +9), for_range:_ (-> +9), free_call:len, free_call:range, index:i, loop:for (-> +9), loop_with_late_exit:for (-> +9), nested_for:1 (-> +9), range:_
            if j != len(a[i - 1]): # binary_operator:Sub, call_argument:, comparison_operator:NotEq, external_free_call:len, free_call:len, if (-> +3), if_test_atom:1, if_test_atom:a, if_test_atom:i, if_test_atom:j, index:_, index_arithmetic, literal:1, verbose_conditional_assignment (-> +3)
                number1 = a[i - 1][j] # assignment, assignment_lhs_identifier:number1, assignment_rhs_atom:1, assignment_rhs_atom:a, assignment_rhs_atom:i, assignment_rhs_atom:j, binary_operator:Sub, if_then_branch, index:_, index:j, index_arithmetic, literal:1, nested_index:2, single_assignment:number1
...
# problem_76/sol1.py
# ----------------------------------------------------------------------------------------
def partition(m): # function:partition (-> +9), function_argument:m, function_argument_flavor:arg, function_returning_something:partition (-> +9), whole_span:10 (-> +9)
    memo = [[0 for _ in range(m)] for _ in range(m + 1)] # addition_operator, assignment, assignment_lhs_identifier:memo, assignment_rhs_atom:0, assignment_rhs_atom:1, assignment_rhs_atom:_, assignment_rhs_atom:m, binary_operator:Add, call_argument:, call_argument:m, comprehension:List, comprehension_for_count:1, external_free_call:range, free_call:range, literal:0, literal:1, range:_, range:m, single_assignment:memo
    for i in range(m + 1): # addition_operator, binary_operator:Add, call_argument:, external_free_call:range, for:i (-> +1), for_range:_ (-> +1), free_call:range, literal:1, loop:for (-> +1), loop_with_late_exit:for (-> +1), range:_
        memo[i][0] = 1 # assignment:1, assignment_rhs_atom:1, index:0, index:i, literal:0, literal:1, nested_index:2
    for n in range(m + 1): # addition_operator, binary_operator:Add, call_argument:, external_free_call:range, for:n (-> +4), for_range:1:m (-> +4), for_range:_ (-> +4), free_call:range, literal:1, loop:for (-> +4), loop_with_late_exit:for (-> +4), range:_
        for k in range(1, m): # call_argument:1, call_argument:m, external_free_call:range, for:k (-> +3), for_range:1:m (-> +3), free_call:range, literal:1, loop:for (-> +3), loop_with_late_exit:for (-> +3), nested_for:1 (-> +3), range:1:m
...
# problem_99/sol1.py
# ----------------------------------------------------------------------------------------
import os # import:os, import_module:os, whole_span:9 (-> +8)
...
def find_largest(data_file: str = "base_exp.txt") -> int: # function:find_largest (-> +6), function_argument:data_file, function_argument_flavor:arg, function_returning_something:find_largest (-> +6), literal:Str
    ...
    for i, line in enumerate(open(os.path.join(os.path.dirname(__file__), data_file))): # call_argument:, call_argument:__file__, call_argument:data_file, composition, external_free_call:enumerate, external_free_call:open, for:i (-> +3), for:line (-> +3), for_indexes_elements (-> +3), free_call:enumerate, free_call:open, literal:Tuple, loop:for (-> +3), loop_with_late_exit:for (-> +3), member_call:dirname, member_call:join

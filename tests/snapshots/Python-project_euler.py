...
def isprime(no): # function:isprime (-> +9), function_argument:no, function_argument_flavor:arg, function_returning_something:isprime (-> +9)
    ...
    for i in range(3, sq, 2): # call_argument:2, call_argument:3, call_argument:sq, external_free_call:range, for:i (-> +2), for_range:3:sq:2 (-> +2), free_call:range, literal:2, literal:3, loop:for (-> +2), loop_with_early_exit:for:return (-> +2), range:3:sq:2, suggest_constant_definition, universal_quantification:i (-> +2)
...
def solution(n): # function:solution (-> +24), function_argument:n, function_argument_flavor:arg, function_returning_something:solution (-> +24)
    ...
    if isprime(n): # call_argument:n, free_call:isprime, if (-> +16), if_test_atom:n, internal_free_call:isprime
        ...
        if isprime(n): # call_argument:n, free_call:isprime, if (-> +11), if_test_atom:n, internal_free_call:isprime, nested_if:1 (-> +11)
            ...
            for i in range(3, n1, 2): # call_argument:2, call_argument:3, call_argument:n1, external_free_call:range, for:i (-> +6), for_range:3:n1:2 (-> +6), free_call:range, literal:2, literal:3, loop:for (-> +6), loop_with_early_exit:for:break (-> +6), range:3:n1:2, suggest_constant_definition
...
def solution(n): # function:solution (-> +18), function_argument:n, function_argument_flavor:arg, function_returning_something:solution (-> +18), whole_span:19 (-> +18)
    ...
    while 1: # infinite_while (-> +10), literal:1, loop:while (-> +10), loop_with_early_exit:while:return (-> +10), while (-> +10)
        ...
        for j in range(2, n): # call_argument:2, call_argument:n, external_free_call:range, for:j (-> +3), for_range:2:n (-> +3), free_call:range, literal:2, loop:for (-> +3), loop_with_early_exit:for:break (-> +3), range:2:n
...
def solution(): # function:solution (-> +7), function_returning_something:solution (-> +7), function_without_arguments:solution (-> +7), whole_span:8 (-> +7)
    for a in range(300): # call_argument:300, external_free_call:range, for:a (-> +6), for_range:300 (-> +6), for_range:400 (-> +6), for_range:500 (-> +6), free_call:range, literal:300, loop:for (-> +6), loop_with_late_exit:for (-> +6), range:300, suggest_constant_definition
        for b in range(400): # call_argument:400, external_free_call:range, for:b (-> +5), for_range:400 (-> +5), for_range:500 (-> +5), free_call:range, literal:400, loop:for (-> +5), loop_with_late_exit:for (-> +5), nested_for:1 (-> +5), range:400, suggest_constant_definition
            for c in range(500): # call_argument:500, external_free_call:range, for:c (-> +4), for_range:500 (-> +4), free_call:range, literal:500, loop:for (-> +4), loop_with_early_exit:for:return (-> +4), nested_for:2 (-> +4), range:500, suggest_constant_definition
...
def solution(): # function:solution (-> +12), function_returning_something:solution (-> +12), function_without_arguments:solution (-> +12)
    ...
    for i, name in enumerate(names): # accumulate_all_elements:Add (-> +4), accumulate_elements:Add (-> +4), call_argument:names, external_free_call:enumerate, for:i (-> +4), for:name (-> +4), for_indexes_elements (-> +4), free_call:enumerate, literal:Tuple, loop:for (-> +4), loop_with_late_exit:for (-> +4)
        for letter in name: # accumulate_all_elements:Add (-> +1), accumulate_elements:Add (-> +1), for:letter (-> +1), for_each (-> +1), loop:for (-> +1), loop_with_late_exit:for (-> +1), nested_for:2 (-> +1)
...
def next_term(a_i, k, i, n): # body_recursive_function:next_term (-> +50), function:next_term (-> +50), function_argument:a_i, function_argument:i, function_argument:k, function_argument:n, function_argument_flavor:arg, function_returning_something:next_term (-> +50), recursive_function:next_term (-> +50)
    ...
    if sub_memo != None: # comparison_operator:NotEq, if (-> +19), if_test_atom:None, if_test_atom:sub_memo, literal:None
        ...
        if jumps != None and len(jumps) > 0: # boolean_operator:And, call_argument:jumps, comparison_operator:Gt, comparison_operator:NotEq, external_free_call:len, free_call:len, if (-> +14), if_test_atom:0, if_test_atom:None, if_test_atom:jumps, literal:0, literal:None, nested_if:1 (-> +14)
            ...
            for _k in range(len(jumps) - 1, -1, -1): # binary_operator:Sub, call_argument:, call_argument:-1, call_argument:jumps, composition, external_free_call:len, external_free_call:range, for:_k (-> +3), for_range:_:-1:-1 (-> +3), free_call:len, free_call:range, literal:-1, literal:1, loop:for (-> +3), loop_with_early_exit:for:break (-> +3), range:_:-1:-1
    ...
    while j < len(jumps): # call_argument:jumps, comparison_operator:Lt, count_states:j (-> +3), external_free_call:len, free_call:len, loop:while (-> +3), loop_with_early_exit:while:break (-> +3), while (-> +3)
